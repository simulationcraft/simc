...
# List of spell ids generated by SpellDataGenerator, used later in SpellIDListGenerator
generated_spell_ids = []
...
class SpellDataGenerator(DataGenerator):
    ...
    _spell_id_list = [
        (
         ...
         1292299, 1292300, 1306870, 1308012, 1308013, 1308014, # Gebbo's Bottomless Bag artifact buffs
        ),
    ...
    def generate(self, ids = None):
        ...
        for id in id_keys:
            ...
            global generated_spell_ids
            generated_spell_ids.append(id)
...
class SpellIDListGenerator(DataGenerator):
    def check_spell(self, spell):
        family = spell.child_ref('SpellClassOptions', field='id_spell').family
        for class_entry in constants.CLASS_INFO:
            if class_entry['family'] == family:
                return class_entry['name'], spell.id

        label = spell.child_ref('SpellLabel').label
        for class_entry in constants.CLASS_INFO:
            if class_entry['label'] == label:
                return class_entry['name'], spell.id
    ...
    def generate(self, data = None):
        ...
        for spell in self.db('SpellName').values():
            class_name, id = self.check_spell(spell)
            if id != -1:
                data[class_name].append([id, spell.name])

        self._out.write('Class Spell ID List, wow build {}\n'.format(self._options.build))

        for class_name, spell_list in sorted(data.items(), key = lambda e: e[0]):
            self._out.write('\n{}\n'.format(class_name))

            for id, name in sorted(spell_list, key = lambda e: e[0]):
                self._out.write('{:<7d} {}{}\n'.format(id, ' ' if id in generated_spell_ids else '*', name))
...
class RankSpellGenerator(DataGenerator):
    ...
    def generate(self, data = None):
        data.sort(key = lambda v: (v[0], v[1], v[2].name, self.db('Spell')[v[2].id].rank))
