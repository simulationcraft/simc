...
RUN apk --no-cache add --virtual build_dependencies \
    ...
    if [ $NONETWORKING -eq 0 ] ; then \
        ...
        make -C /app/SimulationCraft/engine release -j $THREADS LTO=1 OPTS+="-Os -mtune=native" SC_DEFAULT_APIKEY=${APIKEY} ;  \
    else \
        echo "Building no-networking version" && \
        make -C /app/SimulationCraft/engine release -j $THREADS LTO=1 SC_NO_NETWORKING=1 OPTS+="-Os -mtune=native" SC_DEFAULT_APIKEY=${APIKEY} ; \
    fi && \
