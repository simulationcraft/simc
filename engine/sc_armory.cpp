                               ...
                               int cache )
{
  ...
  {
    util_t::fprintf( sim -> output_file, "simucraft: The Death Knight module is still in development, so Armory downloads are disabled.\n" );
    return false;
  }
  ...
  if ( ! members )
  {
    util_t::fprintf( sim -> output_file, "\nsimucraft: Unable to determine members for guild %s|%s|%s from the Armory.\n", region.c_str(), server.c_str(), guild_name.c_str() );
    return false;
  }
  ...
  if ( num_characters > 0 )
  {
    ...
    for ( int i=0; i < num_characters; i++ )
    {
      ...
      util_t::fprintf( sim -> output_file, "\nsimucraft: Downloading character: %s\n", character_name.c_str() );
      player_t* p = armory_t::download_player( sim, region, server, character_name, "active", cache );

      if ( ! p )
      {
        util_t::fprintf( sim -> output_file, "simucraft: Armory failed...  Attempting to download character from wowhead.\n" );
      ...
      {
        util_t::fprintf( sim -> output_file, "\nsimucraft: Setting quiet=1 on healer %s\n", character_name.c_str() );
        p -> quiet = true;
      }
      if ( tree == TREE_PROTECTION )
      {
        util_t::fprintf( sim -> output_file, "\nsimucraft: Setting quiet=1 on tank %s\n", character_name.c_str() );
        p -> quiet = true;
      }
    }
  }
                                     ...
                                     int cache )
{
  ...
  {
    util_t::fprintf( sim -> output_file, "\nsimucraft: Unable to download character %s|%s|%s from the Armory.\n", region.c_str(), server.c_str(), name.c_str() );
    return 0;
  }
  ...
  {
    util_t::fprintf( sim -> output_file, "\nsimucraft: Unable to determine class/name/level/race from armory xml for %s|%s|%s.\n",
                    region.c_str(), server.c_str(), name.c_str() );
    return 0;
  }
  ...
  if ( ! p )
  {
    util_t::fprintf( sim -> output_file, "\nsimucraft: Unable to build player with class '%s' and name '%s' from armory %s|%s|%s.\n",
    ...
    if ( active_talents )
    {
      if ( inactive_talents )
      {
        ...
        else
        {
          ...
          {
            util_t::fprintf( sim -> output_file, "\nsimucraft: Valid values for 'talents' are 'active', 'inactive', or the name of a tree such as 'fury' or 'shadow'.\n" );
            return 0;
          }
      ...
      {
        util_t::fprintf( sim -> output_file, "\nsimucraft: Player %s unable to determine talents from armory xml.\n", p -> name() );
        return 0;
      }
      if ( ! p -> parse_talents_armory( talents_encoding ) )
      {
        util_t::fprintf( sim -> output_file, "\nsimucraft: Player %s unable to parse talents '%s'.\n", p -> name(), talents_encoding.c_str() );
        return 0;
      }
      ...
      for ( int i=0; i < num_glyphs; i++ )
      {
        ...
        {
          util_t::fprintf( sim -> output_file, "\nsimucraft: Player %s unable to determine glyph name from armory xml.\n", p -> name() );
          return 0;
        }
                              ...
                              int cache_only )
{
  ...
  {
    if ( ! cache_only )
      util_t::fprintf( item.sim -> output_file, "\nsimucraft: Unable to download item %s from armory at slot %d for player %s\n", id_str.c_str(), item.slot, p -> name() );
    return false;
  }

  if ( ! parse_item_name( item, slot_xml ) )
  {
    util_t::fprintf( item.sim -> output_file, "\nsimucraft: Player %s unable to parse name for item %s at slot %s.\n", p -> name(), id_str.c_str(), item.slot_name() );
    return false;
  }

  if ( ! parse_item_stats( item, slot_xml ) )
  {
    util_t::fprintf( item.sim -> output_file, "\nsimucraft: Player %s unable to parse stats for item \"%s\" at slot %s.\n", p -> name(), item.name(), item.slot_name() );
    return false;
  }

  if ( ! parse_item_gems( item, slot_xml ) )
  {
    util_t::fprintf( item.sim -> output_file, "\nsimucraft: Player %s unable to parse gems for item \"%s\" at slot %s.\n", p -> name(), item.name(), item.slot_name() );
    return false;
  }

  if ( ! parse_item_enchant( item, slot_xml ) )
  {
    util_t::fprintf( item.sim -> output_file, "\nsimucraft: Player %s unable to parse enchant for item \"%s\" at slot %s.\n", p -> name(), item.name(), item.slot_name() );
    return false;
  }

  if ( ! parse_item_weapon( item, slot_xml ) )
  {
    util_t::fprintf( item.sim -> output_file, "\nsimucraft: Player %s unable to parse weapon info for item \"%s\" at slot %s.\n", p -> name(), item.name(), item.slot_name() );
    return false;
  }
                              ...
                              int cache_only )
{
  ...
  {
    if ( ! cache_only )
      util_t::fprintf( item.sim -> output_file, "\nsimucraft: Player %s unable to download item %s from armory at slot %s.\n", p -> name(), id_str.c_str(), item.slot_name() );
    return false;
  }
