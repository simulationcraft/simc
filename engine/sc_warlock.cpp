  ...
  if ( action_list_str.empty() )
  {
    ...
    case TREE_AFFLICTION:
      ...
      if ( talent_soul_siphon -> rank() ) {
        if ( has_wou )
          action_list_str += "/drain_soul,interrupt=1,interrupt_if=buff.will_of_unbinding.up&cooldown.haunt.remains<tick_time&buff.will_of_unbinding.remains<action.haunt.cast_time+tick_time+cooldown.haunt.remains,if=target.health_pct<=25";
        else
