...
player_t::player_t( sim_t*             s,
  ...
  dps( std::string( "Personal damage per second" ), s -> statistics_level < 1 ), dpse( s -> statistics_level < 2 ),
  dtps( std::string( "Personal damage taken per second" ), s -> statistics_level < 2 ), dmg_taken( s -> statistics_level < 2 ),
  dps_convergence( 0 ),
  // Heal
  iteration_heal( 0 ),iteration_heal_taken( 0 ),
  hps_error( 0 ), hpr( 0 ),
  heal( s -> statistics_level < 2 ), compound_heal( s -> statistics_level < 2 ),
  hps( std::string( "Personal healing per second" ), s -> statistics_level < 1 ), hpse( s -> statistics_level < 2 ),
  ...
  if ( action )
  {
    ...
    if ( ! action -> quiet )
    {
      ...
      if ( action -> marker && sim -> current_iteration == 0 )
        report_information.action_sequence.push_back( new action_sequence_data_t( action, action -> target, sim -> current_time, buff_list ) );
    }
  }
