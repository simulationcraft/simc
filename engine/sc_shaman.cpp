...
struct shaman_t : public player_t
{
  ...
  virtual int       primary_role() SC_CONST     { return talents.stormstrike ? ROLE_HYBRID : ROLE_SPELL; }
...
static void trigger_primal_wisdom ( attack_t* a )
{
  ...
  if ( p -> rng_primal_wisdom -> roll( p -> talents.primal_wisdom * 0.20 ) )
  {
...
static void trigger_static_shock ( attack_t* a )
{
  ...
  if ( p -> talents.static_shock && p -> buffs_lightning_shield -> stack() )
  {
    double chance = p -> talents.static_shock * 0.15 + p -> set_bonus.tier9_2pc_melee() * 0.03;
...
static void trigger_telluric_currents ( spell_t* s )
{
  ...
  if ( p -> talents.telluric_currents )
  {
    double mana_gain = s -> direct_dmg * ( 0.20 * p -> talents.telluric_currents );
...
struct lava_burst_t : public shaman_spell_t
{
      ...
      maelstrom( 0 ), flame_shock( 0 )
  {
    ...
    base_cost_reduction += p -> talents.convection * 0.05;
    base_multiplier     *= 1.0 + p -> talents.concussion * 0.03 + p -> talents.call_of_flame * 0.05;
    base_hit            += p -> talents.elemental_precision      * 0.01;
    direct_power_mod    += p -> primary_tree() == TREE_ELEMENTAL * 0.20;
...
struct wind_shear_t : public shaman_spell_t
{
      ...
      shaman_spell_t( "wind_shear", player, SCHOOL_NATURE, TREE_ELEMENTAL )
  {
    ...
    cooldown -> duration = 6.0 - ( p -> talents.reverberation * 0.5 );
    id = 57994;
  }
...
struct fire_elemental_totem_t : public shaman_spell_t
{
      ...
      shaman_spell_t( "fire_elemental_totem", player, SCHOOL_FIRE, TREE_ELEMENTAL )
  {
    ...
    trigger_gcd = 1.0;
    base_cost_reduction += p -> talents.totemic_focus * 0.10;
    num_ticks = 1;
    base_tick_time = ( 1 + p -> talents.totemic_focus * 0.20 ) * 120.0 + 0.1;
...
struct flametongue_totem_t : public shaman_spell_t
{
      ...
      shaman_spell_t( "flametongue_totem", player, SCHOOL_FIRE, TREE_ENHANCEMENT ), bonus( 0 )
  {
    ...
    bonus = ( p -> talents.totemic_wrath ) ? 0.10 : 0.06;
...
struct magma_totem_t : public shaman_spell_t
{
      ...
      shaman_spell_t( "magma_totem", player, SCHOOL_FIRE, TREE_ELEMENTAL )
  {
    ...
    num_ticks         = int ( ( 1 + p -> talents.totemic_focus * 0.20 ) * 10.0 );
    may_crit          = true;
    trigger_gcd       = 1.0;
...
struct mana_spring_totem_t : public shaman_spell_t
{
      ...
      shaman_spell_t( "mana_spring_totem", player, SCHOOL_NATURE, TREE_RESTORATION ), regen( 0 )
  {
    ...
    regen *= 1.0 + ( p -> talents.restorative_totems * 0.10 );
...
struct searing_totem_t : public shaman_spell_t
{
      ...
      shaman_spell_t( "searing_totem", player, SCHOOL_FIRE, TREE_ELEMENTAL )
  {
    ...
    num_ticks         = int ( ( 1 + p -> talents.totemic_focus * 0.20 ) * 24 );
    may_crit          = true;
    trigger_gcd       = 1.0;
    base_multiplier  *= 1.0 + p -> talents.call_of_flame * 0.10;
    base_hit         += p -> talents.elemental_precision * 0.01;
...
{
  if ( talents.stormstrike ) return TREE_ENHANCEMENT;
  if ( talents.mana_tide_totem ) return TREE_RESTORATION;
...
std::vector<talent_translation_t>& shaman_t::get_talent_list()
{
    ...
    talent_translation_t translation_table[][MAX_TALENT_TREES] =
  {
    ...
    { {  4, 2, &( talents.call_of_flame         ) }, {  4, 3, &( talents.elemental_devastation     ) }, {  4, 0, NULL                                  } },
    { {  5, 0, NULL                               }, {  5, 3, &( talents.flurry                    ) }, {  5, 2, &( talents.totemic_focus            ) } },
    { {  6, 2, &( talents.reverberation         ) }, {  6, 0, NULL                                   }, {  6, 0, NULL                                  } },
    { {  7, 3, &( talents.elemental_precision   ) }, {  7, 0, NULL                                   }, {  7, 0, NULL                                  } },
...
std::vector<option_t>& shaman_t::get_options()
{
  if ( options.empty() )
  {
    ...
    option_t shaman_options[] =
    {
      ...
      { "focused_strikes",           OPT_INT,  &( talents.focused_strikes           ) },
      ...
      { "lava_surge",                OPT_INT,  &( talents.lava_surge                ) },
      ...
      { "telluric_currents",         OPT_INT,  &( talents.telluric_currents         ) },
