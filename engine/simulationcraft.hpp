...
struct item_database_t
{
  ...
  static bool     download_glyph(     player_t* player, std::string& glyph_name, const std::string& glyph_id );
  static gem_e parse_gem(        item_t& item, const std::string& gem_id );
...
public:
  ...
  // FIXME: For now we're adding the pet's own player_t::composite_attack_haste() so that it'll benefit from things like 
  ...
  virtual double composite_attack_haste()
  { return player_t::composite_attack_haste() + std::max( owner -> attack_haste, owner -> spell_haste ); }

  virtual double composite_spell_haste()
  { return player_t::composite_spell_haste() + std::max( owner -> attack_haste, owner -> spell_haste ); }
