...
struct dancing_rune_weapon_pet_t : public pet_t
{
  ...
  struct drw_blood_plague_t : public spell_t
  {
      ...
      spell_t( "blood_plague", player, RESOURCE_NONE, SCHOOL_SHADOW, TREE_UNHOLY )
    {
      ...
      trigger_gcd       = 0;
      base_tick_time    = 3.0;
      num_ticks         = 5 + 4 * o -> talents.epidemic -> rank() / 3;
      base_attack_power_multiplier = 1;
  ...
  struct drw_blood_strike_t : public attack_t
  {
    ...
    virtual void execute()
    {
      ...
      {
        if ( p -> sim -> roll( o -> talents.sudden_doom -> rank() * 0.05 ) ) p -> drw_death_coil -> execute();
      }
      roll_sd = false;
    }
    virtual bool ready() { return false; }
...
void death_knight_attack_t::player_buff()
{
  ...
  if ( weapon )
  {
    ...
    {
      player_multiplier *= 1.0 + p -> talents.nerves_of_cold_steel -> rank() * 0.25/3.0;
    }

    if ( weapon -> group() == WEAPON_1H )
    {
      player_hit += p -> talents.nerves_of_cold_steel -> rank()  * 0.01;
    }
  }
...
struct blood_plague_t : public death_knight_spell_t
{
    ...
    death_knight_spell_t( "blood_plague", player, SCHOOL_SHADOW, TREE_UNHOLY )
  {
    ...
    base_execute_time = 0;
    base_tick_time    = 3.0;
    num_ticks         = 5 + 4 * p -> talents.epidemic -> rank() / 3;
...
static bool trigger_sudden_doom( action_t* a )
{
  ...
  if ( ! p -> sim -> roll( p -> talents.sudden_doom -> rank() * 0.05 ) ) return false;
...
struct blood_strike_t : public death_knight_attack_t
{
    ...
    death_knight_attack_t( "blood_strike", player, SCHOOL_PHYSICAL, TREE_BLOOD )
  {
    ...
    cost_blood = 1;
    convert_runes = p -> passives.reaping->ok();
...
struct frost_fever_t : public death_knight_spell_t
{
  ...
  virtual void execute()
  {
    ...
    target_t* t = sim -> target;
    added_ticks = 0;
    num_ticks = 4 * p -> talents.epidemic -> rank() / 3;
...
struct frost_strike_t : public death_knight_attack_t
{
  ...
  virtual void player_buff()
  {
    ...
    if ( sim -> target -> health_percentage() < 35 )
      player_multiplier *= 1 + 0.06 * p -> talents.merciless_combat -> rank();
  }
};
...
struct icy_touch_t : public death_knight_spell_t
{
    ...
    death_knight_spell_t( "icy_touch", player, SCHOOL_FROST, TREE_FROST )
  {
    ...
    base_crit += p -> talents.rime -> rank() * 0.05;
  }
...
struct obliterate_t : public death_knight_attack_t
{
    ...
    death_knight_attack_t( "obliterate", player, SCHOOL_PHYSICAL, TREE_FROST )
  {
    ...
    base_multiplier *= 1.0 + p -> set_bonus.tier10_2pc_melee() * 0.1 + 0.15 * p -> talents.annihilation -> rank();
    base_crit += p -> talents.rime -> rank() * 0.05;
  }
...
struct festering_strike_t : public death_knight_attack_t
{
    ...
    death_knight_attack_t( "festering_strike", player, SCHOOL_PHYSICAL, TREE_UNHOLY )
  {
    ...
    base_multiplier *= 1.0 + p -> talents.rage_of_rivendare -> rank() * 0.15;
...
struct scourge_strike_t : public death_knight_attack_t
{
    ...
    death_knight_attack_t( "scourge_strike", player, SCHOOL_PHYSICAL, TREE_UNHOLY )
  {
    ...
    base_multiplier *= 1.0 + p -> set_bonus.tier10_2pc_melee() * 0.1 + p -> talents.rage_of_rivendare -> rank() * 0.15;
  }
...
double death_knight_t::composite_attack_haste() SC_CONST
{
  ...
  if ( active_presence == PRESENCE_UNHOLY )
    haste *= 1.0 + 2.0 * talents.improved_unholy_presence -> rank();
...
void death_knight_t::init_base()
{
  ...
  resource_base[ RESOURCE_RUNIC ]  = 100 + talents.runic_power_mastery -> rank() * 10;
...
void death_knight_t::init_talents()
{
  ...
  talents.might_of_the_frozen_wastes  = new talent_t( this, "might_of_the_frozen_wastes", "Might of Frozen Wastes" );
...
void death_knight_t::init_spells()
{
  ...
  spells.festering_strike            = new active_spell_t( this, "festering_strike", "Festering Strke" );
  ...
  spells.blood_plague->init_enabled();
  spells.death_coil->init_enabled();
  spells.festering_strike->init_enabled();
  spells.frost_fever->init_enabled();
  spells.frost_strike->init_enabled();
  spells.heart_strike->init_enabled();
  spells.icy_touch->init_enabled();
  spells.plague_strike->init_enabled();
  spells.scourge_strike->init_enabled();
  ...
  passives.unholy_might->init_enabled();
  passives.reaping->init_enabled();
  passives.master_of_ghouls->init_enabled();
}
...
double death_knight_t::composite_spell_hit() SC_CONST
{
  ...
  hit += 0.02 * talents.virulence -> rank();
  return hit;
}
