...
std::string class_color( player_type_e type )
{
  switch ( type )
  {
...
std::string class_text_color( player_type_e type )
{
  switch ( type )
  {
...
player_type_e get_player_or_owner_type( player_t* p )
{
...
std::string school_color( school_type_e type )
{
  switch ( type )
  {
                        ...
                        bool dps )
{
  ...
  while ( true )
  {
    ...
    for ( size_t i = 0; i < num_players; i++ )
    {
      ...
      snprintf( buffer, sizeof( buffer ), "%st++%.0f++%s,%s,%lu,0,15", ( i?"|":"" ), dps ? p -> dps.mean : p -> hps.mean, formatted_name.c_str(), get_text_color( p ).c_str(), i ); s += buffer;
    }
    s += "&amp;";
    if ( first )
    {
                         ...
                         const sim_t* sim )
{
  ...
  for ( size_t chart = 0; chart < max_charts; chart++ )
  {
    ...
    for ( size_t i = 0; i < num_stats; i++ )
    {
      ...
      snprintf( buffer, sizeof( buffer ), "%st++%.0f++%s+(%s),%s,%lu,0,10", ( i?"|":"" ),
...
std::string chart::scale_factors( const player_t* p )
{
  ...
  for ( size_t i = 0; i < num_scaling_stats; i++ )
  {
    ...
    snprintf( buffer, sizeof( buffer ), "%st++++%.*f++%s,%s,0,%lu,15,0.1", ( i?"|":"" ),
...
std::string chart::timeline_dps_error( const player_t* p )
{
  ...
  for ( unsigned i = 1; i <= 5; i++ )
  {
    ...
    if ( i > 1 ) s += "|";
    snprintf( buffer, sizeof( buffer ), "t%.1f,FFFFFF,0,%lu,10", p -> dps_convergence_error[ j ], j / increment ); s += buffer;

  }
  s += "&amp;";
  snprintf( buffer, sizeof( buffer ), "chxl=0:|0|iterations=%lu|1:|0|max dps error=%.0f", max_buckets + 1, dps_max_error ); s += buffer;
  s += "&amp;";
  s += "chdl=DPS Error";
  s += "&amp;";
