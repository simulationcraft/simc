...
std::string chart_resource_type_string( resource_type_e type )
{
  switch ( type )
  {
                       ...
                       bool dps )
{
  ...
  while ( true )
  {
    ...
    for ( int i=0; i < num_players; i++ )
    {
      ...
      snprintf( buffer, sizeof( buffer ), "%st++%.0f++%s,%s,%d,0,15", ( i?"|":"" ), dps ? p -> dps.mean : p -> hps.mean, formatted_name.c_str(), get_text_color( p ).c_str(), i ); s += buffer;
    }
    s += "&amp;";
    if ( first )
    {
