...
void print_text_core_stats( FILE* file, player_t* p )
{
  util::fprintf( file,
                 ...
                 p -> buffed.attribute[ ATTR_AGILITY   ], p -> agility(),   p -> total_gear.get_stat( STAT_AGILITY   ),
...
void print_text_attack_stats( FILE* file, player_t* p )
{
  if ( p -> dual_wield() )
    util::fprintf( file,
                   ...
                   p -> buffed.attack_power, p -> composite_melee_attack_power() * p -> composite_attack_power_multiplier(), p -> total_gear.attack_power,
                   100 * p -> buffed.attack_hit,         100 * p -> composite_melee_hit(),         p -> total_gear.hit_rating,
                   100 * p -> buffed.attack_crit,        100 * p -> composite_melee_crit(),        p -> total_gear.crit_rating,
                   ...
                   100 * ( 1 / p -> buffed.attack_haste - 1 ), 100 * ( 1 / p -> composite_melee_haste() - 1 ), p -> total_gear.haste_rating,
                   100 * ( 1 / p -> buffed.attack_speed - 1 ), 100 * ( 1 / p -> composite_melee_speed() - 1 ), p -> total_gear.haste_rating );
  else
    util::fprintf( file,
...
void print_text_defense_stats( FILE* file, player_t* p )
{
  util::fprintf( file,
                 ...
                 p -> buffed.armor,       p -> composite_armor(), p -> total_gear.armor,
                 100 * p -> buffed.miss,  100 * ( p -> composite_tank_miss( SCHOOL_PHYSICAL ) ),
                 100 * p -> buffed.dodge, 100 * ( p -> composite_tank_dodge() ), p -> total_gear.dodge_rating,
                 100 * p -> buffed.parry, 100 * ( p -> composite_tank_parry() ), p -> total_gear.parry_rating,
                 100 * p -> buffed.block, 100 * p -> composite_tank_block(), p -> total_gear.block_rating,
