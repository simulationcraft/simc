...
void print_text_buffs( FILE* file, player_processed_report_information_t& ri )
{
  ...
  for ( size_t i = 0; i < ri.dynamic_buffs.size(); i++ )
  {
    ...
    util::fprintf( file, "    %-*s : start=%4.1f refresh=%5.1f interval=%5.1f trigger=%5.1f uptime=%5.2f%%",
...
void print_text_performance( FILE* file, sim_t* sim )
{
  ...
  char date_str[sizeof "2011-10-08 07:07:09"];
  std::strftime(date_str, sizeof date_str, "%Y-%m-%d %H:%M:%S", std::localtime(&cur_time));
  util::fprintf( file,
  ...
  for ( size_t i = 0; i < sim -> event_mgr.event_queue_depth_samples.size(); ++i )
  {
    ...
    util::fprintf( file, "Depth: %-4u Samples: %-7u (%.3f%% / %.3f%%)\n",
        i, sim -> event_mgr.event_queue_depth_samples[ i ].first, p, p2 );
...
void print_text_player( FILE* file, player_t* p )
{
  ...
  if ( ! p -> artifact_str.empty() ) util::fprintf( file, "  Artifact: %s\n", p -> artifact_str.c_str() );
