...
void paladin_t::generate_action_prio_list_ret()
{
  ...
  for ( int i = 0; i < num_items; i++ )
  {
    ...
    {
      std::string item_str;
      item_str = "use_item,name=" + items[i].name_str + ",if=(buff.avenging_wrath.up|buff.crusade.up&buff.crusade.remains<=10)|buff.crusade.down&cooldown.crusade.remains>20";
  ...
  for ( size_t i = 0; i < racial_actions.size(); i++ )
  {
    ...
    {
      def -> add_action( "arcane_torrent,if=holy_power<5&(buff.crusade.up|buff.avenging_wrath.up)" );
    }
    else
    {
      def -> add_action( racial_actions[ i ] );
  ...
  def -> add_action( this, "Judgment", "if=holy_power>=3|((cooldown.zeal.charges_fractional<=1.67|cooldown.crusader_strike.charges_fractional<=1.67)&(cooldown.divine_hammer.remains>gcd|cooldown.blade_of_justice.remains>gcd))|(talent.greater_judgment.enabled&target.health.pct>50)" );
