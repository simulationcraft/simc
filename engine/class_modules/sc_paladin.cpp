...
struct beacon_of_light_t : public paladin_heal_t
{
    ...
    // Phillipuh to-do: Beacon of Faith talent
    // Phillipuh to-do: Beacon of Insight talent
...
struct holy_radiance_t : public paladin_heal_t
{
    ...
    // Phillipuh to-do: Theck says it needs a rework. I should probably rework it.
...
struct holy_shock_damage_t : public paladin_spell_t
{
    ...
    virtual void execute()
    {
        ...
        if ( result_is_hit( execute_state -> result ) )
        {
            ...
            //Phillipuh to-do. Seems to be implemented correctly, but it JUST WON'T WORK.
            if ( p() -> glyphs.illumination -> ok())
            {
                int g = p() -> find_spell(54937) -> effectN(1).percent();
                p() -> resource_gain(RESOURCE_MANA, g * p() -> resources.max[RESOURCE_MANA], p() -> gains.glyph_of_illumination);
...
struct holy_shock_heal_t : public paladin_heal_t
{
        ...
        crit_increase( 0.0 )
    {
        ...
        // Phillipuh to-do: double check Theck's implementation, should be 200% * crit rating
        crit_chance_multiplier = p -> find_class_spell( "Holy Shock" ) -> effectN( 1 ).base_value() / 10.0;
...
void paladin_t::create_buffs()
{
    ...
    // Phillipuh to-do: Holy pally tier set, yeah!
...
void paladin_t::generate_action_prio_list_holy()
{
    ...
    // Beacon probably goes somewhere here? You're damn right it does, Theckie-poo.
