...
void devastation( player_t* p )
{
  ...
  precombat->add_action( "variable,name=dr_prep_time_st,default=10,op=reset", "Variable for when to start holding empowers for upcoming DR in ST." );
  ...
  default_->add_action( "variable,name=next_dragonrage,value=cooldown.dragonrage.remains<?(cooldown.eternity_surge.remains-4)<?(cooldown.fire_breath.remains-4)", "Variable that evaluates when next dragonrage is by working out the maximum between the dragonrage cd and your empowers, ignoring CDR effect estimates." );
  ...
  aoe->add_action( "firestorm,if=!buff.dragonrage.up&debuff.shattering_star_debuff.down&talent.feed_the_flames&((!talent.dragonrage|cooldown.dragonrage.remains>=10)&(essence>=3|buff.essence_burst.up|talent.shattering_star&cooldown.shattering_star.remains<=6)|talent.dragonrage&cooldown.dragonrage.remains<=cast_time&cooldown.fire_breath.remains<6&cooldown.eternity_surge.remains<12)&!debuff.in_firestorm.up", "Hard cast only outside of SS and DR windows" );
  aoe->add_action( "deep_breath,if=talent.maneuverability&talent.melt_armor&(talent.onyx_legacy|cooldown.dragonrage.remains<=2&((cooldown.fire_breath.remains<6|cooldown.eternity_surge.remains<6&(!set_bonus.tww1_4pc|!talent.mass_disintegrate))&(cooldown.fire_breath.remains<10&(cooldown.eternity_surge.remains<10|set_bonus.tww1_4pc&talent.mass_disintegrate))&(cooldown.deep_breath.remains>10|!talent.melt_armor)&target.time_to_die>=32|fight_remains<32))" );
  ...
  aoe->add_action( "firestorm,if=talent.feed_the_flames&(cooldown.dragonrage.remains>=20|cooldown.dragonrage.remains<=10)&(buff.essence_burst.up|essence>=2|cooldown.dragonrage.remains<=10)|buff.snapfire.up", "Cast Firestorm before spending ressources" );
  aoe->add_action( "disintegrate,target_if=min:debuff.bombardments.remains,if=buff.mass_disintegrate_stacks.up&talent.mass_disintegrate" );
  ...
  fb->add_action( "fire_breath,empower_to=1,target_if=max:target.health.pct,if=(buff.dragonrage.remains<1.75*spell_haste&buff.dragonrage.remains>=1*spell_haste)|active_enemies=1", "Fire Breath, use rank appropriate to target count/talents." );
  fb->add_action( "fire_breath,empower_to=2,target_if=max:target.health.pct,if=active_enemies=2|(buff.dragonrage.remains<2.5*spell_haste&buff.dragonrage.remains>=1.75*spell_haste)" );
  fb->add_action( "fire_breath,empower_to=3,target_if=max:target.health.pct,if=!talent.font_of_magic|(buff.dragonrage.remains<=3.25*spell_haste&buff.dragonrage.remains>=2.5*spell_haste)" );
  ...
  st->add_action( "deep_breath,if=talent.maneuverability&talent.melt_armor&(talent.onyx_legacy|cooldown.dragonrage.remains<=2&((cooldown.fire_breath.remains<6|cooldown.eternity_surge.remains<6&(!set_bonus.tww1_4pc|!talent.mass_disintegrate))&(cooldown.fire_breath.remains<10&(cooldown.eternity_surge.remains<10|set_bonus.tww1_4pc&talent.mass_disintegrate))&(cooldown.deep_breath.remains>10|!talent.melt_armor)&target.time_to_die>=32|fight_remains<32))" );
  st->add_action( "dragonrage,if=(cooldown.fire_breath.remains<4|cooldown.eternity_surge.remains<4&(!set_bonus.tww1_4pc|!talent.mass_disintegrate))&(cooldown.fire_breath.remains<8&(cooldown.eternity_surge.remains<8|set_bonus.tww1_4pc&talent.mass_disintegrate))&target.time_to_die>=32|fight_remains<32", "Relaxed Dragonrage Entry Requirements, cannot reliably reach third extend under normal conditions (Bloodlust + Power Infusion/Very high haste gear) DS optimization: Only cast if current fight will last 32s+ or encounter ends in less than 30s" );
  ...
  st->add_action( "variable,name=bombardment_clause,value=(!talent.bombardments|talent.extended_battle|debuff.bombardments.remains<=7&!buff.mass_disintegrate_stacks.up|buff.dragonrage.up),if=talent.bombardments" );
  st->add_action( "call_action_list,name=fb,if=(!talent.dragonrage|variable.next_dragonrage>variable.dr_prep_time_st|!talent.animosity)&(buff.blazing_shards.remains<variable.r1_cast_time|buff.dragonrage.up)&(!cooldown.eternity_surge.up|!talent.event_horizon&!talent.traveling_flame|!buff.dragonrage.up)&(target.time_to_die>=8|fight_remains<30)&(!talent.bombardments|variable.bombardment_clause|!dot.fire_breath_damage.ticking)", "Fire breath logic. Play around blazing shards if outside of DR. DS optimization: Only cast if current fight will last 8s+ or encounter ends in less than 30s" );
  st->add_action( "call_action_list,name=es,if=(!talent.dragonrage|variable.next_dragonrage>variable.dr_prep_time_st|!talent.animosity|set_bonus.tww1_4pc&talent.mass_disintegrate)&(buff.blazing_shards.remains<variable.r1_cast_time|buff.dragonrage.up)&(target.time_to_die>=8|fight_remains<30)&(!talent.bombardments|variable.bombardment_clause|set_bonus.tww1_4pc)", "Eternity Surge logic. Play around blazing shards if outside of DR. DS optimization: Only cast if current fight will last 8s+ or encounter ends in less than 30s" );
  ...
  trinkets->add_action( "use_item,name=spymasters_web,if=buff.spymasters_report.stack=1&buff.dragonrage.up&!buff.spymasters_web.up|buff.dragonrage.up&(fight_remains<130)|(fight_remains<=20|cooldown.engulf.up&talent.engulf&fight_remains<=40&cooldown.dragonrage.remains>=40)" );
