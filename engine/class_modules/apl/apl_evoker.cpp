...
void devastation( player_t* p )
{
  ...
  aoe->add_action( "call_action_list,name=es,if=buff.dragonrage.up|!talent.dragonrage|(cooldown.dragonrage.remains>variable.dr_prep_time_aoe&buff.blazing_shards.remains<variable.r1_cast_time)&(target.time_to_die>=8|fight_remains<30)", "Cast Eternity Surge  DS optimization: Only cast if current fight will last 8s+ or encounter ends in less than 30s" );
  ...
  aoe->add_action( "pyre,if=talent.volatility&active_enemies>=3", "Pyre logic with volatility.  Pyre on 3T+" );
  aoe->add_action( "living_flame,if=buff.burnout.up&buff.leaping_flames.up&!buff.essence_burst.up&essence<essence.max-1", "Cast LF with burnout proc and leaping flames up as long as you have 0 EB" );
