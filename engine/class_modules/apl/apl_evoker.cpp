...
void devastation( player_t* p )
{
  ...
  aoe->add_action( "shattering_star,target_if=max:target.health.pct,if=cooldown.dragonrage.up&talent.arcane_vigor|talent.eternitys_span&active_enemies<=3", "AOE action list; This is kind of a mess again and should prolly be rewritten completely Open with star before DR to save a global and start with a free EB" );
  ...
  aoe->add_action( "pyre,target_if=max:target.health.pct,if=active_enemies>=4&!variable.pool_for_id", "Pyre logic Pyre 4T+ Pyre 3T+ if playing Volatility Pyre with 15 stacks of CB" );
  aoe->add_action( "pyre,target_if=max:target.health.pct,if=active_enemies>=3&talent.volatility&!variable.pool_for_id" );
  ...
  trinkets->add_action( "use_item,name=nymues_unraveling_spindle,if=(cooldown.dragonrage.remains<=3&(cooldown.fire_breath.remains<4|cooldown.eternity_surge.remains<4)&(cooldown.fire_breath.remains<8&cooldown.eternity_surge.remains<8)&target.time_to_die>=32)|cooldown.dragonrage.remains<=3&active_enemies>=3|fight_remains<=20", "Nymues is used before Dragonrage unless you have PI and Event Horizon, then it is used on 2 Stacks of Emerald Trance. In AoE it is used before DR." );
