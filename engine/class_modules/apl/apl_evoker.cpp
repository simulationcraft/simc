...
void devastation( player_t* p )
{
  ...
  action_priority_list_t* st_opener = p->get_action_priority_list( "st_opener" );
  ...
  default_->add_action( "run_action_list,name=st_opener,if=time<9&cooldown.eternity_surge.up&active_enemies<=2&(cooldown.invoke_power_infusion_0.up|!set_bonus.tier30_4pc)", "Go into ST opener on 1-2T if you don't have PI or T30" );
  default_->add_action( "run_action_list,name=aoe,if=active_enemies>=3" );
  default_->add_action( "run_action_list,name=st" );

  st_opener->add_action( "dragonrage", "Opener rotaion" );
  st_opener->add_action( "call_action_list,name=fb" );
  st_opener->add_action( "tip_the_scales" );
  st_opener->add_action( "shattering_star" );
  st_opener->add_action( "living_flame,if=essence<3&!buff.essence_burst.up&buff.power_infusion.up" );
  st_opener->add_action( "call_action_list,name=es,if=!buff.power_swell.up" );
  st_opener->add_action( "disintegrate,chain=1,early_chain_if=evoker.use_early_chaining&buff.dragonrage.up&ticks>=2,interrupt_if=evoker.use_clipping&(buff.power_swell.remains<gcd.max|(essence<3&!buff.essence_burst.up))&ticks>=2" );

  aoe->add_action( "shattering_star,if=cooldown.dragonrage.up", "AOE action list; Actually kinda clean now  Open with star before DR to save a global and start with a free EB" );
  ...
  st->add_action( "verdant_embrace,if=talent.ancient_flame&!buff.dragonrage.up", "Fallback for movement" );
