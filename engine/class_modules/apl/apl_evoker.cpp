...
void augmentation( player_t* p )
{
  ...
  precombat->add_action( "variable,name=opener_cds_detected,op=reset,default=0", "actions.precombat+=/variable,name=opener_delay,op=set,value=0,if=equipped.nymues_unraveling_spindle" );
  precombat->add_action( "variable,name=trinket_1_exclude,value=trinket.1.is.ruby_whelp_shell|trinket.1.is.whispering_incarnate_icon|trinket.1.is.umbrelskuls_fractured_heart|trinket.1.is.neltharions_call_to_chaos|trinket.1.is.vessel_of_searing_shadow|trinket.1.is.screaming_black_dragonscale" );
  precombat->add_action( "variable,name=trinket_2_exclude,value=trinket.2.is.ruby_whelp_shell|trinket.2.is.whispering_incarnate_icon|trinket.2.is.umbrelskuls_fractured_heart|trinket.2.is.neltharions_call_to_chaos|trinket.2.is.vessel_of_searing_shadow|trinket.2.is.screaming_black_dragonscale" );
  ...
  precombat->add_action( "variable,name=trinket_1_buffs,value=trinket.1.has_use_buff&(trinket.1.has_buff.intellect|trinket.1.has_buff.mastery|trinket.1.has_buff.versatility|trinket.1.has_buff.haste|trinket.1.has_buff.crit)&!variable.trinket_1_exclude|trinket.1.is.nymues_unraveling_spindle" );
  precombat->add_action( "variable,name=trinket_2_buffs,value=trinket.2.has_use_buff&(trinket.2.has_buff.intellect|trinket.2.has_buff.mastery|trinket.2.has_buff.versatility|trinket.2.has_buff.haste|trinket.2.has_buff.crit)&!variable.trinket_2_exclude|trinket.2.is.nymues_unraveling_spindle" );
  ...
  precombat->add_action( "variable,name=damage_trinket_priority,op=setif,value=2,value_else=1,condition=(!variable.trinket_1_buffs|variable.trinket_1_exclude|trinket.2.ilvl>=trinket.1.ilvl)&!variable.trinket_2_buffs" );
  ...
  items->add_action( "azure_strike,if=cooldown.item_cd_1141.up&(variable.trinket_1_ogcd_cast&trinket.1.cooldown.up&(variable.damage_trinket_priority=1&(!variable.trinket_2_buffs|variable.trinket_2_exclude)|trinket.2.cooldown.remains)|variable.trinket_2_ogcd_cast&trinket.2.cooldown.up&(variable.damage_trinket_priority=2&(!variable.trinket_1_buffs|variable.trinket_1_exclude)|trinket.1.cooldown.remains))", "Azure Strike for OGCD trinkets. Ideally this would be Prescience casts in reality but this is simpler and seems to have no noticeable diferrence in DPS." );
  items->add_action( "use_item,use_off_gcd=1,slot=trinket1,if=!variable.trinket_1_buffs&!variable.trinket_1_manual&(variable.damage_trinket_priority=1&(!variable.trinket_2_buffs|variable.trinket_2_exclude)|trinket.2.cooldown.remains|trinket.2.cooldown.duration=0)&gcd.remains>0.1&variable.trinket_1_ogcd_cast", "If only one on use trinket provides a buff, use the other on cooldown. Or if neither trinket provides a buff, use both on cooldown." );
  items->add_action( "use_item,use_off_gcd=1,slot=trinket2,if=!variable.trinket_2_buffs&!variable.trinket_2_manual&(variable.damage_trinket_priority=2&(!variable.trinket_1_buffs|variable.trinket_1_exclude)|trinket.1.cooldown.remains|trinket.1.cooldown.duration=0)&gcd.remains>0.1&variable.trinket_2_ogcd_cast" );
  items->add_action( "use_item,slot=trinket1,if=!variable.trinket_1_buffs&!variable.trinket_1_manual&(variable.damage_trinket_priority=1&(!variable.trinket_2_buffs|variable.trinket_2_exclude)|trinket.2.cooldown.remains|trinket.2.cooldown.duration=0)&!variable.trinket_1_ogcd_cast", "Use non OGCD trinkets." );
  items->add_action( "use_item,slot=trinket2,if=!variable.trinket_2_buffs&!variable.trinket_2_manual&(variable.damage_trinket_priority=2&(!variable.trinket_1_buffs|variable.trinket_1_exclude)|trinket.1.cooldown.remains|trinket.1.cooldown.duration=0)&!variable.trinket_2_ogcd_cast" );
  ...
  default_->add_action( "emerald_blossom,if=talent.dream_of_spring&buff.essence_burst.up&(variable.spam_heal=2|variable.spam_heal=1&!buff.ancient_flame.up)&(buff.ebon_might_self.up|essence.deficit=0|buff.essence_burst.stack=buff.essence_burst.max_stack&cooldown.ebon_might.remains>4)&((buff.trembling_earth.stack+evoker.prescience_buffs)<=5|essence>=2)" );
  ...
  default_->add_action( "verdant_embrace,if=!buff.ebon_might_self.up&talent.ancient_flame&talent.scarlet_adaptation&!buff.ancient_flame.up&(!talent.dream_of_spring|mana>=100000)&active_enemies=1" );
  ...
  filler->add_action( "living_flame,if=(buff.ancient_flame.up|mana>=100000|!talent.dream_of_spring|variable.spam_heal=0)&(active_enemies=1|talent.pupil_of_alexstrasza|buff.ancient_flame.up)" );
