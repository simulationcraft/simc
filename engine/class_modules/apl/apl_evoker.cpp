...
void augmentation( player_t* p )
{
  ...
  default_->add_action( "call_action_list,name=fb,if=(raid_event.adds.remains>13|raid_event.adds.in>20|evoker.allied_cds_up>0|!raid_event.adds.exists)&(variable.eons_remains>=variable.hold_empower_for|!talent.breath_of_eons|variable.eons_remains=0)&(!talent.overlord|active_enemies=1|talent.breath_of_eons|cooldown.deep_breath.remains<=gcd.max|cooldown.deep_breath.remains>=25)" );
  default_->add_action( "deep_breath,if=talent.overlord&buff.mass_eruption_stacks.up|!talent.overlord|!talent.mass_eruption|active_enemies=1" );
