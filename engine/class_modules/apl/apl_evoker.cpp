...
void devastation( player_t* p )
{
  ...
  trinkets->add_action( "use_item,name=spoils_of_neltharus,if=buff.dragonrage.up&(!cooldown.fire_breath.up&!cooldown.shattering_star.up|active_enemies>=3)|fight_remains<=20", "Use spoils along with Dragon Rage when both FB & SS is on CD on <=2T or immediately on >=3T" );
