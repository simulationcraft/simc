...
{
  return ( p->true_level > 60 ) ? "phial_of_elemental_chaos_3" : "greater_flask_of_endless_fathoms";
}
...
void devastation( player_t* p )
{
  ...
  aoe->add_action( "deep_breath,if=talent.imminent_destruction&cooldown.fire_breath.remains<=gcd.max*7&cooldown.eternity_surge.remains<gcd.max*7", "AOE action list; less of a mess." );
  aoe->add_action( "dragonrage,if=cooldown.fire_breath.remains<=gcd.max&cooldown.eternity_surge.remains<3*gcd.max" );
  aoe->add_action( "tip_the_scales,if=buff.dragonrage.up&(spell_targets.pyre<=6|!cooldown.fire_breath.up)" );
  aoe->add_action( "fire_breath,empower_to=1,if=buff.dragonrage.remains<1.75*spell_haste&buff.dragonrage.remains>=1*spell_haste|cooldown.dragonrage.remains>10&(spell_targets.pyre>=8|spell_targets.pyre<=3)&buff.dragonrage.up&buff.dragonrage.remains>=10|buff.dragonrage.up&spell_targets.pyre<=3&!talent.raging_inferno&talent.catalyze" );
  aoe->add_action( "fire_breath,empower_to=2,if=buff.dragonrage.remains<2.5*spell_haste&buff.dragonrage.remains>=1.75*spell_haste" );
  aoe->add_action( "fire_breath,empower_to=3,if=(!talent.font_of_magic|(spell_targets.pyre==5&!talent.volatility&!talent.charged_blast&talent.catalyze&!talent.raging_inferno))&cooldown.dragonrage.remains>10|buff.dragonrage.remains<=3.25*spell_haste&buff.dragonrage.remains>=2.5*spell_haste" );
  aoe->add_action( "fire_breath,empower_to=4,if=cooldown.dragonrage.remains>10" );
  ...
  aoe->add_action( "deep_breath,if=!buff.dragonrage.up&!talent.imminent_destruction", "Use DB if not in DR and not running imminent destruction." );
