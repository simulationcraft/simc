...
void devastation( player_t* p )
{
  ...
  default_->add_action( "potion,if=(!talent.dragonrage|buff.dragonrage.up)|fight_remains<35", "Delay pot in ST if you are about to SS - mostly relevant for opener where you want DR->FB->SS->rotation" );
  ...
  default_->add_action( "invoke_external_buff,name=power_infusion,if=buff.dragonrage.up|fight_remains<35", "Invoke External Power Infusions if they're available during dragonrage" );
  ...
  aoe->add_action( "hover,use_off_gcd=1,if=raid_event.movement.in<6&!buff.hover.up&gcd.remains>=0.5&(buff.mass_disintegrate_stacks.up&talent.mass_disintegrate|active_enemies<=4)", "29.12.2025 - Fixed for FS  AOE action list. Use Hover for Mass Disintegrates." );
  aoe->add_action( "deep_breath,if=(talent.imminent_destruction|talent.melt_armor|talent.maneuverability)&talent.strafing_run,cancel_if=gcd.remains=0&ticks>=8", "BaumChange 1: Send DB before DR if FS FS FTF - +30k in 5T" );
  aoe->add_action( "fire_breath,target_if=max:target.health.pct,empower_to=1,if=talent.consume_flame&cooldown.dragonrage.up&active_dot.fire_breath_damage=0&(target.time_to_die>=15|!raid_event.adds.exists)", "FS Opener FB - Use FB when Dragonrage ready and FB not running on target" );
  aoe->add_action( "tip_the_scales,use_off_gcd=1,if=(!talent.dragonrage|buff.dragonrage.up)&(talent.consume_flame&cooldown.eternity_surge.remains<=action.fire_breath.usable_in|!talent.consume_flame&action.fire_breath.usable_in<=cooldown.eternity_surge.remains)", "Tip FB when SC and ES when FS" );
  aoe->add_action( "call_action_list,name=es,if=talent.consume_flame&buff.tip_the_scales.up", "Tip ES for FS" );
  aoe->add_action( "fire_breath,target_if=max:target.health.pct,empower_to=2,if=!talent.consume_flame&variable.can_use_empower&dot.fire_breath_damage.refreshable&target.time_to_die>=duration", "R2 funny breath good default." );
  aoe->add_action( "fire_breath,target_if=max:target.health.pct,empower_to=1,if=talent.consume_flame&variable.can_use_empower&dot.fire_breath_damage.refreshable", "Flameshaper Default Firebreath" );
  ...
  aoe->add_action( "deep_breath,if=(talent.imminent_destruction|talent.melt_armor|talent.maneuverability)&(active_dot.fire_breath_damage=0|!talent.consume_flame),cancel_if=gcd.remains=0&ticks>=8,interrupt_if=gcd.remains=0&ticks>=8", "Use Deep Breath if talented ID." );
  aoe->add_action( "azure_sweep,target_if=max:target.health.pct,if=(active_dot.fire_breath_damage=0|!talent.consume_flame)&!buff.essence_burst.at_max_stacks&essence.deficit>=2&set_bonus.mid1_2pc" );
  aoe->add_action( "pyre,target_if=max:target.health.pct,if=(cooldown.dragonrage.remains>gcd.max*4|!talent.dragonrage|!talent.charged_blast)&!buff.mass_disintegrate_stacks.up&(!buff.mass_disintegrate_stacks.up|buff.essence_burst.stack=2|buff.essence_burst.stack=1&essence>=cost)", "Pyre 4T+ - 3T+ with Volatility - 12 stacks of CB - Pool CB for DR" );
  aoe->add_action( "disintegrate,target_if=min:debuff.bombardments.remains,if=buff.mass_disintegrate_stacks.up&talent.mass_disintegrate,early_chain_if=buff.mass_disintegrate_stacks.up&ticks_remain<=1", "Use Mass Disintegrate if CB wont't overcap" );
  aoe->add_action( "azure_sweep,target_if=max:target.health.pct" );
  aoe->add_action( "living_flame,target_if=max:target.health.pct,if=(!talent.burnout|buff.burnout.up|active_dot.fire_breath_damage=0|buff.inner_flame.up|buff.scarlet_adaptation.up|buff.ancient_flame.up)&buff.leaping_flames.up&(!buff.essence_burst.up&essence.deficit>1|cooldown.fire_breath.remains<=gcd.max*3&buff.essence_burst.stack<buff.essence_burst.max_stack)", "Cast LF with leaping flames up if: not playing burnout, burnout is up or the next firebreath is soon." );
  aoe->add_action( "call_action_list,name=es,if=(!talent.dragonrage|buff.dragonrage.up|cooldown.dragonrage.remains>variable.dr_prep_time_aoe|!talent.animosity)&(talent.azure_sweep&!buff.azure_sweep.up)" );
  aoe->add_action( "living_flame,target_if=max:target.health.pct,if=talent.engulfing_blaze", "Get Ancient Flame as Filler actions.aoe+=/call_action_list,name=green,if=talent.ancient_flame&!buff.ancient_flame.up&!buff.dragonrage.up" );
  aoe->add_action( "azure_strike,target_if=max:target.health.pct", "Fallback filler" );

  es->add_action( "eternity_surge,empower_to=1,target_if=max:target.health.pct,if=active_enemies<=1+talent.eternitys_span|talent.mass_disintegrate|buff.dragonrage.up&talent.animosity", "Eternity Surge, use rank most applicable to targets." );
  ...
  es->add_action( "eternity_surge,empower_to=1,target_if=max:target.health.pct,if=talent.azure_sweep" );

  green->add_action( "emerald_blossom", "Green Spells used to trigger Ancient Flame" );
  green->add_action( "verdant_embrace" );

  st->add_action( "deep_breath,if=(talent.imminent_destruction|talent.melt_armor|talent.maneuverability)&talent.strafing_run,cancel_if=gcd.remains=0&ticks>=8&active_enemies=1", "29.12.2025 - Fixed for FS  SC DB" );
  ...
  st->add_action( "tip_the_scales,use_off_gcd=1,if=buff.dragonrage.up&(action.fire_breath.usable_in<=action.eternity_surge.usable_in&!talent.consume_flame|action.eternity_surge.usable_in<=action.fire_breath.usable_in&talent.consume_flame)", "Become a faster dragon (for one empower)" );
  st->add_action( "eternity_surge,target_if=max:target.health.pct,empower_to=2,if=active_enemies=2&!talent.eternitys_span&variable.can_use_empower" );
  st->add_action( "eternity_surge,target_if=max:target.health.pct,empower_to=1,if=(variable.can_use_empower|set_bonus.mid1_2pc&talent.azure_sweep)&!buff.azure_sweep.up" );
  st->add_action( "fire_breath,target_if=max:target.health.pct,empower_to=2,if=variable.can_use_empower&!buff.dragonrage.up&talent.mass_disintegrate&!talent.scorching_embers&!talent.consume_flame", "R2 funny breath good default for SC." );
  st->add_action( "fire_breath,target_if=max:target.health.pct,empower_to=1,if=variable.can_use_empower&(!buff.tip_the_scales.up|!talent.consume_flame)&dot.fire_breath_damage.refreshable&(cooldown.dragonrage.remains>full_recharge_time|!talent.animosity|buff.dragonrage.up|full_recharge_time<gcd.max*5)&(!buff.essence_burst.at_max_stacks|!talent.essence_well)&(cooldown.eternity_surge.remains>=10|full_recharge_time<gcd.max*2)" );
  st->add_action( "deep_breath,if=(talent.imminent_destruction|talent.melt_armor|talent.maneuverability),interrupt_immediate=1,interrupt_if=gcd.remains=0&ticks>=8,cancel_if=gcd.remains=0&ticks>=8", "Use the Deeper Breath" );
  st->add_action( "disintegrate,target_if=min:debuff.bombardments.remains,early_chain_if=ticks_remain<=1&buff.mass_disintegrate_stacks.up,if=(raid_event.movement.in>2|buff.hover.up)&buff.mass_disintegrate_stacks.up&talent.mass_disintegrate", "Mass Disintegrator pov" );
  st->add_action( "pyre,target_if=min:dot.fire_breath_damage.remains-100*dot.fire_breath_damage.ticking,if=active_enemies>1&talent.feed_the_flames&talent.volatility&(dot.fire_breath_damage.remains<=8&talent.consume_flame|!talent.consume_flame)", "FTF Voal Pyre 2t" );
  st->add_action( "disintegrate,target_if=max:dot.fire_breath_damage.remains,chain=1,if=(raid_event.movement.in>2|buff.hover.up),early_chain_if=ticks_remain<=1,interrupt_if=ticks_remain<=1", "Beam Dragon" );
  st->add_action( "azure_sweep" );
  st->add_action( "azure_strike,if=buff.iridescence_red.stack=1&buff.iridescence_red.remains>action.fire_breath.usable_in+1" );
  ...
  st->add_action( "azure_strike,if=active_enemies>=2", "AS" );
  st->add_action( "living_flame,if=raid_event.movement.in>execute_time", "LF" );
  st->add_action( "call_action_list,name=green,if=talent.ancient_flame&!buff.ancient_flame.up&talent.scarlet_adaptation&!buff.dragonrage.up", "Get Ancient Flame for movement" );
