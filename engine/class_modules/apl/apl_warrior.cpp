...
void arms( player_t* p )
{
  ...
  colossus_execute->add_action( "cleave,if=buff.ravager.remains|talent.mass_execution&active_enemies=2" );
  ...
  colossus_st->add_action( "heroic_strike,if=active_enemies=2" );
  colossus_st->add_action( "mortal_strike" );
  colossus_st->add_action( "heroic_strike" );
  colossus_st->add_action( "overpower,if=active_enemies=2" );
  colossus_st->add_action( "cleave,if=buff.ravager.remains|buff.collateral_damage.stack=3" );
  ...
  colossus_st->add_action( "cleave,if=active_enemies=2" );
