...
{
  return ( p -> true_level > 80 ) ? "disabled" : // Disabled for level 90
...
{
  std::string lvl90_temp_enchant = ( p -> specialization() == HUNTER_SURVIVAL ) ? "disabled" : "disabled"; // Disabled for level 90
...
void marksmanship( player_t* p )
{
  ...
  sentst->add_action( "rapid_fire,if=talent.lunar_storm" );
  ...
  sentcleave->add_action( "rapid_fire,if=buff.precise_shots.down|buff.moving_target.up|cooldown.volley.remains&cooldown.trueshot.remains|!talent.volley" );
  ...
  senttrickshots->add_action( "rapid_fire,if=buff.trick_shots.remains>execute_time&(talent.bulletstorm&buff.bulletstorm.down)" );
...
void marksmanship_ptr( player_t* p )
{
  ...
  sentcleave->add_action( "volley,if=!talent.double_tap&(buff.precise_shots.down|buff.moving_target.up)" );
  sentcleave->add_action( "trueshot,if=variable.trueshot_ready&(buff.double_tap.down|!talent.volley)&(buff.lunar_storm_ready.down|!talent.double_tap|!talent.volley)&(buff.precise_shots.down|buff.moving_target.up|!talent.volley)" );
  sentcleave->add_action( "rapid_fire,if=talent.lunar_storm&(buff.precise_shots.down|buff.moving_target.up|cooldown.volley.remains&cooldown.trueshot.remains|!talent.volley)" );
  sentcleave->add_action( "kill_shot,target_if=max:debuff.spotters_mark.down|action.aimed_shot.in_flight_to_target|max_prio_damage,if=talent.headshot&buff.precise_shots.up&(debuff.spotters_mark.down|buff.moving_target.down)|!talent.headshot&buff.razor_fragments.up" );
  sentcleave->add_action( "multishot,target_if=max:debuff.spotters_mark.down|action.aimed_shot.in_flight_to_target|max_prio_damage,if=buff.precise_shots.up&(debuff.spotters_mark.down|buff.moving_target.down)&!talent.aspect_of_the_hydra" );
  sentcleave->add_action( "arcane_shot,target_if=max:debuff.spotters_mark.down|action.aimed_shot.in_flight_to_target|max_prio_damage,if=buff.precise_shots.up&(debuff.spotters_mark.down|buff.moving_target.down)" );
  sentcleave->add_action( "aimed_shot,target_if=max:debuff.spotters_mark.up,if=(buff.precise_shots.down|debuff.spotters_mark.up&buff.moving_target.up)&full_recharge_time<action.rapid_fire.execute_time+cast_time&(!talent.bulletstorm|buff.bulletstorm.up)&talent.windrunner_quiver" );
...
void survival( player_t* p )
{
  ...
  cds->add_action( "blood_fury", "COOLDOWNS ACTIONLIST" );
  cds->add_action( "invoke_external_buff,name=power_infusion" );
  cds->add_action( "harpoon,if=prev.kill_command" );
  cds->add_action( "ancestral_call" );
  cds->add_action( "fireblood" );
  cds->add_action( "berserking,if=time_to_die<13" );
  cds->add_action( "muzzle" );
  cds->add_action( "potion,if=target.time_to_die<25" );
  cds->add_action( "use_item,use_off_gcd=1,slot=trinket1" );
  ...
  plcleave->add_action( "kill_command,if=buff.relentless_primal_ferocity.up&buff.tip_of_the_spear.stack<1", "PACK LEADER | AOE ACTIONLIST" );
  ...
  plcleave->add_action( "raptor_strike,target_if=max:dot.serpent_sting.remains" );
  ...
  plst->add_action( "raptor_strike,target_if=min:dot.serpent_sting.remains,if=!dot.serpent_sting.ticking&target.time_to_die>12" );
  plst->add_action( "kill_command,if=buff.tip_of_the_spear.stack<1" );
  plst->add_action( "raptor_strike,if=buff.tip_of_the_spear.stack>0" );
  plst->add_action( "wildfire_bomb,if=buff.tip_of_the_spear.stack>0" );
  plst->add_action( "raptor_strike,target_if=max:dot.serpent_sting.remains,if=buff.howl_of_the_pack_leader_cooldown.up&buff.howl_of_the_pack_leader_cooldown.remains<2*gcd" );
  ...
  plst->add_action( "raptor_strike,target_if=max:dot.serpent_sting.remains" );
  plst->add_action( "kill_shot" );
  plst->add_action( "explosive_shot" );

  sentcleave->add_action( "wildfire_bomb", "SENTINEL | DEFAULT AOE ACTIONLIST" );
  sentcleave->add_action( "kill_command,if=buff.relentless_primal_ferocity.up&buff.tip_of_the_spear.stack<1" );
  sentcleave->add_action( "wildfire_bomb,if=buff.tip_of_the_spear.stack>0|cooldown.wildfire_bomb.charges_fractional>1.9" );
  sentcleave->add_action( "raptor_strike,target_if=max:dot.serpent_sting.remains" );
  ...
  sentst->add_action( "wildfire_bomb,if=buff.tip_of_the_spear.stack>0", "SENTINEL | DEFAULT SINGLE TARGET ACTIONLIST." );
  ...
  sentst->add_action( "kill_command,if=buff.tip_of_the_spear.stack<1" );
  ...
  sentst->add_action( "raptor_strike,target_if=max:dot.serpent_sting.remains" );

  trinkets->add_action( "variable,name=buff_sync_ready,value=buff.takedown.up", "True if effects that are desirable to sync a trinket buff with are ready." );
  trinkets->add_action( "variable,name=buff_sync_remains,value=cooldown.takedown.remains", "Time until the effects that are desirable to sync a trinket buff with will be ready." );
  trinkets->add_action( "variable,name=buff_sync_active,value=buff.takedown.up", "True if effecs that are desirable to sync a trinket buff with are active." );
...
void survival_ptr( player_t* p )
{
  ...
  sentst->add_action( "wildfire_bomb", "SENTINEL | DEFAULT SINGLE TARGET ACTIONLIST." );
  sentst->add_action( "kill_command,if=(buff.relentless_primal_ferocity.up&buff.tip_of_the_spear.stack<1)" );
  sentst->add_action( "flanking_strike,if=buff.tip_of_the_spear.stack>0" );
  sentst->add_action( "kill_command,if=buff.tip_of_the_spear.stack<1" );
  sentst->add_action( "wildfire_bomb,if=cooldown.wildfire_bomb.charges_fractional>1.7" );
