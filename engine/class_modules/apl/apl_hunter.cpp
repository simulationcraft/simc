...
void marksmanship( player_t* p )
{
  ...
  drst->add_action( "black_arrow,target_if=min:debuff.spotters_mark.remains|max_prio_damage,if=buff.precise_shots.up" );
  drst->add_action( "explosive_shot,target_if=min:dot.explosive_shot.remains&set_bonus.mid2_4pc|max_prio_damage,if=!talent.tactical_reload|!buff.lock_and_load.up" );
  ...
  drst->add_action( "rapid_fire,target_if=min:debuff.spotters_mark.remains&talent.unload|max_prio_damage,interrupt_if=talent.unload&ticks_remain<2&buff.precise_shots.up&!gcd.remains,interrupt_immediate=1,interrupt_global=1" );
  drst->add_action( "wailing_arrow" );
  drst->add_action( "multishot,target_if=min:debuff.spotters_mark.remains|max_prio_damage,if=buff.precise_shots.up&active_enemies>1&talent.aspect_of_the_hydra&(!max_prio_damage|active_enemies>2)", "Multi-Shot as a PS spender on 2 targets with Hydra talented if we don't care about priority damage. Always do it on 3 or more targets." );
  drst->add_action( "arcane_shot,target_if=min:debuff.spotters_mark.remains|max_prio_damage,if=buff.precise_shots.up" );
  drst->add_action( "aimed_shot,target_if=debuff.spotters_mark.up|max_prio_damage" );
  drst->add_action( "black_arrow" );
  drst->add_action( "steady_shot" );

  draoe->add_action( "explosive_shot,target_if=min:dot.explosive_shot.remains&set_bonus.mid2_4pc|max_prio_damage,if=!talent.tactical_reload|!buff.lock_and_load.up" );
  draoe->add_action( "volley" );
  draoe->add_action( "aimed_shot,target_if=debuff.spotters_mark.up|max_prio_damage,if=buff.trick_shots.remains>cast_time&full_recharge_time<gcd+cast_time" );
  draoe->add_action( "black_arrow,target_if=min:debuff.spotters_mark.remains|max_prio_damage,if=buff.precise_shots.up" );
  draoe->add_action( "multishot,target_if=min:debuff.spotters_mark.remains|max_prio_damage,if=buff.trick_shots.down" );
  ...
  draoe->add_action( "multishot,target_if=min:debuff.spotters_mark.remains|max_prio_damage,if=buff.precise_shots.up" );
  draoe->add_action( "aimed_shot,target_if=debuff.spotters_mark.up|max_prio_damage,if=buff.trick_shots.remains>cast_time" );
  ...
  sentaoe->add_action( "explosive_shot,target_if=min:dot.explosive_shot.remains&set_bonus.mid2_4pc|max_prio_damage,if=!talent.tactical_reload|!buff.lock_and_load.up" );
  ...
  sentaoe->add_action( "multishot,target_if=min:debuff.sentinels_mark.remains,if=buff.precise_shots.up|buff.trick_shots.down" );
  sentaoe->add_action( "rapid_fire,target_if=min:debuff.sentinels_mark.remains&talent.unload,interrupt_if=talent.unload&ticks_remain<2&buff.precise_shots.up&!gcd.remains,interrupt_immediate=1,interrupt_global=1" );
  sentaoe->add_action( "aimed_shot,target_if=debuff.sentinels_mark.up|max_prio_damage,if=buff.trick_shots.remains>cast_time" );
  ...
  sentst->add_action( "explosive_shot,target_if=min:dot.explosive_shot.remains&set_bonus.mid2_4pc|max_prio_damage,if=!talent.tactical_reload|!buff.lock_and_load.up" );
  ...
  sentst->add_action( "kill_shot,target_if=min:debuff.sentinels_mark.remains|max_prio_damage,if=buff.precise_shots.up" );
  sentst->add_action( "multishot,target_if=min:debuff.sentinels_mark.remains,if=buff.precise_shots.up&active_enemies>1&talent.aspect_of_the_hydra&(!max_prio_damage|active_enemies>2)", "Multi-Shot as a PS spender on 2 targets with Hydra talented if we don't care about priority damage. Always do it on 3 or more targets." );
  sentst->add_action( "arcane_shot,target_if=min:debuff.sentinels_mark.remains|max_prio_damage,if=buff.precise_shots.up&(buff.trueshot.up&prev_gcd.1.aimed_shot|!buff.trueshot.up)" );
  sentst->add_action( "rapid_fire,target_if=min:debuff.sentinels_mark.remains&talent.unload|max_prio_damage,interrupt_if=talent.unload&ticks_remain<2&buff.precise_shots.up&!buff.trueshot.up&!gcd.remains,interrupt_immediate=1,interrupt_global=1" );
  sentst->add_action( "aimed_shot,target_if=debuff.sentinels_mark.up|max_prio_damage" );
