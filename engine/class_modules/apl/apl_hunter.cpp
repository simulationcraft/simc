...
void survival( player_t* p )
{
  ...
  bop->add_action( "wildfire_bomb,if=focus+cast_regen<focus.max&full_recharge_time<gcd" );
  ...
  cleave->add_action( "wildfire_bomb,if=full_recharge_time<gcd|target.time_to_die<5" );
  ...
  cleave->add_action( "wildfire_bomb,if=!dot.wildfire_bomb.ticking|raid_event.adds.exists&(charges_fractional>1.2&active_enemies>4|charges_fractional>1.4&active_enemies>3|charges_fractional>1.6)|!raid_event.adds.exists&charges_fractional>1.5" );
  ...
  st->add_action( "serpent_sting,target_if=min:remains,if=!dot.serpent_sting.ticking&target.time_to_die>7&!dot.pheromone_bomb.ticking|!dot.serpent_sting.ticking&target.time_to_die>7" );
  st->add_action( "raptor_strike,target_if=max:debuff.latent_poison_injection.stack,if=(buff.tip_of_the_spear.stack=3&!dot.pheromone_bomb.ticking)|next_wi_bomb.pheromone&buff.tip_of_the_spear.stack=3&(cooldown.wildfire_bomb.full_recharge_time<2*gcd)" );
  st->add_action( "flayed_shot" );
  ...
  st->add_action( "serpent_sting,target_if=min:remains,if=refreshable&target.time_to_die>7" );
