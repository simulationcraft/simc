...
void beast_mastery( player_t* p )
{
  ...
  st->add_action( "aspect_of_the_wild,if=!raid_event.adds.exists|!raid_event.adds.up&(raid_event.adds.duration+raid_event.adds.in<20|(raid_event.adds.count=1&covenant.kyrian))|raid_event.adds.up&raid_event.adds.remains>19" );
...
void marksmanship( player_t* p )
{
  ...
  st->add_action( "wild_spirits,if=(cooldown.trueshot.remains<gcd|buff.trueshot.up)&(!raid_event.adds.exists|!raid_event.adds.up&raid_event.adds.duration+raid_event.adds.in<20|raid_event.adds.up&raid_event.adds.remains>19|active_enemies>1)|fight_remains<80" );
  st->add_action( "flayed_shot" );
  ...
  st->add_action( "resonating_arrow,if=(buff.double_tap.up|!talent.double_tap|fight_remains<12)&(!raid_event.adds.exists|!raid_event.adds.up&(raid_event.adds.duration+raid_event.adds.in<10|raid_event.adds.count=1)|raid_event.adds.up&raid_event.adds.remains>9|active_enemies>1)" );
  ...
  st->add_action( "chimaera_shot,if=set_bonus.tier28_4pc&buff.trick_shots.down&buff.precise_shots.stack*20+focused_trickery_count>40&focus>buff.precise_shots.stack*cost+(action.aimed_shot.cost-buff.precise_shots.stack*cast_regen)" );
  st->add_action( "arcane_shot,if=set_bonus.tier28_4pc&buff.trick_shots.down&buff.precise_shots.stack*20+focused_trickery_count>40&focus>buff.precise_shots.stack*cost+(action.aimed_shot.cost-buff.precise_shots.stack*cast_regen)" );
  st->add_action( "trueshot,if=((buff.precise_shots.down|talent.calling_the_shots)&covenant.venthyr|covenant.necrolord|cooldown.resonating_arrow.remains>30|cooldown.resonating_arrow.remains<10|cooldown.wild_spirits.remains>30|buff.wild_spirits.up)|buff.volley.up&active_enemies>1|fight_remains<25" );
  ...
  trickshots->add_action( "wild_spirits,if=!raid_event.adds.exists|raid_event.adds.remains>10|active_enemies>=raid_event.adds.count*2" );
