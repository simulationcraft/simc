...
void arcane( player_t* p )
{
  ...
  sunfury->add_action( "arcane_missiles,if=buff.clearcasting.react&((talent.high_voltage&buff.arcane_charge.stack<4)|buff.nether_precision.down),interrupt_if=tick_time>gcd.remains&(buff.aether_attunement.react=0|(active_enemies>3&(!talent.time_loop|talent.resonance))),interrupt_immediate=1,interrupt_global=1,chain=1", "Missiles to recoup Charges with High Voltage or maintain Nether Precisioin." );
  ...
  sunfury->add_action( "arcane_barrage,if=buff.glorious_incandescence.up|buff.intuition.react", "Barrage with Intuition or Incandescence." );
