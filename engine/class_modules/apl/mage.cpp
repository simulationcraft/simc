...
void arcane( player_t* p )
{
  ...
  spellslinger->add_action( "arcane_barrage,if=(cooldown.arcane_orb.remains<gcd.max&buff.arcane_charge.stack=4&buff.nether_precision.down&(talent.orb_barrage|(talent.high_voltage&buff.clearcasting.react))&(cooldown.touch_of_the_magi.remains>gcd.max*6|!talent.magis_spark))", "Barrage if you have orb ready and either Orb Barrage or High Voltage, pool for Spark." );
...
void fire( player_t* p )
{
  ...
  precombat->add_action( "variable,name=arcane_explosion,if=variable.arcane_explosion=0,value=999" );
  precombat->add_action( "variable,name=arcane_explosion_mana,default=40,op=reset" );
  ...
  default_->add_action( "use_item,name=treacherous_transmitter,if=variable.time_to_combustion<10|fight_remains<25" );
  default_->add_action( "do_treacherous_transmitter_task,use_off_gcd=1,if=buff.combustion.up|fight_remains<20" );
  default_->add_action( "use_item,name=imperfect_ascendancy_serum,if=variable.time_to_combustion<3", "The War Within S1&2 On-Use items with special use timings" );
  ...
  default_->add_action( "call_action_list,name=sf_combustion_phase,if=talent.spellfire_spheres&(variable.time_to_combustion<=0|buff.combustion.up|variable.time_to_combustion<variable.combustion_precast_time&cooldown.combustion.remains<variable.combustion_precast_time)" );
  default_->add_action( "call_action_list,name=ff_combustion_phase,if=talent.frostfire_bolt&(variable.time_to_combustion<=0|buff.combustion.up|variable.time_to_combustion<variable.combustion_precast_time&cooldown.combustion.remains<variable.combustion_precast_time)" );
  ...
  default_->add_action( "call_action_list,name=sf_standard_rotation,if=talent.spellfire_spheres&variable.time_to_combustion>0&buff.combustion.down" );
  default_->add_action( "call_action_list,name=ff_standard_rotation,if=talent.frostfire_bolt&variable.time_to_combustion>0&buff.combustion.down" );
  default_->add_action( "ice_nova,if=!scorch_execute.active" );
  default_->add_action( "scorch,if=buff.combustion.down" );

  active_talents->add_action( "meteor,if=cooldown.combustion.remains<3|buff.combustion.remains>2", "Meteor when it will impact inside of combust" );
  ...
  ff_combustion_phase->add_action( "call_action_list,name=combustion_cooldowns,if=buff.combustion.remains>variable.skb_duration|fight_remains<20", "Other cooldowns that should be used with Combustion should only be used with an actual Combustion cast and not with a Sun King's Blessing proc." );
  ...
  ff_combustion_phase->add_action( "cancel_buff,name=hyperthermia,if=buff.fury_of_the_sun_king.react", "Cancel Hyperthermia if needed" );
  ff_combustion_phase->add_action( "flamestrike,if=active_enemies>=variable.combustion_flamestrike&(buff.hot_streak.react|buff.hyperthermia.react)" );
  ...
  ff_combustion_phase->add_action( "flamestrike,if=buff.fury_of_the_sun_king.up&active_enemies>=variable.skb_flamestrike" );
  ...
  ff_combustion_phase->add_action( "fireball" );
  ...
  ff_standard_rotation->add_action( "flamestrike,if=active_enemies>=variable.skb_flamestrike&prev_gcd.1.scorch&buff.heating_up.react" );
  ...
  sf_combustion_phase->add_action( "phoenix_flames,if=talent.spellfire_spheres&talent.phoenix_reborn&buff.heating_up.react&!buff.hot_streak.react&buff.born_of_flame.up", "Spend empowered Phoenix Flames at highest priority in combustion when talented into Sunfury." );
  ...
  sf_combustion_phase->add_action( "flamestrike,if=buff.fury_of_the_sun_king.up&buff.fury_of_the_sun_king.remains>cast_time&active_enemies>=variable.skb_flamestrike&buff.fury_of_the_sun_king.expiration_delay_remains=0", "Spend Fury of the Sun King procs inside of combustion." );
