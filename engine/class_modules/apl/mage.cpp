...
void arcane( player_t* p )
{
  ...
  default_->add_action( "use_item,name=hex_lords_dooming_idol,if=(buff.hex_lords_doom.stack>(24-(15*equipped.vile_vial_of_volatile_venom))&cooldown.arcane_surge.ready&variable.hex_double_on_use)|(cooldown.arcane_surge.ready&!variable.hex_double_on_use)|fight_remains<30" );
...
void fire( player_t* p )
{
  ...
  fireblast->add_action( "fire_blast,use_off_gcd=1,use_while_casting=1,if=cooldown_react&!buff.hot_streak.react&action.pyroblast.executing&buff.pyroclasm.up&gcd.remains<gcd.max", "Cast Fire Blast during a hardcasted Pyroblast with Pyroclasm. Doing this can result in casting Fire Blast twice during one cast." );
