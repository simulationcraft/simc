...
void fire( player_t* p )
{
  ...
  default_->add_action( "variable,name=refresh_charring_embers,value=(debuff.charring_embers.down|debuff.charring_embers.remains<=3), if=set_bonus.tier30_2pc", "Variable to trigger refresh of tier 30 2pc if the set bonus is active and it the debuff is down or the remaining duration is less than 3 seconds" );
  default_->add_action( "phoenix_flames,if=(variable.refresh_charring_embers&!buff.hot_streak.react)&time<15&!action.phoenix_flames.in_flight)" );
  ...
  default_->add_action( "fire_blast,use_off_gcd=1,use_while_casting=1,if=firestarter.active&variable.time_to_combustion>0&buff.sun_kings_blessing.stack=6&action.phoenix_flames.in_flight+hot_streak_spells_in_flight+buff.heating_up.react<2&time<15&!buff.hot_streak.react&talent.alexstraszas_fury" );
  ...
  active_talents->add_action( "dragons_breath,if=talent.alexstraszas_fury&(buff.combustion.down&!buff.hot_streak.react)&(buff.feel_the_burn.up|time>15)&!firestarter.remains&!talent.tempered_flames" );
  ...
  combustion_phase->add_action( "phoenix_flames,if=buff.combustion.up&travel_time<buff.combustion.remains&buff.heating_up.react+hot_streak_spells_in_flight<2&!action.phoenix_flames.in_flight&(variable.refresh_charring_embers|buff.flames_fury.up)&talent.alexstraszas_fury" );
  ...
  combustion_phase->add_action( "phoenix_flames,if=buff.combustion.up&travel_time<buff.combustion.remains&buff.heating_up.react+hot_streak_spells_in_flight<2&!action.phoenix_flames.in_flight&(variable.refresh_charring_embers|buff.flames_fury.up|variable.extended_combustion_remains<10&!talent.alexstraszas_fury)" );
  ...
  rop_phase->add_action( "phoenix_flames,if=buff.combustion.up&travel_time<buff.combustion.remains&buff.heating_up.react+hot_streak_spells_in_flight<2&!action.phoenix_flames.in_flight&(variable.refresh_charring_embers|buff.flames_fury.up)" );
  ...
  rop_phase->add_action( "phoenix_flames,if=!variable.phoenix_pooling&(!hot_streak_spells_in_flight&!action.phoenix_flames.in_flight&(buff.heating_up.react|charges>1))&(charges>1|debuff.charring_embers.remains<3|buff.flames_fury.up)&time>15" );
  rop_phase->add_action( "phoenix_flames,if=(variable.refresh_charring_embers|buff.flames_fury.up)&(!hot_streak_spells_in_flight&!action.phoenix_flames.in_flight&(buff.heating_up.react|charges>1))&time>15&!buff.hot_streak.react&talent.alexstraszas_fury" );
  rop_phase->add_action( "phoenix_flames,if=(variable.refresh_charring_embers|buff.flames_fury.up)&(hot_streak_spells_in_flight&!action.phoenix_flames.in_flight&(buff.heating_up.react|charges>1))&time>15&!buff.hot_streak.react&!talent.alexstraszas_fury" );
  ...
  standard_rotation->add_action( "phoenix_flames,if=!variable.phoenix_pooling&(!hot_streak_spells_in_flight&!action.phoenix_flames.in_flight&(buff.heating_up.react))&(charges>1|debuff.charring_embers.remains<3|buff.flames_fury.up)" );
  standard_rotation->add_action( "phoenix_flames,if=(variable.refresh_charring_embers|buff.flames_fury.up)&(!hot_streak_spells_in_flight&!action.phoenix_flames.in_flight&(buff.heating_up.react|charges>1))&!buff.hot_streak.react&talent.alexstraszas_fury" );
  standard_rotation->add_action( "phoenix_flames,if=(variable.refresh_charring_embers|buff.flames_fury.up&cooldown.rune_of_power.remains>10)&(hot_streak_spells_in_flight&!action.phoenix_flames.in_flight&(buff.heating_up.react|charges>1))&!buff.hot_streak.react&!talent.alexstraszas_fury" );
...
void frost( player_t* p )
{
  ...
  aoe->add_action( "flurry,if=cooldown_react&remaining_winters_chill=0&debuff.winters_chill.down&(prev_gcd.1.frostbolt|(active_enemies>=7|charges=max_charges)&buff.fingers_of_frost.react=0)" );
