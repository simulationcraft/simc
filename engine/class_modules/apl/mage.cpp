...
void arcane( player_t* p )
{
  ...
  spellslinger->add_action( "arcane_missiles,if=(buff.clearcasting.react&buff.nether_precision.down)|(buff.clearcasting.react&buff.clearcasting.stack=3),interrupt_if=!gcd.remains,interrupt_immediate=1,interrupt_global=1,chain=1" );
  ...
  sunfury_aoe->add_action( "arcane_barrage,if=buff.arcane_soul.up&buff.clearcasting.stack<3&buff.clearcasting.react", "Spam Arcane Barrage during Arcane Soul, ensuring that you always get to maximum Clearcasting by the end." );
  ...
  sunfury_aoe->add_action( "arcane_orb,if=buff.arcane_charge.stack<2&cooldown.touch_of_the_magi.remains>18&(!talent.high_voltage|buff.clearcasting.react=0)" );
  sunfury_aoe->add_action( "arcane_explosion,if=buff.arcane_charge.stack<3&buff.clearcasting.react=0&buff.burden_of_power.up", "Use Explosion to get Charges for Burden if you don't have a Clearcasting proc" );
  ...
  sunfury_aoe->add_action( "arcane_barrage,if=(buff.arcane_charge.stack=4)" );
...
void fire( player_t* p )
{
  ...
  standard_rotation->add_action( "fire_blast,use_off_gcd=1,use_while_casting=1,if=buff.hyperthermia.up&charges>0&buff.heating_up.react", "We will munch Fireblasts during Hyperthermia" );
  ...
  standard_rotation->add_action( "phoenix_flames,if=talent.sun_kings_blessing&talent.call_of_the_sun_king&!buff.hot_streak.react&hot_streak_spells_in_flight<2", "SKB prefers to not pool Phoenix Flames." );
  standard_rotation->add_action( "phoenix_flames,if=!talent.sun_kings_blessing&talent.call_of_the_sun_king&!buff.hot_streak.react&hot_streak_spells_in_flight<2&(!variable.phoenix_pooling&buff.flames_fury.up|charges_fractional>2.5|charges_fractional>1.5|buff.flames_fury.react)&(!talent.feel_the_burn|buff.feel_the_burn.remains<3*gcd.max|buff.flames_fury.react)", "UI and default fire will pool." );
