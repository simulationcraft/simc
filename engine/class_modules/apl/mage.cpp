...
void arcane( player_t* p )
{
  ...
  precombat->add_action( "variable,name=prepull_evo,op=reset,default=-1" );
  precombat->add_action( "variable,name=prepull_evo,op=set,value=1,if=variable.prepull_evo=-1&runeforge.siphon_storm&active_enemies>2" );
  precombat->add_action( "variable,name=prepull_evo,op=set,value=1,if=variable.prepull_evo=-1&runeforge.siphon_storm&covenant.necrolord&active_enemies>1" );
  precombat->add_action( "variable,name=prepull_evo,op=set,value=0,if=variable.prepull_evo=-1" );
  ...
  precombat->add_action( "variable,name=totm_max_delay_for_ap,op=reset,default=-1" );
  precombat->add_action( "variable,name=totm_max_delay_for_ap,op=set,value=3,if=variable.totm_max_delay_for_ap=-1&runeforge.disciplinary_command" );
  precombat->add_action( "variable,name=totm_max_delay_for_ap,op=set,value=15,if=variable.totm_max_delay_for_ap=-1&covenant.night_fae" );
  precombat->add_action( "variable,name=totm_max_delay_for_ap,op=set,value=15,if=variable.totm_max_delay_for_ap=-1&conduit.arcane_prodigy&active_enemies<3" );
  precombat->add_action( "variable,name=totm_max_delay_for_ap,op=set,value=30,if=variable.totm_max_delay_for_ap=-1&essence.vision_of_perfection.minor" );
  precombat->add_action( "variable,name=totm_max_delay_for_ap,op=set,value=5,if=variable.totm_max_delay_for_ap=-1" );
  precombat->add_action( "variable,name=totm_max_delay_for_rop,op=reset,default=20" );
  precombat->add_action( "variable,name=barrage_mana_pct,op=reset,default=-1" );
  precombat->add_action( "variable,name=barrage_mana_pct,op=set,value=(60-(mastery_value*100)),if=variable.barrage_mana_pct=-1&covenant.night_fae" );
  precombat->add_action( "variable,name=barrage_mana_pct,op=set,value=(80-(mastery_value*100)),if=variable.barrage_mana_pct=-1" );
  precombat->add_action( "variable,name=ap_minimum_mana_pct,op=reset,default=-1" );
  precombat->add_action( "variable,name=ap_minimum_mana_pct,op=set,value=50,if=variable.ap_minimum_mana_pct=-1&runeforge.disciplinary_command" );
  precombat->add_action( "variable,name=ap_minimum_mana_pct,op=set,value=50,if=variable.ap_minimum_mana_pct=-1&runeforge.grisly_icicle" );
  precombat->add_action( "variable,name=ap_minimum_mana_pct,op=set,value=15,if=variable.ap_minimum_mana_pct=-1" );
  ...
  precombat->add_action( "variable,name=inverted_opener,op=reset,default=-1" );
  precombat->add_action( "variable,name=inverted_opener,op=set,value=1,if=variable.inverted_opener=-1&talent.rune_of_power&(talent.arcane_echo|!covenant.kyrian)&(!covenant.necrolord|active_enemies=1|runeforge.siphon_storm)" );
  precombat->add_action( "variable,name=inverted_opener,op=set,value=0,if=variable.inverted_opener=-1" );
  ...
  shared_cds->add_action( "time_warp,if=runeforge.temporal_warp&buff.exhaustion.up&(cooldown.arcane_power.ready|fight_remains<=40)" );
  ...
  essences->add_action( "worldvein_resonance,if=cooldown.arcane_power.remains>=50&cooldown.touch_of_the_magi.remains<=gcd&buff.arcane_charge.stack<=variable.totm_max_charges&talent.rune_of_power&cooldown.rune_of_power.remains<=gcd&cooldown.arcane_power.remains>variable.totm_max_delay_for_ap" );
  ...
  cooldowns->add_action( "touch_of_the_magi,if=buff.arcane_charge.stack<=variable.totm_max_charges&cooldown.arcane_power.remains<=execute_time", "Use totm with ap if it's within the max delay. If not, use with rop if the talent is taken, and it's within the max delay." );
  cooldowns->add_action( "touch_of_the_magi,if=buff.arcane_charge.stack<=variable.totm_max_charges&talent.rune_of_power&cooldown.rune_of_power.remains<=execute_time&cooldown.arcane_power.remains>variable.totm_max_delay_for_ap" );
  ...
  cooldowns->add_action( "rune_of_power,if=buff.arcane_power.down&cooldown.touch_of_the_magi.remains>variable.rop_max_delay_for_totm&buff.arcane_charge.stack=buff.arcane_charge.max_stack&(cooldown.arcane_power.remains>15|debuff.touch_of_the_magi.up)", "Use rop if totm is on cd and won't be up for longer than the max delay, making sure there isn't already a rune down and that ap won't become available during rune." );
  ...
  rotation->add_action( "arcane_barrage,if=cooldown.touch_of_the_magi.ready&(buff.arcane_charge.stack>variable.totm_max_charges&cooldown.arcane_power.remains<=execute_time)", "Barrage if it's time to use totm and we have too many charges" );
  ...
  rotation->add_action( "arcane_barrage,if=target.health.pct<35&buff.arcane_charge.stack=buff.arcane_charge.max_stack&runeforge.arcane_bombardment&active_enemies>1&buff.deathborne.down" );
  ...
  aoe->add_action( "shifting_power,if=buff.arcane_power.down&buff.rune_of_power.down&debuff.touch_of_the_magi.down" );
