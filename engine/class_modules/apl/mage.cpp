...
void arcane( player_t* p )
{
  ...
  default_->add_action( "call_action_list,name=movement,if=!(covenant.kyrian&runeforge.arcane_infinity)" );
  ...
  calculations->add_action( "variable,name=estimated_ap_cooldown,op=set,value=cooldown.arcane_power.remains*(1-(0.3*conduit.arcane_prodigy))", "Placeholder estimate until better support can be added for prodigy" );
  ...
  calculations->add_action( "variable,name=holding_totm,op=set,value=cooldown.touch_of_the_magi.ready&variable.estimated_ap_cooldown<20" );
  ...
  harmony->add_action( "evocation,if=mana.pct<=30&variable.outside_of_cooldowns&((talent.rune_of_power&cooldown.rune_of_power.remains<10)|(cooldown.touch_of_the_magi.remains<10&conduit.arcane_prodigy))" );
  ...
  harmony->add_action( "rune_of_power,if=variable.empowered_barrage&cooldown.radiant_spark.remains<=execute_time&variable.estimated_ap_cooldown>=20" );
  ...
  harmony->add_action( "rune_of_power,if=buff.arcane_power.down&(variable.estimated_ap_cooldown>30|cooldown.radiant_spark.remains>12)" );
  ...
  harmony->add_action( "touch_of_the_magi,if=variable.estimated_ap_cooldown<50&variable.estimated_ap_cooldown>30" );
  ...
  harmony->add_action( "arcane_orb,if=buff.arcane_charge.stack<3&variable.estimated_ap_cooldown>10" );
  ...
  harmony->add_action( "presence_of_mind,if=buff.arcane_charge.stack<buff.arcane_charge.max_stack&buff.arcane_power.up" );
  ...
  harmony->add_action( "arcane_blast,if=buff.arcane_charge.stack&buff.arcane_charge.stack<buff.arcane_charge.max_stack" );
  harmony->add_action( "arcane_missiles,if=!(variable.estimated_ap_cooldown<=10&mana.pct<30),chain=1,interrupt=1,interrupt_global=1" );
  harmony->add_action( "fire_blast" );
