...
void fire( player_t* p )
{
  ...
  default_->add_action( "use_item,name=imperfect_ascendancy_serum,if=variable.time_to_combustion<3", "The War Within S1 On-Use items with special use timings" );
  default_->add_action( "use_item,effect_name=spymasters_web,if=(buff.combustion.remains>10&fight_remains<80)|fight_remains<25" );
  ...
  default_->add_action( "shifting_power,if=buff.combustion.down&&(!improved_scorch.active|debuff.improved_scorch.remains>cast_time+action.scorch.cast_time&!buff.fury_of_the_sun_king.up)&!buff.hot_streak.react&buff.hyperthermia.down" );
  ...
  standard_rotation->add_action( "phoenix_flames,if=talent.sun_kings_blessing&talent.call_of_the_sun_king&!buff.hot_streak.react&hot_streak_spells_in_flight<2&(charges_fractional>1.5|buff.flames_fury.react)", "SKB prefers to not pool Phoenix Flames." );
