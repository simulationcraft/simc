...
void fire( player_t* p )
{
  ...
  precombat->add_action( "variable,name=combustion_shifting_power,default=1,op=reset" );
  precombat->add_action( "snapshot_stats" );
  ...
  combustion_phase->add_action( "shifting_power,if=buff.combustion.up&!action.fire_blast.charges&variable.combustion_shifting_power,interrupt_if=action.fire_blast.charges=action.fire_blast.max_charges" );
