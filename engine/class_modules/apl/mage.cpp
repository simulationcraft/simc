...
void arcane( player_t* p )
{
  ...
  precombat->add_action( "arcane_pulse,if=(talent.splintering_sorcery|variable.sf_touch_surge)&talent.arcane_pulse" );
  ...
  cooldowns->add_action( "arcane_pulse,if=(talent.splintering_sorcery|variable.sf_touch_surge)&buff.arcane_salvo.react<20&(variable.opener|(talent.orb_mastery&cooldown.arcane_surge.remains<(gcd.max*(mana.pct%(8+(8*(active_enemies>=2)))))))", "Spellslinger builds Salvo before going into cds the first time. TODO: Add fight length sensitivity." );
  ...
  spellslinger->add_action( "arcane_pulse,if=(buff.arcane_charge.stack<4)|((active_enemies>=2)&!variable.funnel)" );
  ...
  spellslinger_orbm->add_action( "arcane_pulse,if=(buff.arcane_charge.stack<4)|((active_enemies>=2)&!variable.funnel)" );
  ...
  sunfury->add_action( "arcane_pulse,if=(buff.arcane_charge.stack<4)|((active_enemies>=2)&!variable.funnel)" );
...
void fire( player_t* p )
{
  ...
  precombat->add_action( "variable,name=ff_combustion_flamestrike,if=!talent.spellfire_spheres,value=5", "Do on 4 targets if you dont care about prio" );
  precombat->add_action( "variable,name=ff_filler_flamestrike,if=!talent.spellfire_spheres,value=999", "Do on 5 targets if you dont care about prio but this is so close its basically not even worth even if you dont care about prio" );
  precombat->add_action( "variable,name=sf_combustion_flamestrike,if=talent.spellfire_spheres,value=4+(999*!talent.fuel_the_fire)", "Do on 3 targets if you dont care about prio a lot (+5% AoE -10% Prio)" );
  precombat->add_action( "variable,name=sf_filler_flamestrike,if=talent.spellfire_spheres,value=5+(999*!talent.fuel_the_fire)", "Do on 4 targets if you dont care about prio a lot (+4% AoE -10% prio)" );
  precombat->add_action( "variable,name=combustion_delay,value=10+(8*talent.firestarter)-(10*(!talent.firestarter&talent.sunfury_execution))-(10*(expected_combat_length<60)+10*(expected_combat_length<30))", "Delay combustion to stack up buffs for almost all builds. Only exception is SunfuryExecution when NOT firestarter." );
  ...
  fireblast->add_action( "fire_blast,use_off_gcd=1,use_while_casting=1,if=cooldown_react&!buff.hot_streak.react&(action.fireball.executing&action.fireball.execute_remains>0.1|buff.pyroclasm.react&action.pyroblast.executing&action.pyroblast.execute_remains>0.1)&((target.health.pct>=30|!talent.scorch)&buff.heating_up.react)&(hot_streak_spells_in_flight+buff.heating_up.react=1)&gcd.remains<gcd.max", "During non-execute filler, use Fire Blast with Heating Ups during Fireball/FFB/Pyroblast HARDCASTS only." );
  ...
  fireblast->add_action( "fire_blast,use_off_gcd=1,use_while_casting=1,if=cooldown_react&!buff.hot_streak.react&time<variable.combustion_delay&(hot_streak_spells_in_flight+buff.heating_up.react=1)&gcd.remains<gcd.max&cooldown.combustion.ready", "During the Firestarter period, spend Fire Blasts with Heating Up freely" );
  ...
  ff_combustion->add_action( "flamestrike,if=active_enemies>=variable.ff_combustion_flamestrike&(buff.pyroclasm.up&!buff.hot_streak.react&buff.combustion.down)", "Precast into Combustion. Prioritize Pyroclasm if available." );
  ...
  ff_combustion->add_action( "flamestrike,if=active_enemies>=variable.ff_combustion_flamestrike&(buff.hot_streak.react)", "Spend Hot Streaks on Pyroblast in ST or Flamestrike in AoE." );
  ff_combustion->add_action( "pyroblast,if=buff.hot_streak.react" );
  ff_combustion->add_action( "flamestrike,if=active_enemies>=variable.ff_combustion_flamestrike&(buff.pyroclasm.up&cast_time<buff.combustion.remains)", "Make sure Pyroclasm FINISHES cast before Combustion ends." );
  ...
  ff_filler->add_action( "meteor,if=time>=variable.combustion_delay", "Cast Meteor on CD in filler except when intentionally delaying Combustion" );
  ...
  ff_filler->add_action( "flamestrike,if=active_enemies>=variable.ff_filler_flamestrike&(buff.hot_streak.react&(cooldown.combustion.remains>=5|time<variable.combustion_delay))", "Hold Hot Streak if Combust is nearby. Do not hold if intentionally delaying combustion." );
  ff_filler->add_action( "pyroblast,if=buff.hot_streak.react&(cooldown.combustion.remains>=5|time<variable.combustion_delay)" );
  ff_filler->add_action( "flamestrike,if=active_enemies>=variable.ff_filler_flamestrike&(buff.pyroclasm.up&cooldown.combustion.remains>12|buff.pyroclasm.stack=2)", "Spend Pyroclasm immediately if you have 2 stacks available. Otherwise, hold one stack if it lasts until Combustion comes up." );
  ...
  sf_combustion->add_action( "pyroblast,if=buff.combustion.down&!buff.hot_streak.react&buff.pyroclasm.up", "Precast into Combustion. Prioritize Pyroclasm if available." );
  sf_combustion->add_action( "scorch,if=buff.combustion.down&target.health.pct<30" );
  sf_combustion->add_action( "fireball,if=buff.combustion.down" );
  sf_combustion->add_action( "meteor,if=(talent.burnout&buff.combustion.remains<8)|(!talent.burnout&buff.combustion.remains>2)", "Meteor is used towards the end of Combustion to maximize the Ignite bank for Burnout. If not playing Burnout, just make sure the Meteor lands during Combustion at any time." );
  sf_combustion->add_action( "flamestrike,if=active_enemies>=variable.sf_combustion_flamestrike&(buff.hot_streak.react|prev_gcd.1.scorch&buff.heating_up.react&time-action.scorch.last_used<0.2)", "Spend Hot Streaks on Pyroblast in ST or Flamestrike in AoE. The Scorch condition is simply to simulate predictable guaranteed crits during Combustion." );
  ...
  sf_combustion->add_action( "flamestrike,if=active_enemies>=variable.sf_combustion_flamestrike&(buff.pyroclasm.up&!buff.hot_streak.up&cast_time<buff.combustion.remains)", "Make sure Pyroclasm FINISHES cast before Combustion ends." );
  ...
  sf_filler->add_action( "flamestrike,if=active_enemies>=variable.sf_filler_flamestrike&(buff.pyroclasm.up&(cooldown.combustion.remains>=12|time<variable.combustion_delay)|buff.pyroclasm.stack=2)", "Spend Pyroclasm immediately if you have 2 stacks available. Otherwise, hold one stack if it lasts until Combustion comes up." );
  sf_filler->add_action( "pyroblast,if=buff.pyroclasm.up&(cooldown.combustion.remains>=12|time<variable.combustion_delay)|buff.pyroclasm.stack=2" );
  ...
  sf_filler->add_action( "scorch,if=buff.heat_shimmer.react|talent.scald&target.health.pct<30", "Cast Scorch in execute or with a Heat Shimmer proc." );
