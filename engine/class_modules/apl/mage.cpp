...
void arcane( player_t* p )
{
  ...
  precombat->add_action( "variable,name=inverted_opener,default=-1,op=set,if=variable.inverted_opener=-1,value=1*(equipped.empyreal_ordnance|(talent.rune_of_power&(talent.arcane_echo|!covenant.kyrian)&(!covenant.necrolord|active_enemies=1|runeforge.siphon_storm)))" );
  ...
  opener->add_action( "evocation,if=talent.rune_of_power&cooldown.rune_of_power.remains&cooldown.arcane_power.remains&buff.arcane_power.down&buff.rune_of_power.down&prev_gcd.1.arcane_barrage" );
  ...
  opener->add_action( "radiant_spark,if=mana.pct>40&!variable.inverted_opener=1" );
  ...
  opener->add_action( "radiant_spark,if=buff.rune_of_power.down|prev_gcd.1.arcane_barrage" );
  opener->add_action( "mirrors_of_torment,if=buff.rune_of_power.down|prev_gcd.1.arcane_barrage" );
  opener->add_action( "touch_of_the_magi,if=buff.rune_of_power.down|prev_gcd.1.arcane_barrage|prev_gcd.1.mirrors_of_torment|prev_gcd.1.radiant_spark|prev_gcd.1.deathborne" );
  ...
  opener->add_action( "arcane_barrage,if=buff.rune_of_power.up&cooldown.arcane_power.ready&mana.pct<(40+(10*covenant.kyrian))&buff.arcane_charge.stack=buff.arcane_charge.max_stack&!runeforge.siphon_storm&!runeforge.temporal_warp" );
  opener->add_action( "arcane_barrage,if=buff.arcane_power.up&buff.arcane_power.remains<gcd&runeforge.arcane_infinity" );
  ...
  cooldowns->add_action( "mirrors_of_torment,if=ptr", "Still working on optimizations for the new MoT, but currently using it on cd is better than holding for ap or using it slightly before ap" );
  cooldowns->add_action( "mirrors_of_torment,if=!ptr&cooldown.touch_of_the_magi.remains=0&buff.arcane_charge.stack<=variable.totm_max_charges&cooldown.arcane_power.remains<=gcd", "Always use mirrors with ap. If totm is ready as well, make sure to cast it before totm." );
  cooldowns->add_action( "mirrors_of_torment,if=!ptr&cooldown.arcane_power.remains=0&(!talent.enlightened|(talent.enlightened&mana.pct>=70))&((cooldown.touch_of_the_magi.remains>variable.ap_max_delay_for_totm&buff.arcane_charge.stack=buff.arcane_charge.max_stack)|(cooldown.touch_of_the_magi.remains=0&buff.arcane_charge.stack=0))&buff.rune_of_power.down&mana.pct>=variable.ap_minimum_mana_pct" );
...
void fire( player_t* p )
{
  ...
  precombat->add_action( "variable,name=firestarter_combustion,default=1,op=reset", "This variable specifies whether Combustion should be used during Firestarter." );
  precombat->add_action( "variable,name=hot_streak_flamestrike,if=variable.hot_streak_flamestrike=0,value=2*talent.flame_patch+3*!talent.flame_patch", "This variable specifies the number of targets at which Hot Streak Flamestrikes outside of Combustion should be used." );
  precombat->add_action( "variable,name=hard_cast_flamestrike,if=variable.hard_cast_flamestrike=0,value=2*talent.flame_patch+3*!talent.flame_patch", "This variable specifies the number of targets at which Hard Cast Flamestrikes outside of Combustion should be used as filler." );
  precombat->add_action( "variable,name=combustion_flamestrike,if=variable.combustion_flamestrike=0,value=3*talent.flame_patch+6*!talent.flame_patch", "This variable specifies the number of targets at which Hot Streak Flamestrikes are used during Combustion." );
  precombat->add_action( "variable,name=arcane_explosion,if=variable.arcane_explosion=0,value=99*talent.flame_patch+2*!talent.flame_patch", "This variable specifies the number of targets at which Arcane Explosion outside of Combustion should be used." );
  ...
  precombat->add_action( "variable,name=combustion_on_use,value=equipped.gladiators_badge|equipped.macabre_sheet_music|equipped.inscrutable_quantum_device|equipped.sunblood_amethyst|equipped.empyreal_ordnance|equipped.flame_of_battle|equipped.wakeners_frond|equipped.instructors_divine_bell" );
  ...
  default_->add_action( "variable,name=time_to_combustion,op=max,value=cooldown.gladiators_badge_345228.remains,if=equipped.gladiators_badge&cooldown.gladiators_badge_345228.remains<20", "Delay Combustion for Gladiators Badge, unless it would be delayed longer than 20 seconds." );
  ...
  combustion_phase->add_action( "scorch,if=searing_touch.active" );
