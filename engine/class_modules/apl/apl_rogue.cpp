...
void assassination( player_t* p )
{
  ...
  precombat->add_action( "variable,name=effective_spend_cp,value=cp_max_spend-2<?5*hero_tree.fatebound", "Determine combo point finish condition" );
  ...
  cds->add_action( "kingsbane,if=(debuff.shiv.up|cooldown.shiv.remains<6)&(buff.envenom.up|spell_targets.fan_of_knives>1)&(cooldown.deathmark.remains>=50-15*(hero_tree.fatebound&set_bonus.tww3_fatebound_4pc)|dot.deathmark.ticking)|fight_remains<=15" );
  ...
  cds->add_action( "cold_blood,use_off_gcd=1,if=(buff.fatebound_coin_tails.stack>0&buff.fatebound_coin_heads.stack>0)|debuff.shiv.up&(cooldown.deathmark.remains>50&(!hero_tree.fatebound&!set_bonus.tww3_fatebound_4pc)|dot.kingsbane.ticking&(hero_tree.fatebound&set_bonus.tww3_fatebound_4pc)|!talent.inevitabile_end&effective_combo_points>=variable.effective_spend_cp)", "Cold Blood for Edge Case or Envenoms during shiv" );
  ...
  shiv->add_action( "shiv,if=talent.lightweight_shiv&variable.shiv_kingsbane_condition&(cooldown.deathmark.ready|cooldown.deathmark.remains<=1)&(cooldown.kingsbane.ready|cooldown.kingsbane.remains<=2)&hero_tree.fatebound&set_bonus.tww3_fatebound_2pc", "Shiv for Fatebound Edge Case Coins Before Deathmark + Kingsbane with new Tier Set" );
  ...
  shiv->add_action( "shiv,if=!talent.lightweight_shiv.enabled&variable.shiv_kingsbane_condition&(dot.kingsbane.ticking&dot.kingsbane.remains<(8+3*(hero_tree.deathstalker&set_bonus.tww3_deathstalker_4pc))|!dot.kingsbane.ticking&cooldown.kingsbane.remains>=20)&(!talent.crimson_tempest.enabled|variable.single_target|dot.crimson_tempest.ticking)", "Single-charge Shiv case for Kingsbane" );
  ...
  shiv->add_action( "shiv,if=talent.lightweight_shiv.enabled&variable.shiv_kingsbane_condition&(dot.kingsbane.ticking&dot.kingsbane.remains<(8+3*(hero_tree.deathstalker&set_bonus.tww3_deathstalker_4pc))&dot.kingsbane.remains>4|cooldown.kingsbane.remains<=1&cooldown.shiv.charges_fractional>=1.7)", "Double-charge Shiv case for Kingsbane" );
  ...
  shiv->add_action( "shiv,if=fight_remains<=cooldown.shiv.charges*(8+3*(hero_tree.deathstalker&set_bonus.tww3_deathstalker_4pc))", "Dump Shiv on fight end" );
