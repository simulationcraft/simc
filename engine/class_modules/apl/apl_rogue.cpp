...
void outlaw( player_t* p )
{
  ...
  cds->add_action( "coup_de_grace,if=variable.double_coup&prev_gcd.1.coup_de_grace", "Enable the possibility to chain cast Coup de Grace if the bug option is enabled" );
  ...
  finish->add_action( "coup_de_grace,if=(!prev_gcd.1.coup_de_grace|variable.double_coup)", "Finishers" );
