...
void assassination( player_t* p )
{
  ...
  items->add_action( "variable,name=base_trinket_condition,value=dot.rupture.ticking&cooldown.deathmark.remains<2&target.time_to_die>=12|fight_remains<=22", "Special Case Trinkets" );
  ...
  items->add_action( "do_treacherous_transmitter_task,use_off_gcd=1,if=dot.deathmark.ticking|buff.realigning_nexus_convergence_divergence.remains<1|buff.cryptic_instructions.remains<1|buff.errant_manaforge_emission.remains<1|fight_remains<=15" );
  ...
  shiv->add_action( "shiv,if=!talent.lightweight_shiv.enabled&variable.shiv_kingsbane_condition&(dot.kingsbane.ticking&dot.kingsbane.remains<8|!dot.kingsbane.ticking&cooldown.kingsbane.remains>=24)&(!talent.crimson_tempest.enabled|variable.single_target|dot.crimson_tempest.ticking)", "Shiv cases for Kingsbane" );
  ...
  shiv->add_action( "shiv,if=fight_remains<=charges*8", "Dump Shiv on fight end" );
  ...
  vanish->add_action( "vanish,if=!talent.improved_garrote&buff.darkest_night.up&combo_points.deficit>=3&variable.single_target", "Vanish for slightly more mark uptime since you can apply mark and have darkest night at the same time" );
}
