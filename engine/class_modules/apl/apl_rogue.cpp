...
void outlaw_ptr( player_t* p )
{
  ...
  build->add_action( "sepsis,if=target.time_to_die>11&buff.between_the_eyes.up|fight_remains<11", "Builders" );
  build->add_action( "echoing_reprimand" );
  build->add_action( "ambush,if=talent.hidden_opportunity&(buff.audacity.up|buff.sepsis_buff.up)", "High priority Ambush for HO builds" );
  build->add_action( "pistol_shot,if=talent.fan_the_hammer&talent.audacity&talent.hidden_opportunity&buff.opportunity.up&!buff.audacity.up", "With Audacity + HO + FTH, consume Opportunity to proc Audacity any time Ambush is not available" );
  build->add_action( "pistol_shot,if=buff.greenskins_wickers.up&(!talent.fan_the_hammer&buff.opportunity.up|buff.greenskins_wickers.remains<1.5)", "Use Greenskins Wickers buff immediately with Opportunity unless running Fan the Hammer" );
  ...
  build->add_action( "pistol_shot,if=talent.fan_the_hammer&buff.opportunity.up&combo_points.deficit>((1+talent.quick_draw)*talent.fan_the_hammer.rank)&(cooldown.vanish.remains>10&cooldown.shadow_dance.remains>5|!talent.crackshot|talent.fan_the_hammer.rank<=1)", "With Fan the Hammer, consume Opportunity based on CP deficit, and 2 FTH Crackshot builds can briefly hold stacks for an upcoming stealth cooldown" );
  ...
  stealth_cds->add_action( "vanish,if=(!talent.hidden_opportunity|talent.crackshot)&variable.finish_condition", "Crackshot builds or builds without HO use Vanish at finish condition" );
  ...
  stealth_cds->add_action( "variable,name=shadow_dance_condition,value=buff.between_the_eyes.up&(!talent.hidden_opportunity|!buff.audacity.up&(talent.fan_the_hammer.rank<2|!buff.opportunity.up))&!talent.crackshot", "HO builds without Crackshot use Dance if Audacity and Opportunity are not active" );
  stealth_cds->add_action( "shadow_dance,if=!talent.keep_it_rolling&variable.shadow_dance_condition&buff.slice_and_dice.up&(variable.finish_condition|talent.hidden_opportunity)&(!talent.hidden_opportunity|!cooldown.vanish.ready)" );
  stealth_cds->add_action( "shadow_dance,if=talent.keep_it_rolling&variable.shadow_dance_condition&(cooldown.keep_it_rolling.remains<=30|cooldown.keep_it_rolling.remains>120&(variable.finish_condition|talent.hidden_opportunity))", "KIR builds without Crackshot use Dance at finish condition but hold it for an upcoming KIR" );
}
//outlaw_ptr_apl_end
