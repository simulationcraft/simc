...
void assassination( player_t* p )
{
  ...
  generate->add_action( "shiv,if=spell_targets.fan_of_knives<=1&talent.toxic_stiletto&talent.darkest_night", "Special Edge Case to use Shiv and Fan of Knives in ST as Toxic Stiletto makes it very efficient" );
  generate->add_action( "fan_of_knives,if=buff.darkest_night.up&combo_points.deficit=1&spell_targets.fan_of_knives<=1" );
  ...
  generate->add_action( "fan_of_knives,if=!(main_hand.dagger&off_hand.dagger)", "Fan of Knives fallback for non-dagger weapon swaps" );
