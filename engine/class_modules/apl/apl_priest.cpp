...
void discipline( player_t* p )
{
  ...
  variables->add_action( "variable,name=hd_stacks_current,op=set,value=buff.harsh_discipline.stack+(4*buff.harsh_discipline_ready.stack)" );
  ...
  variables->add_action( "variable,name=expected_penance_reduction,op=setif,condition=talent.train_of_thought,value=1.5,value_else=0", "----------------------------------------------------------------------------  TWILIGHT EQUILIBRIUM VARIABLES  ----------------------------------------------------------------------------  TODO: duration_remains is buggy on penance/dark reprimand, using a static reduction as a stand-in for now" );
  ...
  variables->add_action( "variable,name=shadow_spells_duration,op=add,value=2,if=cooldown.penance.up|((cooldown.penance.remains-variable.expected_penance_reduction)<buff.shadow_covenant.remains)" );
  variables->add_action( "variable,name=shadow_spells_duration,op=add,value=action.mindgames.execute_time,if=cooldown.mindgames.up|(cooldown.mindgames.remains_expected<buff.shadow_covenant.remains)" );
  ...
  variables->add_action( "variable,name=shadow_spells_duration,op=add,value=action.mind_blast.execute_time,if=(@(action.mind_blast.max_charges-action.mind_blast.charges_fractional-1)*action.mind_blast.recharge_time)<buff.shadow_covenant.remains", "Add any fractional charges of mindblast that will recharge in time" );
  variables->add_action( "variable,name=shadow_spells_duration,op=add,value=action.divine_star.execute_time,if=cooldown.divine_star.up|(cooldown.divine_star.remains<buff.shadow_covenant.remains)" );
  variables->add_action( "variable,name=shadow_spells_duration,op=add,value=action.halo.execute_time,if=cooldown.halo.up|(cooldown.halo.remains<buff.shadow_covenant.remains)" );
  variables->add_action( "variable,name=should_te,op=setif,condition=talent.twilight_equilibrium,value=(buff.shadow_covenant.remains-variable.shadow_spells_duration)>gcd.max,value_else=0", "Determine if we have enough scov time remaining to weave in a twilight equilibrium activation. If we don't have TE talented, always false." );
  variables->add_action( "variable,name=hd_prep_time,op=setif,condition=!variable.harsh_discipline_ready,value=gcd.max*(variable.hd_stacks_required-variable.hd_stacks_current),value_else=0", "----------------------------------------------------------------------------  VARIABLES FOR SPELLS BETWEEN SCOV WINDOWS  ----------------------------------------------------------------------------  How many GCDs do we need to spend preparing harsh discipline before entering shadow covenant" );
  variables->add_action( "variable,name=next_penance_time,op=set,value=cooldown.shadow_covenant.remains+variable.hd_prep_time+action.shadow_covenant.execute_time+action.schism.execute_time", "First cast should always be shadow HD prep->covenant->schism. This should be followed by a harsh discipline empowered penance." );
  variables->add_action( "variable,name=next_penance_time,op=add,value=action.purge_the_wicked.execute_time,if=variable.long_scov", "If we're in longscov, add a holy cast before penance to proc twilight equilibrium on the empowered penance" );
  ...
  variables->add_action( "variable,name=next_solace_time,op=set,value=variable.next_penance_time+2", "Solace if we can fit a TE proc during long scov. We have to hardcode penance's 2 second channel because execute_time doesn't work for channeled spells" );
  variables->add_action( "variable,name=can_solace,op=setif,condition=variable.long_scov,value=cooldown.power_word_solace.duration<variable.next_solace_time,value_else=1" );
  variables->add_action( "variable,name=next_mindgames_time,op=set,value=variable.next_penance_time+2", "Mindgames should get a TE buff if there is time available because it is our highest damage spell after penance (even accounting for expiation, etc.)" );
  variables->add_action( "variable,name=next_mindgames_time,op=add,value=action.purge_the_wicked.execute_time,if=variable.long_scov", "Add two executes for holy spells if we're in long scov: PTW>solace>smite" );
  variables->add_action( "variable,name=next_mindgames_time,op=add,value=action.power_word_solace.execute_time,if=variable.long_scov&talent.power_word_solace" );
  variables->add_action( "variable,name=next_mindgames_time,op=add,value=action.smite.execute_time,if=variable.long_scov&!talent.power_word_solace" );
  variables->add_action( "variable,name=next_mindgames_time,op=add,value=action.shadow_word_death.execute_time,if=target.health.pct<20&talent.expiation&!talent.shattered_perceptions", "SW:D is only better than mindgames only if we don't have shattered perceptions, are in execute phase, and have expiation" );
  variables->add_action( "variable,name=next_mindgames_time,op=add,value=action.shadow_word_death.execute_time,if=target.health.pct<20&talent.expiation&!talent.shattered_perceptions&talent.death_and_madness", "Add a second SW:D if we have death and madness" );
  variables->add_action( "variable,name=can_mindgames,op=set,value=cooldown.mindgames.remains_expected<variable.next_mindgames_time" );
  variables->add_action( "variable,name=next_swd_time,op=set,value=variable.next_penance_time+2", "Shadow Word: Death" );
  variables->add_action( "variable,name=next_swd_time,op=add,value=action.purge_the_wicked.execute_time,if=variable.long_scov", "Add two executes for holy spells if we're in long scov: PTW>solace>smite" );
  variables->add_action( "variable,name=next_swd_time,op=add,value=action.power_word_solace.execute_time,if=variable.long_scov&talent.power_word_solace" );
  variables->add_action( "variable,name=next_swd_time,op=add,value=action.smite.execute_time,if=variable.long_scov&!talent.power_word_solace" );
  variables->add_action( "variable,name=next_swd_time,op=add,value=action.mindgames.execute_time,if=talent.mindgames&(talent.shattered_perceptions|(!talent.shattered_perceptions&!talent.expiation))", "SW:D is only better than mindgames only if we don't have shattered perceptions, are in execute phase, and have expiation" );
  variables->add_action( "variable,name=next_swd_time,op=add,value=action.mind_blast.execute_time*2,if=target.health.pct>=20&talent.expiation", "Mind blast is better than SW:D if we aren't in execute and have expiation" );
  variables->add_action( "variable,name=can_swd,op=set,value=cooldown.shadow_word_death.remains<variable.next_swd_time" );
  variables->add_action( "variable,name=next_mind_blast_time,op=set,value=variable.next_penance_time+2", "Mindblast" );
  variables->add_action( "variable,name=next_mind_blast_time,op=add,value=action.purge_the_wicked.execute_time,if=variable.long_scov", "Add two executes for holy spells if we're in long scov: PTW>solace>smite" );
  variables->add_action( "variable,name=next_mind_blast_time,op=add,value=action.power_word_solace.execute_time,if=variable.long_scov&talent.power_word_solace" );
  variables->add_action( "variable,name=next_mind_blast_time,op=add,value=action.smite.execute_time,if=variable.long_scov&!talent.power_word_solace" );
  variables->add_action( "variable,name=next_mind_blast_time,op=add,value=action.mindgames.execute_time,if=talent.mindgames", "Add mindgames if we have it talented" );
  variables->add_action( "variable,name=next_mind_blast_time,op=add,value=action.shadow_word_death.execute_time,if=target.health.pct<20|!talent.expiation", "Add SW:D if we're in execute phase or don't have expiation" );
  variables->add_action( "variable,name=next_mind_blast_time,op=add,value=action.shadow_word_death.execute_time,if=talent.death_and_madness&(target.health.pct<20|!talent.expiation)", "Add a second SW:D if we have death and madness" );
  variables->add_action( "variable,name=can_mind_blast,op=set,value=action.mind_blast.full_recharge_time<variable.next_mind_blast_time", "TODO: This is a little bit simplistic, requiring both charges of mindblast to be available at the moment the first charge needs to be used. Slight optimization should be possible." );
  ...
  main->add_action( "lights_wrath", "For DPS, Lights wrath should always be used outside of scov to maximize the number of smites it buffs. In a real raid setting, you may want to cast it at the beginning of a scov cycle for additional healing." );
  main->add_action( "penance,if=!talent.shadow_covenant|variable.can_penance" );
  main->add_action( "mindgames,if=!talent.shadow_covenant|variable.can_mindgames" );
  main->add_action( "shadow_word_death,if=(!talent.shadow_covenant|variable.can_swd)&target.health.pct<20", "hold for up to 2 GCDs if execute range coming soon" );
  ...
  main->add_action( "power_word_solace,if=!talent.shadow_covenant|variable.can_solace" );
  ...
  scov_prep->add_action( "power_word_solace,if=!variable.harsh_discipline_ready&variable.can_solace" );
  ...
  long_scov->add_action( "purge_the_wicked,if=prev_gcd.1.schism&dot.purge_the_wicked.remains<20" );
  long_scov->add_action( "penance,if=(!variable.should_te|(variable.should_te&variable.te_shadow))&buff.harsh_discipline_ready.up" );
  long_scov->add_action( "power_word_solace,if=prev_gcd.3.schism&talent.power_word_solace" );
  long_scov->add_action( "smite,if=prev_gcd.3.schism&!talent.power_word_solace" );
