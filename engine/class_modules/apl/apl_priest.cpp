...
void shadow( player_t* p )
{
  ...
  main->add_action( "variable,name=pooling_mindblasts,op=setif,value=1,value_else=0,condition=(cooldown.void_torrent.remains<?(variable.holding_crash*raid_event.adds.in))<=gcd.max*4,if=talent.void_blast", "Are we pooling mindblasts? Currently only used for Voidweaver." );
  ...
  main->add_action( "devouring_plague,target_if=max:target.time_to_die*(dot.devouring_plague.remains<=gcd.max|variable.dr_force_prio|!talent.distorted_reality&variable.me_force_prio|!talent.phantasmal_pathogen),if=active_dot.devouring_plague<=1&dot.devouring_plague.remains<=gcd.max|insanity.deficit<=16", "Do not overcap on insanity" );
  ...
  main->add_action( "shadow_word_death,target_if=max:(target.health.pct<=20)*100+dot.devouring_plague.ticking,if=talent.depth_of_shadows", "Snipe SWDs with Depth of Shadows to spawn pets. Prefer targets with Devouring Plague on them." );
  ...
  main->add_action( "void_bolt,target_if=max:target.time_to_die", "Use Voidbolt on the enemy with the largest time to die. We do no care about dots because Voidbolt is only accessible inside voidform which guarantees maximum effect" );
  main->add_action( "call_action_list,name=empowered_filler,if=(buff.mind_spike_insanity.stack=2&talent.mind_spike|buff.mind_flay_insanity.stack=2&!talent.mind_spike)&talent.empowered_surges&buff.power_surge.up", "Do not overcap MSI or MFI during Empowered Surges (Archon)." );
  ...
  main->add_action( "devouring_plague,target_if=max:target.time_to_die*(dot.devouring_plague.remains<=gcd.max|variable.dr_force_prio|!talent.distorted_reality&variable.me_force_prio|!talent.phantasmal_pathogen),if=insanity.deficit<=35&talent.distorted_reality|buff.dark_ascension.up|buff.mind_devourer.up&cooldown.mind_blast.up|buff.entropic_rift.up", "Use Devouring Plague to maximize uptime. Short circuit if you are capping on Insanity within 35 With Distorted Reality can maintain more than one at a time in multi-target." );
  main->add_action( "void_torrent,target_if=max:(dot.devouring_plague.remains*1000+target.time_to_die),if=!variable.holding_crash&talent.idol_of_cthun&talent.void_eruption&!talent.entropic_rift,target_if=dot.devouring_plague.remains>=2.5", "Use Void Torrent if it will get near full Mastery Value and you have Cthun and Void Eruption. Prune this action for Entropic Rift Builds." );
  ...
  main->add_action( "void_torrent,if=!variable.holding_crash&(!talent.idol_of_cthun|!talent.void_eruption)&!talent.entropic_rift,target_if=dot.devouring_plague.remains>=2.5", "Void Torrent if you are not holding Shadow Crash for an add pack coming, prefer the target with the most DoTs active. Only cast if Devouring Plague is on that target and will last at least 2 seconds. Prune item with Entropic Rift." );
  ...
  cds->add_action( "power_infusion,if=(buff.voidform.up|buff.dark_ascension.up&(fight_remains<=80|fight_remains>=140))&(!talent.darkening_horizon|buff.entropic_rift.up)", "Sync Power Infusion with Voidform or Dark Ascension" );
  ...
  cds->add_action( "halo,if=talent.power_surge&(pet.fiend.active&cooldown.fiend.remains>=4|!talent.mindbender&!cooldown.fiend.up|active_enemies>2&!talent.inescapable_torment|!talent.dark_ascension)", "Make sure Mindbender is active before popping Dark Ascension unless you have insignificant talent points or too many targets" );
  ...
  trinkets->add_action( "use_items,if=(buff.voidform.up|buff.power_infusion.up|buff.dark_ascension.up|(cooldown.void_eruption.remains>10&trinket.cooldown.duration<=60))&(buff.entropic_rift.up|!talent.void_blast)|fight_remains<20" );
}
//shadow_apl_end
