...
void shadow( player_t* p )
{
  ...
  main->add_action( "void_blast,if=(dot.devouring_plague.remains>=execute_time|buff.entropic_rift.remains<=gcd.max|action.void_torrent.channeling&talent.void_empowerment)&(insanity.deficit>=16&(!buff.mind_devourer.up|!talent.mind_devourer)|cooldown.mind_blast.full_recharge_time<=gcd.max)" );
  ...
  main->add_action( "void_torrent,if=(dot.devouring_plague.ticking|talent.void_eruption)&talent.void_blast" );
  main->add_action( "shadow_word_death,if=(set_bonus.tier31_4pc|pet.fiend.active&talent.inescapable_torment&set_bonus.tier31_2pc|target.health.pct<=20&talent.depth_of_shadows)&dot.devouring_plague.ticking" );
  ...
  main->add_action( "call_action_list,name=empowered_filler,if=(buff.mind_spike_insanity.stack=2&talent.mind_spike|buff.mind_flay_insanity.stack=2&!talent.mind_spike)&talent.empowered_surges&buff.power_surge.up", "actions.main+=/call_action_list,name=empowered_filler,if=priest.cthun_last_trigger_attempt>=3.5&cooldown.mind_blast.full_recharge_time>=gcd.max*2&talent.idol_of_cthun&talent.void_eruption" );
  main->add_action( "call_action_list,name=heal_for_tof,if=!buff.twist_of_fate.up&buff.twist_of_fate_can_trigger_on_ally_heal.up&(talent.rhapsody|talent.divine_star|talent.halo)", "actions.main+=/call_action_list,name=heal_for_tof,if=equipped.rashoks_molten_heart&buff.molten_radiance.up,line_cd=15" );
  ...
  main->add_action( "void_torrent,if=!variable.holding_crash&talent.idol_of_cthun&talent.void_eruption,target_if=dot.devouring_plague.remains>=2.5" );
