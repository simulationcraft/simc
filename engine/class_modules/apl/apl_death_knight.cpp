...
void frost( player_t* p )
{
  ...
  cooldowns->add_action( "pillar_of_frost,if=talent.obliteration&(runic_power>=35&!buff.abomination_limb.up|buff.abomination_limb.up|talent.rage_of_the_frozen_champion)&(variable.st_planning|variable.adds_remain)&(talent.gathering_storm&buff.remorseless_winter.up|!talent.gathering_storm)" );
...
void unholy( player_t* p )
{
  ...
  precombat->add_action( "army_of_the_dead,precombat_time=4,if=!talent.summon_gargoyle" );
  ...
  default_->add_action( "mind_freeze,if=target.debuff.casting.react", "Interrupt" );
  default_->add_action( "variable,name=pooling_runic_power,value=cooldown.summon_gargoyle.remains<5&talent.summon_gargoyle&(cooldown.dark_transformation.remains<6&talent.commander_of_the_dead|!talent.commander_of_the_dead)", "Prevent specified trinkets being used with automatic lines  Variables" );
  ...
  default_->add_action( "call_action_list,name=racials" );
  default_->add_action( "sequence,if=active_enemies=1&!death_knight.disable_aotd&talent.summon_gargoyle,name=garg_opener:army_of_the_dead:outbreak:festering_strike:festering_strike:apocalypse:summon_gargoyle:dark_transformation:death_coil:death_coil:unholy_blight:empower_rune_weapon" );
  default_->add_action( "sequence,if=active_enemies=1&!death_knight.disable_aotd&!talent.summon_gargoyle,name=opener:festering_strike:festering_strike:potion:unholy_blight:apocalypse:dark_transformation" );
  default_->add_action( "call_action_list,name=cooldowns" );
  default_->add_action( "run_action_list,name=aoe_setup,if=variable.adds_remain&(!talent.plaguebearer|talent.plaguebearer&cooldown.plaguebearer.remains>10)&(cooldown.death_and_decay.remains<10&!talent.defile|cooldown.defile.remains<10&talent.defile|talent.plaguebearer&cooldown.plaguebearer.remains<3*gcd)&!death_and_decay.ticking" );
  ...
  aoe_burst->add_action( "death_coil,if=(buff.sudden_doom.react|!variable.pooling_runic_power)&active_enemies<=3+(talent.eternal_agony*2)+talent.death_rot+talent.improved_death_coil+talent.rotten_touch+talent.coil_of_devastation" );
  ...
  aoe_setup->add_action( "plaguebearer,target_if=max:debuff.festering_wound.stack,if=debuff.festering_wound.stack>=4" );
  ...
  aoe_setup->add_action( "death_coil,if=!variable.pooling_runic_power&active_enemies<=3+(talent.eternal_agony*2)+talent.death_rot+talent.improved_death_coil+talent.rotten_touch+talent.coil_of_devastation" );
  ...
  cooldowns->add_action( "empower_rune_weapon,if=variable.st_planning&pet.gargoyle.active&pet.gargoyle.remains<=21|fight_remains<=21" );
  ...
  cooldowns->add_action( "unholy_assault,if=variable.st_planning&debuff.festering_wound.stack<=2&(!talent.apocalypse|cooldown.apocalypse.remains<3)" );
  ...
  cooldowns->add_action( "abomination_limb_talent,if=variable.adds_remain|fight_remains<13" );

  generic->add_action( "death_coil,if=!variable.pooling_runic_power&(buff.sudden_doom.react|runic_power.deficit<=40)|pet.gargoyle.active|fight_remains<10&!debuff.festering_wound.up|!variable.pooling_runic_power&rune<3", "Single Target" );
  generic->add_action( "wound_spender,if=debuff.festering_wound.up&talent.superstrain&talent.plaguebringer&buff.plaguebringer.remains<gcd&(cooldown.apocalypse.remains_expected>rune.time_to_3|!talent.apocalypse)" );
  generic->add_action( "any_dnd,if=talent.defile.enabled&(!variable.pooling_runes|fight_remains<5)" );
  generic->add_action( "festering_strike,if=debuff.festering_wound.stack<4&(cooldown.apocalypse.remains<gcd*2|!talent.apocalypse)&(!talent.unholy_assault|talent.unholy_assult&cooldown.unholy_assault.remains>5)" );
  generic->add_action( "wound_spender,if=debuff.festering_wound.stack>(3-talent.infected_claws)&!variable.pooling_runes&(cooldown.apocalypse.remains>rune.time_to_3|!talent.apocalypse)|debuff.festering_wound.up&fight_remains<(debuff.festering_wound.stack*gcd)" );
  generic->add_action( "festering_strike,if=debuff.festering_wound.stack<(4-talent.infected_claws)&!variable.pooling_runes" );
  generic->add_action( "death_coil,if=!variable.pooling_runic_power" );
  generic->add_action( "wound_spender,if=debuff.festering_wound.stack>=1&rune<2&!variable.pooling_runes&(cooldown.apocalypse.remains_expected>rune.time_to_3|!talent.apocalypse)" );
  ...
  generic_aoe->add_action( "death_coil,if=(!variable.pooling_runic_power|buff.sudden_doom.react)&active_enemies<=3+(talent.eternal_agony*2)+talent.death_rot+talent.improved_death_coil+talent.rotten_touch+talent.coil_of_devastation" );
  ...
  trinkets->add_action( "use_item,name=scars_of_fraternal_strife", "Trinkets  The trinket with the highest estimated value, will be used first and paired with Apocalypse (if buff is 15 seconds or less) or Blight/DT (if greater than 15 seconds)" );
