...
void frost( player_t* p )
{
  ...
  default_->add_action( "variable,name=adds_remain,value=active_enemies>=2&(!raid_event.adds.exists|raid_event.adds.exists&(raid_event.adds.remains>5|target.1.time_to_die>20))" );
  ...
  default_->add_action( "howling_blast,target_if=dot.frost_fever.refreshable&(talent.icecap|!breath_of_sindragosa.ticking&talent.breath_of_sindragosa|talent.obliteration&cooldown.pillar_of_frost.remains&!buff.killing_machine.up)" );
  ...
  covenants->add_action( "swarming_mist,if=talent.breath_of_sindragosa&(buff.breath_of_sindragosa.up&(variable.st_planning&runic_power.deficit>40|variable.adds_remain&runic_power.deficit>60|variable.adds_remain)|!buff.breath_of_sindragosa.up&cooldown.breath_of_sindragosa.remains)" );
...
void unholy( player_t* p )
{
  ...
  cooldowns->add_action( "unholy_assault,target_if=min:debuff.festering_wound.stack,if=active_enemies>=2&variable.major_cooldowns_active&debuff.festering_wound.stack<2" );
