...
void unholy( player_t* p )
{
  ...
  default_->add_action( "call_action_list,name=cooldowns" );
  default_->add_action( "run_action_list,name=aoe_setup,if=variable.adds_remain&(variable.aoe_setup_time|talent.plaguebearer&cooldown.plaguebearer.remains<3*gcd)&!death_and_decay.ticking" );
  ...
  aoe_setup->add_action( "abomination_limb_talent,if=variable.adds_remain&rune<3|fight_remains<13" );
  ...
  cooldowns->add_action( "potion,if=variable.cd_timing|fight_remains<26", "Potion" );
  cooldowns->add_action( "army_of_the_dead,if=talent.commander_of_the_dead&cooldown.dark_transformation.remains_expected<8|!talent.commander_of_the_dead&talent.unholy_assault&cooldown.unholy_assault.remains<10|!talent.unholy_assault&!talent.commander_of_the_dead|fight_remains<=30", "Cooldowns" );
  ...
  cooldowns->add_action( "unholy_blight,if=variable.adds_remain|fight_remains<duration" );
  ...
  cooldowns->add_action( "unholy_assault,if=variable.st_planning&(!talent.apocalypse|cooldown.apocalypse.remains<gcd|variable.cd_timing)" );
  ...
  generic->add_action( "death_coil,if=!variable.pooling_runic_power&(buff.sudden_doom.react|runic_power.deficit<=40|rune<3)|pet.gargoyle.active|fight_remains<(30%gcd)", "Single Target" );
  ...
  generic_aoe->add_action( "festering_strike,target_if=max:debuff.festering_wound.stack,if=debuff.festering_wound.stack<=3&cooldown.apocalypse.remains<variable.apoc_timing|debuff.festering_wound.stack<1" );
  generic_aoe->add_action( "festering_strike,target_if=min:debuff.festering_wound.stack,if=cooldown.apocalypse.remains>variable.apoc_timing&debuff.festering_wound.stack<1" );
  ...
  generic_aoe->add_action( "wound_spender,target_if=max:debuff.festering_wound.stack,if=(cooldown.apocalypse.remains>variable.apoc_timing&debuff.festering_wound.up|debuff.festering_wound.stack>4)&(fight_remains<cooldown.death_and_decay.remains+10|fight_remains>cooldown.apocalypse.remains)" );

  racials->add_action( "arcane_torrent,if=runic_power.deficit>20&(pet.gargoyle.active|!talent.summon_gargoyle.enabled)", "Racials" );
  racials->add_action( "blood_fury,if=variable.cd_timing|fight_remains<=duration" );
  racials->add_action( "berserking,if=variable.cd_timing|fight_remains<=duration" );
  ...
  racials->add_action( "ancestral_call,if=variable.cd_timing|fight_remains<=duration" );
  racials->add_action( "arcane_pulse,if=active_enemies>=2|(rune.deficit>=5&runic_power.deficit>=60)" );
  racials->add_action( "fireblood,if=variable.cd_timing|fight_remains<=duration" );
