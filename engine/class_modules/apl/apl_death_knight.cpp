...
void unholy( player_t* p )
{
  ...
  aoe_burst->add_action( "clawing_shadows,if=active_enemies<=6&(talent.bursting_sores|(active_enemies*(variable.clawing_shadows_damage*variable.scourge_strike_rank2))+(death_knight.fwounded_targets*variable.wound_damage)>(active_enemies*variable.epidemic_main_damage)+(variable.epidemic_cleave_damage*(active_enemies*(active_enemies-1))))", "AoE Burst, Clawing shadows evaluates the damage per global of Clawing shadows versus Epidemic, using clawing shadows if it would deal more damage." );
  ...
  cooldowns->add_action( "unholy_assault,if=variable.st_planning&debuff.festering_wound.stack<2&(pet.apoc_ghoul.active|buff.dark_transformation.up&!pet.army_ghoul.active&cooldown.apocalypse.remains>20)" );
