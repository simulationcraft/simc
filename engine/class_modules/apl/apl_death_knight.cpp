...
void unholy( player_t* p )
{
  ...
  default_->add_action( "call_action_list,name=racials" );
  default_->add_action( "call_action_list,name=trinkets" );
  default_->add_action( "outbreak,if=dot.dread_plague.ticks_remain<3&!talent.pestilence|talent.pestilence&dot.virulent_plague.ticking" );
  ...
  default_->add_action( "putrefy,if=(lesser_ghoul.count>1|talent.necromancers_cunning)&(buff.forbidden_knowledge.up&runic_power.deficit>10|charges=max_charges|talent.pestilence&!dot.virulent_plague.ticking)" );
  ...
  default_->add_action( "putrefy,if=lesser_ghoul.count>1|talent.necromancers_cunning" );
