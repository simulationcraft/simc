...
void unholy( player_t* p )
{
  ...
  default_->add_action( "call_action_list,name=covenants" );
  default_->add_action( "sequence,if=talent.unholy_blight&active_enemies=1&!death_knight.disable_aotd,name=opener:army_of_the_dead:festering_strike:festering_strike:unholy_blight:potion:dark_transformation:abomination_limb:apocalypse" );
