...
void unholy( player_t* p )
{
  ...
  precombat->add_action( "variable,name=trinket_1_sync,op=setif,value=1,value_else=0.5,condition=trinket.1.cooldown.duration%%cooldown.unholy_blight.duration=0", "Evaluates a trinkets cooldown, divided by unholy blight's cooldown. If it's value has no remainder return 1, else return 0.5." );
