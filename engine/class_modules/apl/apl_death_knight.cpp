...
void unholy( player_t* p )
{
  ...
  cooldowns->add_action( "soul_reaper,if=active_enemies=1&target.time_to_pct_35<5&target.time_to_die>(debuff.soul_reaper.remains+5)" );
  cooldowns->add_action( "soul_reaper,target_if=min:debuff.soul_reaper.remains,if=target.time_to_pct_35<5&active_enemies>=2&target.time_to_die>(debuff.soul_reaper.remains+5)&(active_enemies<=3|set_bonus.tier28_4pc&active_enemies<=5&(buff.dark_transformation.up|runeforge.reanimated_shambler)&(!death_and_decay.ticking|covenant.night_fae))" );
