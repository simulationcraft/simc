...
void unholy( player_t* p )
{
  ...
  default_->add_action( "variable,name=major_procs_active,value=soulbind.heirmirs_arsenal_marrowed_gemstone&buff.marrowed_gemstone_enhancement.up|soulbind.thrill_seeker&buff.thrill_seeker.up|buff.frenzied_monstrosity.up|soulbind.lead_by_example&buff.lead_by_example.up|soulbind.combat_meditation&buff.combat_meditation.up|buff.unholy_strength.up", "Checks for major buffs to adjust scourge strike/clawing shadows use." );
  ...
  covenants->add_action( "abomination_limb,if=active_enemies>=2&rune.time_to_4>(3+buff.runic_corruption.remains)&(raid_event.adds.remains>5|!raid_event.adds.exists)" );
