...
void frost( player_t* p )
{
  ...
  default_->add_action( "obliterate,if=covenant.night_fae&death_and_decay.ticking&death_and_decay.active_remains<(gcd*1.5)&(!talent.obliteration|set_bonus.tier28_4pc&talent.obliteration&!buff.pillar_of_frost.up)" );
  default_->add_action( "mind_freeze,if=target.debuff.casting.react", "Interrupt" );
  ...
  default_->add_action( "run_action_list,name=obliteration_pooling,if=!set_bonus.tier28_4pc&talent.obliteration&cooldown.pillar_of_frost.remains<7&(variable.st_planning|raid_event.adds.exists&raid_event.adds.in<10|!raid_event.adds.exists)" );
  ...
  obliteration_pooling->add_action( "remorseless_winter,if=variable.rw_buffs|active_enemies>=2", "Pooling For Obliteration: Starts 7 seconds before Pillar of Frost comes off CD" );
  ...
  obliteration_pooling->add_action( "obliterate,target_if=max:(debuff.razorice.stack+1)%(debuff.razorice.remains+1)*death_knight.runeforge.razorice,if=rune>=3&!main_hand.2h|rune>=4&main_hand.2h" );
