...
void demonology( player_t* p )
{
  ...
  items->add_action( "use_item,name=spoils_of_neltharus" );
  items->add_action( "use_item,name=erupting_spear_fragment" );
  items->add_action( "use_item,name=mirror_of_fractured_tomorrows" );
  items->add_action( "use_item,name=ashes_of_the_embersoul" );
  items->add_action( "use_item,name=balefire_branch" );
  items->add_action( "use_item,name=timethiefs_gambit" );
  items->add_action( "use_item,name=energy_siphon" );
  items->add_action( "use_item,name=nymues_unraveling_spindle,if=pet.demonic_tyrant.active&!cooldown.demonic_strength.ready|fight_remains<22", "Post Tyrant" );
  ...
  items->add_action( "use_item,use_off_gcd=1,name=beacon_to_the_beyond,if=cooldown.summon_demonic_tyrant.remains>20" );
