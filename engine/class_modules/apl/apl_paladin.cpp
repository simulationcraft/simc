...
void protection( player_t* p )
{
  ...
  standard->add_action( "shield_of_the_righteous,if=!buff.hammer_of_light_ready.up&(buff.luck_of_the_draw.up&((holy_power+judgment_holy_power>=5)|(!talent.righteous_protector.enabled|cooldown.righteous_protector_icd.remains=0)))", "actions.standard+=/shield_of_the_righteous,if=buff.luck_of_the_draw.up|((!talent.righteous_protector.enabled|(!talent.righteous_protector.enabled|cooldown.righteous_protector_icd.remains=0)|holy_power+judgment_holy_power>=4)&((holy_power+judgment_holy_power)>4)&!buff.hammer_of_light_ready.up) during luck of the draw(TWW2 4P)spam sotr as much as possible without wasting icd where possible" );
  standard->add_action( "shield_of_the_righteous,if=!buff.hammer_of_light_ready.up&set_bonus.thewarwithin_season_2_4pc&((holy_power+judgment_holy_power>5)|(holy_power+judgment_holy_power>=5&cooldown.righteous_protector_icd.remains=0))", "outside of luck of the draw, pool holy power as much as possible to be able to enter luck of the draw windows with more holy power" );
  ...
  standard->add_action( "avengers_shield,if=talent.refining_fire.enabled" );
