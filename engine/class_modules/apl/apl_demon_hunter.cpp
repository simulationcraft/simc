...
void vengeance( player_t* p )
{
  ...
  default_->add_action( "variable,name=trinket_1_buffs,value=trinket.1.has_use_buff|(trinket.1.has_buff.strength|trinket.1.has_buff.mastery|trinket.1.has_buff.versatility|trinket.1.has_buff.haste|trinket.1.has_buff.crit)" );
  ...
  default_->add_action( "variable,name=dont_spend_fury,op=setif,condition=(cooldown.fel_devastation.remains<(action.soul_cleave.execute_time+gcd.remains))&fury<50,value=1,value_else=0" );
  ...
  default_->add_action( "metamorphosis,use_off_gcd=1" );
  ...
  big_aoe->add_action( "fel_devastation,if=talent.collective_anguish.enabled|talent.stoke_the_flames.enabled" );
  big_aoe->add_action( "the_hunt" );
  big_aoe->add_action( "elysian_decree" );
  ...
  big_aoe->add_action( "soul_cleave,if=soul_fragments<1" );
  ...
  fiery_demise->add_action( "immolation_aura" );
  ...
  fiery_demise->add_action( "spirit_bomb,if=spell_targets=1&soul_fragments>=5" );
  fiery_demise->add_action( "spirit_bomb,if=spell_targets>1&spell_targets<=5&soul_fragments>=4" );
  fiery_demise->add_action( "spirit_bomb,if=spell_targets>=6&soul_fragments>=3" );
  fiery_demise->add_action( "the_hunt" );
  fiery_demise->add_action( "elysian_decree" );
  fiery_demise->add_action( "soul_cleave,if=fury.deficit<=30&!variable.dont_spend_fury" );

  filler->add_action( "sigil_of_chains,if=talent.cycle_of_binding.enabled&talent.sigil_of_chains.enabled" );
  filler->add_action( "sigil_of_misery,if=talent.cycle_of_binding.enabled&talent.sigil_of_misery.enabled" );
  filler->add_action( "sigil_of_silence,if=talent.cycle_of_binding.enabled&talent.sigil_of_silence.enabled" );
  filler->add_action( "throw_glaive" );

  maintenance->add_action( "fiery_brand,if=(active_dot.fiery_brand=0&(cooldown.sigil_of_flame.remains<(execute_time+gcd.remains)|cooldown.soul_carver.remains<(execute_time+gcd.remains)|cooldown.fel_devastation.remains<(execute_time+gcd.remains)))|(talent.down_in_flames&full_recharge_time<(execute_time+gcd.remains))" );
  ...
  maintenance->add_action( "bulk_extraction,if=prev_gcd.1.spirit_bomb" );
  maintenance->add_action( "felblade,if=fury.deficit>=40" );
  ...
  maintenance->add_action( "spirit_bomb,if=fury.deficit<30&((spell_targets>=2&soul_fragments>=5)|(spell_targets>=6&soul_fragments>=4))&!variable.dont_spend_fury" );
  maintenance->add_action( "soul_cleave,if=fury.deficit<30&soul_fragments<=3&!variable.dont_spend_fury" );

  single_target->add_action( "the_hunt" );
  single_target->add_action( "soul_carver" );
  single_target->add_action( "fel_devastation,if=talent.collective_anguish.enabled|(talent.stoke_the_flames.enabled&talent.burning_blood.enabled)" );
  single_target->add_action( "elysian_decree" );
  single_target->add_action( "fel_devastation" );
  single_target->add_action( "soul_cleave,if=talent.focused_cleave&!variable.dont_spend_fury" );
  single_target->add_action( "fracture" );
  single_target->add_action( "shear" );
  single_target->add_action( "soul_cleave,if=!variable.dont_spend_fury" );
  ...
  small_aoe->add_action( "elysian_decree" );
  small_aoe->add_action( "fel_devastation" );
  small_aoe->add_action( "soul_carver" );
  small_aoe->add_action( "spirit_bomb,if=soul_fragments>=5" );
  small_aoe->add_action( "soul_cleave,if=talent.focused_cleave&soul_fragments<=2" );
  small_aoe->add_action( "fracture" );
  small_aoe->add_action( "shear" );
  small_aoe->add_action( "soul_cleave,if=soul_fragments<=2" );
  small_aoe->add_action( "call_action_list,name=filler" );

  trinkets->add_action( "use_item,use_off_gcd=1,slot=trinket1,if=!variable.trinket_1_buffs" );
