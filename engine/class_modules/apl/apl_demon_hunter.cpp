...
void havoc( player_t* p )
{
  ...
  default_->add_action( "run_action_list,name=opener,if=(cooldown.eye_beam.up|cooldown.metamorphosis.up|cooldown.essence_break.up)&time<15&(raid_event.adds.in>40)" );
  ...
  default_->add_action( "immolation_aura,if=talent.inertia&buff.unbound_chaos.down&cooldown.eye_beam.remains<5&(active_enemies>=desired_targets+raid_event.adds.count|raid_event.adds.in>full_recharge_time&(!talent.essence_break|cooldown.essence_break.remains<5))" );
  ...
  default_->add_action( "sigil_of_flame,if=talent.student_of_suffering&cooldown.eye_beam.remains<gcd.max&(cooldown.essence_break.remains<gcd.max*4|!talent.essence_break)&(cooldown.metamorphosis.remains>10|cooldown.blade_dance.remains<gcd.max*3)" );
  ...
  default_->add_action( "felblade,if=buff.out_of_range.down|fury.deficit>40", "actions+=/sigil_of_flame,if=talent.student_of_suffering&((cooldown.eye_beam.remains<4&cooldown.metamorphosis.remains>20)|(cooldown.eye_beam.remains<gcd.max&cooldown.metamorphosis.up))" );
  ...
  cooldown->add_action( "use_item,name=mad_queens_mandate,use_off_gcd=1,if=(buff.metamorphosis.remains>5|buff.metamorphosis.down)&(target.time_to_die>125|target.time_to_die<10)&time>0&(!talent.initiative|buff.initiative.up|time>5)" );
  cooldown->add_action( "use_item,slot=trinket1,use_off_gcd=1,if=((cooldown.eye_beam.remains<gcd.max&active_enemies>1|buff.metamorphosis.up)&(raid_event.adds.in>trinket.1.cooldown.duration-15|raid_event.adds.remains>8)|!trinket.1.has_buff.any|fight_remains<25)&(!equipped.witherbarks_branch|trinket.2.cooldown.remains>20)&time>0&(!talent.initiative|buff.initiative.up|time>5)&!trinket.1.is.mad_queens_mandate" );
  cooldown->add_action( "use_item,slot=trinket2,use_off_gcd=1,if=((cooldown.eye_beam.remains<gcd.max&active_enemies>1|buff.metamorphosis.up)&(raid_event.adds.in>trinket.2.cooldown.duration-15|raid_event.adds.remains>8)|!trinket.2.has_buff.any|fight_remains<25)&(!equipped.witherbarks_branch|trinket.1.cooldown.remains>20)&time>0&(!talent.initiative|buff.initiative.up|time>5)&!trinket.2.is.mad_queens_mandate" );
  cooldown->add_action( "the_hunt,if=debuff.essence_break.down&(active_enemies>=desired_targets+raid_event.adds.count|raid_event.adds.in>(1+!set_bonus.tier31_2pc)*45)&(debuff.reavers_mark.up|!hero_tree.aldrachi_reaver)&buff.reavers_glaive.down&(buff.metamorphosis.remains>5|buff.metamorphosis.down)" );
  ...
  meta->add_action( "annihilation,if=(hero_tree.felscarred&buff.demonsurge_annihilation.up)&(cooldown.eye_beam.remains<gcd.max*3&cooldown.blade_dance.remains|cooldown.metamorphosis.remains<gcd.max*3)" );
  meta->add_action( "fel_rush,if=buff.unbound_chaos.up&talent.inertia&cooldown.blade_dance.remains<gcd.max*3" );
  ...
  meta->add_action( "essence_break,if=fury>20&(cooldown.metamorphosis.remains>10|cooldown.blade_dance.remains<gcd.max*2)&(buff.unbound_chaos.down|buff.inertia.up|!talent.inertia)&buff.out_of_range.remains<gcd.max&(!talent.shattered_destiny|cooldown.eye_beam.remains>4)|fight_remains<10" );
  ...
  meta->add_action( "annihilation,if=cooldown.blade_dance.remains|fury>60|buff.metamorphosis.remains<5&cooldown.felblade.up" );
  ...
  opener->add_action( "annihilation,if=hero_tree.felscarred&buff.demonsurge_annihilation.up,line_cd=10" );
  opener->add_action( "fel_rush,if=talent.momentum&buff.momentum.remains<6|talent.inertia&buff.unbound_chaos.up&talent.a_fire_inside&(buff.inertia.down&buff.metamorphosis.up|hero_tree.felscarred)&debuff.essence_break.down" );
  opener->add_action( "fel_rush,if=talent.inertia&buff.unbound_chaos.up&!talent.a_fire_inside&buff.metamorphosis.up" );
  opener->add_action( "the_hunt,if=buff.metamorphosis.up|!talent.shattered_destiny" );
  opener->add_action( "felblade,if=(buff.metamorphosis.down|fury<40)&(!talent.a_fire_inside|hero_tree.aldrachi_reaver)" );
  ...
  opener->add_action( "metamorphosis,if=buff.metamorphosis.up&cooldown.blade_dance.remains&buff.inner_demon.down&(!hero_tree.felscarred&(!talent.restless_hunter|prev_gcd.1.death_sweep)|buff.demonsurge.stack=2)&(cooldown.essence_break.remains|talent.shattered_destiny|!talent.essence_break)" );
  ...
  opener->add_action( "eye_beam,if=buff.metamorphosis.down|debuff.essence_break.down&buff.inner_demon.down&(cooldown.blade_dance.remains|cooldown.essence_break.up)" );
  opener->add_action( "abyssal_gaze,if=debuff.essence_break.down&cooldown.blade_dance.remains" );
  opener->add_action( "essence_break,if=(cooldown.blade_dance.remains<gcd.max&!talent.shattered_destiny&buff.metamorphosis.up|cooldown.eye_beam.remains&cooldown.metamorphosis.remains)&(!hero_tree.felscarred|buff.demonsurge_annihilation.down)" );
