...
void vengeance( player_t* p )
{
  ...
  ar->add_action( "variable,name=dont_soul_cleave,value=variable.can_spb|variable.can_spb_soon|variable.can_spb_one_gcd|prev_gcd.1.fracture" );
  ...
  ar->add_action( "run_action_list,name=rg_active,if=buff.glaive_flurry.up|buff.rending_strike.up" );
  ...
  ar->add_action( "vengeful_retreat,use_off_gcd=1,if=talent.unhindered_assault&!cooldown.felblade.up&(((talent.spirit_bomb&(fury<40&(variable.can_spb|variable.can_spb_soon)))|(talent.spirit_bomb&(cooldown.sigil_of_spite.up|cooldown.soul_carver.up)&cooldown.fel_devastation.up&fury<50))|fury<30)" );
  ...
  ar->add_action( "soul_cleave,if=(debuff.reavers_mark.remains<=(gcd.remains+execute_time+(gcd.max*2)))&(buff.art_of_the_glaive.stack+soul_fragments>=30&buff.art_of_the_glaive.stack>=28)&(fury<40|!variable.can_spb)" );
  ...
  ar->add_action( "bulk_extraction,if=(debuff.reavers_mark.remains<=(gcd.remains+execute_time+(gcd.max*2)))&(buff.art_of_the_glaive.stack+(spell_targets>?5)>=30)" );
  ...
  ar->add_action( "fel_devastation,if=talent.spirit_bomb&!variable.can_spb&(variable.can_spb_soon|soul_fragments.inactive>=2)" );
  ...
  ar->add_action( "soul_carver,if=!talent.spirit_bomb|(((soul_fragments.total+3)<=6)&fury>=15&!prev_gcd.1.sigil_of_spite)" );
  ar->add_action( "sigil_of_spite,if=!talent.spirit_bomb|((variable.can_spb&fury>=40)|variable.can_spb_soon|soul_fragments<=1)" );
  ...
  ar->add_action( "felblade,if=(((talent.spirit_bomb&(fury<40&(variable.can_spb|variable.can_spb_soon)))|(talent.spirit_bomb&(cooldown.sigil_of_spite.up|cooldown.soul_carver.up)&cooldown.fel_devastation.up&fury<50))|fury<30)" );
  ...
  fel_dev->add_action( "soul_sunder,if=buff.demonsurge_soul_sunder.up|!variable.dont_soul_cleave|(buff.metamorphosis.remains<(gcd.remains+execute_time+1)&buff.demonsurge_soul_sunder.up)" );
  ...
  fel_dev->add_action( "immolation_aura" );
  fel_dev->add_action( "sigil_of_flame,if=!variable.hold_sof" );
  fel_dev->add_action( "felblade" );
  fel_dev->add_action( "fracture" );

  fel_dev_prep->add_action( "fiery_brand,if=talent.fiery_demise&((talent.darkglare_boon&fury>=70)|(!talent.darkglare_boon&fury>=100))&(variable.can_spburst|variable.can_spburst_soon)&active_dot.fiery_brand=0&(cooldown.metamorphosis.up|cooldown.metamorphosis.remains<(gcd.remains+execute_time+action.fel_devastation.execute_time+(gcd.max*2)))" );
  fel_dev_prep->add_action( "fel_devastation,if=((talent.darkglare_boon&fury>=70)|(!talent.darkglare_boon&fury>=100))&(variable.can_spburst|variable.can_spburst_soon)" );
  fel_dev_prep->add_action( "sigil_of_spite,if=!(variable.can_spburst|variable.can_spburst_soon)&soul_fragments.total<=2&((talent.darkglare_boon&fury>=70)|(!talent.darkglare_boon&fury>=100))" );
  fel_dev_prep->add_action( "felblade,if=!((talent.darkglare_boon&fury>=70)|(!talent.darkglare_boon&fury>=100))" );
  fel_dev_prep->add_action( "fracture,if=!(variable.can_spburst|variable.can_spburst_soon)|!((talent.darkglare_boon&fury>=70)|(!talent.darkglare_boon&fury>=100))" );
  ...
  fs->add_action( "call_action_list,name=meta_prep,if=(cooldown.metamorphosis.up|cooldown.metamorphosis.remains<=(gcd.max*3))&!cooldown.fel_devastation.up&!buff.demonsurge_soul_sunder.up&!buff.demonsurge_spirit_burst.up" );
  fs->add_action( "the_hunt" );
  fs->add_action( "soul_carver,if=(!talent.fiery_demise|talent.fiery_demise&dot.fiery_brand.ticking)&(((soul_fragments.total+3)<=6)&fury>=15&!prev_gcd.1.sigil_of_spite)" );
  fs->add_action( "sigil_of_spite,if=(((variable.can_spbomb|(buff.metamorphosis.up&variable.can_spburst))&fury>=40))|((variable.can_spbomb_soon|(buff.metamorphosis.up&variable.can_spburst_soon))|soul_fragments<=1)" );
  ...
  metamorphosis->add_action( "sigil_of_spite,if=((variable.can_spburst&fury>=40)|variable.can_spburst_soon)" );
  metamorphosis->add_action( "spirit_burst,if=talent.spirit_bomb&variable.can_spburst&buff.demonsurge_spirit_burst.up|soul_fragments>=5" );
  ...
  rg_active->add_action( "bulk_extraction,if=!buff.soul_furnace_damage_amp.up&buff.soul_furnace_stack.stack+(spell_targets>?5)>=10" );
  ...
  rg_active->add_action( "felblade" );
  rg_active->add_action( "fracture,if=!buff.rending_strike.up" );
}
//vengeance_apl_end
