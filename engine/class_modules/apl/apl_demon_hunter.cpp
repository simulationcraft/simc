...
void havoc( player_t* p )
{
  ...
  default_->add_action( "reavers_glaive,if=buff.glaive_flurry.down&buff.rending_strike.down&buff.thrill_of_the_fight_damage.remains<4+(variable.rg_ds=2)&(variable.rg_ds=0|variable.rg_ds=1&cooldown.blade_dance.up|variable.rg_ds=2&cooldown.blade_dance.remains)&(buff.thrill_of_the_fight_damage.up|!prev_gcd.1.death_sweep|!variable.rg_inc)&active_enemies<3&!action.reavers_glaive.last_used<5&debuff.essence_break.down|fight_remains<10" );
  ...
  default_->add_action( "immolation_aura,if=hero_tree.felscarred&cooldown.metamorphosis.remains<10&cooldown.eye_beam.remains<10&(buff.unbound_chaos.down|action.fel_rush.charges=0|(cooldown.eye_beam.remains<?cooldown.metamorphosis.remains)<5)" );
  ...
  default_->add_action( "vengeful_retreat,use_off_gcd=1,if=talent.initiative&(cooldown.eye_beam.remains>15&gcd.remains<0.3|gcd.remains<0.1&cooldown.eye_beam.remains<=gcd.remains&(buff.unbound_chaos.up|action.immolation_aura.recharge_time>6|!talent.inertia)&(cooldown.metamorphosis.remains>10|cooldown.blade_dance.remains<gcd.max*2))&time>10" );
  ...
  default_->add_action( "sigil_of_flame,if=active_enemies>3&!talent.student_of_suffering|buff.out_of_range.down&talent.art_of_the_glaive|!hero_tree.aldrachi_reaver&(buff.student_of_suffering.remains<1&cooldown.eye_beam.remains<3|!talent.student_of_suffering)" );
  ...
  cooldown->add_action( "sigil_of_spite,if=debuff.essence_break.down&(debuff.reavers_mark.remains>=2-talent.quickened_sigils|!hero_tree.aldrachi_reaver)&time>15" );
  ...
  opener->add_action( "vengeful_retreat,use_off_gcd=1,if=prev_gcd.1.death_sweep&buff.initiative.remains<2|buff.initiative.remains<0.5&debuff.initiative_tracker.up" );
  opener->add_action( "fel_rush,if=talent.momentum&buff.momentum.remains<6|talent.inertia&buff.unbound_chaos.up&talent.a_fire_inside&(buff.inertia.down&buff.metamorphosis.up&cooldown.blade_dance.remains<gcd.max|hero_tree.felscarred)&debuff.essence_break.down" );
  ...
  opener->add_action( "immolation_aura,if=(charges=2-!talent.a_fire_inside&cooldown.blade_dance.remains|hero_tree.felscarred)&(!hero_tree.felscarred&buff.unbound_chaos.down|hero_tree.felscarred&(cooldown.metamorphosis.up|buff.inertia.remains<gcd.max*2))&debuff.essence_break.down&buff.immolation_aura.stack<5" );
  ...
  opener->add_action( "metamorphosis,if=buff.metamorphosis.up&cooldown.blade_dance.remains&buff.inner_demon.down&(!hero_tree.felscarred|buff.demonsurge.stack=2)&(cooldown.essence_break.remains|talent.shattered_destiny|!talent.essence_break)" );
  ...
  opener->add_action( "eye_beam,if=buff.metamorphosis.down|debuff.essence_break.down&buff.inner_demon.down" );
  opener->add_action( "abyssal_gaze,if=debuff.essence_break.down&cooldown.blade_dance.remains" );
  opener->add_action( "annihilation,if=hero_tree.felscarred&(talent.essence_break|talent.restless_hunter)&buff.demonsurge_annihilation.up,line_cd=10" );
  opener->add_action( "essence_break,if=cooldown.blade_dance.remains<gcd.max&!talent.shattered_destiny&buff.metamorphosis.up|cooldown.eye_beam.remains&cooldown.metamorphosis.remains" );
