...
void havoc_ptr( player_t* p )
{
  ...
  default_->add_action(
      ...
      "Blade Dance with First Blood, Trail of Ruin, Chaos Theory, or at 2+ targets" );
  default_->add_action(
  ...
  default_->add_action(
      "variable,name=pooling_for_eye_beam,value=talent.demonic&!talent.blind_fury&cooldown.eye_beam.remains<(gcd.max*2)"
      "&fury<60" );
  default_->add_action( "variable,name=waiting_for_momentum,value=talent.momentum&!buff.momentum.up" );
  default_->add_action(
  ...
  default_->add_action(
      "throw_glaive,if=talent.serrated_glaive&(buff.metamorphosis.remains>gcd.max*6&(debuff.serrated_glaive.down|"
      "debuff.serrated_glaive.remains<cooldown.essence_break.remains+5&cooldown.essence_break.remains<gcd.max*2)&("
      "cooldown.blade_dance.remains|cooldown.essence_break.remains<gcd.max*2)|time<0.5)&debuff.essence_break.down&"
      "target.time_to_die>gcd.max*8" );
  default_->add_action(
      "throw_glaive,if=talent.serrated_glaive&cooldown.eye_beam.remains<gcd.max*2&debuff.serrated_glaive.remains<(2+"
      "buff.metamorphosis.down*6)&(cooldown.blade_dance.remains|buff.metamorphosis.down)&debuff.essence_break.down&"
      "target.time_to_die>gcd.max*8" );
  default_->add_action( "disrupt" );
  default_->add_action(
  ...
  default_->add_action(
      "pick_up_fragment,mode=nearest,type=lesser,if=talent.demonic_appetite&fury.deficit>=45&(!cooldown.eye_beam.ready|"
      "fury<30)" );
  default_->add_action( "annihilation,if=buff.inner_demon.up&cooldown.metamorphosis.remains<=gcd*3" );
  default_->add_action(
      ...
      "10*talent.cycle_of_hatred)))&(time<30|gcd.remains-1<0)&!talent.any_means_necessary&(!talent.initiative|buff."
      "initiative.remains<gcd.max|time>4)" );
  default_->add_action(
      "vengeful_retreat,use_off_gcd=1,if=talent.initiative&talent.essence_break&time>1&(cooldown.essence_break.remains>"
      ...
      "metamorphosis.up|cooldown.eye_beam.remains>15+(10*talent.cycle_of_hatred)))&talent.any_means_necessary" );
  default_->add_action(
      "vengeful_retreat,use_off_gcd=1,if=talent.initiative&!talent.essence_break&time>1&!buff.momentum.up" );
  default_->add_action(
      "fel_rush,if=talent.momentum.enabled&buff.momentum.remains<gcd.max*2&(charges_fractional>1.8|cooldown.eye_beam."
      "remains<3)&debuff.essence_break.down&cooldown.blade_dance.remains" );
  default_->add_action(
      "essence_break,if=(active_enemies>desired_targets|raid_event.adds.in>40)&!variable.waiting_for_momentum&(buff."
      "metamorphosis.remains>gcd.max*3|cooldown.eye_beam.remains>10)&(!talent.tactical_retreat|buff.tactical_retreat."
      "up|time<10)&buff.vengeful_retreat_movement.remains<gcd.max*0.5&cooldown.blade_dance.remains<=3.1*gcd.max|fight_"
      "remains<6" );
  default_->add_action(
  ...
  default_->add_action( "annihilation,if=buff.inner_demon.up&cooldown.eye_beam.remains<=gcd" );
  default_->add_action(
      "fel_rush,if=talent.momentum.enabled&cooldown.eye_beam.remains<gcd.max*3&buff.momentum.remains<5&buff."
      "metamorphosis.down" );
  default_->add_action(
      "the_hunt,if=debuff.essence_break.down&(time<10|cooldown.metamorphosis.remains>10|!equipped.algethar_puzzle_box)&"
      "(raid_event.adds.in>90|active_enemies>3|time_to_die<10)&(time>6&debuff.essence_break.down&(!talent.furious_gaze|"
      "buff.furious_gaze.up)|!set_bonus.tier30_2pc)" );
  default_->add_action(
      "eye_beam,if=active_enemies>desired_targets|raid_event.adds.in>(40-talent.cycle_of_hatred*15)&!debuff.essence_"
      "break.up&(cooldown.metamorphosis.remains>40-talent.cycle_of_hatred*15|!variable.holding_meta)&(buff."
      "metamorphosis.down|buff.metamorphosis.remains>gcd.max|!talent.restless_hunter)|fight_remains<15" );
  default_->add_action(
  ...
  default_->add_action( "sigil_of_flame,if=talent.any_means_necessary&debuff.essence_break.down&active_enemies>=4" );
  default_->add_action(
      "throw_glaive,if=talent.soulrend&(active_enemies>desired_targets|raid_event.adds.in>full_recharge_time+9)&spell_"
  ...
  default_->add_action(
      "throw_glaive,if=talent.soulrend&(active_enemies>desired_targets|raid_event.adds.in>full_recharge_time+9)&spell_"
      "targets>=(2-talent.furious_throws)&!debuff.essence_break.up" );
  default_->add_action(
      "immolation_aura,if=!buff.immolation_aura.up&(!talent.ragefire|active_enemies>desired_targets|raid_event.adds.in>"
      "15)&buff.out_of_range.down" );
  default_->add_action( "fel_rush,if=talent.isolated_prey&active_enemies=1&fury.deficit>=35" );
  default_->add_action( "chaos_strike,if=!variable.pooling_for_blade_dance&!variable.pooling_for_eye_beam" );
  default_->add_action( "sigil_of_flame,if=raid_event.adds.in>15&fury.deficit>=30&buff.out_of_range.down" );
  default_->add_action( "felblade,if=fury.deficit>=40&buff.out_of_range.down" );
  ...
  cooldown->add_action(
      "metamorphosis,if=talent.demonic&(!talent.chaotic_transformation|cooldown.eye_beam.remains>20&(!variable.blade_"
      "dance|cooldown.blade_dance.remains>gcd.max)|fight_remains<25+talent.shattered_destiny*70&cooldown.eye_beam."
      "remains&cooldown.blade_dance.remains)" );
  cooldown->add_action(
...
void vengeance_ptr( player_t* p )
{
  ...
  default_->add_action( "variable,name=next_fire_cd_time,value=cooldown.fel_devastation.remains",
  ...
  default_->add_action( "variable,name=incoming_souls,op=add,value=3,if=prev_gcd.1.fracture&buff.metamorphosis.up" );
  default_->add_action(
      "variable,name=incoming_souls,op=add,value=2,if=talent.soul_sigils&(prev_gcd.2.sigil_of_flame|prev_gcd.2.sigil_"
      "of_silence|prev_gcd.2.sigil_of_chains|prev_gcd.2.elysian_decree)" );
  default_->add_action(
      "variable,name=incoming_souls,op=add,value=active_enemies>?3,if=talent.elysian_decree&prev_gcd.2.elysian_"
      "decree" );
  default_->add_action(
      "variable,name=incoming_souls,op=add,value=0.6*active_enemies>?5,if=talent.fallout&prev_gcd.1.immolation_aura" );
  default_->add_action(
      "variable,name=incoming_souls,op=add,value=active_enemies>?5,if=talent.bulk_extraction&prev_gcd.1.bulk_"
      "extraction" );
  default_->add_action(
      "variable,name=incoming_souls,op=add,value=3-(cooldown.soul_carver.duration-ceil(cooldown.soul_carver.remains)),"
      "if=talent.soul_carver&cooldown.soul_carver.remains>57" );
  default_->add_action( "variable,name=fd,value=talent.fiery_demise&dot.fiery_brand.ticking", "Fiery Demise is up" );
  default_->add_action( "auto_attack" );
  default_->add_action( "disrupt,if=target.debuff.casting.react" );
  default_->add_action( "demon_spikes,use_off_gcd=1,if=!buff.demon_spikes.up&!cooldown.pause_action.remains" );
  default_->add_action( "call_action_list,name=externals" );
  default_->add_action(
      "metamorphosis,use_off_gcd=1,if=talent.first_of_the_illidari&(trinket.beacon_to_the_beyond.cooldown.remains<10|"
      "fight_remains<=20|!equipped.beacon_to_the_beyond|fight_remains%%120>5&fight_remains%%120<30)" );
  default_->add_action( "infernal_strike,use_off_gcd=1" );
  default_->add_action( "potion" );
  default_->add_action( "call_action_list,name=trinkets" );
  default_->add_action( "call_action_list,name=maintenance" );
  ...
  externals->add_action( "invoke_external_buff,name=symbol_of_hope,if=cooldown.fiery_brand.charges=0",
                         "External buffs" );
  externals->add_action( "invoke_external_buff,name=power_infusion" );

  trinkets->add_action( "use_item,name=elementium_pocket_anvil,use_off_gcd=1", "Trinkets" );
  trinkets->add_action(
      "use_item,name=beacon_to_the_beyond,use_off_gcd=1,if=!talent.first_of_the_illidari|buff.metamorphosis.up|fight_"
      "remains<=10|cooldown.metamorphosis.remains>25" );
  trinkets->add_action(
  ...
  big_aoe->add_action( "spirit_bomb,if=(active_enemies<=7&soul_fragments>=4)|(active_enemies>7&soul_fragments>=3)",
                       "6+ targets" );
  big_aoe->add_action( "fel_devastation,if=!(talent.demonic&buff.metamorphosis.up)" );
  big_aoe->add_action( "the_hunt" );
  big_aoe->add_action( "elysian_decree,if=(soul_fragments+variable.incoming_souls)<=2" );
