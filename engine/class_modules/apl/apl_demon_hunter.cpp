...
void havoc( player_t* p )
{
  ...
  meta->add_action( "death_sweep,if=debuff.essence_break.up|cooldown.essence_break.remains>=gcd.max*2|(cooldown.metamorphosis.up&cooldown.eye_beam.remains&!talent.restless_hunter)", "actions.meta+=/death_sweep" );
  ...
  opener->add_action( "vengeful_retreat,use_off_gcd=1,if=buff.metamorphosis.up&cooldown.metamorphosis.remains&cooldown.essence_break.up&cooldown.eye_beam.remains&talent.shattered_destiny&talent.essence_break&(cooldown.sigil_of_spite.remains|!hero_tree.aldrachi_reaver)", "actions.opener+=/vengeful_retreat,use_off_gcd=1,if=(prev_gcd.1.death_sweep&buff.initiative.remains<2&buff.inner_demon.down|buff.initiative.remains<0.5&debuff.initiative_tracker.up&(!talent.inertia|buff.unbound_chaos.down))&((!talent.essence_break&talent.shattered_destiny)|(talent.essence_break&!talent.shattered_destiny)|(!talent.shattered_destiny&!talent.essence_break))|prev_gcd.2.death_sweep&prev_gcd.1.annihilation&!talent.shattered_destiny" );
  opener->add_action( "vengeful_retreat,use_off_gcd=1,if=(cooldown.blade_dance.up|buff.blade_rhapsody.up|cooldown.blade_dance.remains<=gcd.max)&buff.metamorphosis.remains>=gcd.max*2&buff.thrill_of_the_fight_damage.up&cooldown.essence_break.up&cooldown.metamorphosis.up" );
  opener->add_action( "vengeful_retreat,use_off_gcd=1,if=cooldown.metamorphosis.remains&buff.demonsurge.stack=4&cooldown.eye_beam.up&talent.initiative&(cooldown.essence_break.up|!talent.essence_break)", "actions.opener+=/vengeful_retreat,use_off_gcd=1,if=cooldown.metamorphosis.remains&buff.demonsurge.stack=5&cooldown.eye_beam.remains&talent.initiative&(cooldown.essence_break.up|!talent.essence_break)" );
  ...
  opener->add_action( "fel_rush,if=talent.inertia&buff.unbound_chaos.up&cooldown.metamorphosis.remains&buff.demonsurge.stack=3" );
  opener->add_action( "the_hunt,if=(buff.metamorphosis.up|!talent.shattered_destiny)&(!talent.initiative|buff.initiative.up|time>5)" );
  opener->add_action( "death_sweep,if=hero_tree.felscarred&talent.chaos_theory&buff.metamorphosis.up&buff.demonsurge.stack=0&!talent.restless_hunter&(buff.out_of_range.up|buff.out_of_range.down)" );
  ...
  opener->add_action( "immolation_aura,if=talent.inertia&buff.unbound_chaos.down&buff.metamorphosis.up&debuff.essence_break.down&(cooldown.blade_dance.remains|buff.demonsurge_death_sweep.down&buff.inertia.up)&(buff.inner_demon.down|hero_tree.felscarred&buff.demonsurge_annihilation.down)&buff.demonsurge.stack<5" );
  ...
  opener->add_action( "essence_break,if=cooldown.eye_beam.remains&cooldown.metamorphosis.up&buff.blade_rhapsody.up&buff.inner_demon.down&!hero_tree.felscarred&buff.metamorphosis.remains>=gcd.max" );
  opener->add_action( "metamorphosis,if=debuff.essence_break.up&cooldown.blade_dance.remains" );
...
void vengeance( player_t* p )
{
  ...
  fel_dev_prep->add_action( "sigil_of_flame,if=cooldown.metamorphosis.remains>recharge_time" );
  fel_dev_prep->add_action( "fiery_brand,if=talent.fiery_demise&((fury+variable.fel_dev_passive_fury_gen)>=120)&(variable.can_spburst|variable.can_spburst_soon|soul_fragments.total>=4)&active_dot.fiery_brand=0&(cooldown.metamorphosis.remains<(execute_time+action.fel_devastation.execute_time+(gcd.max*2)))" );
  ...
  fel_dev_prep->add_action( "felblade,if=!((fury+variable.fel_dev_passive_fury_gen)>=120)" );
  ...
  fs->add_action( "variable,name=fel_dev_passive_fury_gen,op=add,value=30+(2*talent.flames_of_fury*spell_targets.sigil_of_flame),if=(cooldown.sigil_of_flame.remains<variable.fel_dev_sequence_time)&!variable.hold_sof" );
  ...
  metamorphosis->add_action( "spirit_burst,if=variable.can_spburst&(buff.demonsurge_spirit_burst.up|talent.fiery_demise&dot.fiery_brand.ticking&soul_fragments>=4)" );
  ...
  metamorphosis->add_action( "fracture,if=talent.spirit_bomb&buff.demonsurge_spirit_burst.up&!(variable.can_spburst|variable.can_spburst_soon)" );
