...
void windwalker( player_t* p )
{
  ...
  aoe->add_action( p, "Spinning Crane Kick",
                   "if=combo_strike&((cooldown.bonedust_brew.remains>2&(chi>3|cooldown.fists_of_fury.remains>6)&(chi>="
                   "5|cooldown.fists_of_fury.remains>2))|energy.time_to_max<=3)" );
  ...
  if ( monk->spec.invoke_xuen->ok() )
  {
    cd_serenity->add_action( p, "Touch of Death",
                             "if=fight_remains>180|pet.xuen_the_white_tiger.active|fight_remains<10" );
    cd_serenity->add_action( p, "Touch of Karma",
                             "if=fight_remains>90|pet.xuen_the_white_tiger.active|fight_remains<10" );
  }
  else
  {
    cd_serenity->add_action( p, "Touch of Death", "if=fight_remains>180|fight_remains<10" );
  ...
  if ( monk->spec.invoke_xuen->ok() )
    cd_sef->add_action(
        p, "Touch of Death",
        "if=buff.storm_earth_and_fire.down&pet.xuen_the_white_tiger.active|fight_remains<10|fight_remains>180" );
  else
    cd_sef->add_action( p, "Touch of Death", "if=buff.storm_earth_and_fire.down|fight_remains<10|fight_remains>180" );
  ...
  for ( size_t i = 0; i < p->items.size(); i++ )
  {
    if ( p->items[ i ].has_special_effect( SPECIAL_EFFECT_SOURCE_ITEM, SPECIAL_EFFECT_USE ) )
    {
      serenity->add_action( "use_item,name=" + p->items[ i ].name_str );
    }
  }
  ...
  serenity->add_action( p, "Blackout Kick",
                        "target_if=min:debuff.mark_of_the_crane.remains,if=combo_strike&buff.weapons_of_order_ww.up&"
                        "cooldown.rising_sun_kick.remains>2" );
  ...
  weapons_of_order->add_action( p, "Spinning Crane Kick", "if=combo_strike&buff.dance_of_chiji.up" );
  ...
  weapons_of_order->add_action( p, "Blackout Kick",
                                "target_if=min:debuff.mark_of_the_crane.remains,if=combo_strike&active_enemies<=2" );
  weapons_of_order->add_talent( p, "Whirling Dragon Punch" );
  weapons_of_order->add_action(
      p, "Fists of Fury",
  ...
  weapons_of_order->add_action(
      p, "Blackout Kick", "target_if=min:debuff.mark_of_the_crane.remains,if=chi>=3|buff.weapons_of_order_ww.up" );
