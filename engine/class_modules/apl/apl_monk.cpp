...
namespace monk_apl
{
  ...
  void windwalker( player_t *p )
  {
    ...
    cd_serenity->add_action( "actions.cd_serenity+=/invoke_xuen_the_white_tiger,if=fight_style.dungeonslice&talent.bonedust_brew&target.time_to_die>16&!cooldown.serenity.remains&cooldown.bonedust_brew.remains<2");
    ...
    serenity_2t->add_action( "actions.serenity_2t+=/spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike&buff.dance_of_chiji.up&!buff.blackout_reinforcement.up&set_bonus.tier31_2pc" );
