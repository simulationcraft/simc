...
namespace monk_apl
{
  ...
  void windwalker( player_t *p )
  {
    ...
    // Snapshot stats
    pre->add_action( "snapshot_stats", "Snapshot raid buffed stats before combat begins and pre-potting is done." );
    pre->add_action( "variable,name=sync_serenity,default=0,value=1,if=(equipped.neltharions_call_to_dominance|equipped.ashes_of_the_embersoul|equipped.mirror_of_fractured_tomorrows|equipped.witherbarks_branch)&!fight_style.dungeonslice" );
    ...
    cd_sef->add_action( "invoke_xuen_the_white_tiger,if=target.time_to_die>25&fight_remains>120&((trinket.1.is.ashes_of_the_embersoul|trinket.1.is.witherbark_branch)&!trinket.1.cooldown.remains|(trinket.2.is.ashes_of_the_embersoul|trinket.2.is.witherbark_branch)&!trinket.2.cooldown.remains)" );
    ...
    cd_serenity->add_action( "invoke_xuen_the_white_tiger,if=(target.time_to_die>25|fight_style.dungeonslice&cooldown.serenity.remains<2)&fight_remains>120&((trinket.1.is.ashes_of_the_embersoul|trinket.1.is.witherbark_branch)&!trinket.1.cooldown.remains|(trinket.2.is.ashes_of_the_embersoul|trinket.2.is.witherbark_branch)&!trinket.2.cooldown.remains)" );
