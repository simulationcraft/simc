...
void survival( player_t* p )
{
  ...
  bop -> add_action( "kill_command,target_if=min:bloodseeker.remains,if=focus+cast_regen<focus.max&!runeforge.nessingwarys_trapping_apparatus" , "If you don't have Nessingwary's Trapping Apparatus, simply cast Kill Command if you won't overcap on Focus from doing so" ); 
  bop -> add_action( "kill_command,target_if=min:bloodseeker.remains,if=focus+cast_regen<focus.max&((runeforge.nessingwarys_trapping_apparatus&!talent.steel_trap&cooldown.freezing_trap.remains&cooldown.tar_trap.remains)|(runeforge.nessingwarys_trapping_apparatus&talent.steel_trap&cooldown.freezing_trap.remains>focus%(30-cast_regen)*gcd&cooldown.tar_trap.remains>focus%(30-cast_regen)*gcd&cooldown.steel_trap.remains>focus%(30-cast_regen)*gcd))|focus<30", "With Nessingwary's Trapping Apparatus only Kill Command if your traps are on cooldown or if you lack the focus to use Raptor Strike or Mongoose Bite. Otherwise stop using Kill Command if your current focus amount is enough to sustain the amount of time left for any of your traps to come off cooldown" );
