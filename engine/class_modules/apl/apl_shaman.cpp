...
void elemental( player_t* p )
{
  ...
  aoe->add_action( "elemental_blast,if=talent.tempest" );
  ...
  single_target->add_action( "earth_shock,if=buff.flowing_elements.up&buff.overcharge_tier.up" );
  single_target->add_action( "lava_burst,if=talent.master_of_the_elements&!buff.master_of_the_elements.up&maelstrom.deficit>15&cooldown.lava_burst.charges_fractional>1.8&((!buff.storm_elemental.up|buff.wind_gust.stack=4)+talent.call_of_the_ancestors>=spell_targets.chain_lightning)", "Lava Burst to proc MotE if you are about to reach 2 charges and (1 target OR wind gust stacked for Farseer) or (1 target AND wind gust stacked for SB)." );
  single_target->add_action( "lava_burst,if=talent.master_of_the_elements&!buff.master_of_the_elements.up&maelstrom.deficit>15&(maelstrom>52-5*talent.eye_of_the_storm*(1+talent.elemental_blast)+30*talent.elemental_blast)", "Lava Burst to proc MotE if you have enough maelstrom for a spender." );
  ...
  single_target->add_action( "earthquake,if=buff.call_of_the_ancestors.stack>=3&(buff.master_of_the_elements.up|!talent.master_of_the_elements|maelstrom.deficit<15)&spell_targets.chain_lightning=2" );
  single_target->add_action( "elemental_blast,target_if=min:debuff.lightning_rod.remains,if=buff.master_of_the_elements.up|maelstrom.deficit<15" );
  single_target->add_action( "earthquake,if=(buff.master_of_the_elements.up|!talent.master_of_the_elements|maelstrom.deficit<15)&buff.call_of_the_ancestors.up&!talent.elemental_blast&spell_targets.chain_lightning=2" );
  single_target->add_action( "earth_shock,target_if=min:debuff.lightning_rod.remains,if=buff.master_of_the_elements.up|maelstrom.deficit<15" );
  single_target->add_action( "flame_shock,target_if=min:dot.flame_shock.remains,if=buff.master_of_the_elements.up&dot.flame_shock.refreshable", "Consume spare MotE to buff Flame Shock, mostly on off-targets." );
  single_target->add_action( "voltaic_blaze,if=talent.crackling_fury&!buff.ascendance.up" );
  single_target->add_action( "tempest" );
  single_target->add_action( "chain_lightning,if=spell_targets.chain_lightning=2", "Filler spell. Always available. Always the bottom line." );
