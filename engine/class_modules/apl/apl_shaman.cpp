...
void elemental( player_t* p )
{
  ...
  default_->add_action( "use_item,slot=trinket1,use_off_gcd=1", "Dmg trinkets" );
  default_->add_action( "use_item,slot=trinket2,use_off_gcd=1" );
...
void elemental_ptr( player_t* p )
{
  ...
  aoe->add_action( "ancestral_swiftness" );
  aoe->add_action(
      "flame_shock,if=active_dot.flame_shock=0&spell_targets.chain_lightning<=3&cooldown.ascendance.remains>10" );
  aoe->add_action( "voltaic_blaze,if=active_dot.flame_shock=0|talent.purging_flames" );
  aoe->add_action( "ascendance,if=cooldown.stormkeeper.remains>10" );
  aoe->add_action( "tempest,target_if=min:debuff.lightning_rod.remains,if=buff.tempest.stack=2" );
  aoe->add_action( "earthquake" );
  aoe->add_action(
      "lava_burst,target_if=dot.flame_shock.remains>2,if=buff.purging_flames.up&(buff.ascendance.up&buff.ascendance."
      "remains<3|buff.lava_surge.up)",
  ...
  single_target->add_action( "stormkeeper" );
  single_target->add_action( "ancestral_swiftness" );
  single_target->add_action( "flame_shock,if=active_dot.flame_shock=0&!buff.master_of_the_elements.up",
