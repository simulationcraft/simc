...
public:
  ...
  struct talents_t
  {
    ...
    const spell_data_t* gladiators_resolve;
...
struct slam_sweeping_strikes_attack_t : public warrior_attack_t
{
  ...
  {
    return data().effectN( 2 ).percent(); //does not double dip on anything
  }
...
struct shield_wall_t : public warrior_spell_t
{
    ...
    warrior_spell_t( "shield_wall", p, p -> find_class_spell( "Shield Wall" ) )
  {
    ...
    if ( p -> spec.bastion_of_defense -> ok() && p -> active_stance != STANCE_GLADIATOR )
...
double warrior_t::composite_dodge() const
{
  ...
  if( spec.bastion_of_defense -> ok() && active_stance != STANCE_GLADIATOR )
