...
struct death_knight_pet_t : public pet_t
{
  ...
  {
    pet_t::arise();
    owner_attack_power          = owner->cache.total_attack_power() * owner_coeff.ap_from_ap;
...
struct stat_event_t : public player_event_t
{
  ...
  void execute() override
  {
    for ( auto pet : p()->active_pets )
    {
      ...
      dk_pet->owner_attack_power = dk_pet->owner->cache.total_attack_power() * dk_pet->owner_coeff.ap_from_ap;
