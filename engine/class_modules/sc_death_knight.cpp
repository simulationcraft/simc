...
struct howling_blast_t : public death_knight_spell_t
{
  ...
  {
    // If we have rime, and no runes left, and rime is about to expire, do not queue howling blast
    // this avoids a crash where the buff would get removed during the actions executing, before
    // howling blast goes off.  However, if we do have runes left, we will allow howling blast to
    // get fired off, to more accurately reflect what a player would do
    if ( p() -> buffs.rime -> check() && 
         p() -> _runes.runes_full() == 0 && p() -> buffs.rime -> remains() == 0_ms )
    {
