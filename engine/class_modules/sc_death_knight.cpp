...
struct gargoyle_pet_t : public death_knight_pet_t
{
  ...
  struct gargoyle_strike_t : public pet_spell_t<gargoyle_pet_t>
  {
    ...
    void execute() override
    {
      ...
      if( dk() -> bugs && gargoyle_strike_count % 2 == 0 && ( dk() -> pet_spell.gargoyle_strike -> cast_time() / dk() -> composite_spell_haste() <= 1_s ) && ( dk() -> pet_spell.gargoyle_strike -> cast_time() / dk() -> composite_spell_haste() >= 0.751_s ) )
      {
        min_gcd = 1_s;
      }
      else if ( dk() -> bugs && gargoyle_strike_count % 2 == 0 && dk() -> pet_spell.gargoyle_strike -> cast_time() / dk() -> composite_spell_haste() <= 0.75_s )
      {
        min_gcd = 0.75_s;
      }
      pet_spell_t::execute();
