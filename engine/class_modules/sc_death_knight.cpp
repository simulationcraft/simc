...
namespace { // UNNAMED NAMESPACE
  ...
  namespace pets {
    ...
    struct dt_pet_melee_t : auto_attack_melee_t < base_ghoul_pet_t > {
      dt_pet_melee_t(base_ghoul_pet_t * pet) : auto_attack_melee_t < base_ghoul_pet_t >(pet) {}

      void impact(action_state_t * s) override {
        auto_attack_melee_t < base_ghoul_pet_t > ::impact(s);

        if (result_is_hit(s->result) && rng().roll(p()->o()->legendary.uvanimor_the_unbeautiful)) {
          if (sim->debug) {
            log_rune_status(p()->o());
          }
    ...
    struct dt_pet_t : public base_ghoul_pet_t
    {
      ...
      attack_t* create_auto_attack() override
      {
        return new dt_pet_melee_t(this);
      }
    };
