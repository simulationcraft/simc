...
void death_knight_t::default_apl_blood()
{
  ...
  // Essences
  essences -> add_action( "Concentraded Flame,if=dot.concentrated_flame_burn.remains<2&!buff.dancing_rune_weapon.up" );
  essences -> add_action( "Anima of Death,if=buff.vampiric_blood.up&(raid_event.adds.exists|raid_event.adds.in>15)" );
  essences -> add_action( "Memory of Lucid Dreams,if=rune.time_to_1>gcd&runic_power<40" );
  essences -> add_action( "Worldvein Resonance" );
  essences -> add_action( "Ripple in Space,if=!buff.dancing_rune_weapon.up" );
