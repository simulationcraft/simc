...
namespace priestspace
{
  namespace actions
  {
    namespace spells
    {
      ...
      public:
        ...
        void init() override
        {
          if (priest().active_items.mangazas_madness) // FIXME is this check needed? && priest().cooldowns.shadow_word_void->charges < 1)
          {
  ...
  void priest_t::create_buffs_shadow()
  {
    ...
    buffs.shadowform                 = make_buff(this, "shadowform", find_class_spell("Shadowform"))
                                         ->add_invalidate(CACHE_PLAYER_DAMAGE_MULTIPLIER);
    buffs.shadowform_state           = make_buff(this, "shadowform_state")
                                         ->set_chance(1.0)->set_quiet(true);
    buffs.shadowy_insight            = make_buff(this, "shadowy_insight", talents.shadowy_insight)
                                         ->set_rppm(RPPM_HASTE)->set_max_stack(1U);  // Spell Data says 2, really is 1 -- 2016/04/17 Twintop
    ...
    buffs.void_vb                    = make_buff(this, "void", find_spell(211657))
                                         ->set_chance(1.0);
  ...
  action_t* priest_t::create_action_shadow( const std::string& name, const std::string& options_str )
  {
    ...
    if (name == "void_bolt")              return new void_bolt_t(*this, options_str);
    if (name == "void_eruption")          return new void_eruption_t(*this, options_str);
    if (name == "mind_sear")              return new mind_sear_t(*this, options_str);
    if (name == "shadow_crash")           return new shadow_crash_t(*this, options_str);
    if (name == "shadow_word_death")      return new shadow_word_death_t(*this, options_str);
    if (name == "void_torrent")           return new void_torrent_t(*this, options_str);
