...
namespace warlock {
  ...
  void warlock_t::create_apl_destruction() {
    ...
    def->add_action(this, "Havoc", "cycle_targets=1,if=!(target=current_target)&(dot.immolate.remains>dot.immolate.duration*0.5|!talent.internal_combustion.enabled)&(!cooldown.summon_infernal.ready|!talent.grimoire_of_supremacy.enabled|talent.grimoire_of_supremacy.enabled&pet.infernal.remains<=10)", "The if condition here always applies Havoc to something other than the primary target");
    ...
    aoe->add_action(this, "Havoc", "cycle_targets=1,if=!(target=current_target)&active_enemies<4", "Three targets is an in-between case that gives a slight benefit to higher priority Havoc");
    ...
    aoe->add_action(this, "Havoc", "cycle_targets=1,if=!(target=sim.target)&(!talent.grimoire_of_supremacy.enabled|!talent.inferno.enabled|talent.grimoire_of_supremacy.enabled&pet.infernal.remains<=10)", "Even if the Havoc priority list won't be used, Havoc is pretty much free damage and resources and should be used almost on cooldown");
