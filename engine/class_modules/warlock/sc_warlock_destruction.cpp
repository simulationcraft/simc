...
namespace warlock {
  ...
  void warlock_t::create_apl_destruction() {
    ...
    cds->add_action( "worldvein_resonance,if=pet.infernal.active&(pet.infernal.remains<18.5|pet.infernal.remains<20&soul_shard>=3.6|!talent.grimoire_of_supremacy.enabled)" );
    ...
    cds->add_action( "worldvein_resonance,if=cooldown.summon_infernal.remains>target.time_to_die&pet.infernal.remains<18.5" );
