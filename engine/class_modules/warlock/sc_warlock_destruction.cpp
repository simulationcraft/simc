...
public:
    ...
    if ( resource_current == RESOURCE_SOUL_SHARD && p()->in_combat )
    {
      if ( p()->legendary.the_master_harvester )
      {
        double sh_proc_chance =
            p()->find_spell( p()->legendary.the_master_harvester->spell_id )->effectN( 3 ).percent();
  ...
  double composite_target_multiplier( player_t* t ) const override
  {
    ...
    if ( p()->legendary.odr_shawl_of_the_ymirjar && target == p()->havoc_target &&
         affected_by_odr_shawl_of_the_ymirjar )
  ...
  virtual void impact( action_state_t* s ) override
  {
    ...
    if ( result_is_hit( s->result ) )
    {
      ...
      p()->resource_gain( RESOURCE_SOUL_SHARD,
                          ( std::double_t( p()->find_spell( 245731 )->effectN( 1 ).base_value() ) / 10 ),
                          p()->gains.shadowburn );
    }
  }
};
...
struct conflagrate_t : public destruction_spell_t
{
  ...
  void impact( action_state_t* s ) override
  {
    ...
    if ( result_is_hit( s->result ) )
    {
      ...
      p()->resource_gain( RESOURCE_SOUL_SHARD,
                          ( std::double_t( p()->find_spell( 245330 )->effectN( 1 ).base_value() ) / 10 ),
                          p()->gains.conflagrate );
    }
  }
...
struct incinerate_t : public destruction_spell_t
{
  ...
  incinerate_t( warlock_t* p, const std::string& options_str ) : destruction_spell_t( p, "Incinerate" )
  {
    parse_options( options_str );
  ...
  void impact( action_state_t* s ) override
  {
    ...
    if ( p()->sets->has_set_bonus( WARLOCK_DESTRUCTION, T21, B4 ) )
    {
      residual_action::trigger(
          flames_of_argus, s->target,
          s->result_amount * p()->sets->set( WARLOCK_DESTRUCTION, T21, B4 )->effectN( 1 ).percent() );
    }
  }
};
...
struct rain_of_fire_t : public destruction_spell_t
{
  ...
  virtual void execute() override
  {
    ...
    if ( p()->legendary.alythesss_pyrogenics )
      p()->buffs.alythesss_pyrogenics->trigger( 1, buff_t::DEFAULT_VALUE(), -1.0,
                                                data().duration() * player->cache.spell_haste() );
  }
};
...
void warlock_t::create_buffs_destruction()
{
  ...
  buffs.embrace_chaos =
      make_buff( this, "embrace_chaos", sets->set( WARLOCK_DESTRUCTION, T19, B2 )->effectN( 1 ).trigger() )
...
void warlock_t::create_apl_destruction()
{
  ...
  def->add_action(
      "chaos_bolt,cycle_targets=1,if=!debuff.havoc.remains&!talent.internal_combustion.enabled&soul_shard>=4|(talent."
      "eradication.enabled&debuff.eradication.remains<=cast_time)|buff.dark_soul_instability.remains>cast_time|pet."
      "infernal.active&talent.grimoire_of_supremacy.enabled" );
  def->add_action(
      "chaos_bolt,cycle_targets=1,if=!debuff.havoc.remains&talent.internal_combustion.enabled&dot.immolate.remains>8|"
      "soul_shard=5" );
  def->add_action(
      "conflagrate,cycle_targets=1,if=target=sim.target&(talent.flashover.enabled&buff.backdraft.stack<=2)|(!talent."
      "flashover.enabled&buff.backdraft.stack<2)" );
  def->add_talent( this, "Sahdowburn",
                   ",cycle_targets=1,if=target=sim.target&charges=2|!buff.backdraft.remains|buff.backdraft.remains>"
  ...
  aoe->add_action(
      "immolate,cycle_targets=1,if=refreshable&(!talent.fire_and_brimstone.enabled|talent.cataclysm.enabled&cooldown."
      "cataclysm.remains>=12|talent.inferno.enabled)" );
  aoe->add_action( "rain_of_fire" );
  aoe->add_action(
      "conflagrate,if=(talent.flashover.enabled&buff.backdraft.stack<=2)|(!talent.flashover.enabled&buff.backdraft."
      "stack<2)" );
  aoe->add_talent( this, "Shadowburn",
                   "if=!talent.fire_and_brimstone.enabled&(charges=2|!buff.backdraft.remains|buff.backdraft.remains>"
                   "buff.backdraft.stack*action.incinerate.execute_time)" );
