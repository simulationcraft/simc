  ...
  void consume_resource() override
  {
    ...
    if ( resource_current == RESOURCE_SOUL_SHARD && p()->in_combat )
    {
      ...
      if ( p()->talents.soul_fire->ok() )
        p()->cooldowns.soul_fire->adjust(
            ( -1 * ( p()->talents.soul_fire->effectN( 2 ).time_value() * last_resource_cost ) ) );
    }
  }
...
{
  roaring_blaze_t( warlock_t* p ) : destruction_spell_t( "roaring_blaze", p, p->find_spell( 265931 ) )
  {
    background     = true;
    destro_mastery = false;
    hasted_ticks   = true;
...
struct conflagrate_t : public destruction_spell_t
{
  ...
  void execute() override
  {
    ...
    p()->buffs.backdraft->trigger(
        1 + ( p()->talents.flashover->ok() ? p()->talents.flashover->effectN( 1 ).base_value() : 0 ) );
...
void warlock_t::create_buffs_destruction()
{
  ...
  buffs.backdraft = make_buff( this, "backdraft", find_spell( 117828 ) )
                        ...
                        ->set_max_stack( find_spell( 117828 )->max_stacks() +
...
void warlock_t::create_apl_destruction()
{
  ...
  def->add_action(
      "run_action_list,name=cata,if=spell_targets.infernal_awakening>=3+raid_event.invulnerable.up&talent.cataclysm."
      "enabled" );
  def->add_action(
      "run_action_list,name=fnb,if=spell_targets.infernal_awakening>=3+raid_event.invulnerable.up&talent.fire_and_"
      "brimstone.enabled" );
  def->add_action(
      "run_action_list,name=inf,if=spell_targets.infernal_awakening>=3+raid_event.invulnerable.up&talent.inferno."
      "enabled" );
  def->add_talent( this, "Cataclysm" );
  def->add_action(
      "immolate,cycle_targets=1,if=!debuff.havoc.remains&(refreshable|talent.internal_combustion.enabled&action.chaos_"
      "bolt.in_flight&remains-action.chaos_bolt.travel_time-5<duration*0.3)" );
  def->add_action( "call_action_list,name=cds" );
  def->add_talent( this, "Channel Demonfire", "if=!buff.active_havoc.remains" );
  def->add_action(
      "havoc,cycle_targets=1,if=!(target=sim.target)&target.time_to_die>10&active_enemies>1+raid_event.invulnerable."
      "up" );
  ...
  def->add_action(
      "chaos_bolt,cycle_targets=1,if=!debuff.havoc.remains&execute_time+travel_time<target.time_to_die&(trinket.proc."
      "intellect.react&trinket.proc.intellect.remains>cast_time|trinket.proc.mastery.react&trinket.proc.mastery."
      "remains>cast_time|trinket.proc.versatility.react&trinket.proc.versatility.remains>cast_time|trinket.proc.crit."
      "react&trinket.proc.crit.remains>cast_time|trinket.proc.spell_power.react&trinket.proc.spell_power.remains>cast_"
      "time)" );
  def->add_action(
      "chaos_bolt,cycle_targets=1,if=!debuff.havoc.remains&execute_time+travel_time<target.time_to_die&(trinket."
      "stacking_proc.intellect.react&trinket.stacking_proc.intellect.remains>cast_time|trinket.stacking_proc.mastery."
      "react&trinket.stacking_proc.mastery.remains>cast_time|trinket.stacking_proc.versatility.react&trinket.stacking_"
      "proc.versatility.remains>cast_time|trinket.stacking_proc.crit.react&trinket.stacking_proc.crit.remains>cast_"
      "time|trinket.stacking_proc.spell_power.react&trinket.stacking_proc.spell_power.remains>cast_time)" );
  def->add_action(
      "chaos_bolt,cycle_targets=1,if=!debuff.havoc.remains&execute_time+travel_time<target.time_to_die&(cooldown."
      "summon_infernal.remains>=20|!talent.grimoire_of_supremacy.enabled)&(cooldown.dark_soul_instability.remains>=20|!"
      "talent.dark_soul_instability.enabled)&(talent.eradication.enabled&debuff.eradication.remains<=cast_time|buff."
      "backdraft.remains|talent.internal_combustion.enabled)" );
  def->add_action(
      "chaos_bolt,cycle_targets=1,if=!debuff.havoc.remains&execute_time+travel_time<target.time_to_die&(soul_shard>=4|"
      "buff.dark_soul_instability.remains>cast_time|pet.infernal.active|buff.active_havoc.remains>cast_time)" );
  def->add_action(
      "conflagrate,cycle_targets=1,if=!debuff.havoc.remains&((talent.flashover.enabled&buff.backdraft.stack<=2)|(!"
      "talent.flashover.enabled&buff.backdraft.stack<2))" );
  def->add_talent( this, "Shadowburn",
                   "cycle_targets=1,if=!debuff.havoc.remains&((charges=2|!buff.backdraft.remains|buff.backdraft."
                   "remains>buff.backdraft.stack*action.incinerate.execute_time))" );
  def->add_action( "incinerate,cycle_targets=1,if=!debuff.havoc.remains" );

  cds->add_action(
      "summon_infernal,if=target.time_to_die>=210|!cooldown.dark_soul_instability.remains|target.time_to_die<=30+gcd|!"
      "talent.dark_soul_instability.enabled" );
  cds->add_talent( this, "Dark Soul: Instability",
                   "if=target.time_to_die>=140|pet.infernal.active|target.time_to_die<=20+gcd" );
  ...
  fnb->add_action(
      "immolate,if=talent.channel_demonfire.enabled&!remains&cooldown.channel_demonfire.remains<=action.chaos_bolt."
      "execute_time" );
  fnb->add_action( "channel_demonfire,if=!buff.active_havoc.remains" );
  fnb->add_action(
      "havoc,cycle_targets=1,if=!(target=sim.target)&target.time_to_die>10&spell_targets.rain_of_fire<=4+raid_event."
      "invulnerable.up&talent.grimoire_of_supremacy.enabled&pet.infernal.active&pet.infernal.remains<=10" );
  fnb->add_action(
      "havoc,if=spell_targets.rain_of_fire<=4+raid_event.invulnerable.up&talent.grimoire_of_supremacy.enabled&pet."
      "infernal.active&pet.infernal.remains<=10" );
  fnb->add_action(
      "chaos_bolt,cycle_targets=1,if=!debuff.havoc.remains&talent.grimoire_of_supremacy.enabled&pet.infernal.remains>"
      "execute_time&active_enemies<=4+raid_event.invulnerable.up&((108*(spell_targets.rain_of_fire+raid_event."
      "invulnerable.up)%3)<(240*(1+0.08*buff.grimoire_of_supremacy.stack)%2*(1+buff.active_havoc.remains>execute_time))"
      ")" );
  fnb->add_action(
      "havoc,cycle_targets=1,if=!(target=sim.target)&target.time_to_die>10&spell_targets.rain_of_fire<=4+raid_event."
      "invulnerable.up" );
  ...
  fnb->add_action(
      "chaos_bolt,cycle_targets=1,if=!debuff.havoc.remains&buff.active_havoc.remains>execute_time&spell_targets.rain_"
      "of_fire<=4+raid_event.invulnerable.up" );
  fnb->add_action(
      "immolate,cycle_targets=1,if=!debuff.havoc.remains&refreshable&spell_targets.incinerate<=8+raid_event."
      "invulnerable.up" );
  ...
  fnb->add_action(
      "conflagrate,cycle_targets=1,if=!debuff.havoc.remains&(talent.flashover.enabled&buff.backdraft.stack<=2|spell_"
      "targets.incinerate<=7+raid_event.invulnerable.up|talent.roaring_blaze.enabled&spell_targets.incinerate<=9+raid_"
      "event.invulnerable.up)" );
  ...
  cata->add_action(
      "havoc,cycle_targets=1,if=!(target=sim.target)&target.time_to_die>10&spell_targets.rain_of_fire<=8+raid_event."
      "invulnerable.up&talent.grimoire_of_supremacy.enabled&pet.infernal.active&pet.infernal.remains<=10" );
  cata->add_action(
      "havoc,if=spell_targets.rain_of_fire<=8+raid_event.invulnerable.up&talent.grimoire_of_supremacy.enabled&pet."
      "infernal.active&pet.infernal.remains<=10" );
  cata->add_action(
      "chaos_bolt,cycle_targets=1,if=!debuff.havoc.remains&talent.grimoire_of_supremacy.enabled&pet.infernal.remains>"
      "execute_time&active_enemies<=8+raid_event.invulnerable.up&((108*(spell_targets.rain_of_fire+raid_event."
      "invulnerable.up)%3)<(240*(1+0.08*buff.grimoire_of_supremacy.stack)%2*(1+buff.active_havoc.remains>execute_time))"
      ")" );
  cata->add_action(
      "havoc,cycle_targets=1,if=!(target=sim.target)&target.time_to_die>10&spell_targets.rain_of_fire<=4+raid_event."
  ...
  cata->add_action(
      "shadowburn,cycle_targets=1,if=!debuff.havoc.remains&((charges=2|!buff.backdraft.remains|buff.backdraft.remains>"
      "buff.backdraft.stack*action.incinerate.execute_time))" );
  ...
  inf->add_action(
      "havoc,cycle_targets=1,if=!(target=sim.target)&target.time_to_die>10&spell_targets.rain_of_fire<=4+raid_event."
      "invulnerable.up+talent.internal_combustion.enabled&talent.grimoire_of_supremacy.enabled&pet.infernal.active&pet."
      "infernal.remains<=10" );
  inf->add_action(
      "havoc,if=spell_targets.rain_of_fire<=4+raid_event.invulnerable.up+talent.internal_combustion.enabled&talent."
      "grimoire_of_supremacy.enabled&pet.infernal.active&pet.infernal.remains<=10" );
  inf->add_action(
      "chaos_bolt,cycle_targets=1,if=!debuff.havoc.remains&talent.grimoire_of_supremacy.enabled&pet.infernal.remains>"
      "execute_time&spell_targets.rain_of_fire<=4+raid_event.invulnerable.up+talent.internal_combustion.enabled&((108*("
      "spell_targets.rain_of_fire+raid_event.invulnerable.up)%(3-0.16*(spell_targets.rain_of_fire+raid_event."
      "invulnerable.up)))<(240*(1+0.08*buff.grimoire_of_supremacy.stack)%2*(1+buff.active_havoc.remains>execute_time))"
      ")" );
  inf->add_action(
      "havoc,cycle_targets=1,if=!(target=sim.target)&target.time_to_die>10&spell_targets.rain_of_fire<=3+raid_event."
      "invulnerable.up&(talent.eradication.enabled|talent.internal_combustion.enabled)" );
  inf->add_action(
      "havoc,if=spell_targets.rain_of_fire<=3+raid_event.invulnerable.up&(talent.eradication.enabled|talent.internal_"
      "combustion.enabled)" );
  inf->add_action(
      "chaos_bolt,cycle_targets=1,if=!debuff.havoc.remains&buff.active_havoc.remains>execute_time&spell_targets.rain_"
      "of_fire<=3+raid_event.invulnerable.up&(talent.eradication.enabled|talent.internal_combustion.enabled)" );
