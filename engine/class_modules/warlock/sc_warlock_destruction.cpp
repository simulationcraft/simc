...
namespace warlock {
  ...
  void warlock_t::create_apl_destruction() {
    ...
    cds->add_action(  "memory_of_lucid_dreams,if=pet.infernal.active" );
    ...
    cds->add_talent( this, "Dark Soul: Instability", "if=target.time_to_die>cooldown.dark_soul_instability.duration+20&!(essence.vision_of_perfection.minor&azerite.crashing_chaos.rank)", "If you can get in more than one more DSI before the end of the fight, use that now" );
    cds->add_action( "memory_of_lucid_dreams,if=target.time_to_die>cooldown.dark_soul_instability.duration+20&!(essence.vision_of_perfection.minor&azerite.crashing_chaos.rank)" );
    ...
    aoe->add_action( "concentrated_flame,if=!dot.concentrated_flame_burn.remains&!action.concentrated_flame.in_flight&active_enemies<5" );
