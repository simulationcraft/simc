...
using namespace helpers;
  ...
  struct demonic_soul_t : public warlock_spell_t
  {
      ...
      demoniacs_fervor( false )
    {
      ...
      if ( affliction() && sim->dbc->wowv() >= wowv_t{ 11, 2, 0 }  )
        base_dd_multiplier *= 1.0 + p->warlock_base.affliction_warlock->effectN( 27 ).percent();
  ...
  struct wicked_reaping_t : public warlock_spell_t
  {
      ...
      : warlock_spell_t( "Wicked Reaping", p, p->hero.wicked_reaping_dmg )
    {
      ...
      if ( affliction() && sim->dbc->wowv() >= wowv_t{ 11, 2, 0 } )
        base_dd_multiplier *= 1.0 + p->warlock_base.affliction_warlock->effectN( 26 ).percent();
