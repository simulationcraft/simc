...
{
  debuff_havoc_t( warlock_td_t& p ) : base_t( p, "havoc", p.source->find_spell( 80240 ) )
  {
  }
...
void warlock_t::create_buffs()
{
  ...
  buffs.sephuzs_secret = make_buff( this, "sephuzs_secret", find_spell( 208052 ) )->add_invalidate( CACHE_HASTE );
...
std::string warlock_t::default_food() const
{
  ...
  return ( true_level > 100 ) ? lvl110_food : ( true_level > 90 ) ? lvl100_food : "disabled";
}
...
expr_t* warlock_t::create_expression( const std::string& name_str )
{
  if ( name_str == "time_to_shard" )
  {
    return make_fn_expr( name_str, [this]() {
      ...
      double average           = 1 / ( 0.16 / std::sqrt( active_agonies ) * ( active_agonies == 1 ? 1.15 : 1.0 ) *
                             active_agonies / dot_tick_time.total_seconds() );
      if ( sim->log )
...
static void do_trinket_init( warlock_t* player, specialization_e spec, const special_effect_t*& ptr,
