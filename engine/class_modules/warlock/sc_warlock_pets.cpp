...
wild_imp_pet_t::wild_imp_pet_t( sim_t* sim, warlock_t* owner )
  : warlock_pet_t( sim, owner, "wild_imp", PET_WILD_IMP ), firebolt(), isnotdoge(), power_siphon( false )
{
}
...
void wild_imp_pet_t::arise()
{
  ...
  {
    o()->cooldowns.demonic_tyrant->adjust(
        o()->legendary.wilfreds_sigil_of_superior_summoning->driver()->effectN( 1 ).time_value() );
...
dreadstalker_t::dreadstalker_t( sim_t* sim, warlock_t* owner )
...
{
  demonfire_blast_t( warlock_pet_t* p, const std::string& options_str )
    : warlock_pet_spell_t( "demonfire_blast", p, p->find_spell( 265279 ) )
  {
...
action_t* demonic_tyrant_t::create_action( const std::string& name, const std::string& options_str )
{
  ...
  if ( name == "demonfire_blast" )
    return new demonfire_blast_t( this, options_str );
...
{
  multi_slash_damage_t( warlock_pet_t* p, int slash_num )
...
shivarra_t::shivarra_t( sim_t* sim, warlock_t* owner, const std::string& name )
  : warlock_pet_t( sim, owner, name, PET_WARLOCK_RANDOM, name != "shivarra" ), multi_slash()
{
...
darkhound_t::darkhound_t( sim_t* sim, warlock_t* owner, const std::string& name )
  : warlock_pet_t( sim, owner, name, PET_WARLOCK_RANDOM, name != "darkhound" ), fel_bite()
{
...
bilescourge_t::bilescourge_t( sim_t* sim, warlock_t* owner, const std::string& name )
  : warlock_pet_t( sim, owner, name, PET_WARLOCK_RANDOM, name != "bilescourge" )
{
...
urzul_t::urzul_t( sim_t* sim, warlock_t* owner, const std::string& name )
  : warlock_pet_t( sim, owner, name, PET_WARLOCK_RANDOM, name != "urzul" ), many_faced_bite()
{
...
{
  double_breath_damage_t( warlock_pet_t* p, int breath_num )
...
void_terror_t::void_terror_t( sim_t* sim, warlock_t* owner, const std::string& name )
  : warlock_pet_t( sim, owner, name, PET_WARLOCK_RANDOM, name != "void_terror" ), double_breath()
{
  action_list_str = "travel";
...
wrathguard_t::wrathguard_t( sim_t* sim, warlock_t* owner, const std::string& name )
  : warlock_pet_t( sim, owner, name, PET_WARLOCK_RANDOM, name != "wrathguard" ), overhead_assault()
{
...
vicious_hellhound_t::vicious_hellhound_t( sim_t* sim, warlock_t* owner, const std::string& name )
  : warlock_pet_t( sim, owner, name, PET_WARLOCK_RANDOM, name != "vicious_hellhound" ), demon_fang()
{
  action_list_str = "travel";
...
illidari_satyr_t::illidari_satyr_t( sim_t* sim, warlock_t* owner, const std::string& name )
  : warlock_pet_t( sim, owner, name, PET_WARLOCK_RANDOM, name != "illidari_satyr" ), shadow_slash()
{
...
eyes_of_guldan_t::eyes_of_guldan_t( sim_t* sim, warlock_t* owner, const std::string& name )
  : warlock_pet_t( sim, owner, name, PET_WARLOCK_RANDOM, name != "eyes_of_guldan" )
{
...
// prince malchezaar
prince_malchezaar_t::prince_malchezaar_t( sim_t* sim, warlock_t* owner, const std::string& name )
  : warlock_pet_t( sim, owner, name, PET_WARLOCK_RANDOM, name != "prince_malchezaar" )
{
...
{
  if ( name == "immolation" )
    return new immolation_t( this, options_str );
...
void warlock_pet_t::create_buffs()
{
  ...
  buffs.rage_of_guldan =
      make_buff( this, "rage_of_guldan", find_spell( 257926 ) )
...
pet_t* warlock_t::create_main_pet( const std::string& pet_name, const std::string& pet_type )
{
  ...
  if ( pet_name == "felhunter" )
    return new felhunter_pet_t( sim, this, pet_name );
  if ( pet_name == "imp" )
    return new imp_pet_t( sim, this, pet_name );
  if ( pet_name == "succubus" )
    return new succubus_pet_t( sim, this, pet_name );
  if ( pet_name == "voidwalker" )
    return new voidwalker_pet_t( sim, this, pet_name );
...
void warlock_t::create_all_pets()
{
  if ( specialization() == WARLOCK_DEMONOLOGY )
  {
    ...
    if ( talents.inner_demons->ok() or talents.nether_portal->ok() )
    {
      ...
      {
        warlock_pet_list.vicious_hellhounds[ i ] =
            new pets::demonology::random_demons::vicious_hellhound_t( sim, this );
      }
...
expr_t* warlock_t::create_pet_expression( const std::string& name_str )
{
  if ( name_str == "last_cast_imps" )
  {
    struct wild_imp_last_cast_expression_t : public expr_t
    {
      ...
      wild_imp_last_cast_expression_t( warlock_t& p ) : expr_t( "last_cast_imps" ), player( p )
      {
      }
  ...
  else if ( name_str == "two_cast_imps" )
  {
    struct wild_imp_two_cast_expression_t : public expr_t
    {
      ...
      wild_imp_two_cast_expression_t( warlock_t& p ) : expr_t( "two_cast_imps" ), player( p )
      {
      }
