...
struct shadow_bolt_t : public affliction_spell_t
{
  ...
  void impact( action_state_t* s ) override
  {
    ...
    if ( result_is_hit( s->result ) && ( !p()->min_version_check( "9.1" ) || p()->talents.shadow_embrace->ok() ) )
    {
      // Add passive check
...
struct drain_soul_t : public affliction_spell_t
{
  ...
  {
    affliction_spell_t::tick( d );
    if ( result_is_hit( d->state->result ) && ( !p()->min_version_check( "9.1" ) || p()->talents.shadow_embrace->ok() ) )
    {
