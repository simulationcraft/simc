  ...
  virtual std::tuple<double, double> get_db_dot_state( dot_t* dot )
  {
    ...
    if ( sim->debug )
    {
      sim->out_debug.printf(
          "%s %s dot_remains=%.3f duration=%.3f time_to_next=%.3f tick_time=%.3f "
          "ticks_left=%.3f amount=%.3f total=%.3f",
          name(), dot->name(), dot->remains().total_seconds(), dot->duration().total_seconds(),
          dot->time_to_next_tick().total_seconds(), dot_tick_time.total_seconds(), ticks_left, state->result_raw,
          state->result_raw * ticks_left );
    }
...
struct agony_t : public affliction_spell_t
{
  ...
  agony_t( warlock_t* p, const std::string& options_str ) : affliction_spell_t( p, "Agony" )
  {
    parse_options( options_str );
    may_crit                   = false;
...
struct unstable_affliction_t : public affliction_spell_t
{
  struct real_ua_t : public affliction_spell_t
  {
    ...
    void last_tick( dot_t* d ) override
    {
      ...
      if ( p()->azerite_essence.conflict_and_strife.enabled() && p()->azerite_essence.conflict_and_strife.is_major() &&
           p()->buffs.active_uas->stack() < 1 )
      {
...
struct deathbolt_t : public affliction_spell_t
{
  ...
  double get_contribution_from_dot( dot_t* dot )
  {
    ...
    return std::get<DB_DOT_TICKS_LEFT>( tick_state ) * std::get<DB_DOT_DAMAGE>( tick_state );
  }
...
std::unique_ptr<expr_t> warlock_t::create_aff_expression( const std::string& name_str )
{
  if ( name_str == "deathbolt_setup" )
  {
    return make_fn_expr( "deathbolt_setup", [this]() {
      ...
      gcds_required += talents.absolute_corruption->ok()
                           ? 0
                           : ( td->dots_corruption->remains() <= td->dots_corruption->duration() ? 1 : 0 );
      gcds_required += talents.siphon_life->ok()
                           ? ( td->dots_siphon_life->remains() <= td->dots_siphon_life->duration() ? 1 : 0 )
                           : 0;
