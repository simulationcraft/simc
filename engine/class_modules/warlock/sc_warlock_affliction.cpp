...
public:
  ...
  affliction_spell_t( warlock_t* p, const std::string& n ) : affliction_spell_t( n, p, p->find_class_spell( n ) )
  {
  }
    ...
    if ( resource_current == RESOURCE_SOUL_SHARD && p()->in_combat )
    {
      if ( p()->legendary.the_master_harvester )
      {
        double sh_proc_chance =
            p()->find_spell( p()->legendary.the_master_harvester->spell_id )->effectN( 1 ).percent();
...
struct agony_t : public affliction_spell_t
{
  ...
  {
    agony_max_stacks =
        ( p()->talents.writhe_in_agony->ok() ? p()->talents.writhe_in_agony->effectN( 2 ).base_value() : 10 );
  ...
  void tick( dot_t* d ) override
  {
    ...
    double accumulator_increment =
        rng().range( 0.0, p()->sets->has_set_bonus( WARLOCK_AFFLICTION, T19, B4 ) ? 0.368 * tier_bonus : 0.32 ) /
        std::sqrt( active_agonies );
      ...
      if ( warlock_td_t* target_data = find_td( d->state->target ) )
      {
        for ( auto& current_ua : target_data->dots_unstable_affliction )
        {
          if ( current_ua->is_ticking() )
            current_ua->extend_duration( p()->sets->set( WARLOCK_AFFLICTION, T21, B2 )->effectN( 1 ).time_value(),
                                         true );
        }
      }
...
struct unstable_affliction_t : public affliction_spell_t
{
  ...
  unstable_affliction_t( warlock_t* p, const std::string& options_str )
    : affliction_spell_t( "unstable_affliction", p, p->spec.unstable_affliction ), ua_dots()
  {
    parse_options( options_str );
...
struct summon_darkglare_t : public affliction_spell_t
{
  ...
  void execute() override
  {
    ...
    for ( const auto target : sim->target_non_sleeping_list )
    {
      ...
      {
        td->dots_agony->extend_duration(
            timespan_t::from_seconds( p()->spec.summon_darkglare->effectN( 2 ).base_value() ) );
      }
      if ( td->dots_corruption->is_ticking() )
      {
        td->dots_corruption->extend_duration(
            timespan_t::from_seconds( p()->spec.summon_darkglare->effectN( 2 ).base_value() ) );
      }
      if ( td->dots_siphon_life->is_ticking() )
      {
        td->dots_siphon_life->extend_duration(
            timespan_t::from_seconds( p()->spec.summon_darkglare->effectN( 2 ).base_value() ) );
      }
      if ( td->dots_phantom_singularity->is_ticking() )
      {
        td->dots_phantom_singularity->extend_duration(
            timespan_t::from_seconds( p()->spec.summon_darkglare->effectN( 2 ).base_value() ) );
      }
      if ( td->dots_vile_taint->is_ticking() )
      {
        td->dots_vile_taint->extend_duration(
            timespan_t::from_seconds( p()->spec.summon_darkglare->effectN( 2 ).base_value() ) );
      }
      for ( auto& current_ua : td->dots_unstable_affliction )
      {
        if ( current_ua->is_ticking() )
          current_ua->extend_duration(
              timespan_t::from_seconds( p()->spec.summon_darkglare->effectN( 2 ).base_value() ) );
      }
    }
  }
...
struct seed_of_corruption_t : public affliction_spell_t
{
  struct seed_of_corruption_aoe_t : public affliction_spell_t
  {
    ...
    seed_of_corruption_aoe_t( warlock_t* p ) : affliction_spell_t( "seed_of_corruption_aoe", p, p->find_spell( 27285 ) )
    {
      aoe                              = -1;
      dual                             = true;
      background                       = true;
...
{
  haunt_t( warlock_t* p, const std::string& options_str ) : affliction_spell_t( "haunt", p, p->talents.haunt )
  {
    parse_options( options_str );
  }
...
struct deathbolt_t : public affliction_spell_t
{
  ...
  double get_contribution_from_dot( dot_t* dot )
  {
    ...
    if ( sim->debug )
    {
      sim->out_debug.printf(
          "%s %s dot_remains=%.3f duration=%.3f time_to_next=%.3f tick_time=%.3f ticks_left=%.3f amount=%.3f "
          "total=%.3f",
          name(), dot->name(), dot->remains().total_seconds(), dot->duration().total_seconds(),
          dot->time_to_next_tick().total_seconds(), dot_tick_time.total_seconds(), ticks_left, tick_base_damage,
          total_damage );
    }
  ...
  void execute() override
  {
    ...
    const double total_dot_dmg = total_damage_agony + total_damage_corruption + total_damage_siphon_life +
                                 total_damage_unstable_afflictions + total_damage_phantom_singularity +
                                 total_damage_vile_taint;
...
action_t* warlock_t::create_action_affliction( const std::string& action_name, const std::string& options_str )
{
  ...
  if ( action_name == "deathbolt" )
    return new deathbolt_t( this, options_str );
...
{
  // spells
  buffs.active_uas = make_buff( this, "active_uas" )->set_max_stack( 20 );
  // talents
...
void warlock_t::create_apl_affliction()
{
  ...
  def->add_action(
      "unstable_affliction,if=(dot.unstable_affliction_1.ticking+dot.unstable_affliction_2.ticking+dot.unstable_"
      "affliction_3.ticking+dot.unstable_affliction_4.ticking+dot.unstable_affliction_5.ticking=0)|soul_shard>2" );
