...
public:
  ...
  demonology_spell_t( warlock_t* p, const std::string& n ) : demonology_spell_t( n, p, p->find_class_spell( n ) )
  {
  }
    ...
    if ( resource_current == RESOURCE_SOUL_SHARD && p()->in_combat )
    {
      if ( p()->legendary.the_master_harvester )
      {
        double sh_proc_chance =
            p()->find_spell( p()->legendary.the_master_harvester->spell_id )->effectN( 2 ).percent();
...
struct hand_of_guldan_t : public demonology_spell_t
{
  ...
  hand_of_guldan_t( warlock_t* p, const std::string& options_str )
    : demonology_spell_t( p, "Hand of Gul'dan" ), blaze( new umbral_blaze_t( p ) )
  {
    parse_options( options_str );
    aoe         = -1;
    shards_used = 0;
...
struct call_dreadstalkers_t : public demonology_spell_t
{
  ...
  call_dreadstalkers_t( warlock_t* p, const std::string& options_str ) : demonology_spell_t( p, "Call Dreadstalkers" )
  {
    ...
    dreadstalker_duration = p->find_spell( 193332 )->duration() +
                            ( p->sets->has_set_bonus( WARLOCK_DEMONOLOGY, T19, B4 )
                                  ? p->sets->set( WARLOCK_DEMONOLOGY, T19, B4 )->effectN( 1 ).time_value()
                                  : timespan_t::zero() );
    ...
    for ( size_t i = 0; i < p()->warlock_pet_list.dreadstalkers.size(); i++ )
    {
      if ( p()->warlock_pet_list.dreadstalkers[ i ]->is_sleeping() )
      {
        ...
        if ( p()->sets->has_set_bonus( WARLOCK_DEMONOLOGY, T21, B2 ) )
        {
          ...
          p()->warlock_pet_list.dreadstalkers[ i ]->buffs.rage_of_guldan->set_default_value(
              p()->buffs.rage_of_guldan->stack_value() );
          p()->warlock_pet_list.dreadstalkers[ i ]->buffs.rage_of_guldan->trigger();
        }

        if ( p()->legendary.wilfreds_sigil_of_superior_summoning )
        {
          p()->cooldowns.demonic_tyrant->adjust(
              p()->legendary.wilfreds_sigil_of_superior_summoning->driver()->effectN( 1 ).time_value() );
          p()->procs.wilfreds_dog->occur();
        }

        if ( ++j == dreadstalker_count )
          break;
      }
    }
...
struct implosion_t : public demonology_spell_t
{
  ...
  implosion_t( warlock_t* p, const std::string& options_str )
...
{
  power_siphon_t( warlock_t* p, const std::string& options_str )
  ...
  void execute() override
  {
    ...
    struct lower_energy
    {
      inline bool operator()( const pets::demonology::wild_imp_pet_t* imp1,
...
struct doom_t : public demonology_spell_t
{
  ...
  void tick( dot_t* d ) override
  {
    ...
    {
      if ( p()->sets->has_set_bonus( WARLOCK_DEMONOLOGY, T19, B2 ) &&
           rng().roll( p()->sets->set( WARLOCK_DEMONOLOGY, T19, B2 )->effectN( 1 ).percent() ) )
  ...
  void summon_random_pet( random_pet_e random_pet )
  {
    switch ( random_pet )
    {
      ...
      case random_pet_e::eyes_of_guldan:
      {
        ...
        summon_random_pet_helper( p()->warlock_pet_list.eyes_of_guldan,
                                  p()->warlock_pet_list.eyes_of_guldan.size() / 2 );
        break;
...
pet_t* warlock_t::create_demo_pet( const std::string& pet_name, const std::string& pet_type )
{
  ...
  if ( pet_name == "felguard" )
    return new felguard_pet_t( sim, this, pet_name );
...
void warlock_t::create_buffs_demonology()
{
  ...
  // Tier
  buffs.rage_of_guldan =
      make_buff( this, "rage_of_guldan", sets->set( WARLOCK_DEMONOLOGY, T21, B2 )->effectN( 1 ).trigger() )
  ...
  buffs.dreaded_haste =
      make_buff( this, "dreaded_haste", sets->set( WARLOCK_DEMONOLOGY, T20, B4 )->effectN( 1 ).trigger() )
          ->set_default_value(
              sets->set( WARLOCK_DEMONOLOGY, T20, B4 )->effectN( 1 ).trigger()->effectN( 1 ).percent() )
...
void warlock_t::create_apl_demonology()
{
  ...
  npa->add_action(
      ...
      "dreadstalkers.remains>action.shadow_bolt.cast_time))&cooldown.nether_portal.remains>(160+action.hand_of_guldan."
      "cast_time)" );
  ...
  def->add_action(
      "summon_vilefiend,if=cooldown.summon_demonic_tyrant.remains>30|(cooldown.summon_demonic_tyrant.remains<10&"
      "cooldown.call_dreadstalkers.remains<10)" );
  def->add_action( "grimoire_felguard" );
  def->add_action( "hand_of_guldan,if=soul_shard>=5" );
  def->add_action(
      "hand_of_guldan,if=soul_shard>=3&cooldown.call_dreadstalkers.remains>4&(!talent.summon_vilefiend.enabled|"
      "cooldown.summon_vilefiend.remains>4)" );
  def->add_action( "call_dreadstalkers" );
  def->add_action( "bilescourge_bombers" );
  def->add_action(
      "summon_demonic_tyrant,if=talent.summon_vilefiend.enabled&buff.dreadstalkers.remains>action.summon_demonic_"
      "tyrant.cast_time&buff.vilefiend.remains>action.summon_demonic_tyrant.cast_time" );
  def->add_action(
      "summon_demonic_tyrant,if=!talent.summon_vilefiend.enabled&buff.dreadstalkers.remains>action.summon_demonic_"
      "tyrant.cast_time&soul_shard=0" );
  ...
  def->add_action(
      "doom,cycle_targets=1,if=(talent.doom.enabled&target.time_to_die>duration&(!ticking|remains<duration*0.3))" );
