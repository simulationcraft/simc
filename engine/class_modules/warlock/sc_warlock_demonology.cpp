...
namespace warlock {
  ...
  void warlock_t::create_apl_demonology() {
    ...
    def->add_action( "potion,if=pet.demonic_tyrant.active&(!essence.vision_of_perfection.major|!talent.demonic_consumption.enabled|cooldown.summon_demonic_tyrant.remains>=cooldown.summon_demonic_tyrant.duration-5)&(!talent.nether_portal.enabled|cooldown.nether_portal.remains>160)|time_to_die<30" );
    ...
    def->add_action( "use_items,if=pet.demonic_tyrant.active&(!essence.vision_of_perfection.major|!talent.demonic_consumption.enabled|cooldown.summon_demonic_tyrant.remains>=cooldown.summon_demonic_tyrant.duration-5)|time_to_die<=15" );
    def->add_action( "berserking,if=pet.demonic_tyrant.active&(!essence.vision_of_perfection.major|!talent.demonic_consumption.enabled|cooldown.summon_demonic_tyrant.remains>=cooldown.summon_demonic_tyrant.duration-5)|time_to_die<=15" );
    def->add_action( "blood_fury,if=pet.demonic_tyrant.active&(!essence.vision_of_perfection.major|!talent.demonic_consumption.enabled|cooldown.summon_demonic_tyrant.remains>=cooldown.summon_demonic_tyrant.duration-5)|time_to_die<=15" );
    def->add_action( "fireblood,if=pet.demonic_tyrant.active&(!essence.vision_of_perfection.major|!talent.demonic_consumption.enabled|cooldown.summon_demonic_tyrant.remains>=cooldown.summon_demonic_tyrant.duration-5)|time_to_die<=15" );
    def->add_action( "blood_of_the_enemy,if=pet.demonic_tyrant.active&pet.demonic_tyrant.remains<=15-gcd*3&(!essence.vision_of_perfection.major|!talent.demonic_consumption.enabled|cooldown.summon_demonic_tyrant.remains>=cooldown.summon_demonic_tyrant.duration-5)" );
    def->add_action( "worldvein_resonance,if=(pet.demonic_tyrant.active&(!essence.vision_of_perfection.major|!talent.demonic_consumption.enabled|cooldown.summon_demonic_tyrant.remains>=cooldown.summon_demonic_tyrant.duration-5)|time_to_die<=15)" );
    def->add_action( "ripple_in_space,if=pet.demonic_tyrant.active&(!essence.vision_of_perfection.major|!talent.demonic_consumption.enabled|cooldown.summon_demonic_tyrant.remains>=cooldown.summon_demonic_tyrant.duration-5)|time_to_die<=15" );
    def->add_action( "use_item,name=pocketsized_computation_device,if=cooldown.summon_demonic_tyrant.remains>=20&cooldown.summon_demonic_tyrant.remains<=cooldown.summon_demonic_tyrant.duration-15|time_to_die<=30" );
    def->add_action( "use_item,name=rotcrusted_voodoo_doll,if=(cooldown.summon_demonic_tyrant.remains>=25|time_to_die<=30)" );
    def->add_action( "use_item,name=shiver_venom_relic,if=(cooldown.summon_demonic_tyrant.remains>=25|time_to_die<=30)" );
    def->add_action( "use_item,name=aquipotent_nautilus,if=(cooldown.summon_demonic_tyrant.remains>=25|time_to_die<=30)" );
    def->add_action( "use_item,name=tidestorm_codex,if=(cooldown.summon_demonic_tyrant.remains>=25|time_to_die<=30)" );
    def->add_action( "use_item,name=vial_of_storms,if=(cooldown.summon_demonic_tyrant.remains>=25|time_to_die<=30)" );
    def->add_action( "call_action_list,name=opener,if=!talent.nether_portal.enabled&time<30&!cooldown.summon_demonic_tyrant.remains" );
    def->add_action( "use_item,name=azsharas_font_of_power,if=(time>30|!talent.nether_portal.enabled)&talent.grimoire_felguard.enabled&(time_to_die>120|time_to_die<cooldown.summon_demonic_tyrant.remains+15)|time_to_die<=35" );
    ...
    def->add_action( "demonic_strength,if=pet.demonic_tyrant.active|time_to_die<9" );
    ...
    def->add_action( "guardian_of_azeroth,if=cooldown.summon_demonic_tyrant.remains<=15|time_to_die<=30" );
    def->add_action( "grimoire_felguard,if=(time_to_die>120|time_to_die<cooldown.summon_demonic_tyrant.remains+15|cooldown.summon_demonic_tyrant.remains<13)" );
    ...
    def->add_action( this, "Summon Demonic Tyrant", "if=soul_shard<3&(!talent.demonic_consumption.enabled|buff.wild_imps.stack+imps_spawned_during.2000%spell_haste>=6&time_to_imps.all.remains<cast_time)|time_to_die<20", "2000%spell_haste is shorthand for the cast time of Demonic Tyrant. The intent is to only begin casting if a certain number of imps will be out by the end of the cast." );
    ...
    def->add_action( "demonbolt,if=soul_shard<=3&buff.demonic_core.up&((cooldown.summon_demonic_tyrant.remains<6|cooldown.summon_demonic_tyrant.remains>22&!azerite.shadows_bite.enabled)|buff.demonic_core.stack>=3|buff.demonic_core.remains<5|time_to_die<25|buff.shadows_bite.remains)" );
    ...
    def->add_action( "concentrated_flame,if=!dot.concentrated_flame_burn.remains&!action.concentrated_flame.in_flight&!pet.demonic_tyrant.active" );
    ...
    npb->add_action( "power_siphon,if=time>=30&buff.wild_imps.stack>=2&buff.demonic_core.stack<=2&buff.demonic_power.down&soul_shard>=3" );
    ...
    opener->add_action( "implosion,if=azerite.explosive_potential.enabled&buff.wild_imps.stack>2&buff.explosive_potential.down" );
    opener->add_action( "doom,line_cd=30" );
    opener->add_action( "guardian_of_azeroth" );
    opener->add_action( "hand_of_guldan,if=prev_gcd.1.hand_of_guldan&soul_shard>0&(prev_gcd.2.soul_strike|(buff.wild_imps.stack>1&action.hand_of_guldan.in_flight))" );
    ...
    imp->add_action( "concentrated_flame,if=!dot.concentrated_flame_burn.remains&!action.concentrated_flame.in_flight&spell_targets.implosion<5" );
