...
void rogue_t::init_action_list()
{
  ...
  if ( specialization() == ROGUE_ASSASSINATION )
  {
    ...
    dot->add_action( "variable,name=energy_sat,value=energy.regen_combined>35", "Combined Energy Regen needed to saturate" );
  ...
  else if ( specialization() == ROGUE_SUBTLETY )
  {
    ...
    cds->add_action( this, "Symbols of Death", "if=variable.snd_condition&(talent.enveloping_shadows.enabled|cooldown.shadow_dance.charges>=1)&(!talent.shuriken_tornado.enabled|talent.shadow_focus.enabled|cooldown.shuriken_tornado.remains>2)&(!runeforge.obedience|cooldown.flagellation.remains>10)", "Use Symbols on cooldown (after first SnD) unless we are going to pop Tornado and do not have Shadow Focus, or if Flagellation is up soon." );
