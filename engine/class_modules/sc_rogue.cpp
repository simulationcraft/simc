...
void rogue_t::init_action_list()
{
  ...
  if ( specialization() == ROGUE_ASSASSINATION )
  {
    ...
    def -> add_action( "call_action_list,name=cds,if=(!talent.master_assassin.enabled|dot.garrote.ticking)&(!equipped.azsharas_font_of_power|!cooldown.latent_arcana.up)" );
  ...
  else if ( specialization() == ROGUE_OUTLAW )
  {
    ...
    cds -> add_action( "use_item,name=ashvanes_razor_coral,if=debuff.razor_coral_debuff.down|(buff.adrenaline_rush.up|debuff.conductive_ink_debuff.up)&(target.health.pct<31|target.time_to_die<60)" );
    ...
    build -> add_action( this, "Pistol Shot", "if=buff.opportunity.up&(buff.keep_your_wits_about_you.stack<10|buff.deadshot.up|energy<45)", "Use Pistol Shot if it won't cap combo points and the Oppotunity buff is up. Avoid using when Keep Your Wits stacks are high unless the Deadshot buff is also up." );
  ...
  else if ( specialization() == ROGUE_SUBTLETY )
  {
    ...
    def -> add_action( this, "Nightblade", "if=azerite.nights_vengeance.enabled&(spell_targets.shuriken_storm<2|variable.use_priority_rotation)&(cooldown.symbols_of_death.remains<=3|(buff.symbols_of_death.remains>3&!stealthed.all&azerite.nights_vengeance.rank>=3))&!buff.nights_vengeance.up&combo_points>=2", "Night's Vengeance: Nightblade before Symbols at low CP to combine early refresh with getting the buff up. Also low CP during Symbols outside Dance with 3 NV." );
    ...
    cds -> add_action( "use_item,name=ashvanes_razor_coral,if=debuff.razor_coral_debuff.down|(buff.symbols_of_death.up|debuff.conductive_ink_debuff.up)&(target.health.pct<31|target.time_to_die<60)" );
