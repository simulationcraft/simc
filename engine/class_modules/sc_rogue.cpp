...
struct garrote_t : public rogue_attack_t
{
  ...
  void snapshot_state( action_state_t* state, result_amount_type type ) override
  {
    ...
    assert( !debug_cast<garrote_state_t*>( state )->exsanguinated );
...
void rogue_t::init_action_list()
{
  ...
  {
    precombat->add_action( "variable,name=over_rp_crit_threshold,value=attack_crit>0.42" );
  }
  ...
  const std::string reaping_flames_init = "variable,name=reaping_delay,value=target.time_to_die";
  ...
  if ( specialization() == ROGUE_ASSASSINATION )
  {
    ...
    cds -> add_action( this, "Vendetta", "if=!stealthed.rogue&((dot.rupture.ticking&!debuff.vendetta.up&variable.vendetta_subterfuge_condition&variable.vendetta_nightstalker_condition&variable.vendetta_font_condition)|fight_remains<=20)" );
    ...
    cds -> add_action( "use_item,name=galecallers_boon,if=(debuff.vendetta.up|(!talent.exsanguinate.enabled&cooldown.vendetta.remains>45|talent.exsanguinate.enabled&(cooldown.exsanguinate.remains<6|cooldown.exsanguinate.remains>20&fight_remains>65)))&(!exsanguinated.rupture|!exsanguinated.garrote&dot.garrote.pmultiplier>1)" );
    cds -> add_action( "use_item,name=ashvanes_razor_coral,if=debuff.razor_coral_debuff.down|fight_remains<20" );
    ...
    essences->add_action( "blood_of_the_enemy,if=debuff.vendetta.up&(exsanguinated.garrote|debuff.toxic_blade.up&combo_points.deficit<=1|debuff.vendetta.remains<=10)|fight_remains<=10", "Always use Blood with Vendetta up. Hold for Exsanguinate. Use with TB up before a finisher as long as it runs for 10s during Vendetta." );
    essences->add_action( "guardian_of_azeroth,if=cooldown.vendetta.remains<3|debuff.vendetta.up|fight_remains<30", "Attempt to align Guardian with Vendetta as long as it won't result in losing a full-value cast over the remaining duration of the fight" );
    essences->add_action( "guardian_of_azeroth,if=floor((fight_remains-30)%cooldown)>floor((fight_remains-30-cooldown.vendetta.remains)%cooldown)" );
    essences->add_action( "focused_azerite_beam,if=spell_targets.fan_of_knives>=2|raid_event.adds.in>60&energy<70|fight_remains<10" );
    essences->add_action( "purifying_blast,if=spell_targets.fan_of_knives>=2|raid_event.adds.in>60|fight_remains<10" );
    ...
    essences->add_action( reaping_flames_init, reaping_flames_comment );
    essences->add_action( reaping_flames_condition );
    ...
    dot -> add_action( this, "Rupture", "if=talent.exsanguinate.enabled&!dot.garrote.refreshable&(combo_points>=cp_max_spend&cooldown.exsanguinate.remains<1&dot.rupture.remains*0.5<target.time_to_die)" );
    ...
    dot -> add_talent( this, "Crimson Tempest", "target_if=min:remains,if=spell_targets>3&remains<2+(spell_targets>=5)&combo_points>=4", "Crimson Tempest on multiple targets at 4+ CP when running out in 2-3s (based on target count) on any target" );
    ...
    dot -> add_talent( this, "Crimson Tempest", "target_if=min:remains,if=spell_targets>1&spell_targets<4&remains<2&combo_points>=4", "Crimson Tempest on 1-3 multiple targets at 4+ CP when running out in 2s on any target" );
    ...
    dot -> add_talent( this, "Crimson Tempest", "if=spell_targets>(7-buff.envenom.up)&combo_points>=4+talent.deeper_stratagem.enabled&!debuff.vendetta.up&!debuff.toxic_blade.up&!azerite.twist_the_knife.enabled&energy.deficit<=25+variable.energy_regen_combined", "Cast Crimson Tempest on AoE instead of Envenom at 7+ targets when Vendetta/TB is not up" );
  ...
  else if ( specialization() == ROGUE_OUTLAW )
  {
    ...
    def -> add_action( "variable,name=rtb_reroll,op=set,if=azerite.deadshot.enabled|variable.over_rp_crit_threshold,value=rtb_buffs<2&(buff.loaded_dice.up|!buff.broadside.up)", "Reroll for 2+ buffs or Broadside with Deadshot." );
    def -> add_action( "variable,name=rtb_reroll,op=set,if=azerite.ace_up_your_sleeve.enabled&azerite.ace_up_your_sleeve.rank>=azerite.deadshot.rank&!variable.over_rp_crit_threshold,value=rtb_buffs<2&(buff.loaded_dice.up|buff.ruthless_precision.remains<=cooldown.between_the_eyes.remains)", "Reroll for 2+ buffs or Ruthless Precision with Ace up your Sleeve, unless there are more Deadshot ranks." );
    ...
    essences->add_action( "blood_of_the_enemy,if=variable.blade_flurry_sync&cooldown.between_the_eyes.up&variable.bte_condition&(spell_targets.blade_flurry>=2|raid_event.adds.in>45)|fight_remains<=10" );
    essences->add_action( "guardian_of_azeroth" );
    essences->add_action( "focused_azerite_beam,if=spell_targets.blade_flurry>=2|raid_event.adds.in>60&!buff.adrenaline_rush.up|fight_remains<10" );
    essences->add_action( "purifying_blast,if=spell_targets.blade_flurry>=2|raid_event.adds.in>60|fight_remains<10" );
    essences->add_action( "the_unbound_force,if=buff.reckless_force.up|buff.reckless_force_counter.stack<10" );
    essences->add_action( "ripple_in_space" );
    essences->add_action( "worldvein_resonance" );
    ...
    finish -> add_talent( this, "Slice and Dice", "if=buff.slice_and_dice.remains<fight_remains&buff.slice_and_dice.remains<(1+combo_points)*1.8" );
