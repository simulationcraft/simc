...
void rogue_t::init_action_list()
{
  ...
  else if ( specialization() == ROGUE_SUBTLETY )
  {
    ...
    stealthed -> add_action( this, "Shadowstrike", "if=variable.use_priority_rotation&(debuff.find_weakness.remains<1|azerite.inevitability.enabled&buff.symbols_of_death.up&spell_targets.shuriken_storm<=3+azerite.blade_in_the_shadows.enabled)", "For priority rotation, use Shadowstrike over Storm if FW is running off (on any amount of targets) or to maximize SoD extension with Inevitability on 3 targets (4 with BitS)." );
