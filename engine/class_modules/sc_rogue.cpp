...
struct weapon_info_t
{
  ...
  weapon_info_t() : current_weapon( WEAPON_PRIMARY ), initialized( false ), secondary_weapon_uptime( nullptr )
  {
...
struct rogue_td_t : public actor_target_data_t
{
  ...
  {
    return debuffs.crippling_poison->check() || debuffs.leeching_poison->check();
  }
...
struct rogue_attack_t : public melee_attack_t
{
  ...
  rogue_attack_t( const std::string& token, rogue_t* p, const spell_data_t* s = spell_data_t::nil(),
                  const std::string& options = std::string() )
    : melee_attack_t( token, p, s ),
      requires_stealth( false ),
      requires_position( POSITION_NONE ),
      requires_weapon_type( WEAPON_NONE ),
      requires_weapon_group( WEAPON_NONE ),
  ...
  double composite_da_multiplier( const action_state_t* state ) const override
  {
    ...
    if ( p()->buffs.elaborate_planning->up() &&
         data().affected_by( p()->buffs.elaborate_planning->data().effectN( 1 ) ) )
    {
  ...
  double composite_ta_multiplier( const action_state_t* state ) const override
  {
    ...
    if ( p()->buffs.elaborate_planning->up() &&
         data().affected_by( p()->buffs.elaborate_planning->data().effectN( 2 ) ) )
    {
  ...
  double composite_target_multiplier( player_t* target ) const override
  {
    ...
    if ( tdata->dots.nightblade->is_ticking() &&
         data().affected_by( tdata->dots.nightblade->current_action->data().effectN( 6 ) ) )
    {
  ...
  double composite_persistent_multiplier( const action_state_t* state ) const override
  {
    ...
    if ( p()->talent.nightstalker->ok() && snapshots_nightstalker() &&
         p()->stealthed( STEALTH_BASIC | STEALTH_SHADOWDANCE ) )
    {
      m *=
          1.0 + ( p()->talent.nightstalker->effectN( 2 ).percent() + p()->spec.subtlety_rogue->effectN( 4 ).percent() );
    }
...
struct secondary_ability_trigger_t : public event_t
{
  ...
  secondary_ability_trigger_t( action_state_t* s, secondary_trigger_e source_,
                               const timespan_t& delay = timespan_t::zero() )
    : event_t( *s->action->sim, delay ), action( s->action ), state( s ), target( nullptr ), cp( 0 ), source( source_ )
  {
  }

  secondary_ability_trigger_t( player_t* target, action_t* action, int cp, secondary_trigger_e source_,
                               const timespan_t& delay = timespan_t::zero() )
...
{
  main_gauche_t( rogue_t* p ) : rogue_attack_t( "main_gauche", p, p->find_spell( 86392 ) )
  {
...
{
  blade_flurry_attack_t( rogue_t* p ) : rogue_attack_t( "blade_flurry_attack", p, p->find_spell( 22482 ) )
  {
...
{
  internal_bleeding_t( rogue_t* p ) : rogue_attack_t( "internal_bleeding", p, p->find_spell( 154953 ) )
  {
    background   = true;
    hasted_ticks = true;
  }
...
{
  poison_bomb_t( rogue_t* p ) : rogue_attack_t( "poison_bomb", p, p->find_spell( 255546 ) )
  {
    background = true;
    aoe        = -1;
...
struct rogue_poison_t : public rogue_attack_t
{
  ...
  rogue_poison_t( const std::string& token, rogue_t* p, const spell_data_t* s = spell_data_t::nil() )
    : actions::rogue_attack_t( token, p, s ), proc_chance_( 0 )
  {
    proc        = true;
    background  = true;
  ...
  virtual void trigger( const action_state_t* source_state )
  {
    ...
    if ( sim->debug )
      sim->out_debug.printf( "%s attempts to proc %s, target=%s source_action=%s proc_chance=%.3f: %d", player->name(),
                             name(), source_state->target->name(), source_state->action->name(),
                             proc_chance( source_state ), result );
...
struct deadly_poison_t : public rogue_poison_t
{
  ...
  {
    deadly_poison_dd_t( rogue_t* p ) : rogue_poison_t( "deadly_poison_instant", p, p->find_spell( 113780 ) )
    {
      harmful = true;
    }
  };
  ...
  struct deadly_poison_dot_t : public rogue_poison_t
  {
    deadly_poison_dot_t( rogue_t* p )
      : rogue_poison_t( "deadly_poison_dot", p,
                        p->find_specialization_spell( "Deadly Poison" )->effectN( 1 ).trigger() )
    {
      may_crit     = false;
      proc         = false;
      harmful      = true;
    ...
    {
      // 12/29/2017 - Deadly Poison uses an older style of refresh, adding the origial duration worth of ticks up to 50%
      // more than the base number of ticks
      //              Deadly Poison shouldn't have partial ticks, so we just add the amount of time relative to how many
      //              additional ticks we want to add
...
struct crippling_poison_t : public rogue_poison_t
{
  ...
  {
    crippling_poison_proc_t( rogue_t* rogue ) : rogue_poison_t( "crippling_poison", rogue, rogue->find_spell( 3409 ) )
    {
    }
...
struct leeching_poison_t : public rogue_poison_t
{
  ...
  {
    leeching_poison_proc_t( rogue_t* rogue ) : rogue_poison_t( "leeching_poison", rogue, rogue->find_spell( 112961 ) )
    {
    }
...
struct apply_poison_t : public action_t
{
  ...
  apply_poison_t( rogue_t* p, const std::string& options_str )
    : action_t( ACTION_OTHER, "apply_poison", p ),
      lethal_poison( POISON_NONE ),
      nonlethal_poison( POISON_NONE ),
      executed( false )
  {
    ...
    if ( p->main_hand_weapon.type != WEAPON_NONE || p->off_hand_weapon.type != WEAPON_NONE )
    {
      ...
      if ( lethal_str.empty() )
        lethal_poison = DEADLY_POISON;
      else if ( lethal_str == "deadly" )
        lethal_poison = DEADLY_POISON;
      else if ( lethal_str == "wound" )
        lethal_poison = WOUND_POISON;

      if ( nonlethal_str == "crippling" )
        nonlethal_poison = CRIPPLING_POISON;
      if ( nonlethal_str == "leeching" )
        nonlethal_poison = LEECHING_POISON;
    }

    if ( !p->active_lethal_poison )
    {
      if ( lethal_poison == DEADLY_POISON )
        p->active_lethal_poison = new deadly_poison_t( p );
      if ( lethal_poison == WOUND_POISON )
        p->active_lethal_poison = new wound_poison_t( p );
    }

    if ( !p->active_nonlethal_poison )
    {
      if ( nonlethal_poison == CRIPPLING_POISON )
        p->active_nonlethal_poison = new crippling_poison_t( p );
      if ( nonlethal_poison == LEECHING_POISON )
        p->active_nonlethal_poison = new leeching_poison_t( p );
    }
  }
...
struct melee_t : public rogue_attack_t
{
  ...
  melee_t( const char* name, rogue_t* p, int sw ) : rogue_attack_t( name, p ), sync_weapons( sw ), first( true )
  {
...
struct auto_melee_attack_t : public action_t
{
  ...
  auto_melee_attack_t( rogue_t* p, const std::string& options_str )
    : action_t( ACTION_OTHER, "auto_attack", p ), sync_weapons( 0 )
  {
    trigger_gcd = timespan_t::zero();
  ...
  void execute() override
  {
    ...
    if ( precombat_seconds && !p()->in_combat )
    {
      ...
      if ( p()->azerite.brigands_blitz.ok() )
        p()->buffs.brigands_blitz->trigger(
            as<int>( floor( -precombat_lost_seconds / p()->buffs.brigands_blitz_driver->buff_period ) ) );
    }
  }
};
...
struct between_the_eyes_t : public rogue_attack_t
{
  between_the_eyes_t( rogue_t* p, const std::string& options_str )
    : rogue_attack_t( "between_the_eyes", p, p->find_specialization_spell( "Between the Eyes" ), options_str )
  {
  ...
  void execute() override
  {
    ...
    if ( result_is_hit( execute_state->result ) )
    {
      ...
      if ( p()->azerite.ace_up_your_sleeve.ok() &&
           rng().roll( rs->cp * p()->azerite.ace_up_your_sleeve.spell_ref().effectN( 2 ).percent() ) )
        p()->trigger_combo_point_gain( as<int>( p()->azerite.ace_up_your_sleeve.spell_ref().effectN( 3 ).base_value() ),
                                       p()->gains.ace_up_your_sleeve, this );
    }
  }
...
struct blade_rush_t : public rogue_attack_t
{
  ...
  {
    blade_rush_attack_t( rogue_t* p ) : rogue_attack_t( "blade_rush_attack", p, p->find_spell( 271881 ) )
    {
      aoe = -1;
    }
...
struct blindside_t : public rogue_attack_t
{
  ...
  {
    if ( !p()->buffs.blindside->check() && candidate_target->health_percentage() >= data().effectN( 4 ).base_value() )
      return false;
...
// This ability does nothing but for some odd reasons throughout the history of Rogue spaghetti, we may want to look at
// using it. So, let's support it.
...
struct fan_of_knives_t : public rogue_attack_t
{
  ...
  {
    echoing_blades_t( rogue_t* p ) : rogue_attack_t( "echoing_blades", p, p->find_spell( 287653 ) )
    {
      aoe        = -1;
      background = true;
  ...
  void impact( action_state_t* state ) override
  {
    ...
    if ( echoing_blades_attack && state->result == RESULT_CRIT &&
         ++echoing_blades_crit_count <= p()->azerite.echoing_blades.spell_ref().effectN( 4 ).base_value() )
    {
...
struct garrote_state_t : public rogue_attack_state_t
{
  ...
  void initialize() override
  {
    rogue_attack_state_t::initialize();
    shrouded_suffocation = false;
  }
...
{
  garrote_t( rogue_t* p, const std::string& options_str ) : rogue_attack_t( "garrote", p, p->spec.garrote, options_str )
  {
    may_crit  = false;
    may_dodge = may_block = may_parry = false;
    hasted_ticks                      = true;
...
{
  killing_spree_tick_t( rogue_t* p, const char* name, const spell_data_t* s ) : rogue_attack_t( name, p, s )
  {
...
struct marked_for_death_t : public rogue_attack_t
{
  ...
  marked_for_death_t( rogue_t* p, const std::string& options_str )
    : rogue_attack_t( "marked_for_death", p, p->find_talent_spell( "Marked for Death" ) ), precombat_seconds( 0.0 )
  {
    add_option( opt_float( "precombat_seconds", precombat_seconds ) );
    parse_options( options_str );
...
{
  mutilate_strike_t( rogue_t* p, const char* name, const spell_data_t* s ) : rogue_attack_t( name, p, s )
  {
    background = true;
  }
...
{
  double_dose_t( rogue_t* p ) : rogue_attack_t( "double_dose", p, p->find_spell( 273009 ) )
  {
    background = true;
    may_miss = may_block = may_dodge = may_parry = false;
...
struct replicating_shadows_t : public rogue_attack_t
{
  ...
  replicating_shadows_t( rogue_t* p )
    : rogue_attack_t( "replicating_shadows", p, p->find_spell( 286131 ) ), nightblade_action( nullptr )
  {
    background = true;
    may_miss = may_block = may_dodge = may_parry = false;
  ...
  void execute() override
  {
    ...
    if ( last_nb_tdata->dots.nightblade->is_ticking() )
    {
      ...
      for ( const auto enemy : sim->target_non_sleeping_list )
      {
        ...
        if ( !tdata->dots.nightblade->is_ticking() )
        {
          double dist = enemy->get_position_distance( p()->last_nightblade_target->x_position,
                                                      p()->last_nightblade_target->y_position );
      ...
      // If it exists, trigger a new nightblade with 0 CP duration. We also copy the persistent multiplier in
      // nightblade_t. Estimated 10 yd spread radius.
      if ( minDistTarget && minDist < 10.0 )
      {
        ...
        if ( nightblade_action )
          make_event<actions::secondary_ability_trigger_t>( *sim, minDistTarget, nightblade_action,
                                                            cast_state( last_nb_tdata->dots.nightblade->state )->cp,
                                                            TRIGGER_REPLICATING_SHADOWS );
      }
    }
  }
...
struct roll_the_bones_t : public rogue_attack_t
{
  ...
  roll_the_bones_t( rogue_t* p, const std::string& options_str )
    : rogue_attack_t( "roll_the_bones", p, p->spec.roll_the_bones ), precombat_seconds( 0.0 )
  {
    add_option( opt_float( "precombat_seconds", precombat_seconds ) );
    parse_options( options_str );
      ...
      struct new_duration_expr_t : public expr_t
      {
        ...
        double evaluate() override
        {
          double duration = base_duration *
                            ( 1.0 + std::min( cp_max_spend, rupture->p()->resources.current[ RESOURCE_COMBO_POINT ] ) );

          return std::min( duration * 1.3,
                           duration + rupture->td( rupture->target )->dots.rupture->remains().total_seconds() );
        }
      };
...
struct secret_technique_t : public rogue_attack_t
{
  ...
  {
    secret_technique_attack_t( const std::string& n, rogue_t* p ) : rogue_attack_t( n, p, p->find_spell( 280720 ) )
    {
      weapon     = &( p->main_hand_weapon );
      background = true;
      aoe        = -1;
  ...
  void execute() override
  {
    ...
    make_event<actions::secondary_ability_trigger_t>( *sim, execute_state->target, player_attack, cp,
                                                      TRIGGER_SECRET_TECHNIQUE );
    ...
    {
      make_event<actions::secondary_ability_trigger_t>( *sim, execute_state->target, clone_attack, cp,
                                                        TRIGGER_SECRET_TECHNIQUE, delay );
    }
  }
};
...
{
  shadow_blades_attack_t( rogue_t* p ) : rogue_attack_t( "shadow_blades_attack", p, p->find_spell( 279043 ) )
  {
    background = true;
...
struct shadow_blades_t : public rogue_attack_t
{
  ...
  shadow_blades_t( rogue_t* p, const std::string& options_str )
    : rogue_attack_t( "shadow_blades", p, p->find_specialization_spell( "Shadow Blades" ) ), precombat_seconds( 0.0 )
  {
    add_option( opt_float( "precombat_seconds", precombat_seconds ) );
    parse_options( options_str );
...
struct shadow_dance_t : public rogue_attack_t
{
  ...
  shadow_dance_t( rogue_t* p, const std::string& options_str )
    : rogue_attack_t( "shadow_dance", p, p->spec.shadow_dance ), icd( p->get_cooldown( "shadow_dance_icd" ) )
  {
    parse_options( options_str );
  ...
  void execute() override
  {
    ...
    if ( p()->azerite.the_first_dance.ok() )
    {
      ...
      p()->trigger_combo_point_gain(
          as<int>( p()->buffs.the_first_dance->data().effectN( 3 ).resource( RESOURCE_COMBO_POINT ) ),
...
struct sinister_strike_t : public rogue_attack_t
{
  ...
  void execute() override
  {
    ...
    {
      make_event<actions::secondary_ability_trigger_t>( *sim, execute_state->target, extra_attack, 0,
                                                        TRIGGER_SINISTER_STRIKE, extra_attack_delay );
    }
  }
};
...
struct slice_and_dice_t : public rogue_attack_t
{
  ...
  slice_and_dice_t( rogue_t* p, const std::string& options_str )
    : rogue_attack_t( "slice_and_dice", p, p->talent.slice_and_dice ), precombat_seconds( 0.0 )
  {
    add_option( opt_float( "precombat_seconds", precombat_seconds ) );
    parse_options( options_str );
...
{
  sprint_t( rogue_t* p, const std::string& options_str ) : rogue_attack_t( "sprint", p, p->spell.sprint, options_str )
  {
...
struct vendetta_t : public rogue_attack_t
{
  ...
  {
    nothing_personal_t( rogue_t* p ) : rogue_attack_t( "nothing_personal", p, p->find_spell( 286581 ) )
    {
      may_dodge = may_parry = may_block = false;
      may_crit                          = false;
      tick_may_crit                     = true;
...
struct stealth_t : public rogue_attack_t
{
  ...
  bool ready() override
  {
    ...
    if ( p()->sim->fight_style == "DungeonSlice" && p()->player_t::buffs.shadowmeld->check() &&
         target->type == ENEMY_ADD )
      return true;
...
struct kidney_shot_t : public rogue_attack_t
{
  ...
  void impact( action_state_t* state ) override
  {
    ...
    {
      make_event<actions::secondary_ability_trigger_t>( *sim, state->target, internal_bleeding, cast_state( state )->cp,
                                                        TRIGGER_INTERNAL_BLEEDING );
    }
  }
};
...
struct cancel_autoattack_t : public action_t
{
  ...
  cancel_autoattack_t( rogue_t* rogue_, const std::string& options_str )
    : action_t( ACTION_OTHER, "cancel_autoattack", rogue_ ), rogue( rogue_ )
  {
    parse_options( options_str );
...
struct weapon_swap_t : public action_t
{
      ...
      rogue( rogue_ )
  {
    ...
    else if ( util::str_compare_ci( slot_str, "main" ) || util::str_compare_ci( slot_str, "main_hand" ) )
    {
      swap_type = SWAP_MAIN_HAND;
    }
    else if ( util::str_compare_ci( slot_str, "off" ) || util::str_compare_ci( slot_str, "off_hand" ) )
    {
    ...
    if ( swap_type != SWAP_BOTH )
    {
      ...
      {
        background = true;
        sim->errorf( "Player %s weapon_swap: No weapon info for %s/%s", player->name(), slot_str.c_str(),
                     swap_to_str.c_str() );
      }
    }
    else
  ...
  bool ready() override
  {
    if ( swap_type == SWAP_MAIN_HAND && rogue->weapon_data[ WEAPON_MAIN_HAND ].current_weapon == swap_to_type )
    {
      return false;
    }
    else if ( swap_type == SWAP_OFF_HAND && rogue->weapon_data[ WEAPON_OFF_HAND ].current_weapon == swap_to_type )
    {
      return false;
    }
    else if ( swap_type == SWAP_BOTH && rogue->weapon_data[ WEAPON_MAIN_HAND ].current_weapon == swap_to_type &&
...
struct exsanguinated_expr_t : public expr_t
{
  ...
  exsanguinated_expr_t( action_t* a ) : expr_t( "exsanguinated_expr" ), action( a )
  {
  }
...
void weapon_info_t::callback_state( current_weapon_e weapon, bool state )
{
  ...
  for ( size_t i = 0, end = cb_data[ weapon ].size(); i < end; ++i )
  {
    if ( state )
    {
      ...
      if ( sim->debug )
      {
        sim->out_debug.printf( "%s enabling callback %s on item %s", item_data[ WEAPON_PRIMARY ]->player->name(),
                               cb_data[ weapon ][ i ]->effect.name().c_str(), item_data[ weapon ]->name() );
      }
    }
    else
    {
      ...
      if ( sim->debug )
      {
        sim->out_debug.printf( "%s disabling callback %s on item %s", item_data[ WEAPON_PRIMARY ]->player->name(),
                               cb_data[ weapon ][ i ]->effect.name().c_str(), item_data[ weapon ]->name() );
      }
    }
  }
...
struct proxy_rupture_t : public buff_t
{
  ...
  proxy_rupture_t( rogue_td_t& r )
    : buff_t( r, "rupture", r.source->find_specialization_spell( "Rupture" ) ), rupture_dot( r.dots.rupture )
  {
    set_quiet( true );
...
{
  adrenaline_rush_t( rogue_t* p ) : buff_t( p, "adrenaline_rush", p->find_class_spell( "Adrenaline Rush" ) )
  {
    set_cooldown( timespan_t::zero() );
...
{
  blade_flurry_t( rogue_t* p ) : buff_t( p, "blade_flurry", p->spec.blade_flurry )
  {
    set_cooldown( timespan_t::zero() );
...
struct subterfuge_t : public buff_t
{
  ...
  subterfuge_t( rogue_t* r ) : buff_t( r, "subterfuge", r->find_spell( 115192 ) ), rogue( r )
  {
  }
...
{
  stealth_t( rogue_t* r ) : stealth_like_buff_t( r, "stealth", r->find_spell( 1784 ) )
  {
...
{
  vanish_t( rogue_t* r ) : stealth_like_buff_t( r, "vanish", r->find_spell( 11327 ) )
  {
  }
...
{
  shadow_dance_t( rogue_t* p ) : stealth_like_buff_t( p, "shadow_dance", p->spec.shadow_dance )
  {
...
struct shuriken_tornado_t : public buff_t
{
  ...
  shuriken_tornado_t( rogue_t* r )
    : buff_t( r, "shuriken_tornado", r->talent.shuriken_tornado ), rogue( r ), shuriken_storm_action( nullptr )
  {
    set_cooldown( timespan_t::zero() );
    ...
    set_tick_callback( [this]( buff_t*, int, const timespan_t& ) {
      ...
      if ( shuriken_storm_action )
        make_event<actions::secondary_ability_trigger_t>( *sim, rogue->target, shuriken_storm_action, 0,
                                                          TRIGGER_SHURIKEN_TORNADO );
    } );
  }
};
...
{
  rogue_poison_buff_t( rogue_td_t& r, const std::string& name, const spell_data_t* spell ) : buff_t( r, name, spell )
  {
  }
...
{
  wound_poison_t( rogue_td_t& r ) : rogue_poison_buff_t( r, "wound_poison", r.source->find_spell( 8680 ) )
  {
  }
};
...
{
  crippling_poison_t( rogue_td_t& r ) : rogue_poison_buff_t( r, "crippling_poison", r.source->find_spell( 3409 ) )
  {
  }
};
...
{
  leeching_poison_t( rogue_td_t& r ) : rogue_poison_buff_t( r, "leeching_poison", r.source->find_spell( 112961 ) )
  {
  }
};
...
struct roll_the_bones_t : public buff_t
{
  ...
  roll_the_bones_t( rogue_t* r ) : buff_t( r, "roll_the_bones", r->spec.roll_the_bones ), rogue( r )
  {
  ...
  std::vector<buff_t*> random_roll( bool loaded )
  {
    ...
    if ( !rogue->fixed_rtb_odds.empty() )
    {
      ...
      if ( loaded )
      {
        // At some point Loaded Dice were apparently changed to just convert 1 buffs straight into two buffs.
        // (2020-03-09)
  ...
  {
    std::vector<buff_t*> rolled;
    range::for_each( rogue->fixed_rtb, [this, &rolled]( size_t idx ) { rolled.push_back( buffs[ idx ] ); } );
    return rolled;
  }
...
void rogue_t::trigger_venomous_wounds( const action_state_t* state )
{
  ...
  resource_gain( RESOURCE_ENERGY, spec.venomous_wounds->effectN( 2 ).base_value(), gains.venomous_wounds );
}
...
void rogue_t::trigger_combo_point_gain( int cp, gain_t* gain, action_t* action )
{
...
void rogue_t::trigger_shadow_techniques( const action_state_t* state )
{
  ...
  if ( sim->debug )
    sim->out_debug.printf( "Melee attack landed, so shadow techniques increment from %d to %d", shadow_techniques,
                           shadow_techniques + 1 );
  ...
  if ( ++shadow_techniques == 5 || ( shadow_techniques == 4 && rng().roll( 0.5 ) ) )
  {
    ...
    trigger_combo_point_gain( as<int>( spec.shadow_techniques_effect->effectN( 1 ).base_value() ),
                              gains.shadow_techniques, state->action );
    resource_gain( RESOURCE_ENERGY, spec.shadow_techniques_effect->effectN( 2 ).base_value(), gains.shadow_techniques,
                   state->action );
    if ( sim->debug )
...
void rogue_t::trigger_weaponmaster( const action_state_t* s )
{
  ...
  make_event<actions::secondary_ability_trigger_t>(
      *sim, s->target, s->action, actions::rogue_attack_t::cast_state( s )->cp, TRIGGER_WEAPONMASTER );
}
...
void rogue_t::trigger_relentless_strikes( const action_state_t* state )
{
  ...
  grant_energy += rogue_attack_t::cast_state( state )->cp *
                  spell.relentless_strikes_energize->effectN( 1 ).resource( RESOURCE_ENERGY );
...
void rogue_t::spend_combo_points( const action_state_t* state )
{
  ...
  if ( sim->log )
    sim->out_log.printf( "%s consumes %.1f %s for %s (%.0f)", name(), max_spend,
                         util::resource_type_string( RESOURCE_COMBO_POINT ), state->action->name(),
                         resources.current[ RESOURCE_COMBO_POINT ] );
  ...
  if ( specialization() == ROGUE_SUBTLETY && replicating_shadows &&
       rng().roll( max_spend * azerite.replicating_shadows.spell_ref().effectN( 2 ).percent() ) )
  {
...
void rogue_t::trigger_inevitability( const action_state_t* state )
{
  ...
  buffs.symbols_of_death->extend_duration(
      this, timespan_t::from_seconds( azerite.inevitability.spell_ref().effectN( 2 ).base_value() / 10.0 ) );
}
...
rogue_td_t::rogue_td_t( player_t* target, rogue_t* source )
  : actor_target_data_t( target, source ), dots( dots_t() ), debuffs( debuffs_t() )
{
  ...
  debuffs.toxic_blade =
      make_buff( *this, "toxic_blade", source->talent.toxic_blade->effectN( 4 ).trigger() )
  ...
  debuffs.find_weakness =
      make_buff( *this, "find_weakness", fw_debuff )->set_default_value( fw_debuff->effectN( 1 ).percent() );
  ...
  {
    target->callbacks_on_demise.emplace_back(
        std::bind( &rogue_t::trigger_venomous_wounds_death, source, std::placeholders::_1 ) );
  }
}
...
{
  return ( true_level > 110 )
             ? ( specialization() == ROGUE_ASSASSINATION ? "potion_of_focused_resolve" : "potion_of_unbridled_fury" )
             : ( true_level > 100 )
                   ? "prolonged_power"
                   : ( true_level >= 90 )
                         ? "draenic_agility"
...
void rogue_t::init_action_list()
{
  ...
  const std::string reaping_flames_comment =
      "Hold Reaping Flames for execute range or kill buffs, if possible. Always try to get the lowest cooldown based "
      "on available enemies.";
  const std::string reaping_flames_condition =
      "cycling_variable,name=reaping_delay,op=min,if=essence.breath_of_the_dying.major,value=target.time_to_die";
  const std::string reaping_flames_action =
      "reaping_flames,target_if=target.time_to_die<1.5|((target.health.pct>80|target.health.pct<=20)&(active_enemies=1|"
      "variable.reaping_delay>29))|(target.time_to_pct_20>30&(active_enemies=1|variable.reaping_delay>44))";
  ...
  if ( specialization() == ROGUE_ASSASSINATION )
  {
    ...
    cds->add_action(
        "use_item,name=azsharas_font_of_power,if=!stealthed.all&master_assassin_remains=0&(cooldown.vendetta.remains<?("
        "cooldown.toxic_blade.remains*equipped.ashvanes_razor_coral))<10+10*equipped.ashvanes_razor_coral&!debuff."
        "vendetta.up&!debuff.toxic_blade.up" );
    cds->add_action( "call_action_list,name=essences,if=!stealthed.all&dot.rupture.ticking&master_assassin_remains=0" );
    cds->add_talent(
        this, "Marked for Death",
        "target_if=min:target.time_to_die,if=raid_event.adds.up&(target.time_to_die<combo_points.deficit*1.5|combo_"
        "points.deficit>=cp_max_spend)",
        "If adds are up, snipe the one with lowest TTD. Use when dying faster than CP deficit or without any CP." );
    cds->add_talent( this, "Marked for Death",
                     "if=raid_event.adds.in>30-raid_event.adds.duration&combo_points.deficit>=cp_max_spend",
                     "If no adds will die within the next 30s, use MfD on boss without any CP." );
    cds->add_action(
        "variable,name=vendetta_subterfuge_condition,value=!talent.subterfuge.enabled|!azerite.shrouded_suffocation."
        "enabled|dot.garrote.pmultiplier>1&(spell_targets.fan_of_knives<6|!cooldown.vanish.up)",
        "Vendetta logical conditionals based on current spec" );
    cds->add_action(
        "variable,name=vendetta_nightstalker_condition,value=!talent.nightstalker.enabled|!talent.exsanguinate.enabled|"
        "cooldown.exsanguinate.remains<5-2*talent.deeper_stratagem.enabled" );
    cds->add_action(
        "variable,name=variable,name=vendetta_font_condition,value=!equipped.azsharas_font_of_power|azerite.shrouded_"
        "suffocation.enabled|debuff.razor_coral_debuff.down|trinket.ashvanes_razor_coral.cooldown.remains<10&(cooldown."
        "toxic_blade.remains<1|debuff.toxic_blade.up)" );
    cds->add_action( this, "Vendetta",
                     "if=!stealthed.rogue&dot.rupture.ticking&!debuff.vendetta.up&variable.vendetta_subterfuge_"
                     "condition&variable.vendetta_nightstalker_condition&variable.vendetta_font_condition" );
    cds->add_action( this, "Vanish",
                     "if=talent.exsanguinate.enabled&talent.nightstalker.enabled&combo_points>=cp_max_spend&cooldown."
                     "exsanguinate.remains<1",
                     "Vanish with Exsg + Nightstalker: Maximum CP and Exsg ready for next GCD" );
    cds->add_action( this, "Vanish",
                     "if=talent.nightstalker.enabled&!talent.exsanguinate.enabled&combo_points>=cp_max_spend&(debuff."
                     "vendetta.up|essence.vision_of_perfection.enabled)",
                     "Vanish with Nightstalker + No Exsg: Maximum CP and Vendetta up (unless using VoP)" );
    cds->add_action(
        "variable,name=ss_vanish_condition,value=azerite.shrouded_suffocation.enabled&(non_ss_buffed_targets>=1|spell_"
        "targets.fan_of_knives=3)&(ss_buffed_targets_above_pandemic=0|spell_targets.fan_of_knives>=6)",
    ...
    cds->add_action( this, "Vanish",
                     "if=talent.subterfuge.enabled&!stealthed.rogue&cooldown.garrote.up&(variable.ss_vanish_condition|!"
                     "azerite.shrouded_suffocation.enabled&(dot.garrote.refreshable|debuff.vendetta.up&dot.garrote."
                     "pmultiplier<=1))&combo_points.deficit>=((1+2*azerite.shrouded_suffocation.enabled)*spell_targets."
                     "fan_of_knives)>?4&raid_event.adds.in>12" );
    cds->add_action(
        this, "Vanish",
        "if=talent.master_assassin.enabled&!stealthed.all&master_assassin_remains<=0&!dot.rupture.refreshable&dot."
        "garrote.remains>3&(debuff.vendetta.up&(!talent.toxic_blade.enabled|debuff.toxic_blade.up)&(!essence.blood_of_"
        "the_enemy.major|debuff.blood_of_the_enemy.up)|essence.vision_of_perfection.enabled)",
        "Vanish with Master Assasin: No stealth and no active MA buff, Rupture not in refresh range, during "
        "Vendetta+TB+BotE (unless using VoP)" );
    cds->add_action(
        "shadowmeld,if=!stealthed.all&azerite.shrouded_suffocation.enabled&dot.garrote.refreshable&dot.garrote."
        "pmultiplier<=1&combo_points.deficit>=1",
        "Shadowmeld for Shrouded Suffocation" );
    cds->add_talent( this, "Exsanguinate",
                     "if=!stealthed.rogue&!dot.garrote.refreshable&dot.rupture.remains>4+4*cp_max_spend",
                     "Exsanguinate when not stealthed and both Rupture and Garrote are up for long enough." );
    cds->add_talent( this, "Toxic Blade",
                     "if=dot.rupture.ticking&(!equipped.azsharas_font_of_power|cooldown.vendetta.remains>10)" );
    ...
    cds->add_action(
        "use_item,name=galecallers_boon,if=(debuff.vendetta.up|(!talent.exsanguinate.enabled&cooldown.vendetta.remains>"
        "45|talent.exsanguinate.enabled&cooldown.exsanguinate.remains<6))&!exsanguinated.rupture" );
    cds->add_action( "use_item,name=ashvanes_razor_coral,if=debuff.razor_coral_debuff.down|target.time_to_die<20" );
    cds->add_action(
        "use_item,name=ashvanes_razor_coral,if=(!talent.exsanguinate.enabled|!talent.subterfuge.enabled)&debuff."
        "vendetta.remains>10-4*equipped.azsharas_font_of_power" );
    cds->add_action(
        "use_item,name=ashvanes_razor_coral,if=(talent.exsanguinate.enabled&talent.subterfuge.enabled)&debuff.vendetta."
        "up&(exsanguinated.garrote|azerite.shrouded_suffocation.enabled&dot.garrote.pmultiplier>1)" );
    cds->add_action(
        "use_item,effect_name=cyclotronic_blast,if=master_assassin_remains=0&!debuff.vendetta.up&!debuff.toxic_blade."
        "up&buff.memory_of_lucid_dreams.down&energy<80&dot.rupture.remains>4" );
    ...
    essences->add_action(
        "concentrated_flame,if=energy.time_to_max>1&!debuff.vendetta.up&(!dot.concentrated_flame_burn.ticking&!action."
        "concentrated_flame.in_flight|full_recharge_time<gcd.max)" );
    essences->add_action(
        "blood_of_the_enemy,if=debuff.vendetta.up&(exsanguinated.garrote|debuff.toxic_blade.up&combo_points.deficit<=1|"
        "debuff.vendetta.remains<=10)|target.time_to_die<=10",
        "Always use Blood with Vendetta up. Hold for Exsanguinate. Use with TB up before a finisher as long as it runs "
        "for 10s during Vendetta." );
    essences->add_action( "guardian_of_azeroth,if=cooldown.vendetta.remains<3|debuff.vendetta.up|target.time_to_die<30",
                          "Attempt to align Guardian with Vendetta as long as it won't result in losing a full-value "
                          "cast over the remaining duration of the fight" );
    essences->add_action(
        "guardian_of_azeroth,if=floor((target.time_to_die-30)%cooldown)>floor((target.time_to_die-30-cooldown.vendetta."
        "remains)%cooldown)" );
    ...
    stealthed->add_action( this, "Rupture",
                           "if=talent.nightstalker.enabled&combo_points>=4&target.time_to_die-remains>6",
                           "Nighstalker on 1T: Snapshot Rupture" );
    stealthed->add_action( "pool_resource,for_next=1",
                           "Subterfuge + Shrouded Suffocation: Ensure we use one global to apply Garrote to the main "
                           "target if it is not snapshot yet, so all other main target abilities profit." );
    stealthed->add_action(
        this, "Garrote",
        "if=azerite.shrouded_suffocation.enabled&buff.subterfuge.up&buff.subterfuge.remains<1.3&!ss_buffed" );
    stealthed->add_action( "pool_resource,for_next=1", "Subterfuge: Apply or Refresh with buffed Garrotes" );
    stealthed->add_action(
        this, "Garrote",
        "target_if=min:remains,if=talent.subterfuge.enabled&(remains<12|pmultiplier<=1)&target.time_to_die-remains>2" );
    stealthed->add_action(
        this, "Rupture",
        "if=talent.subterfuge.enabled&azerite.shrouded_suffocation.enabled&!dot.rupture.ticking&variable.single_target",
    ...
    stealthed->add_action(
        this, "Garrote",
        "target_if=min:remains,if=talent.subterfuge.enabled&azerite.shrouded_suffocation.enabled&(active_enemies>1|!"
        "talent.exsanguinate.enabled)&target.time_to_die>remains&(remains<18|!ss_buffed)" );
    stealthed->add_action(
        "pool_resource,for_next=1",
        "Subterfuge + Exsg on 1T: Refresh Garrote at the end of stealth to get max duration before Exsanguinate" );
    stealthed->add_action(
        this, "Garrote",
        "if=talent.subterfuge.enabled&talent.exsanguinate.enabled&active_enemies=1&buff.subterfuge.remains<1.3" );

    // Damage over time abilities
    action_priority_list_t* dot = get_action_priority_list( "dot", "Damage over time abilities" );
    dot->add_action(
        "variable,name=skip_cycle_garrote,value=priority_rotation&spell_targets.fan_of_knives>3&(dot.garrote.remains<"
        "cooldown.garrote.duration|poisoned_bleeds>5)",
        "Limit Garrotes on non-primrary targets for the priority rotation if 5+ bleeds are already up" );
    dot->add_action(
        "variable,name=skip_cycle_rupture,value=priority_rotation&spell_targets.fan_of_knives>3&(debuff.toxic_blade.up|"
        "(poisoned_bleeds>5&!azerite.scent_of_blood.enabled))",
        "Limit Ruptures on non-primrary targets for the priority rotation if 5+ bleeds are already up" );
    dot->add_action(
        "variable,name=skip_rupture,value=debuff.vendetta.up&(debuff.toxic_blade.up|master_assassin_remains>0)&dot."
        "rupture.remains>2",
        "Limit Ruptures if Vendetta+Toxic Blade/Master Assassin is up and we have 2+ seconds left on the Rupture DoT" );
    dot->add_action( this, "Rupture",
                     "if=talent.exsanguinate.enabled&((combo_points>=cp_max_spend&cooldown.exsanguinate.remains<1)|(!"
                     "ticking&(time>10|combo_points>=2)))",
                     "Special Rupture setup for Exsg" );
    dot->add_action( "pool_resource,for_next=1",
                     "Garrote upkeep, also tries to use it as a special generator for the last CP before a finisher" );
    dot->add_action(
        this, "Garrote",
        "if=(!talent.subterfuge.enabled|!(cooldown.vanish.up&cooldown.vendetta.remains<=4))&combo_points.deficit>=1+3*("
        "azerite.shrouded_suffocation.enabled&cooldown.vanish.up)&refreshable&(pmultiplier<=1|remains<=tick_time&spell_"
        "targets.fan_of_knives>=3+azerite.shrouded_suffocation.enabled)&(!exsanguinated|remains<=tick_time*2&spell_"
        "targets.fan_of_knives>=3+azerite.shrouded_suffocation.enabled)&!ss_buffed&(target.time_to_die-remains)>4&("
        "master_assassin_remains=0|!ticking&azerite.shrouded_suffocation.enabled)" );
    dot->add_action( "pool_resource,for_next=1" );
    dot->add_action(
        this, "Garrote",
        "cycle_targets=1,if=!variable.skip_cycle_garrote&target!=self.target&(!talent.subterfuge.enabled|!(cooldown."
        "vanish.up&cooldown.vendetta.remains<=4))&combo_points.deficit>=1+3*(azerite.shrouded_suffocation.enabled&"
        "cooldown.vanish.up)&refreshable&(pmultiplier<=1|remains<=tick_time&spell_targets.fan_of_knives>=3+azerite."
        "shrouded_suffocation.enabled)&(!exsanguinated|remains<=tick_time*2&spell_targets.fan_of_knives>=3+azerite."
        "shrouded_suffocation.enabled)&!ss_buffed&(target.time_to_die-remains)>12&(master_assassin_remains=0|!ticking&"
        "azerite.shrouded_suffocation.enabled)" );
    dot->add_talent( this, "Crimson Tempest", "if=spell_targets>=2&remains<2+(spell_targets>=5)&combo_points>=4",
                     "Crimson Tempest only on multiple targets at 4+ CP when running out in 2s (up to 4 targets) or 3s "
                     "(5+ targets)" );
    dot->add_action(
        this, "Rupture",
        "if=!variable.skip_rupture&combo_points>=4&refreshable&(pmultiplier<=1|remains<=tick_time&spell_targets.fan_of_"
        "knives>=3+azerite.shrouded_suffocation.enabled)&(!exsanguinated|remains<=tick_time*2&spell_targets.fan_of_"
        "knives>=3+azerite.shrouded_suffocation.enabled)&target.time_to_die-remains>4",
        "Keep up Rupture at 4+ on all targets (when living long enough and not snapshot)" );
    dot->add_action( this, "Rupture",
                     "cycle_targets=1,if=!variable.skip_cycle_rupture&!variable.skip_rupture&target!=self.target&combo_"
                     "points>=4&refreshable&(pmultiplier<=1|remains<=tick_time&spell_targets.fan_of_knives>=3+azerite."
                     "shrouded_suffocation.enabled)&(!exsanguinated|remains<=tick_time*2&spell_targets.fan_of_knives>="
                     "3+azerite.shrouded_suffocation.enabled)&target.time_to_die-remains>4" );

    // Direct damage abilities
    action_priority_list_t* direct = get_action_priority_list( "direct", "Direct damage abilities" );
    direct->add_action( this, "Envenom",
                        "if=combo_points>=4+talent.deeper_stratagem.enabled&(debuff.vendetta.up|debuff.toxic_blade.up|"
                        "energy.deficit<=25+variable.energy_regen_combined|!variable.single_target)&(!talent."
                        "exsanguinate.enabled|cooldown.exsanguinate.remains>2)",
                        "Envenom at 4+ (5+ with DS) CP. Immediately on 2+ targets, with Vendetta, or with TB; "
                        "otherwise wait for some energy. Also wait if Exsg combo is coming up." );
    direct->add_action(
        "variable,name=use_filler,value=combo_points.deficit>1|energy.deficit<=25+variable.energy_regen_combined|!"
        "variable.single_target" );
    direct->add_action( this, "Fan of Knives",
                        "if=variable.use_filler&azerite.echoing_blades.enabled&spell_targets.fan_of_knives>=2+(debuff."
                        "vendetta.up*(1+(azerite.echoing_blades.rank=1)))",
                        "With Echoing Blades, Fan of Knives at 2+ targets, or 3-4+ targets when Vendetta is up" );
    direct->add_action( this, "Fan of Knives",
                        "if=variable.use_filler&(buff.hidden_blades.stack>=19|(!priority_rotation&spell_targets.fan_of_"
                        "knives>=4+(azerite.double_dose.rank>2)+stealthed.rogue))",
                        "Fan of Knives at 19+ stacks of Hidden Blades or against 4+ (5+ with Double Dose) targets." );
    direct->add_action(
        this, "Fan of Knives",
        "target_if=!dot.deadly_poison_dot.ticking,if=variable.use_filler&spell_targets.fan_of_knives>=3",
        "Fan of Knives to apply Deadly Poison if inactive on any target at 3 targets." );
    direct->add_talent(
        this, "Blindside",
        "if=variable.use_filler&(buff.blindside.up|!talent.venom_rush.enabled&!azerite.double_dose.enabled)" );
    direct->add_action( this, "Mutilate",
                        "target_if=!dot.deadly_poison_dot.ticking,if=variable.use_filler&spell_targets.fan_of_knives=2",
  ...
  else if ( specialization() == ROGUE_OUTLAW )
  {
    // Pre-Combat
    precombat->add_action( this, "Stealth",
                           "if=(!equipped.pocketsized_computation_device|!cooldown.cyclotronic_blast.duration|raid_"
                           "event.invulnerable.exists)" );
    ...
    precombat->add_action( this, "Adrenaline Rush",
                           "precombat_seconds=1,if=(!equipped.pocketsized_computation_device|!cooldown.cyclotronic_"
                           "blast.duration|raid_event.invulnerable.exists)" );
    precombat->add_action( "use_item,name=azsharas_font_of_power" );
    precombat->add_action( "use_item,effect_name=cyclotronic_blast,if=!raid_event.invulnerable.exists" );

    // Main Rotation
    def->add_action( "variable,name=rtb_reroll,value=rtb_buffs<2&!buff.grand_melee.up&!buff.ruthless_precision.up",
                     "Reroll for 2+ or Grand Melee or Ruthless Precision." );
    def->add_action(
        "variable,name=rtb_reroll,op=set,if=azerite.deadshot.enabled,value=rtb_buffs<2&(buff.loaded_dice.up|!buff."
        "broadside.up)",
        "Reroll for 2+ buffs or Broadside with Deadshot." );
    def->add_action(
        "variable,name=rtb_reroll,op=set,if=azerite.ace_up_your_sleeve.enabled&azerite.ace_up_your_sleeve.rank>="
        "azerite.deadshot.rank,value=rtb_buffs<2&(buff.loaded_dice.up|buff.ruthless_precision.remains<=cooldown."
        "between_the_eyes.remains)",
        "Reroll for 2+ buffs or Ruthless Precision with Ace up your Sleeve, unless there are more Deadshot ranks." );
    def->add_action( "variable,name=rtb_reroll,op=set,if=azerite.snake_eyes.rank>=2,value=rtb_buffs<2",
    ...
    def->add_action(
        "variable,name=rtb_reroll,op=set,if=buff.blade_flurry.up,value=rtb_buffs-buff.skull_and_crossbones.up<2&(buff."
        "loaded_dice.up|!buff.grand_melee.up&!buff.ruthless_precision.up&!buff.broadside.up)",
        "With Blade Flurry up, ignore rules above and take everything that is 2+ (not counting SaC) or single BS, GM, "
        "RP" );
    def->add_action(
        "variable,name=rtb_reroll,op=set,if=buff.loaded_dice.up,value=(rtb_buffs-buff.buried_treasure.up)<2|buff.roll_"
        "the_bones.remains<10.8+(1.8*talent.deeper_stratagem.enabled)",
        "With Loaded Dice up, reroll any single buff, any 2 buff with Buried Treasure, or in pandemic." );
    def->add_action(
        "variable,name=ambush_condition,value=combo_points.deficit>=2+2*(talent.ghostly_strike.enabled&cooldown."
        "ghostly_strike.remains<1)+buff.broadside.up&energy>60&!buff.skull_and_crossbones.up&!buff.keep_your_wits_"
        "about_you.up" );
    def->add_action(
        "variable,name=bte_condition,value=buff.ruthless_precision.up|(azerite.deadshot.enabled|azerite.ace_up_your_"
        "sleeve.enabled)&buff.roll_the_bones.up" );
    def->add_action(
        "variable,name=blade_flurry_sync,value=spell_targets.blade_flurry<2&raid_event.adds.in>20|buff.blade_flurry.up",
        "With multiple targets, this variable is checked to decide whether some CDs should be synced with Blade "
        "Flurry" );
    def->add_action( "call_action_list,name=stealth,if=stealthed.all" );
    def->add_action( "call_action_list,name=cds" );
    def->add_action(
        "run_action_list,name=finish,if=combo_points>=cp_max_spend-(buff.broadside.up+buff.opportunity.up)*(talent."
        "quick_draw.enabled&(!talent.marked_for_death.enabled|cooldown.marked_for_death.remains>1))*(azerite.ace_up_"
        "your_sleeve.rank<2|!cooldown.between_the_eyes.up|!buff.roll_the_bones.up)",
        "Finish at maximum CP. Substract one for each Broadside and Opportunity when Quick Draw is selected and MfD is "
        "not ready after the next second. Always max BtE with 2+ Ace." );
    ...
    cds->add_action(
        ...
        "if=!buff.adrenaline_rush.up&(!equipped.azsharas_font_of_power|cooldown.latent_arcana.remains>20)" );
    cds->add_talent(
        this, "Marked for Death",
        "target_if=min:target.time_to_die,if=raid_event.adds.up&(target.time_to_die<combo_points.deficit|!stealthed."
        "rogue&combo_points.deficit>=cp_max_spend-1)",
        "If adds are up, snipe the one with lowest TTD. Use when dying faster than CP deficit or without any CP." );
    cds->add_talent(
        this, "Marked for Death",
        "if=raid_event.adds.in>30-raid_event.adds.duration&!stealthed.rogue&combo_points.deficit>=cp_max_spend-1",
        "If no adds will die within the next 30s, use MfD on boss without any CP." );
    cds->add_action( this, "Blade Flurry",
                     "if=spell_targets>=2&!buff.blade_flurry.up&(!raid_event.adds.exists|raid_event.adds.remains>8|"
                     "raid_event.adds.in>(2-cooldown.blade_flurry.charges_fractional)*25)",
                     "Blade Flurry on 2+ enemies. With adds: Use if they stay for 8+ seconds or if your next charge "
                     "will be ready in time for the next wave." );
    cds->add_talent( this, "Ghostly Strike", "if=combo_points.deficit>=1+buff.broadside.up" );
    cds->add_talent( this, "Killing Spree",
                     "if=variable.blade_flurry_sync&spell_targets.blade_flurry>1&energy.time_to_max>2" );
    cds->add_talent( this, "Blade Rush", "if=variable.blade_flurry_sync&energy.time_to_max>2" );
    cds->add_action( this, "Vanish", "if=!stealthed.all&variable.ambush_condition",
                     "Using Vanish/Ambush is only a very tiny increase, so in reality, you're absolutely fine to use "
                     "it as a utility spell." );
    ...
    cds->add_action(
        "use_item,effect_name=cyclotronic_blast,if=!stealthed.all&buff.adrenaline_rush.down&buff.memory_of_lucid_"
        "dreams.down&energy.time_to_max>4&rtb_buffs<5" );
    cds->add_action(
        "use_item,name=azsharas_font_of_power,if=!buff.adrenaline_rush.up&!buff.blade_flurry.up&cooldown.adrenaline_"
        "rush.remains<15" );
    cds->add_action(
        "use_item,name=ashvanes_razor_coral,if=debuff.razor_coral_debuff.down|debuff.conductive_ink_debuff.up&target."
        "health.pct<32&target.health.pct>=30|!debuff.conductive_ink_debuff.up&(debuff.razor_coral_debuff.stack>=20-10*"
        "debuff.blood_of_the_enemy.up|target.time_to_die<60)&buff.adrenaline_rush.remains>18",
        "Very roughly rule of thumbified maths below: Use for Inkpod crit, otherwise with AR at 20+ stacks or 10+ with "
        "also Blood up." );
    cds->add_action( "use_items,if=buff.bloodlust.react|target.time_to_die<=20|combo_points.deficit<=2",
                     "Default fallback for usable items." );

    // Azerite Essences
    action_priority_list_t* essences = get_action_priority_list( "essences", "Essences" );
    essences->add_action(
        "concentrated_flame,if=energy.time_to_max>1&!buff.blade_flurry.up&(!dot.concentrated_flame_burn.ticking&!"
        "action.concentrated_flame.in_flight|full_recharge_time<gcd.max)" );
    essences->add_action(
        "blood_of_the_enemy,if=variable.blade_flurry_sync&cooldown.between_the_eyes.up&variable.bte_condition&(spell_"
        "targets.blade_flurry>=2|raid_event.adds.in>45)" );
    ...
    stealth->add_action(
        this, "Cheap Shot",
        "target_if=min:debuff.prey_on_the_weak.remains,if=talent.prey_on_the_weak.enabled&!target.is_boss" );
    stealth->add_action( this, "Ambush" );

    // Finishers
    action_priority_list_t* finish = get_action_priority_list( "finish", "Finishers" );
    finish->add_action( this, "Between the Eyes", "if=variable.bte_condition",
                        "BtE over RtB rerolls with Deadshot/Ace traits or Ruthless Precision." );
    finish->add_talent(
        this, "Slice and Dice",
        "if=buff.slice_and_dice.remains<target.time_to_die&buff.slice_and_dice.remains<(1+combo_points)*1.8" );
    finish->add_action( this, "Roll the Bones", "if=buff.roll_the_bones.remains<=3|variable.rtb_reroll" );
    finish->add_action( this, "Between the Eyes", "if=azerite.ace_up_your_sleeve.enabled|azerite.deadshot.enabled",
    ...
    build->add_action( this, "Pistol Shot",
                       "if=(talent.quick_draw.enabled|azerite.keep_your_wits_about_you.rank<2)&buff.opportunity.up&("
                       "buff.keep_your_wits_about_you.stack<14|energy<45)",
                       "Use Pistol Shot if it won't cap combo points and the Opportunity buff is up. Avoid using when "
                       "Keep Your Wits stacks are high or when using Weaponmaster, unless the Deadshot buff is up." );
  ...
  else if ( specialization() == ROGUE_SUBTLETY )
  {
    ...
    def->add_action( this, "Nightblade", "if=target.time_to_die>6&remains<gcd.max&combo_points>=4-(time<10)*2",
                     "Apply Nightblade at 2+ CP during the first 10 seconds, after that 4+ CP if it expires within the "
                     "next GCD or is not up" );
    def->add_action( "variable,name=use_priority_rotation,value=priority_rotation&spell_targets.shuriken_storm>=2",
                     "Only change rotation if we have priority_rotation set and multiple targets up." );
    def->add_action( "call_action_list,name=stealth_cds,if=variable.use_priority_rotation",
                     "Priority Rotation? Let's give a crap about energy for the stealth CDs (builder still respect "
                     "it). Yup, it can be that simple." );
    def->add_action(
        "variable,name=stealth_threshold,value=25+talent.vigor.enabled*35+talent.master_of_shadows.enabled*25+talent."
        "shadow_focus.enabled*20+talent.alacrity.enabled*10+15*(spell_targets.shuriken_storm>=3)",
        "Used to define when to use stealth CDs or builders" );
    def->add_action( "call_action_list,name=stealth_cds,if=energy.deficit<=variable.stealth_threshold",
                     "Consider using a Stealth CD when reaching the energy threshold" );
    def->add_action(
        this, "Nightblade",
        "if=azerite.nights_vengeance.enabled&!buff.nights_vengeance.up&combo_points.deficit>1&(spell_targets.shuriken_"
        "storm<2|variable.use_priority_rotation)&(cooldown.symbols_of_death.remains<=3|(azerite.nights_vengeance.rank>="
        "2&buff.symbols_of_death.remains>3&!stealthed.all&cooldown.shadow_dance.charges_fractional>=0.9))",
        "Night's Vengeance: Nightblade before Symbols at low CP to combine early refresh with getting the buff up. "
        "Also low CP during Symbols between Dances with 2+ NV." );
    def->add_action( "call_action_list,name=finish,if=combo_points.deficit<=1|target.time_to_die<=1&combo_points>=3",
                     "Finish at 4+ without DS, 5+ with DS (outside stealth)" );
    def->add_action( "call_action_list,name=finish,if=spell_targets.shuriken_storm=4&combo_points>=4",
    ...
    cds->add_action(
        this, "Shadow Dance",
        "use_off_gcd=1,if=!buff.shadow_dance.up&buff.shuriken_tornado.up&buff.shuriken_tornado.remains<=3.5",
        "Use Dance off-gcd before the first Shuriken Storm from Tornado comes in." );
    cds->add_action( this, "Symbols of Death",
                     "use_off_gcd=1,if=buff.shuriken_tornado.up&buff.shuriken_tornado.remains<=3.5",
                     "(Unless already up because we took Shadow Focus) use Symbols off-gcd before the first Shuriken "
                     "Storm from Tornado comes in." );
    cds->add_action(
        "call_action_list,name=essences,if=!stealthed.all&dot.nightblade.ticking|essence.breath_of_the_dying.major&"
        "time>=2" );
    cds->add_action( "pool_resource,for_next=1,if=!talent.shadow_focus.enabled",
                     "Pool for Tornado pre-SoD with ShD ready when not running SF." );
    cds->add_talent(
        this, "Shuriken Tornado",
        "if=energy>=60&dot.nightblade.ticking&cooldown.symbols_of_death.up&cooldown.shadow_dance.charges>=1",
        "Use Tornado pre SoD when we have the energy whether from pooling without SF or just generally." );
    cds->add_action(
        this, "Symbols of Death",
        "if=dot.nightblade.ticking&!cooldown.shadow_blades.up&(!talent.shuriken_tornado.enabled|talent.shadow_focus."
        "enabled|cooldown.shuriken_tornado.remains>2)&(!essence.blood_of_the_enemy.major|cooldown.blood_of_the_enemy."
        "remains>2)&(azerite.nights_vengeance.rank<2|buff.nights_vengeance.up)",
        "Use Symbols on cooldown (after first Nightblade) unless we are going to pop Tornado and do not have Shadow "
        "Focus." );
    cds->add_talent( this, "Marked for Death",
                     "target_if=min:target.time_to_die,if=raid_event.adds.up&(target.time_to_die<combo_points.deficit|!"
                     "stealthed.all&combo_points.deficit>=cp_max_spend)",
                     "If adds are up, snipe the one with lowest TTD. Use when dying faster than CP deficit or not "
                     "stealthed without any CP." );
    cds->add_talent(
        this, "Marked for Death",
        "if=raid_event.adds.in>30-raid_event.adds.duration&!stealthed.all&combo_points.deficit>=cp_max_spend",
    ...
    cds->add_talent( this, "Shuriken Tornado",
                     "if=talent.shadow_focus.enabled&dot.nightblade.ticking&buff.symbols_of_death.up",
                     "With SF, if not already done, use Tornado with SoD up." );
    cds->add_action( this, "Shadow Dance",
                     "if=!buff.shadow_dance.up&target.time_to_die<=5+talent.subterfuge.enabled&!raid_event.adds.up" );
    ...
    cds->add_action(
        "use_item,effect_name=cyclotronic_blast,if=!stealthed.all&dot.nightblade.ticking&!buff.symbols_of_death.up&"
        "energy.deficit>=30" );
    cds->add_action(
        "use_item,name=azsharas_font_of_power,if=!buff.shadow_dance.up&cooldown.symbols_of_death.remains<10" );
    cds->add_action(
        "use_item,name=ashvanes_razor_coral,if=debuff.razor_coral_debuff.down|debuff.conductive_ink_debuff.up&target."
        "health.pct<32&target.health.pct>=30|!debuff.conductive_ink_debuff.up&(debuff.razor_coral_debuff.stack>=25-10*"
        "debuff.blood_of_the_enemy.up|target.time_to_die<40)&buff.symbols_of_death.remains>8",
        "Very roughly rule of thumbified maths below: Use for Inkpod crit, otherwise with SoD at 25+ stacks or 15+ "
        "with also Blood up." );
    cds->add_action( "use_item,name=mydas_talisman" );
    cds->add_action( "use_items,if=buff.symbols_of_death.up|target.time_to_die<20",
                     "Default fallback for usable items: Use with Symbols of Death." );

    // Azerite Essences
    action_priority_list_t* essences = get_action_priority_list( "essences", "Essences" );
    essences->add_action(
        "concentrated_flame,if=energy.time_to_max>1&!buff.symbols_of_death.up&(!dot.concentrated_flame_burn.ticking&!"
        "action.concentrated_flame.in_flight|full_recharge_time<gcd.max)" );
    essences->add_action(
        "blood_of_the_enemy,if=!cooldown.shadow_blades.up&cooldown.symbols_of_death.up|target.time_to_die<=10" );
    essences->add_action( "guardian_of_azeroth" );
    essences->add_action(
        "focused_azerite_beam,if=(spell_targets.shuriken_storm>=2|raid_event.adds.in>60)&!cooldown.symbols_of_death.up&"
        "!buff.symbols_of_death.up&energy.deficit>=30" );
    ...
    stealth_cds->add_action( "variable,name=shd_threshold,value=cooldown.shadow_dance.charges_fractional>=1.75",
                             "Helper Variable" );
    stealth_cds->add_action(
        this, "Vanish",
        "if=!variable.shd_threshold&combo_points.deficit>1&debuff.find_weakness.remains<1&cooldown.symbols_of_death."
        "remains>=3",
    ...
    stealth_cds->add_action( "pool_resource,for_next=1,extra_amount=40",
                             "Pool for Shadowmeld + Shadowstrike unless we are about to cap on Dance charges. Only "
                             "when Find Weakness is about to run out." );
    stealth_cds->add_action(
        "shadowmeld,if=energy>=40&energy.deficit>=10&!variable.shd_threshold&combo_points.deficit>1&debuff.find_"
        "weakness.remains<1" );
    stealth_cds->add_action(
        "variable,name=shd_combo_points,value=combo_points.deficit>=4-(talent.deeper_stratagem.enabled&(azerite.the_"
        "first_dance.enabled&!talent.dark_shadow.enabled&!talent.subterfuge.enabled&spell_targets.shuriken_storm<3))",
        "CP requirement: Dance at low CP by default. (Subtraction is a copy from the stealhed finisher call for TFD "
        "handling.)" );
    stealth_cds->add_action(
        "variable,name=shd_combo_points,value=combo_points.deficit<=1+2*azerite.the_first_dance.enabled,if=variable."
        "use_priority_rotation&(talent.nightstalker.enabled|talent.dark_shadow.enabled)",
    ...
    stealth_cds->add_action(
        ...
        "&(variable.shd_threshold|buff.symbols_of_death.remains>=1.2|spell_targets.shuriken_storm>=4&cooldown.symbols_"
        "of_death.remains>10)&(azerite.nights_vengeance.rank<2|buff.nights_vengeance.up)",
        "With Dark Shadow only Dance when Nightblade will stay up. Use during Symbols or above threshold. Wait for NV "
        "buff with 2+NV." );
    stealth_cds->add_action(
        this, "Shadow Dance",
        "if=variable.shd_combo_points&target.time_to_die<cooldown.symbols_of_death.remains&!raid_event.adds.up",
    ...
    stealthed->add_action(
        this, "Shadowstrike",
        "if=(talent.find_weakness.enabled|spell_targets.shuriken_storm<3)&(buff.stealth.up|buff.vanish.up)",
        "If Stealth/vanish are up, use Shadowstrike to benefit from the passive bonus and Find Weakness, even if we "
        "are at max CP (from the precombat MfD)." );
    stealthed->add_action(
    ...
    stealthed->add_action( "call_action_list,name=finish,if=spell_targets.shuriken_storm=4&combo_points>=4",
    ...
    stealthed->add_action(
        ...
        "azerite.the_first_dance.enabled&spell_targets.shuriken_storm<3&buff.nights_vengeance.up))",
        "Finish at 4+ CP without DS, 5+ with DS, and 6 with DS after Vanish or The First Dance + Nights Vegeance" );
    stealthed->add_talent( this, "Gloomblade",
                           "if=azerite.perforate.rank>=2&spell_targets.shuriken_storm<=2&position_back",
                           "Use Gloomblade over Shadowstrike and Storm with 2+ Perforate at 2 or less targets." );
    stealthed->add_action( this, "Shadowstrike",
                           "cycle_targets=1,if=talent.secret_technique.enabled&talent.find_weakness.enabled&debuff."
                           "find_weakness.remains<1&spell_targets.shuriken_storm=2&target.time_to_die-remains>6",
                           "At 2 targets with Secret Technique keep up Find Weakness by cycling Shadowstrike." );
    stealthed->add_action(
        this, "Shadowstrike",
        "if=!talent.deeper_stratagem.enabled&azerite.blade_in_the_shadows.rank=3&spell_targets.shuriken_storm=3",
        "Without Deeper Stratagem and 3 Ranks of Blade in the Shadows it is worth using Shadowstrike on 3 targets." );
    stealthed->add_action(
        this, "Shadowstrike",
        "if=variable.use_priority_rotation&(talent.find_weakness.enabled&debuff.find_weakness.remains<1|talent."
        "weaponmaster.enabled&spell_targets.shuriken_storm<=4|azerite.inevitability.enabled&buff.symbols_of_death.up&"
        "spell_targets.shuriken_storm<=3+azerite.blade_in_the_shadows.enabled)",
        "For priority rotation, use Shadowstrike over Storm 1) with WM against up to 4 targets, 2) if FW is running "
        "off (on any amount of targets), or 3) to maximize SoD extension with Inevitability on 3 targets (4 with "
        "BitS)." );
    ...
    finish->add_action(
        this, "Eviscerate",
        "if=buff.nights_vengeance.up&(spell_targets.shuriken_storm<2|variable.use_priority_rotation|!talent.secret_"
        "technique.enabled|!cooldown.secret_technique.up)",
        "Eviscerate has highest priority with Night's Vengeance up. Exception is AoE damage when SecTec is ready." );
    finish->add_action(
        this, "Nightblade",
        "if=(!talent.dark_shadow.enabled|!buff.shadow_dance.up)&target.time_to_die-remains>6&remains<tick_time*2",
        "Keep up Nightblade if it is about to run out. Do not use NB during Dance, if talented into Dark Shadow." );
    finish->add_action(
        this, "Nightblade",
        "cycle_targets=1,if=!variable.use_priority_rotation&spell_targets.shuriken_storm>=2&(azerite.nights_vengeance."
        "enabled|!azerite.replicating_shadows.enabled|spell_targets.shuriken_storm-active_dot.nightblade>=2)&!buff."
        "shadow_dance.up&target.time_to_die>=(5+(2*combo_points))&refreshable",
        "Multidotting outside Dance on targets that will live for the duration of Nightblade, refresh during pandemic. "
        "Multidot as long as 2+ targets do not have Nightblade up with Replicating Shadows (unless you have Night's "
        "Vengeance too)." );
    finish->add_action( this, "Nightblade",
                        "if=remains<cooldown.symbols_of_death.remains+10&cooldown.symbols_of_death.remains<=5&target."
                        "time_to_die-remains>cooldown.symbols_of_death.remains+5",
                        "Refresh Nightblade early if it will expire during Symbols. Do that refresh if SoD gets ready "
                        "in the next 5s." );
    ...
    // Builders
    action_priority_list_t* build = get_action_priority_list( "build", "Builders" );
    build->add_action( this, "Shuriken Storm",
                       "if=spell_targets>=2+(talent.gloomblade.enabled&azerite.perforate.rank>=2&position_back)" );
...
action_t* rogue_t::create_action( const std::string& name, const std::string& options_str )
{
  using namespace actions;

  if ( name == "adrenaline_rush" )
    return new adrenaline_rush_t( this, options_str );
  if ( name == "ambush" )
    return new ambush_t( this, options_str );
  if ( name == "apply_poison" )
    return new apply_poison_t( this, options_str );
  if ( name == "auto_attack" )
    return new auto_melee_attack_t( this, options_str );
  if ( name == "backstab" )
    return new backstab_t( this, options_str );
  if ( name == "between_the_eyes" )
    return new between_the_eyes_t( this, options_str );
  if ( name == "blade_flurry" )
    return new blade_flurry_t( this, options_str );
  if ( name == "blade_rush" )
    return new blade_rush_t( this, options_str );
  if ( name == "blindside" )
    return new blindside_t( this, options_str );
  if ( name == "cheap_shot" )
    return new cheap_shot_t( this, options_str );
  if ( name == "crimson_tempest" )
    return new crimson_tempest_t( this, options_str );
  if ( name == "detection" )
    return new detection_t( this, options_str );
  if ( name == "dispatch" )
    return new dispatch_t( this, options_str );
  if ( name == "envenom" )
    return new envenom_t( this, options_str );
  if ( name == "eviscerate" )
    return new eviscerate_t( this, options_str );
  if ( name == "exsanguinate" )
    return new exsanguinate_t( this, options_str );
  if ( name == "fan_of_knives" )
    return new fan_of_knives_t( this, options_str );
  if ( name == "feint" )
    return new feint_t( this, options_str );
  if ( name == "garrote" )
    return new garrote_t( this, options_str );
  if ( name == "gouge" )
    return new gouge_t( this, options_str );
  if ( name == "ghostly_strike" )
    return new ghostly_strike_t( this, options_str );
  if ( name == "gloomblade" )
    return new gloomblade_t( this, options_str );
  if ( name == "kick" )
    return new kick_t( this, options_str );
  if ( name == "kidney_shot" )
    return new kidney_shot_t( this, options_str );
  if ( name == "killing_spree" )
    return new killing_spree_t( this, options_str );
  if ( name == "marked_for_death" )
    return new marked_for_death_t( this, options_str );
  if ( name == "mutilate" )
    return new mutilate_t( this, options_str );
  if ( name == "nightblade" )
    return new nightblade_t( this, options_str );
  if ( name == "pistol_shot" )
    return new pistol_shot_t( this, options_str );
  if ( name == "poisoned_knife" )
    return new poisoned_knife_t( this, options_str );
  if ( name == "roll_the_bones" )
    return new roll_the_bones_t( this, options_str );
  if ( name == "rupture" )
    return new rupture_t( this, options_str );
  if ( name == "secret_technique" )
    return new secret_technique_t( this, options_str );
  if ( name == "shadow_blades" )
    return new shadow_blades_t( this, options_str );
  if ( name == "shadow_dance" )
    return new shadow_dance_t( this, options_str );
  if ( name == "shadowstep" )
    return new shadowstep_t( this, options_str );
  if ( name == "shadowstrike" )
    return new shadowstrike_t( this, options_str );
  if ( name == "shuriken_storm" )
    return new shuriken_storm_t( this, options_str );
  if ( name == "shuriken_tornado" )
    return new shuriken_tornado_t( this, options_str );
  if ( name == "shuriken_toss" )
    return new shuriken_toss_t( this, options_str );
  if ( name == "sinister_strike" )
    return new sinister_strike_t( this, options_str );
  if ( name == "slice_and_dice" )
    return new slice_and_dice_t( this, options_str );
  if ( name == "sprint" )
    return new sprint_t( this, options_str );
  if ( name == "stealth" )
    return new stealth_t( this, options_str );
  if ( name == "symbols_of_death" )
    return new symbols_of_death_t( this, options_str );
  if ( name == "toxic_blade" )
    return new toxic_blade_t( this, options_str );
  if ( name == "vanish" )
    return new vanish_t( this, options_str );
  if ( name == "vendetta" )
    return new vendetta_t( this, options_str );
  if ( name == "cancel_autoattack" )
    return new cancel_autoattack_t( this, options_str );
  if ( name == "swap_weapon" )
    return new weapon_swap_t( this, options_str );
...
std::unique_ptr<expr_t> rogue_t::create_expression( const std::string& name_str )
{
  ...
  else if ( util::str_compare_ci( name_str, "master_assassin_remains" ) )
  {
    return make_fn_expr( name_str, [this]() {
      ...
      {
        timespan_t nominal_master_assassin_duration =
            timespan_t::from_seconds( talent.master_assassin->effectN( 1 ).base_value() );
        timespan_t gcd_remains =
            timespan_t::from_seconds( std::max( ( gcd_ready - sim->current_time() ).total_seconds(), 0.0 ) );
  ...
  else if ( util::str_compare_ci( name_str, "bleeds" ) )
  {
    return make_fn_expr( name_str, [this]() {
      rogue_td_t* tdata = get_target_data( target );
      return tdata->dots.garrote->is_ticking() + tdata->dots.internal_bleeding->is_ticking() +
             tdata->dots.rupture->is_ticking() + tdata->dots.crimson_tempest->is_ticking();
    } );
  }
  else if ( util::str_compare_ci( name_str, "poisoned_bleeds" ) )
  {
    return make_fn_expr( name_str, [this]() {
      ...
      for ( size_t i = 0, actors = sim->target_non_sleeping_list.size(); i < actors; i++ )
      {
        ...
        {
          poisoned_bleeds += tdata->dots.garrote->is_ticking() + tdata->dots.internal_bleeding->is_ticking() +
    ...
    return make_fn_expr( name_str, [this]() {
      ...
      for ( size_t i = 0, actors = sim->target_non_sleeping_list.size(); i < actors; i++ )
      {
        ...
        if ( !tdata->dots.garrote->is_ticking() ||
             !debug_cast<const actions::garrote_state_t*>( tdata->dots.garrote->state )->shrouded_suffocation )
        {
  ...
  if ( split.size() == 2 && util::str_compare_ci( split[ 0 ], "stealthed" ) )
  {
    ...
    {
      return make_fn_expr( split[ 0 ], [this]() { return stealthed( STEALTH_BASIC | STEALTH_ROGUE ); } );
    }
    else if ( util::str_compare_ci( split[ 1 ], "mantle" ) )
    {
      return make_fn_expr( split[ 0 ], [this]() { return stealthed( STEALTH_BASIC ); } );
    }
    else if ( util::str_compare_ci( split[ 1 ], "all" ) )
    {
      return make_fn_expr( split[ 0 ], [this]() { return stealthed(); } );
    }
  }
  // exsanguinated.(garrote|internal_bleeding|rupture|crimson_tempest)
            ...
            ( util::str_compare_ci( split[ 1 ], "garrote" ) ||
              ...
              util::str_compare_ci( split[ 1 ], "rupture" ) || util::str_compare_ci( split[ 1 ], "crimson_tempest" ) ) )
  {
  ...
  else if ( split.size() == 3 && util::str_compare_ci( split[ 0 ], "rtb_list" ) && !split[ 1 ].empty() )
  {
    ...
    const std::array<const buff_t*, 6> rtb_buffs = {{buffs.broadside, buffs.buried_treasure, buffs.grand_melee,
                                                     buffs.ruthless_precision, buffs.skull_and_crossbones,
                                                     buffs.true_bearing}};
  ...
  if ( split.size() == 2 && util::str_compare_ci( split[ 0 ], "time_to_sht" ) )
  {
    return make_fn_expr( split[ 0 ], [this, split]() {
      ...
      if ( main_hand_attack && attack_x > shadow_techniques && attack_x <= 5 )
      {
        ...
        if ( sim->debug )
          sim->out_debug.printf( "Inside the shadowtechniques handler, attack_x = %u, remaining_aa = %u", attack_x,
                                 remaining_aa );
        ...
        if ( sim->debug )
          sim->out_debug.printf( "Main hand next_swing in: %.3f", mh_next_swing.total_seconds() );
        ...
        if ( off_hand_attack )
        {
          ...
          if ( sim->debug )
            sim->out_debug.printf( "Off hand next_swing in: %.3f", oh_next_swing.total_seconds() );
        }
        else
        {
          if ( sim->debug )
            sim->out_debug.printf( "Off hand attack not found, using only main hand timers" );
        }
      ...
      else if ( main_hand_attack == nullptr )
      {
        ...
        if ( sim->debug )
          sim->out_debug.printf( "Main hand attack is required but was not found" );
      }
      else if ( attack_x > 5 )
      {
        return_value = timespan_t::from_seconds( 0.0 );
        if ( sim->debug )
          sim->out_debug.printf( "Invalid value %u for attack_x (must be 5 or less)", attack_x );
      }
      else
      {
        return_value = timespan_t::from_seconds( 0.0 );
        if ( sim->debug )
          sim->out_debug.printf( "attack_x value %u is not greater than shadow techniques count %u, returning %.3f",
                                 attack_x, shadow_techniques, return_value.total_seconds() );
      }
      if ( sim->debug )
        sim->out_debug.printf( "Shadow techniques return value is: %.3f", return_value.total_seconds() );
      return return_value;
    } );
  }
      ...
      return make_fn_expr( name_str, [this, regen] {
        ...
        if ( buffs.blade_rush->check() )
        {
          energy_regen_per_second +=
              ( buffs.blade_rush->data().effectN( 1 ).base_value() / buffs.blade_rush->buff_period.total_seconds() );
        }

        // TODO - Add support for Venomous Vim, Master of Shadows, and potentially also estimated Combat Potency, ShT
        // etc.
        //        Also consider if buffs such as Adrenaline Rush/Lucid should be prorated based on duration for
        //        time_to_max
...
void rogue_t::init_base_stats()
{
  ...
  if ( options.rogue_ready_trigger )
  {
    for ( size_t i = 0; i < sim->player_list.size(); ++i )
    {
      ...
      if ( p->specialization() != ROGUE_ASSASSINATION && p->specialization() != ROGUE_OUTLAW &&
           p->specialization() != ROGUE_SUBTLETY )
      {
    ...
    if ( options.rogue_ready_trigger )
    {
      ...
      // Disabled for now
      // sim -> errorf( "[Rogue] ready_trigger=1 has been enabled. You can disable by adding rogue_ready_trigger=0 to
      // your actor" );
    }
  }
}
...
void rogue_t::init_spells()
{
  ...
  // Shared
  talent.weaponmaster =
      find_talent_spell( "Weaponmaster" );  // Note: this will return a different spell depending on the spec.
...
void rogue_t::init_scaling()
{
  ...
  if ( sim->scaling->scale_stat == STAT_WEAPON_DPS && weapon_data[ WEAPON_MAIN_HAND ].secondary_weapon_data.active() )
  {
...
void rogue_t::create_buffs()
{
  ...
  buffs.shadowstep = make_buff( this, "shadowstep", spec.shadowstep )->set_cooldown( timespan_t::zero() );
  buffs.sprint =
      make_buff( this, "sprint", spell.sprint )->set_cooldown( timespan_t::zero() )->add_invalidate( CACHE_RUN_SPEED );
  ...
  // Roll the bones buffs
  buffs.broadside =
      make_buff( this, "broadside", find_spell( 193356 ) )->add_invalidate( CACHE_PLAYER_DAMAGE_MULTIPLIER );
  ...
  // Assassination
  buffs.elaborate_planning =
      make_buff( this, "elaborate_planning", talent.elaborate_planning->effectN( 1 ).trigger() )
  ...
  buffs.master_assassin =
      make_buff( this, "master_assassin", talent.master_assassin )
          ->set_default_value( spec.master_assassin->effectN( 1 ).percent() )
          ->set_duration( timespan_t::from_seconds( talent.master_assassin->effectN( 1 ).base_value() ) );
  buffs.hidden_blades_driver =
      make_buff( this, "hidden_blades_driver", talent.hidden_blades )
  ...
  // Subtlety
  buffs.master_of_shadows =
      make_buff( this, "master_of_shadows", find_spell( 196980 ) )
  ...
  // Azerite
  buffs.blade_in_the_shadows =
      make_buff( this, "blade_in_the_shadows", find_spell( 279754 ) )
  ...
  buffs.brigands_blitz = make_buff<stat_buff_t>( this, "brigands_blitz", find_spell( 277724 ) )
                             ...
                             ->set_refresh_behavior( buff_refresh_behavior::DURATION );
  buffs.brigands_blitz_driver =
      make_buff( this, "brigands_blitz_driver", find_spell( 277725 ) )
          ->set_trigger_spell( azerite.brigands_blitz.spell_ref().effectN( 1 ).trigger() )
          ->set_quiet( true )
          ->set_tick_callback( [this]( buff_t*, int, const timespan_t& ) { buffs.brigands_blitz->trigger(); } );
  ...
  buffs.double_dose = make_buff( this, "double_dose", find_spell( 273009 ) )->set_quiet( true );
  buffs.keep_your_wits_about_you =
      make_buff( this, "keep_your_wits_about_you", find_spell( 288988 ) )
...
static bool parse_fixed_rtb( sim_t* sim, const std::string& /* name */, const std::string& value )
{
  ...
  {
    sim->errorf( "%s: No valid 'fixed_rtb' buffs given by string '%s'", sim->active_player->name(), value.c_str() );
    return false;
  }
...
static bool parse_fixed_rtb_odds( sim_t* sim, const std::string& /* name */, const std::string& value )
{
  ...
  if ( sum != 100.0 )
  {
    sim->errorf( "Warning: %s: 'fixed_rtb_odds' adding up to %f instead of 100, re-scaling accordingly",
                 sim->active_player->name(), sum );
    for ( size_t i = 0; i < odds.size(); i++ )
    {
...
std::string rogue_t::create_profile( save_e stype )
{
  ...
  if ( weapon_data[ WEAPON_MAIN_HAND ].secondary_weapon_data.active() )
  {
    ...
    if ( sim->save_gear_comments && !weapon_data[ WEAPON_MAIN_HAND ].secondary_weapon_data.encoded_comment().empty() )
    {
      profile_str += "# ";
  ...
  if ( weapon_data[ WEAPON_OFF_HAND ].secondary_weapon_data.active() )
  {
    ...
    if ( sim->save_gear_comments && !weapon_data[ WEAPON_OFF_HAND ].secondary_weapon_data.encoded_comment().empty() )
    {
      profile_str += "# ";
...
void rogue_t::init_items()
{
  ...
  if ( !weapon_data[ WEAPON_MAIN_HAND ].secondary_weapon_data.options_str.empty() )
  {
    ...
    weapon_data[ WEAPON_MAIN_HAND ].item_data[ WEAPON_SECONDARY ] =
        &( weapon_data[ WEAPON_MAIN_HAND ].secondary_weapon_data );
  ...
  if ( !weapon_data[ WEAPON_OFF_HAND ].secondary_weapon_data.options_str.empty() )
  {
    ...
    weapon_data[ WEAPON_OFF_HAND ].item_data[ WEAPON_SECONDARY ] =
        &( weapon_data[ WEAPON_OFF_HAND ].secondary_weapon_data );
...
void rogue_t::init_special_effects()
{
  ...
  {
    for ( size_t i   = 0,
                 end = weapon_data[ WEAPON_MAIN_HAND ].item_data[ WEAPON_SECONDARY ]->parsed.special_effects.size();
          i < end; ++i )
    {
      special_effect_t* effect =
          weapon_data[ WEAPON_MAIN_HAND ].item_data[ WEAPON_SECONDARY ]->parsed.special_effects[ i ];
  ...
  {
    for ( size_t i   = 0,
                 end = weapon_data[ WEAPON_OFF_HAND ].item_data[ WEAPON_SECONDARY ]->parsed.special_effects.size();
          i < end; ++i )
    {
      special_effect_t* effect =
          weapon_data[ WEAPON_OFF_HAND ].item_data[ WEAPON_SECONDARY ]->parsed.special_effects[ i ];
      unique_gear::initialize_special_effect_2( effect );
    }
  }
...
void rogue_t::swap_weapon( weapon_slot_e slot, current_weapon_e to_weapon, bool in_combat )
{
  ...
  if ( sim->debug )
  {
    sim->out_debug.printf( "%s performing weapon swap from %s to %s", name(),
  ...
  if ( !sim->optimize_expressions )
  {
    if ( options.rogue_optimize_expressions )
    {
      ...
      if ( options.rogue_optimize_expressions )
      {
        ...
        // Disabled for now
        // sim -> errorf( "[Rogue] optimize_expressions=1 has been enabled. You can disable by adding
        // rogue_ready_trigger=0 to every rogue actor" );
      }
    }
  }
...
void rogue_t::regen( timespan_t periodicity )
{
  ...
  // their benefits. IMPORTANT NOTE: If anything is updated/added here, rogue_t::create_resource_expression() needs to
  // be updated as well to reflect this
  if ( !resources.is_infinite( RESOURCE_ENERGY ) )
  {
    if ( buffs.adrenaline_rush->up() )
    {
      double energy_regen = periodicity.total_seconds() * resource_regen_per_second( RESOURCE_ENERGY ) *
                            buffs.adrenaline_rush->data().effectN( 1 ).percent();
      resource_gain( RESOURCE_ENERGY, energy_regen, gains.adrenaline_rush );
    }

    // Additional energy gains
    if ( buffs.nothing_personal->up() )
      resource_gain( RESOURCE_ENERGY, buffs.nothing_personal->check_value() * periodicity.total_seconds(),
                     gains.nothing_personal );
  ...
  switch ( specialization() )
  {
    ...
    case ROGUE_SUBTLETY:
    {
      const timespan_t duration =
          this->buffs.shadow_blades->data().duration() * azerite.vision_of_perfection_percentage;
    ...
    {
      const timespan_t duration =
          this->buffs.adrenaline_rush->data().duration() * azerite.vision_of_perfection_percentage;
