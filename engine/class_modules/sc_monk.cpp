  ...
  if ( mh && mh -> type != WEAPON_NONE && weapon_power_mod > 0 )
  {
    ...
    if ( sim->debug )
    {
      sim -> out_debug.printf( "%s main hand weapon damage portion for %s: td=%.3f wd=%.3f bd=%.3f ws=%.3f ap=%.3f",
                               player -> name(), action -> name(), total_dmg, dmg, mh -> bonus_dmg, mh -> swing_time.total_seconds(), ap );
    }
  }
  ...
  if ( oh && oh -> type != WEAPON_NONE && weapon_power_mod > 0 )
  {
    ...
    if ( sim -> debug )
    {
      sim -> out_debug.printf( "%s off-hand weapon damage portion for %s: td=%.3f wd=%.3f bd=%.3f ws=%.3f ap=%.3f",
                               player -> name(), action -> name(), total_dmg, dmg, oh -> bonus_dmg, oh -> swing_time.total_seconds(), ap );
    }
  }
...
public:
    ...
    tiger_power( 0 ), attacks( SEF_MAX )
  {
    ...
    main_hand_weapon.min_dmg = mh_dps * main_hand_weapon.swing_time.total_seconds();
    main_hand_weapon.max_dmg = mh_dps * main_hand_weapon.swing_time.total_seconds();

    if ( dual_wield )
    {
      off_hand_weapon.type = WEAPON_BEAST;
      off_hand_weapon.swing_time = timespan_t::from_seconds( 2.6 );
      off_hand_weapon.min_dmg = mh_dps * off_hand_weapon.swing_time.total_seconds();
      off_hand_weapon.max_dmg = mh_dps * off_hand_weapon.swing_time.total_seconds();
    }
...
void monk_t::apl_combat_brewmaster()
{
  ...
  aoe -> add_talent( this, "Zen Sphere", "cycle_targets=1,if=talent.zen_sphere.enabled&!dot.zen_sphere.ticking&(energy+(energy.regen*gcd))<100" );
