...
struct storm_earth_and_fire_pet_t : public pet_t
{
  ...
  struct sef_rising_sun_kick_t : public sef_melee_attack_t
  {
    ...
    {
      if ( player -> o() -> talent.tornado_kicks )
    ...
    void impact( action_state_t* state ) override
    {
      ...
      if ( result_is_hit( state -> result ) )
      {
        ...
        if ( o() -> talent.tornado_kicks )
        {
          double raw = state -> result_raw * o() -> talent.tornado_kicks -> effectN( 1 ).percent();
  ...
  void consume_resource() override
  {
    ...
    if ( current_resource() == RESOURCE_CHI )
    {
      if ( ab::cost() > 0 )
      {
        ...
        if ( p() -> legendary.drinking_horn_cover && p() -> level() < 120 )
        {
...
// Rising Sun Kick Tornado Kick Windwalker Talent ===========================
  ...
  virtual void impact( action_state_t* s ) override
  {
    ...
    if ( result_is_hit( s -> result ) )
    {
      ...
      if ( p() -> specialization() == MONK_WINDWALKER )
      {
        ...
        if ( p() -> talent.tornado_kicks )
        {
          double raw = s -> result_raw * p() -> talent.tornado_kicks -> effectN( 1 ).percent();
...
struct fists_of_fury_t: public monk_melee_attack_t
{
  ...
  virtual double cost() const override
  {
    ...
    if ( p() -> legendary.katsuos_eclipse && !p() -> buff.serenity -> up() && p() -> level() < 120 )
  ...
  virtual void consume_resource() override
  {
    ...
    if ( p() -> buff.serenity -> up() )
    {
      if ( p() -> legendary.katsuos_eclipse && p() -> level() < 120 )
...
struct keg_smash_t: public monk_melee_attack_t
{
  ...
  virtual double action_multiplier() const override
  {
    ...
    if ( p() -> legendary.stormstouts_last_gasp && p() -> level() < 120 )
  ...
  virtual void execute() override
  {
    ...
    if ( p() -> legendary.salsalabims_lost_tunic != nullptr && p() -> level() < 120 )
...
struct touch_of_death_t: public monk_spell_t
{
  ...
  double calculate_tick_amount( action_state_t* s, double /*dot_multiplier*/ ) const override
  {
    ...
    if ( p() -> legendary.hidden_masters_forbidden_touch && p() -> level() < 120 )
...
struct touch_of_karma_t: public monk_melee_attack_t
{
  ...
  void execute() override
  {
    ...
    if ( pct_health > 0 )
    {
      ...
      if ( p() -> legendary.cenedril_reflector_of_hatred && p() -> level() < 120 )
...
struct flying_serpent_kick_t: public monk_melee_attack_t
{
  ...
  void execute() override
  {
    ...
    if ( p() -> legendary.sephuzs_secret != spell_data_t::not_found() && execute_state -> target -> type == ENEMY_ADD && p() -> level() < 120 )
    {
...
struct crackling_jade_lightning_t: public monk_spell_t
{
  ...
  virtual double cost_per_tick( resource_e resource ) const override
  {
    ...
    if ( p() -> buff.the_emperors_capacitor -> up() && resource == RESOURCE_ENERGY && p() -> level() < 120 )
  ...
  virtual double cost() const override
  {
    ...
    if ( p() -> buff.the_emperors_capacitor -> up() && p() -> level() < 120 )
      c *= 1 + ( p() -> buff.the_emperors_capacitor -> current_stack * p() -> passives.the_emperors_capacitor -> effectN( 2 ).percent() );
...
void monk_t::apl_combat_windwalker()
{
  ...
  def -> add_action( "call_action_list,name=sef,if=!talent.serenity.enabled&equipped.drinking_horn_cover&(cooldown.fist_of_the_white_tiger.remains<=18&cooldown.fists_of_fury.remains<=12&chi>=3&cooldown.rising_sun_kick.remains<=1|target.time_to_die<=25|cooldown.touch_of_death.remains>112)&cooldown.storm_earth_and_fire.charges=1" );
  def -> add_action( "call_action_list,name=sef,if=!talent.serenity.enabled&!equipped.drinking_horn_cover&(cooldown.fist_of_the_white_tiger.remains<=14&cooldown.fists_of_fury.remains<=6&chi>=3&cooldown.rising_sun_kick.remains<=1|target.time_to_die<=15|cooldown.touch_of_death.remains>112)&cooldown.storm_earth_and_fire.charges=1" );
  ...
  st -> add_talent( this, "Energizing Elixir", "if=!prev_gcd.1.tiger_palm&chi<=1&(cooldown.rising_sun_kick.remains=0|(talent.fist_of_the_white_tiger.enabled&cooldown.fist_of_the_white_tiger.remains=0)|energy<50)" );
