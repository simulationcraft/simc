...
void hunter_t::apl_surv()
{
  ...
  moknathal -> add_action( "flanking_strike,if=focus>75-buff.moknathal_tactics.remains*focus.regen&(cooldown.mongoose_bite.charges<=1&buff.aspect_of_the_eagle.remains>=gcd|cooldown.mongoose_bite.charges<=1&buff.aspect_of_the_eagle.down)" );
  moknathal -> add_action( "lacerate,if=(focus>60-buff.moknathal_tactics.remains*focus.regen&buff.mongoose_fury.duration>=gcd&refreshable&cooldown.mongoose_bite.charges=0&buff.mongoose_fury.stack<3)|(focus>65-buff.moknathal_tactics.remains*focus.regen&buff.mongoose_fury.down&cooldown.mongoose_bite.charges<3&refreshable)" );
