...
std::unique_ptr<expr_t> hunter_t::create_action_expression ( action_t& action, util::string_view expression_str )
{
  ...
  {
    const bool talented   = talents.black_arrow.ok();
    const double low_pct  = talents.black_arrow->effectN( 2 ).base_value();
    const double high_pct = talents.black_arrow->effectN( 3 ).base_value();
