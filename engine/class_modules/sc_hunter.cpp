...
public:
  ...
  hunter_t( sim_t* sim, const std::string& name, race_e r = RACE_NONE ) :
    ...
    gains( gains_t() ),
    procs( procs_t() ),
    ppm_tier15_2pc_melee( "tier15_2pc", *this, std::numeric_limits<double>::min(), RPPM_HASTE ),
    ppm_tier15_4pc_melee( "tier15_4pc", *this, std::numeric_limits<double>::min(), RPPM_HASTE ),
    talents( talents_t() ),
    specs( specs_t() ),
    glyphs( glyphs_t() ),
