...
namespace { // UNNAMED NAMESPACE
	...
	public:
		...
		void trigger_enhance_lashing_flames(const action_state_t* state);
		...
		actor_target_data_t(target, p)
	{
		...
		debuff.lashing_flames = buff_creator_t(*this, "lashing_flames", p->find_spell(238142))
			.cd(timespan_t::zero());
	}
	...
	public:
		...
		void trigger_maelstrom_weapon(const action_state_t* source_state, double amount = 0)
		{
			...
			if (p()->artifact.winds_of_change.rank())
			{
				if (source_state->action->name_str == "windfury_attack" || source_state->action->name_str == "windfury_attack_oh")
				{
					amount += p()->artifact.winds_of_change.value();
				}
			}
	...
	struct shaman_spell_t : public shaman_spell_base_t<spell_t>
	{
			...
			base_t(token, p, s), overload(nullptr)
		{
			...
			{
				crit_bonus_multiplier *= 1.0 + p->spec.elemental_fury->effectN(1).percent() + p->artifact.elemental_destabilization.percent();
			}
	...
	struct lashing_flames_debuff : public shaman_spell_t
	{
		lashing_flames_debuff(shaman_t* p) :
			shaman_spell_t("lashing flames", p, p -> find_spell(240842))
		{
			callbacks = may_crit = false; //handled in flametongue_attack
			dot_max_stack = data().max_stacks();
		}
	};
	...
	struct lava_lash_t : public shaman_attack_t
	{
		...
		double action_multiplier() const override
		{
			...
			{
				m *= 1.0 + (td(target)->debuff.lashing_flames->stack() * p()->artifact.lashing_flames.percent()); //Modify hit
			}
		...
		void impact(action_state_t* state) override
		{
			...
			if (result_is_hit(state->result))
			{
				td(target)->debuff.lashing_flames->decrement(td(target)->debuff.lashing_flames->current_stack); // get rid of all the lashing flames stacks on the target.
			}
	...
	struct flametongue_t : public shaman_spell_t
	{
		...
		void execute() override
		{
			...
			if (p()->artifact.lashing_flames.rank())
			{
				td(target)->debuff.lashing_flames->increment(1); //TODO-LF: Is this even the right place for this?
			}
		}
	};
	...
	struct totem_pulse_action_t : public spell_t
	{
			...
			spell_t(token, p, s), hasted_pulse(false), pulse_multiplier(1.0), totem(p)
		{
			...
			crit_bonus_multiplier *= 1.0 + totem->o()->spec.elemental_fury->effectN(1).percent() + totem->o()->artifact.elemental_destabilization.percent();
		}
	...
	void shaman_t::trigger_enhance_lashing_flames(const action_state_t* state) 
	{
		//TODO-LF: apply lashing flames debuff to target struck. Is this even the right place for this?
	}
	...
	void shaman_t::init_action_list_elemental()
	{
		...
		action_priority_list_t* def = get_action_priority_list("default");
		action_priority_list_t* single_lr = get_action_priority_list("single_lr", "Single Target Action Priority List for Lightning Rod Spec");
		action_priority_list_t* single_if = get_action_priority_list("single_if", "Single Target Action Priority List for Ice Fury Spec");
