...
void shaman_t::init_action_list_elemental()
{
  ...
  if (options.rotation == ROTATION_SIMPLE || options.rotation == ROTATION_STANDARD) {
    ...
    aoe->add_action(
        "lava_burst,target_if=dot.flame_shock.remains,if=cooldown_react&buff.lava_surge.up&(talent.master_of_the_elements.enabled&talent.deeply_rooted_elements.enabled)&!buff.master_of_the_elements.up&enemies>=6",
...
void shaman_t::init_action_list_enhancement()
{
  ...
  //_Use_items
    ...
    single->add_action( "ice_strike,if=buff.doom_winds_talent.up" );
    single->add_action( "sundering,if=buff.doom_winds_talent.up" );
