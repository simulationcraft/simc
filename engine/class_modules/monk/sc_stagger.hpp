...
struct stagger_report_t : public player_report_extension_t
{
...
private:
  // stagger_p_t* stagger;
  virtual const stagger_level_t& get_level( size_t index );
...
public:
  ...
  // virtual void assess_damage_imminent_pre_absorb( school_e school, result_amount_type damage_type, action_state_t *state ) override;
  ...
  stagger_t( Args&&... args )
    : base_actor_t( std::forward<Args>( args )... )
  {
    static_assert( std::is_base_of_v<player_t, base_actor_t>, "stagger_t<base_actor_t, ..> base_actor is not derived from player_t (should be actor base type)" );
    static_assert( std::is_base_of_v<player_t, derived_actor_t>, "stagger_t<.., derived_actor_t> derived_actor is not derived from player_t (should be actor derived type)" );

    this->mixin_reports.push_back( std::make_unique<stagger_report_t>( this ) );
  }
};
