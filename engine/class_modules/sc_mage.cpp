...
struct ignite_spread_event_t : public event_t
{
  ...
  static bool ignite_compare ( dot_t* a, dot_t* b )
  {
    ...
    if ( lv == rv )
    {
      timespan_t lr = a -> remains();
      timespan_t rr = b -> remains();
      if ( lr == rr )
      {
        return a < b;
      }
...
void mage_t::apl_precombat()
{
  ...
  if ( specialization() == MAGE_ARCANE )
  {
    ...
    precombat -> add_action( "variable,name=bs_rotation,op=set,value=1,if=azerite.brain_storm.rank>=2&talent.rune_of_power.enabled", "We change the rotation to burn AFTER Evocation instead of before when we have enough brain storm traits." );
  }
  // Snapshot Stats
...
void mage_t::apl_arcane()
{
  ...
  burn -> add_action( "stop_burn_phase,if=burn_phase&variable.bs_rotation=1&buff.arcane_power.down&cooldown.arcane_power.remains>0&buff.arcane_power.down&target.time_to_die>variable.average_burn_length&burn_phase_duration>0", "When running the Brain Storm rotation, we instead end the Burn Phase when Arcane Power ends. " );
  burn -> add_talent( this, "Charged Up", "if=buff.arcane_charge.stack<=1" );
  burn -> add_talent( this, "Rune of Power", "if=variable.bs_rotation=1&!buff.arcane_power.up&mana.pct>=50&(buff.arcane_charge.stack=buff.arcane_charge.max_stack)&(full_recharge_time<=execute_time|full_recharge_time<=cooldown.arcane_power.remains|target.time_to_die<=cooldown.arcane_power.remains)", "During Brain Storm rotation, we can use 1 RoP before Arcane Power, as long as it means we will still have one for Arcane Power." );
  ...
  conserve -> add_action( this, "Arcane Barrage", "if=((buff.arcane_charge.stack=buff.arcane_charge.max_stack)&((mana.pct<=variable.conserve_mana&variable.bs_rotation=0|mana.pct<=variable.conserve_mana-30&variable.bs_rotation=1)|(cooldown.arcane_power.remains>cooldown.rune_of_power.full_recharge_time&mana.pct<=variable.conserve_mana+25))|(talent.arcane_orb.enabled&cooldown.arcane_orb.remains<=gcd&cooldown.arcane_power.remains>10))|mana.pct<=(variable.conserve_mana-10)&variable.bs_rotation=0|mana.pct<=(variable.conserve_mana-50)&variable.bs_rotation=1", "During conserve, we still just want to continue not dropping charges as long as possible.So keep 'burning' as long as possible (aka conserve_mana threshhold) and then swap to a 4x AB->Abarr conserve rotation. If we do not have 4 AC, we can dip slightly lower to get a 4th AC. We also sustain at a higher mana percentage when we plan to use a Rune of Power during conserve phase, so we can burn during the Rune of Power." );
