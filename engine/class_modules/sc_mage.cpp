...
void mage_t::apl_fire()
{
  ...
  default_list -> add_talent( this, "Rune of Power",
                              "if=buff.rune_of_power.remains<action.fireball.execute_time+spell_gcd&!(buff.heating_up.up&action.fireball.in_flight)",
  ...
  crystal_sequence -> add_action( this, "Pyroblast",
                                  "if=execute_time=spell_gcd&pet.prismatic_crystal.remains<spell_gcd+travel_time&pet.prismatic_crystal.remains>travel_time",
                                  "Use pyros before PC's expiration" );
  crystal_sequence -> add_action( "call_action_list,name=single_target" );


  init_combust -> add_action( "start_pyro_chain,if=talent.meteor.enabled&cooldown.meteor.up&((cooldown.combustion.remains<spell_gcd*3&buff.pyroblast.up&(buff.heating_up.up^action.fireball.in_flight))|(buff.pyromaniac.up&(cooldown.combustion.remains<ceil(buff.pyromaniac.remains%spell_gcd)*spell_gcd)))",
                              ...
                              "# Meteor Combustion" );
  init_combust -> add_action( "start_pyro_chain,if=talent.prismatic_crystal.enabled&cooldown.prismatic_crystal.up&((cooldown.combustion.remains<spell_gcd*2&buff.pyroblast.up&(buff.heating_up.up^action.fireball.in_flight))|(buff.pyromaniac.up&(cooldown.combustion.remains<ceil(buff.pyromaniac.remains%spell_gcd)*spell_gcd)))",
                              "Prismatic Crystal Combustion" );
  init_combust -> add_action( "start_pyro_chain,if=talent.prismatic_crystal.enabled&!glyph.combustion.enabled&cooldown.prismatic_crystal.remains>20&((cooldown.combustion.remains<spell_gcd*2&buff.pyroblast.up&buff.heating_up.up&action.fireball.in_flight)|(buff.pyromaniac.up&(cooldown.combustion.remains<ceil(buff.pyromaniac.remains%spell_gcd)*spell_gcd)))" );
  init_combust -> add_action( "start_pyro_chain,if=!talent.prismatic_crystal.enabled&!talent.meteor.enabled&((cooldown.combustion.remains<spell_gcd*4&buff.pyroblast.up&buff.heating_up.up&action.fireball.in_flight)|(buff.pyromaniac.up&cooldown.combustion.remains<ceil(buff.pyromaniac.remains%spell_gcd)*(spell_gcd+talent.kindling.enabled)))",
  ...
  combust_sequence -> add_action( this, "Inferno Blast",
                                  "if=talent.meteor.enabled&cooldown.meteor.duration-cooldown.meteor.remains<spell_gcd*3",
  ...
  living_bomb -> add_talent( this, "Living Bomb",
                             "cycle_targets=1,if=target!=prismatic_crystal&(active_dot.living_bomb=0|(ticking&active_dot.living_bomb=1))&(((!talent.incanters_flow.enabled|incanters_flow_dir<0|buff.incanters_flow.stack=5)&remains<3.6)|((incanters_flow_dir>0|buff.incanters_flow.stack=1)&remains<spell_gcd))&target.time_to_die>remains+12" );
  ...
  single_target -> add_action( this, "Pyroblast",
                               "if=buff.pyroblast.up&buff.potent_flames.up&buff.potent_flames.remains<spell_gcd" );
  single_target -> add_action( this, "Pyroblast",
...
void mage_t::apl_frost()
{
  ...
  default_list -> add_talent( this, "Rune of Power", "if=(cooldown.icy_veins.remains<spell_gcd&buff.rune_of_power.remains<20)|(cooldown.prismatic_crystal.remains<spell_gcd&buff.rune_of_power.remains<10)" );
  default_list -> add_action( "call_action_list,name=cooldowns,if=time_to_die<24" );
  default_list -> add_action( "call_action_list,name=crystal_sequence,if=talent.prismatic_crystal.enabled&(cooldown.prismatic_crystal.remains<=spell_gcd|pet.prismatic_crystal.active)" );
  default_list -> add_action( "call_action_list,name=aoe,if=active_enemies>=5" );
  default_list -> add_action( "call_action_list,name=single_target" );
  ...
  aoe -> add_talent( this, "Frost Bomb",
                     "if=remains<action.ice_lance.travel_time&(cooldown.frozen_orb.remains<spell_gcd|buff.fingers_of_frost.react=2)" );
  ...
  single_target -> add_talent( this, "Frost Bomb",
                               "if=!talent.prismatic_crystal.enabled&cooldown.frozen_orb.remains<spell_gcd&debuff.frost_bomb.remains<10",
  ...
  single_target -> add_talent( this, "Frost Bomb",
                               "if=remains<action.ice_lance.travel_time&(buff.fingers_of_frost.react=2|(buff.fingers_of_frost.react&(talent.thermal_void.enabled|buff.fingers_of_frost.remains<spell_gcd*2)))",
...
expr_t* mage_t::create_expression( action_t* a, const std::string& name_str )
{
  ...
  if ( util::str_compare_ci( name_str, "spell_gcd" ) )
  {
    struct spell_gcd_expr_t : public mage_expr_t
    {
      spell_gcd_expr_t( mage_t& m ) : mage_expr_t( "spell_gcd", m )
      { }

      double evaluate()
      {
        return std::max( 1.5 * mage.cache.spell_haste(), 1.0 );
      }
    };

    return new spell_gcd_expr_t( *this );
  }
