...
void mage_t::apl_precombat()
{
  ...
  if ( specialization() == MAGE_ARCANE )
    ...
    precombat -> add_action( "variable,name=conserve_mana,op=set,value=35,if=talent.overpowered.enabled" ) ;
...
void mage_t::apl_arcane()
{
  ...
  conserve -> add_action( this, "Arcane Explosion", "if=active_enemies>=3&(mana.pct>=variable.conserve_mana|buff.arcane_charge.stack=3)", "Keep 'burning' in aoe situations until conserve pct. After that only cast AE with 3 Arcane charges, since it's almost equal mana cost to a 3 stack AB anyway. At that point AoE rotation will be AB x3 -> AE -> Abarr" );
