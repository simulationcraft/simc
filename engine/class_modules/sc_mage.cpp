...
{
  static const snapshot_state_e STATE_FROZEN = STATE_USER_1;
...
public:
  ...
  {
    const mage_td_t* t = td( s -> target );

    return ( t -> debuffs.winters_chill -> check() )
        || ( t -> debuffs.frozen -> check() );
  }

  virtual void snapshot_internal( action_state_t* s, unsigned flags, dmg_e rt ) override
  {
    auto mss = debug_cast<mage_spell_state_t*>( s );
    if ( flags & STATE_FROZEN )
      mss -> frozen = frozen( mss );
...
double mage_spell_state_t::composite_crit_chance() const
{
  ...
  if ( frozen )
  {
    const mage_t* p = debug_cast<const mage_spell_t*>( action ) -> p();
