...
struct starfire_t : public druid_spell_t
{
  ...
  virtual void impact( action_state_t* s )
  {
    ...
    if ( p() -> talent.balance_of_power && result_is_hit( s -> result ) )
      td( s -> target ) -> dots.moonfire -> extend_duration( timespan_t::from_seconds( p() -> talent.balance_of_power -> effectN( 1 ).base_value() ) );
  }
};
...
struct wrath_t : public druid_spell_t
{
  ...
  virtual void impact( action_state_t* s )
  {
    ...
    if ( p() -> talent.balance_of_power && result_is_hit( s -> result ) )
      td( s -> target ) -> dots.sunfire -> extend_duration( timespan_t::from_seconds( p() -> talent.balance_of_power -> effectN( 2 ).base_value() ) );
  }
};
