...
struct stalwart_guardian_t : public absorb_t
{
  ...
  {
    stalwart_guardian_reflect_t( druid_t* p ) : attack_t( "stalwart_guardian_reflect", p, p->find_spell( 185321 ) )
    {
...
struct fey_moonwing_t : public pet_t
{
  ...
  {
    fey_missile_t( fey_moonwing_t* player ) : spell_t( "fey_missile", player, player->find_spell( 188046 ) )
    {
  ...
  fey_moonwing_t( sim_t* sim, druid_t* owner ) : pet_t( sim, owner, "fey_moonwing", true /*GUARDIAN*/, true )
  {
...
public:
  druid_buff_t( druid_t& p, const std::string& name, const spell_data_t* s = spell_data_t::nil(),
                const item_t* item = nullptr )
    : BuffBase( &p, name, s, item )
  {
  }
  druid_buff_t( druid_td_t& td, const std::string& name, const spell_data_t* s = spell_data_t::nil(),
                const item_t* item = nullptr )
...
{
  berserk_buff_t( druid_t& p ) : berserk_buff_base_t( p, "berserk", p.find_spell( 106951 ) )
  {
...
{
  incarnation_moonkin_buff_t( druid_t& p ) : base_t( p, "incarnation_chosen_of_elune", p.talent.incarnation_moonkin )
  {
...
{
  celestial_alignment_buff_t( druid_t& p ) : base_t( p, "celestial_alignment", p.spec.celestial_alignment )
  {
...
{
  moonkin_form_t( druid_t& p ) : base_t( p, "moonkin_form", p.find_affinity_spell( "Moonkin Form" ) )
  {
    add_invalidate( CACHE_PLAYER_DAMAGE_MULTIPLIER );
    add_invalidate( CACHE_ARMOR );
    set_chance( 1.0 );
...
{
  moonkin_form_affinity_t( druid_t& p ) : base_t( p, "moonkin_form_affinity", p.spec.moonkin_form_affinity )
  {
    add_invalidate( CACHE_PLAYER_DAMAGE_MULTIPLIER );
    add_invalidate( CACHE_ARMOR );
    set_chance( 1.0 );
...
struct druid_action_t : public Base
{
  ...
  bool may_autounshift;  // Allows a spell that may be cast in NO_FORM but not in current form to be cast by exiting
                         // form.
  unsigned autoshift;    // Allows a spell that may not be cast in the current form to be cast by automatically changing
...
public:
  ...
  void trigger_gushing_wound( player_t* t, double dmg )
  {
    ...
    residual_action::trigger( p()->active.gushing_wound,  // ignite spell
                              t,                          // target
  ...
  bool verify_actor_spec() const override
  {
    ...
    {
      p()->sim->out_log.printf( "%s failed verification", ab::name() );
    }
#endif
  ...
  expr_t* create_expression( const std::string& name_str )
  {
    ...
    if ( splits[ 0 ] == "dot" && splits[ 2 ] == "ticks_gained_on_refresh" )
    {
...
public:
  ...
  druid_attack_t( const std::string& n, druid_t* player, const spell_data_t* s = spell_data_t::nil() )
    : ab( n, player, s ),
...
public:
  ...
  druid_spell_base_t( const std::string& n, druid_t* player, const spell_data_t* s = spell_data_t::nil() )
    : ab( n, player, s ), cat_form_gcd( ab::data().affected_by( player->spec.cat_form->effectN( 4 ) ) )
  {
  }
...
{
  proc_sephuz_t( druid_t* p, const std::string& options_str ) : action_t( ACTION_USE, "proc_sephuz", p )
  {
...
public:
  ...
  druid_spell_t( const std::string& token, druid_t* p, const spell_data_t* s = spell_data_t::nil(),
    ...
    : base_t( token, p, s ), warrior_of_elune( data().affected_by( p->talent.warrior_of_elune->effectN( 1 ) ) )
  {
...
{
  shooting_stars_t( druid_t* player ) : druid_spell_t( "shooting_stars", player, player->find_spell( 202497 ) )
  {
    background = true;
  }
};
...
struct moonfire_t : public druid_spell_t
{
  ...
  public:
    ...
    moonfire_damage_t( druid_t* p, const std::string& n ) : druid_spell_t( n, p, p->find_spell( 164812 ) )
    {
...
namespace heals
{
  struct druid_heal_t : public druid_spell_base_t<heal_t>
  {
    ...
    druid_heal_t( const std::string& token, druid_t* p, const spell_data_t* s = spell_data_t::nil(),
                  const std::string& options_str = std::string() )
      : base_t( token, p, s ), living_seed( nullptr ), target_self( 0 )
    {
...
{
  caster_attack_t( const std::string& token, druid_t* p, const spell_data_t* s = spell_data_t::nil(),
                   const std::string& options = std::string() )
    : base_t( token, p, s )
  {
    parse_options( options );
...
{
  druid_melee_t( druid_t* p ) : caster_attack_t( "melee", p )
  {
    school     = SCHOOL_PHYSICAL;
...
public:
      ...
      tigers_fury( data().affected_by( p->spec.tigers_fury->effectN( 1 ) ) )
  {
    ...
    if ( p->specialization() == DRUID_FERAL && p->talent.soul_of_the_forest->ok() &&
         ( data().affected_by( p->talent.soul_of_the_forest->effectN( 2 ) ) |
           data().affected_by( p->talent.soul_of_the_forest->effectN( 3 ) ) ) )
    {
  ...
  virtual void consume_resource() override
  {
    ...
    if ( consumes_combo_points && hit_any_target )
    {
      ...
      {
        p()->buff.tigers_fury->extend_duration(
            p(), timespan_t::from_seconds( p()->legendary.behemoth_headdress * consumed ) );
      }
      ...
      if ( sim->log )
      {
        sim->out_log.printf( "%s consumes %d %s for %s (0)", player->name(), consumed,
  ...
  void tick( dot_t* d ) override
  {
    ...
    {
      p()->resource_gain( RESOURCE_ENERGY, p()->sets->set( DRUID_FERAL, T17, B2 )->effectN( 1 ).base_value(),
  ...
  void trigger_wildshapers_clutch( action_state_t* s )
  {
    ...
    if ( !dbc::is_school( school, SCHOOL_PHYSICAL ) &&
         s->action->id != 210705 )  // bleeds only TODO(feral): Refactor this.
      return;
...
struct ferocious_bite_t : public cat_attack_t
{
  ...
  void ApexPredatorResource()
  {
    ...
    if ( p()->talent.soul_of_the_forest->ok() && p()->specialization() == DRUID_FERAL )
    {
      p()->resource_gain( RESOURCE_ENERGY, 5 * p()->talent.soul_of_the_forest->effectN( 1 ).resource( RESOURCE_ENERGY ),
                          p()->gain.soul_of_the_forest );
    }
...
struct rake_t : public cat_attack_t
{
  ...
  {
    rake_bleed_t( druid_t* p ) : cat_attack_t( "rake_bleed", p, p->find_spell( 155722 ) )
    {
      background = dual = true;
  ...
  rake_t( druid_t* p, const std::string& options_str ) : cat_attack_t( "rake", p, p->find_affinity_spell( "Rake" ) )
  {
    parse_options( options_str );
...
struct rip_t : public cat_attack_t
{
  ...
  {
    bloody_gash_t( druid_t* p ) : cat_attack_t( "bloody_gash", p, p->spec.bloody_gash )
    {
      background = dual = proc = true;
...
struct shred_t : public cat_attack_t
{
    ...
    : cat_attack_t( "shred", p, p->find_class_spell( "Shred" ), options_str )
  {
    ...
    energize_amount += p->spec.feral->effectN( 3 ).base_value() + p->talent.feral_affinity->effectN( 8 ).base_value();
  }
...
struct thrash_bear_t : public bear_attack_t
{
  ...
  thrash_bear_t( druid_t* p, const std::string& options_str )
    : bear_attack_t( "thrash_bear", p, p->find_spell( 77758 ), options_str ), blood_frenzy_amount( 0 )
  {
    aoe                    = -1;
    spell_power_mod.direct = 0;
    gore                   = true;
...
struct regrowth_t : public druid_heal_t
{
  ...
  timespan_t execute_time() const override
  {
    ...
    et *= 1.0 + p()->buff.power_of_elune->current_stack * p()->buff.power_of_elune->data().effectN( 2 ).percent();
  ...
  double action_multiplier() const override
  {
    ...
    am *= 1.0 + p()->buff.power_of_elune->current_stack * p()->buff.power_of_elune->data().effectN( 1 ).percent();
...
struct swiftmend_t : public druid_heal_t
{
  ...
  {
    swiftmend_aoe_heal_t( druid_t* p, const spell_data_t* s ) : druid_heal_t( "swiftmend_aoe", p, s )
    {
      aoe            = 3;
      background     = true;
  ...
  virtual bool ready() override
  {
    ...
    if ( !( td( t )->dots.regrowth->is_ticking() || td( t )->dots.rejuvenation->is_ticking() ) )
      return false;
...
{
  streaking_stars_t( druid_t* p ) : druid_spell_t( "streaking_stars", p, p->find_spell( 272873 ) )
  {
    background  = 1;
...
struct fury_of_elune_t : public druid_spell_t
{
  ...
  {
    fury_of_elune_tick_t( const std::string& n, druid_t* p, const spell_data_t* s ) : druid_spell_t( n, p, s )
    {
...
struct swipe_proxy_t : public druid_spell_t
{
  ...
  swipe_proxy_t( druid_t* p, const std::string& options_str )
    : druid_spell_t( "swipe", p, spell_data_t::nil(), options_str )
  {
...
struct incarnation_t : public druid_spell_t
{
  ...
  incarnation_t( druid_t* p, const std::string& options_str )
    : druid_spell_t(
          "incarnation", p,
          p->specialization() == DRUID_BALANCE
              ? p->talent.incarnation_moonkin
              : p->specialization() == DRUID_FERAL
                    ? p->talent.incarnation_cat
                    : p->specialization() == DRUID_GUARDIAN
                          ? p->talent.incarnation_bear
                          : p->specialization() == DRUID_RESTORATION ? p->talent.incarnation_tree : spell_data_t::nil(),
          options_str )
  {
...
struct lunar_strike_t : public druid_spell_t
{
  lunar_strike_t( druid_t* player, const std::string& options_str )
    : druid_spell_t( "lunar_strike", player,
                     player->talent.balance_affinity->ok() ? player->find_spell( 197628 )
                                                           : player->find_affinity_spell( "Lunar Strike" ),
                     options_str )
  {
    aoe = -1;
    base_aoe_multiplier =
...
struct sunfire_t : public druid_spell_t
{
  struct sunfire_damage_t : public druid_spell_t
  {
    ...
    sunfire_damage_t( druid_t* p ) : druid_spell_t( "sunfire_dmg", p, p->find_spell( 164815 ) ), sunfire_action_id( 0 )
    {
      if ( p->talent.shooting_stars->ok() && !p->active.shooting_stars )
      {
        p->active.shooting_stars = new shooting_stars_t( p );
...
{
  bool empowered;
  solar_wrath_state_t( action_t* action, player_t* target ) : action_state_t( action, target ), empowered( false )
  {
  }
...
{
  solar_empowerment_t( druid_t* p ) : druid_spell_t( "solar_empowerment", p, p->find_spell( 164545 ) )
  {
    background = true;
    may_crit   = false;
    aoe        = -1;
...
struct solar_wrath_t : public druid_spell_t
{
  ...
  void execute() override
  {
    ...
    {
      p()->cooldown.celestial_alignment->adjust( -p()->sets->set( DRUID_BALANCE, T17, B4 )->effectN( 1 ).time_value() );
    }
...
struct starfall_t : public druid_spell_t
{
  ...
  {
    starfall_tick_t( const std::string& n, druid_t* p, const spell_data_t* s ) : druid_spell_t( n, p, s )
    {
      aoe        = -1;
...
{
  starshards_t( druid_t* player ) : starfall_t( player, "" )
  {
    background = true;
    target     = sim->target;
    radius     = 40;
...
struct ailuro_pouncers_event_t : public event_t
{
  ...
  ailuro_pouncers_event_t( druid_t* p, timespan_t set_delay = timespan_t::zero() )
    : event_t( *p, set_delay > timespan_t::zero() ? set_delay : p->legendary.ailuro_pouncers ), druid( p )
  {
  }
    ...
    range::for_each( sim->actor_list, [this]( player_t* target ) -> void {
      ...
      target->callbacks_on_demise.push_back( [this]( player_t* target ) -> void {
        ...
        if ( get_target_data( target )->dots.thrash_cat->is_ticking() ||
             get_target_data( target )->dots.rip->is_ticking() || get_target_data( target )->dots.rake->is_ticking() )
        {
          if ( !p->cooldown.tigers_fury->down() )
            p->proc.predator_wasted->occur();
...
action_t* druid_t::create_action( const std::string& name, const std::string& options_str )
{
  ...
  if ( name == "barkskin" )
    return new barkskin_t( this, options_str );
  if ( name == "berserk" )
    return new berserk_t( this, options_str );
  if ( name == "bear_form" )
    return new spells::bear_form_t( this, options_str );
  if ( name == "brutal_slash" )
    return new brutal_slash_t( this, options_str );
  if ( name == "bristling_fur" )
    return new bristling_fur_t( this, options_str );
  if ( name == "cat_form" )
    return new spells::cat_form_t( this, options_str );
  if ( name == "celestial_alignment" || name == "ca" )
    return new celestial_alignment_t( this, options_str );
  if ( name == "cenarion_ward" )
    return new cenarion_ward_t( this, options_str );
  if ( name == "dash" )
    return new dash_t( this, options_str );
  if ( name == "displacer_beast" )
    return new displacer_beast_t( this, options_str );
  if ( name == "feral_frenzy" )
    return new feral_frenzy_driver_t( this, options_str );
  if ( name == "ferocious_bite" )
    return new ferocious_bite_t( this, options_str );
  if ( name == "force_of_nature" )
    return new force_of_nature_t( this, options_str );
  if ( name == "frenzied_regeneration" )
    return new frenzied_regeneration_t( this, options_str );
  if ( name == "full_moon" )
    return new full_moon_t( this, options_str );
  if ( name == "fury_of_elune" )
    return new fury_of_elune_t( this, options_str );
  if ( name == "growl" )
    return new growl_t( this, options_str );
  if ( name == "half_moon" )
    return new half_moon_t( this, options_str );
  if ( name == "innervate" )
    return new innervate_t( this, options_str );
  if ( name == "ironfur" )
    return new ironfur_t( this, options_str );
  if ( name == "lifebloom" )
    return new lifebloom_t( this, options_str );
  if ( name == "lunar_beam" )
    return new lunar_beam_t( this, options_str );
  if ( name == "lunar_strike" )
    return new lunar_strike_t( this, options_str );
  if ( name == "maim" )
    return new maim_t( this, options_str );
  if ( name == "mangle" )
    return new mangle_t( this, options_str );
  if ( name == "maul" )
    return new maul_t( this, options_str );
  if ( name == "moonfire" )
    return new moonfire_t( this, options_str );
  if ( name == "moonfire_cat" || name == "lunar_inspiration" )
    return new lunar_inspiration_t( this, options_str );
  if ( name == "new_moon" )
    return new new_moon_t( this, options_str );
  if ( name == "proc_sephuz" )
    return new proc_sephuz_t( this, options_str );
  if ( name == "sunfire" )
    return new sunfire_t( this, options_str );
  if ( name == "moonkin_form" )
    return new spells::moonkin_form_t( this, options_str );
  if ( name == "moonkin_form_affinity" )
    return new spells::moonkin_form_affinity_t( this, options_str );
  if ( name == "pulverize" )
    return new pulverize_t( this, options_str );
  if ( name == "rake" )
    return new rake_t( this, options_str );
  if ( name == "renewal" )
    return new renewal_t( this, options_str );
  if ( name == "regrowth" )
    return new regrowth_t( this, options_str );
  if ( name == "rejuvenation" )
    return new rejuvenation_t( this, options_str );
  if ( name == "rip" )
    return new rip_t( this, options_str );
  if ( name == "savage_roar" )
    return new savage_roar_t( this, options_str );
  if ( name == "shred" )
    return new shred_t( this, options_str );
  if ( name == "skull_bash" )
    return new skull_bash_t( this, options_str );
  if ( name == "solar_wrath" )
    return new solar_wrath_t( this, options_str );
  if ( name == "stampeding_roar" )
    return new stampeding_roar_t( this, options_str );
  if ( name == "starfall" )
    return new starfall_t( this, options_str );
  if ( name == "starsurge" )
    return new starsurge_t( this, options_str );
  if ( name == "stellar_flare" )
    return new stellar_flare_t( this, options_str );
  if ( name == "prowl" )
    return new prowl_t( this, options_str );
  if ( name == "survival_instincts" )
    return new survival_instincts_t( this, options_str );
  if ( name == "swipe_cat" )
    return new swipe_cat_t( this, options_str );
  if ( name == "swipe_bear" )
    return new swipe_bear_t( this, options_str );
  if ( name == "swipe" )
    return new swipe_proxy_t( this, options_str );
  if ( name == "swiftmend" )
    return new swiftmend_t( this, options_str );
  if ( name == "tigers_fury" )
    return new tigers_fury_t( this, options_str );
  if ( name == "thrash_bear" )
    return new thrash_bear_t( this, options_str );
  if ( name == "thrash_cat" )
    return new thrash_cat_t( this, options_str );
  if ( name == "thrash" )
    return new thrash_proxy_t( this, options_str );
  if ( name == "tranquility" )
    return new tranquility_t( this, options_str );
  if ( name == "typhoon" )
    return new typhoon_t( this, options_str );
  if ( name == "warrior_of_elune" )
    return new warrior_of_elune_t( this, options_str );
  if ( name == "wild_charge" )
    return new wild_charge_t( this, options_str );
  if ( name == "wild_growth" )
    return new wild_growth_t( this, options_str );
  if ( name == "incarnation" )
...
pet_t* druid_t::create_pet( const std::string& pet_name, const std::string& /* pet_type */ )
{
...
void druid_t::init_spells()
{
  ...
  spec.balance_tier19_2pc =
      sets->has_set_bonus( DRUID_BALANCE, T19, B2 ) ? find_spell( 211089 ) : spell_data_t::not_found();
  ...
  spec.feline_swiftness =
      specialization() == DRUID_FERAL || talent.feral_affinity->ok() ? find_spell( 131768 ) : spell_data_t::not_found();

  spec.astral_influence = specialization() == DRUID_BALANCE || talent.balance_affinity->ok()
                              ? find_spell( 197524 )
                              : spell_data_t::not_found();

  spec.thick_hide = specialization() == DRUID_GUARDIAN || talent.guardian_affinity->ok() ? find_spell( 16931 )
                                                                                         : spell_data_t::not_found();
...
void druid_t::init_base_stats()
{
  ...
  base.spell_power_per_intellect =
      specialization() == DRUID_BALANCE || specialization() == DRUID_RESTORATION ? 1.0 : 0.0;

  // Resources
  resources.base[ RESOURCE_RAGE ]        = 100;
  resources.base[ RESOURCE_COMBO_POINT ] = 5;
  resources.base[ RESOURCE_ASTRAL_POWER ] =
      100 + sets->set( DRUID_BALANCE, T20, B2 )->effectN( 1 ).resource( RESOURCE_ASTRAL_POWER );
  ...
  resources.active_resource[ RESOURCE_ENERGY ] =
      primary_role() == ROLE_ATTACK || talent.feral_affinity->ok() || specialization() == DRUID_RESTORATION;
...
void druid_t::create_buffs()
{
  ...
  buff.clearcasting =
      buff_creator_t( this, "clearcasting", spec.omen_of_clarity->effectN( 1 ).trigger() )
          ...
          .default_value( specialization() != DRUID_RESTORATION ? talent.moment_of_clarity->effectN( 4 ).percent()
                                                                : 0.0 );
  ...
  buff.incarnation_proxy = buff_creator_t( this, "incarnation", spell_data_t::nil() ).quiet( true ).chance( 1 );
  ...
  buff.bloodtalons =
      buff_creator_t( this, "bloodtalons", talent.bloodtalons->ok() ? find_spell( 145152 ) : spell_data_t::not_found() )
  ...
  buff.guardian_of_elune =
      buff_creator_t( this, "guardian_of_elune", talent.guardian_of_elune->effectN( 1 ).trigger() )
  ...
  buff.natures_balance = buff_creator_t( this, "natures_balance", talent.natures_balance )
                             ...
                             .tick_callback( [this]( buff_t*, int, const timespan_t& ) {
                               resource_gain( RESOURCE_ASTRAL_POWER,
                                              talent.natures_balance->effectN( 1 ).resource( RESOURCE_ASTRAL_POWER ),
  ...
  buff.fury_of_elune =
      buff_creator_t( this, "fury_of_elune", talent.fury_of_elune )  // Astral Power Gain Buff
          .tick_callback( [this]( buff_t*, int, const timespan_t& ) {
  ...
  buff.lunar_empowerment =
      buff_creator_t( this, "lunar_empowerment", find_spell( 164547 ) )
  ...
  buff.solar_empowerment =
      buff_creator_t( this, "solar_empowerment", find_spell( 164545 ) )
  ...
  buff.starfall = buff_creator_t( this, "starfall", find_spell( 191034 ) ).duration( timespan_t::from_seconds( 8 ) );

  // Feral

  buff.apex_predator =
      buff_creator_t( this, "apex_predator", find_spell( 252752 ) )
  ...
  buff.predatory_swiftness =
      buff_creator_t( this, "predatory_swiftness", find_spell( 69369 ) ).chance( spec.predatory_swiftness->ok() );
  ...
  buff.bristling_fur = buff_creator_t( this, "bristling_fur", talent.bristling_fur ).cd( timespan_t::zero() );

  buff.earthwarden = buff_creator_t( this, "earthwarden", find_spell( 203975 ) )
                         .default_value( talent.earthwarden->effectN( 1 ).percent() );

  buff.earthwarden_driver =
      buff_creator_t( this, "earthwarden_driver", talent.earthwarden )
          .quiet( true )
  ...
  buff.ironfur =
      buff_creator_t( this, "ironfur", spec.ironfur )
          .duration( spec.ironfur->duration() )
          .default_value( spec.ironfur->effectN( 1 ).percent() )
          .add_invalidate( CACHE_ARMOR )
          .add_invalidate( CACHE_AGILITY )
          .max_stack( (unsigned)( specialization() == DRUID_GUARDIAN ? spec.ironfur_2->effectN( 1 ).base_value() : 1 ) )
  ...
  // Restoration
  buff.harmony =
      buff_creator_t( this, "harmony", mastery.harmony->ok() ? find_spell( 100977 ) : spell_data_t::not_found() );

  buff.soul_of_the_forest =
      buff_creator_t( this, "soul_of_the_forest",
  ...
  {
    buff.yseras_gift =
        buff_creator_t( this, "yseras_gift_driver", spec.yseras_gift )
            .quiet( true )
            .tick_callback( [this]( buff_t*, int, const timespan_t& ) { active.yseras_gift->schedule_execute(); } )
            .tick_zero( true );
  }
  ...
  buff.feral_tier17_4pc = buff_creator_t( this, "feral_tier17_4pc", find_spell( 166639 ) ).quiet( true );
  ...
  buff.guardian_tier19_4pc =
      buff_creator_t( this, "natural_defenses", sets->set( DRUID_GUARDIAN, T19, B4 )->effectN( 1 ).trigger() )
          .trigger_spell( sets->set( DRUID_GUARDIAN, T19, B4 ) )
          .default_value( sets->set( DRUID_GUARDIAN, T19, B4 )->effectN( 1 ).trigger()->effectN( 1 ).base_value() /
                          1000.0 );
}
...
std::string druid_t::default_food() const
{
  return ( true_level > 110 )
             ...
             : ( true_level > 100 && specialization() == DRUID_FERAL )
                   ? "lemon_herb_filet"
                   : ( true_level > 100 )
...
void druid_t::apl_precombat()
{
  ...
  {
    /*  precombat -> add_action( "variable,name=rake_refresh,op=set,value=7", "Rake_refresh controls how aggresively to
      refresh rake. Lower means less aggresively." ); precombat -> add_action(
      "variable,name=rake_refresh,op=set,value=3,if=equipped.ailuro_pouncers" ); precombat -> add_action(
      "variable,name=pooling,op=set,value=3", "Pooling controlls how aggresively to pool. Lower means more aggresively"
      ); precombat -> add_action( "variable,name=pooling,op=set,value=10,if=equipped.chatoyant_signet" ); precombat ->
      add_action( "variable,name=pooling,op=set,value=3,if=equipped.the_wildshapers_clutch&!equipped.chatoyant_signet"
      );*/
  ...
  if ( specialization() == DRUID_BALANCE )
  {
    ...
    precombat->add_talent( this, "Cenarion Ward" );
            ...
            ( find_item( "soul_capacitor" ) || find_item( "maalus_the_blood_drinker" ) ) )
...
void druid_t::apl_feral()
{
  ...
  def->add_action(
      "regrowth,if=(talent.sabertooth.enabled|buff.predatory_swiftness.up)&talent.bloodtalons.enabled&buff.bloodtalons."
      "down&combo_points=5" );
  ...
  cooldowns->add_action(
      "shadowmeld,if=combo_points<5&energy>=action.rake.cost&dot.rake.pmultiplier<2.1&buff.tigers_fury.up&(buff."
      "bloodtalons.up|!talent.bloodtalons.enabled)&(!talent.incarnation.enabled|cooldown.incarnation.remains>18)&!buff."
      "incarnation.up" );
  ...
  st->add_action(
      "ferocious_bite,target_if=dot.rip.ticking&dot.rip.remains<3&target.time_to_die>10&(target.health.pct<25|talent."
      "sabertooth.enabled)" );
  st->add_action(
      "regrowth,if=combo_points=5&buff.predatory_swiftness.up&talent.bloodtalons.enabled&buff.bloodtalons.down&(!buff."
      "incarnation.up|dot.rip.remains<8)" );
  st->add_action(
      "regrowth,if=combo_points>3&talent.bloodtalons.enabled&buff.predatory_swiftness.up&buff.apex_predator.up&buff."
      "incarnation.down" );
  st->add_action(
      "ferocious_bite,if=buff.apex_predator.up&((combo_points>4&(buff.incarnation.up|talent.moment_of_clarity.enabled))"
      "|(talent.bloodtalons.enabled&buff.bloodtalons.up&combo_points>3))" );
  ...
  finisher->add_action( "pool_resource,for_next=1" );
  finisher->add_action(
      "rip,target_if=!ticking|(remains<=duration*0.3)&(target.health.pct>25&!talent.sabertooth.enabled)|(remains<="
      "duration*0.8&persistent_multiplier>dot.rip.pmultiplier)&target.time_to_die>8" );
  finisher->add_action( "pool_resource,for_next=1" );
  ...
  generator->add_action(
      "regrowth,if=talent.bloodtalons.enabled&buff.predatory_swiftness.up&buff.bloodtalons.down&combo_points=4&dot."
      "rake.remains<4" );
  generator->add_action(
      "regrowth,if=equipped.ailuro_pouncers&talent.bloodtalons.enabled&(buff.predatory_swiftness.stack>2|(buff."
      "predatory_swiftness.stack>1&dot.rake.remains<3))&buff.bloodtalons.down" );
  ...
  generator->add_action( "pool_resource,for_next=1" );
  generator->add_action(
      "rake,target_if=talent.bloodtalons.enabled&buff.bloodtalons.up&((remains<=7)&persistent_multiplier>dot.rake."
      "pmultiplier*0.85)&target.time_to_die>4" );
  generator->add_action(
  ...
  //  opener->add_action(this, artifact.ashamanes_frenzy, "frenzy",
  //  "if=buff.bloodtalons.up&!equipped.draught_of_souls"); opener->add_action(this, "Regrowth",
  //  "if=combo_points=5&buff.bloodtalons.down&buff.predatory_swiftness.up&!equipped.draught_of_souls");
  ...
  //  //       line +=
  //  ",if=(buff.tigers_fury.up&(target.time_to_die>trinket.stat.any.cooldown|target.time_to_die<45))|buff.incarnation.remains>20";
  //  //    def -> add_action( line );
  //  //  }
  //  //}
  //  if ( items[SLOT_FINGER_1].name_str == "ring_of_collapsing_futures" || items[SLOT_FINGER_2].name_str ==
  //  "ring_of_collapsing_futures")
  ...
  // def -> add_action( this, "Ferocious Bite",
  // "cycle_targets=1,if=dot.rip.ticking&dot.rip.remains<3&target.time_to_die>3&(target.health.pct<25|talent.sabertooth.enabled)",
  ...
  //                    "Use Healing Touch at 5 Combo Points, if Predatory Swiftness is about to fall off, at 2 Combo
  //                    Points before Ashamane's Frenzy, " "before Elune's Guidance is cast or before the Elune's
  //                    Guidance buff gives you a 5th Combo Point." );
  // def->add_action(this, "Regrowth",
  ...
  // def -> add_action( this, "Regrowth",
  // "if=equipped.ailuro_pouncers&talent.bloodtalons.enabled&(buff.predatory_swiftness.stack>2|(buff.predatory_swiftness.stack>1&dot.rake.remains<3))&buff.bloodtalons.down",
  ...
  // def -> add_action( "wait,sec=1,if=energy.time_to_max>3", "The Following line massively increases performance of the
  // simulation but can be safely removed." );
  ...
  // "cycle_targets=1,if=(!ticking|(remains<8&target.health.pct>25&!talent.sabertooth.enabled)|"
  ...
  // finish -> add_talent( this, "Savage Roar",
  // "if=((buff.savage_roar.remains<=10.5&talent.jagged_wounds.enabled)|(buff.savage_roar.remains<=7.2))&"
  ...
  // generate -> add_talent( this, "Elune's Guidance",
  // "if=talent.elunes_guidance.enabled&combo_points=0&energy>=action.ferocious_bite.cost+25" ); generate -> add_action(
  ...
  // if ( race == RACE_NIGHT_ELF )
  //   generate -> add_action(
  //   "shadowmeld,if=combo_points<5&energy>=action.rake.cost&dot.rake.pmultiplier<2.1&buff.tigers_fury.up&"
  ...
  // generate -> add_action( this, "Rake",
  // "cycle_targets=1,if=combo_points<5&(!ticking|(!talent.bloodtalons.enabled&remains<duration*0.3)|"
  ...
  // generate -> add_action(
  // "moonfire_cat,cycle_targets=1,if=combo_points<5&remains<=4.2&target.time_to_die-remains>tick_time*2" ); generate ->
  // add_action( "pool_resource,for_next=1" );
  ///* if ( sets -> has_set_bonus( DRUID_FERAL, T19, B4 ) )
  //    generate->add_action(
  //    "thrash_cat,cycle_targets=1,if=remains<=duration*0.3&(spell_targets.swipe_cat>=2|(buff.clearcasting.up&buff.bloodtalons.down))",
  //    "Use Thrash on Clearcasting, or during Berserk/Incarnation if you have T19 4set" );
  // else
  ...
  // "Use Thrash on single-target if you have T19 4set" ); generate -> add_action(
  ...
  // // Sabertooth Opener
  // sbt -> add_action( this, "Regrowth",
  // "if=talent.bloodtalons.enabled&combo_points=5&!buff.bloodtalons.up&!dot.rip.ticking",
...
void druid_t::apl_balance()
{
  ...
  default_list->add_action(
      ...
      "raid_event.adds.duration%15)*(4)<(raid_event.adds.in%180))|(raid_event.adds.up))" );
  default_list->add_action(
      "incarnation,if=astral_power>=40&(((raid_event.adds.duration%30)*(4)<(raid_event.adds.in%180))|(raid_event.adds."
      "up))" );
  default_list->add_action( this, "Celestial Alignment",
                            "if=astral_power>=40&(!azerite.lively_spirit.enabled|buff.lively_spirit.up)&(((raid_event."
                            "adds.duration%15)*(4)<(raid_event.adds.in%180))|(raid_event.adds.up))" );
  ...
  ST->add_talent( this, "Fury of Elune",
                  "if=(((raid_event.adds.duration%8)*(4)<(raid_event.adds.in%60))|(raid_event.adds.up))&((buff."
                  "celestial_alignment.up|buff.incarnation.up)|(cooldown.celestial_alignment.remains>30|cooldown."
                  "incarnation.remains>30))" );
  ST->add_talent( this, "Force of Nature",
                  "if=(buff.celestial_alignment.up|buff.incarnation.up)|(cooldown.celestial_alignment.remains>30|"
  ...
  ST->add_action( this, "Solar Wrath",
                  "if=(buff.solar_empowerment.stack=3|buff.solar_empowerment.stack=2&buff.lunar_empowerment.stack=2&"
                  "astral_power>=40)&astral_power.deficit>10" );
  ...
  ST->add_action( this, "Starsurge",
                  "if=!buff.starlord.up|buff.starlord.remains>=4|(gcd.max*(astral_power%40))>target.time_to_die" );
  ST->add_action( this, "Lunar Strike",
                  "if=(buff.warrior_of_elune.up|!buff.solar_empowerment.up)&buff.lunar_empowerment.up" );
  ...
  AoE->add_talent( this, "Fury of Elune",
                   "if=(((raid_event.adds.duration%8)*(4)<(raid_event.adds.in%60))|(raid_event.adds.up))&((buff."
                   "celestial_alignment.up|buff.incarnation.up)|(cooldown.celestial_alignment.remains>30|cooldown."
                   "incarnation.remains>30))" );
  ...
  AoE->add_action( this, "Lunar Strike",
                   "if=(buff.lunar_empowerment.stack=3|buff.solar_empowerment.stack=2&buff.lunar_empowerment.stack=2&"
                   "astral_power>=40)&astral_power.deficit>14" );
  ...
  AoE->add_action( this, "Solar Wrath",
                   "if=(buff.solar_empowerment.up&!buff.warrior_of_elune.up|buff.solar_empowerment.stack>=3)&buff."
                   "lunar_empowerment.stack<3" );
  AoE->add_action( this, "Lunar Strike" );
  ...
  ED->add_talent( this, "Fury of Elune",
                  "if=(buff.celestial_alignment.up|buff.incarnation.up)|(cooldown.celestial_alignment.remains>30|"
                  "cooldown.incarnation.remains>30)&(buff.the_emerald_dreamcatcher.remains>gcd.max|!buff.the_emerald_"
                  "dreamcatcher.up)" );
  ED->add_talent( this, "Force of Nature",
                  "if=(buff.celestial_alignment.up|buff.incarnation.up)|(cooldown.celestial_alignment.remains>30|"
                  "cooldown.incarnation.remains>30)&(buff.the_emerald_dreamcatcher.remains>gcd.max|!buff.the_emerald_"
                  "dreamcatcher.up)" );
  ...
  ED->add_action(
      this, "Moonfire",
      "target_if=refreshable,if=buff.the_emerald_dreamcatcher.remains>gcd.max|!buff.the_emerald_dreamcatcher.up" );
  ED->add_action(
      this, "Sunfire",
      "target_if=refreshable,if=buff.the_emerald_dreamcatcher.remains>gcd.max|!buff.the_emerald_dreamcatcher.up" );
  ED->add_talent(
      this, "Stellar Flare",
      "target_if=refreshable,if=buff.the_emerald_dreamcatcher.remains>gcd.max|!buff.the_emerald_dreamcatcher.up" );
  ED->add_action( this, "Starfall",
                  "if=buff.oneths_overconfidence.up&(buff.the_emerald_dreamcatcher.remains>gcd.max|!buff.the_emerald_"
                  "dreamcatcher.up)" );
  ED->add_action( this, "New Moon",
                  "if=buff.the_emerald_dreamcatcher.remains>execute_time|!buff.the_emerald_dreamcatcher.up" );
  ED->add_action( this, "Half Moon",
                  "if=astral_power.deficit>=20&(buff.the_emerald_dreamcatcher.remains>execute_time|!buff.the_emerald_"
                  "dreamcatcher.up)" );
  ED->add_action( this, "Full Moon",
                  "if=astral_power.deficit>=40&(buff.the_emerald_dreamcatcher.remains>execute_time|!buff.the_emerald_"
                  "dreamcatcher.up)" );
  ED->add_action( this, "Lunar Strike",
                  ",if=buff.lunar_empowerment.up&buff.the_emerald_dreamcatcher.remains>execute_time" );
  ED->add_action( this, "Solar Wrath",
                  "if=buff.solar_empowerment.up&buff.the_emerald_dreamcatcher.remains>execute_time" );
  ED->add_action(
      this, "Starsurge",
      "if=(buff.the_emerald_dreamcatcher.up&buff.the_emerald_dreamcatcher.remains<gcd.max)|astral_power>=50" );
  ED->add_action( this, "Solar Wrath" );
}
...
void druid_t::apl_guardian()
{
  ...
  if ( catweave_bear && talent.feral_affinity->ok() )
  {
    ...
    default_list->add_action(
        "call_action_list,name=cat,if=talent.feral_affinity.enabled&((cooldown.thrash_bear.remains>0&cooldown.mangle."
        "remains>0&rage<45&buff.incarnation.down&buff.galactic_guardian.down)|(buff.cat_form.up&energy>20)|(dot.rip."
        "ticking&dot.rip.remains<3&target.health.pct<25))" );
...
void druid_t::init_resources( bool force )
{
  ...
  if ( talent.natures_balance->ok() )
  {
    resources.current[ RESOURCE_ASTRAL_POWER ] =
        42;  // Nature's Balance causes you to drop to 50 instead of 20 Astral Power at the start of combat
  }
  else
  {
...
double druid_t::composite_player_multiplier( school_e school ) const
{
  ...
  if ( buff.bear_form->check() &&
       ( dbc::is_school( school, SCHOOL_ARCANE ) || dbc::is_school( school, SCHOOL_NATURE ) ) )
...
double druid_t::composite_spell_power( school_e school ) const
{
  ...
  if ( specialization() == DRUID_GUARDIAN )
  {
    ap_coeff = spec.guardian->effectN( 11 ).percent();
  }
  if ( specialization() == DRUID_FERAL )
  {
    ap_coeff = spec.feral->effectN( 11 ).percent();
  }
...
expr_t* druid_t::create_expression( const std::string& name_str )
{
  ...
  {
    druid_t& druid;
    druid_expr_t( const std::string& n, druid_t& p ) : expr_t( n ), druid( p )
    {
    }
  };
  ...
  else if ( util::str_compare_ci( name_str, "new_moon" ) || util::str_compare_ci( name_str, "half_moon" ) ||
            util::str_compare_ci( name_str, "full_moon" ) )
  {
    struct moon_stage_expr_t : public druid_expr_t
    {
      ...
      moon_stage_expr_t( druid_t& p, const std::string& name_str ) : druid_expr_t( name_str, p )
      {
...
void druid_t::assess_damage( school_e school, dmg_e dtype, action_state_t* s )
{
  if ( dbc::is_school( school, SCHOOL_PHYSICAL ) && dtype == DMG_DIRECT && s->result == RESULT_HIT )
  {
    buff.ironfur->up();
  }
...
void druid_t::assess_heal( school_e school, dmg_e dmg_type, action_state_t* s )
{
    ...
    for ( size_t i = 0, end = p.stats_list.size(); i < end; i++ )
    {
      ...
      if ( tf_exe_total > 0 || bt_exe_total > 0 || sr_exe_total > 0 )
      {
        ...
        os.printf( "<tr%s><td class=\"left\">%s</td><td class=\"right\">%.2f %%</td><td class=\"right\">%.2f %%</td>\n",
                   row_class_str.c_str(), name_str.c_str(), util::round( tf_exe_up / tf_exe_total * 100, 2 ),
...
{
  wildcat_celerity_t( const std::string& n ) : super( DRUID_FERAL, n )
  {
  }
...
{
  luffa_wrappings_t( const std::string& name ) : scoped_action_callback_t<T>( DRUID, name )
  {
  }
...
struct oneths_intuition_t : public class_buff_cb_t<druid_t>
{
  ...
  buff_t* creator( const special_effect_t& e ) const override
  {
    return make_buff( e.player, buff_name, e.player->find_spell( 209406 ) )->set_chance( e.driver()->proc_chance() );
  }
};
...
struct oneths_overconfidence_t : public class_buff_cb_t<druid_t>
{
  ...
  buff_t* creator( const special_effect_t& e ) const override
  {
    return make_buff( e.player, buff_name, e.player->find_spell( 209407 ) )->set_chance( e.driver()->proc_chance() );
  }
};
...
struct the_emerald_dreamcatcher_t : public class_buff_cb_t<druid_t>
{
  ...
  {
    return make_buff( e.player, buff_name, e.driver()->effectN( 1 ).trigger() )
        ->set_default_value( e.driver()->effectN( 1 ).trigger()->effectN( 1 ).resource( RESOURCE_ASTRAL_POWER ) );
  }
};
...
{
  lady_and_the_child_t( const std::string& name ) : scoped_action_callback_t<T>( DRUID, name )
  {
  }
...
struct skysecs_hold_t : public scoped_action_callback_t<frenzied_regeneration_t>
{
  struct skysecs_hold_heal_t : public druid_heal_t
  {
    skysecs_hold_heal_t( const special_effect_t& effect )
      : druid_heal_t( "skysecs_hold", debug_cast<druid_t*>( effect.player ), effect.driver()->effectN( 1 ).trigger() )
    {
      background   = true;
...
struct oakhearts_puny_quods_t : public scoped_action_callback_t<barkskin_t>
{
  ...
  void manipulate( barkskin_t* a, const special_effect_t& e ) override
  {
    ...
    a->energize_amount   = e.driver()->effectN( 1 ).trigger()->effectN( 1 ).resource( RESOURCE_RAGE );
...
struct oakhearts_puny_quods_buff_t : public class_buff_cb_t<druid_t>
{
  ...
  {
    return make_buff( e.player, buff_name, e.driver()->effectN( 1 ).trigger() )
        ->set_default_value( e.driver()->effectN( 1 ).trigger()->effectN( 2 ).resource( RESOURCE_RAGE ) )
        ->set_tick_callback( []( buff_t* b, int, const timespan_t& ) {
          assert( b->player );
          b->player->resource_gain( RESOURCE_RAGE, b->default_value,
...
struct druid_module_t : public module_t
{
  ...
  virtual void static_init() const override
  {
    ...
    register_special_effect( 200818,
                             lady_and_the_child_t<moonfire_t::galactic_guardian_damage_t>( "galactic_guardian" ) );
