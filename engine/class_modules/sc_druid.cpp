...
void druid_t::apl_balance()
{
  ...
  st->add_action( "ravenous_frenzy,if=buff.ca_inc.up","Use the Venthyr ability on cd which is with CA/Inc" );
  st->add_action( "starsurge,if=runeforge.timeworn_dreambinder.equipped&(eclipse.in_any&!((buff.timeworn_dreambinder.remains>action.wrath.execute_time+0.1&(eclipse.in_both|eclipse.in_solar|eclipse.lunar_next)|buff.timeworn_dreambinder.remains>action.starfire.execute_time+0.1&(eclipse.in_lunar|eclipse.solar_next|eclipse.any_next))|!buff.timeworn_dreambinder.up)|(buff.ca_inc.up|variable.convoke_desync)&cooldown.convoke_the_spirits.ready&covenant.night_fae)&(!covenant.kyrian|cooldown.empower_bond.remains>8)&(buff.ca_inc.up|!cooldown.ca_inc.ready)","Calculates whether Starsurge needs to be used to keep up the Dreambinder buff or to dump before convoke" );
  ...
  aoe->add_action( "ravenous_frenzy,if=buff.ca_inc.up","Use frenzy with ca" );
