...
void druid_t::apl_restoration()
{
  ...
  pre->add_action( this, "Prowl", "if=talent.feral_affinity.enabled" );
  ...
  owl->add_action( "heart_of_the_wild,if=cooldown.convoke_the_spirits.remains<30|!covenant.night_fae" );
  owl->add_action( "convoke_the_spirits,if=(buff.heart_of_the_wild.up|cooldown.heart_of_the_wild.remains>60|!talent.heart_of_the_wild.enabled)&(buff.eclipse_solar.up|spell_targets.starfire>1&eclipse.in_any|spell_targets.starfire>=9)&(!equipped.soulleting_ruby|cooldown.soulleting_ruby.remains<114&!cooldown.soulleting_ruby.ready)" );
  ...
  cat->add_action( "heart_of_the_wild,if=cooldown.convoke_the_spirits.remains<30|!covenant.night_fae" );
  cat->add_action( "rake,if=buff.shadowmeld.up|buff.prowl.up" );
  cat->add_action( "auto_attack,if=!buff.prowl.up&!buff.shadowmeld.up" );
  cat->add_action( "convoke_the_spirits,if=(buff.heart_of_the_wild.up|cooldown.heart_of_the_wild.remains>60|!talent.heart_of_the_wild.enabled)&energy<50&buff.cat_form.up" );
  cat->add_action( "sunfire,target_if=refreshable&target.time_to_die>5" );
  cat->add_action( "moonfire,target_if=refreshable&time_to_die>12&(spell_targets.swipe_cat<=4|energy<50)&!ticking&spell_targets.swipe_cat<3|(prev_gcd.1.sunfire&remains<duration*0.8&spell_targets.sunfire=1)" );
  ...
  cat->add_action( "rip,target_if=((refreshable&(combo_points=5&time_to_die>remains+24|(remains+combo_points*4<time_to_die&remains+4+combo_points*4>time_to_die)))|combo_points=5&energy>90&remains<=10)&spell_targets.swipe_cat<11" );
