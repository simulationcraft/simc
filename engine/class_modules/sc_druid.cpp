...
public:
  ...
  struct talents_t
  {
    ...
    player_talent_t swipe;  // removed in PTR
    ...
    player_talent_t ursine_adept;  // removed in PTR
...
typedef druid_buff_base_t<> druid_buff_t;
...
struct ursine_vigor_buff_t : public druid_buff_t
{
  ...
  ursine_vigor_buff_t( druid_t* p )
    : base_t( p, "ursine_vigor", p->talent.ursine_vigor->effectN( 1 ).trigger() )
  {
...
public:
  ...
  void apply_buff_effects()
  {
    ...
    // TODO fix fix fix once data is not a disaster
  ...
  virtual bool check_form_restriction()
  {
    ...
    if ( ( maybe_ptr( p()->dbc->ptr ) ? p()->spec.ursine_adept->ok() : p()->talent.ursine_adept.ok() ) &&
      p()->buff.bear_form->check() && ab::data().affected_by( p()->buff.bear_form->data().effectN( 2 ) ) )
    {
      return true;
    }
...
struct druid_heal_t : public druid_spell_base_t<heal_t>
{
  ...
  druid_heal_t( std::string_view n, druid_t* p, const spell_data_t* s = spell_data_t::nil(), std::string_view opt = {} )
    : base_t( n, p, s ),
      affected_by(),
      forestwalk_mul( p->buff.forestwalk->data().effectN( 1 ).percent() ),
...
{
  struct rake_bleed_t : public druid_mixin_t<trigger_waning_twilight_t<cat_attack_t>>
  {
...
// Rip ======================================================================
struct rip_t : public druid_mixin_t<trigger_waning_twilight_t<cat_finisher_t>>
{
...
struct swipe_cat_t : public cat_attack_t
{
  ...
  swipe_cat_t( druid_t* p, std::string_view opt )
    : cat_attack_t( "swipe_cat", p, p->apply_override( maybe_ptr( p->dbc->ptr ) ? p->spec.swipe : p->talent.swipe, p->spec.cat_form_override ), opt ),
      berserk_swipe_cp( p->spec.berserk_cat->effectN( 2 ).base_value() )
  {
    aoe = -1;
  ...
  bool ready() override
  {
    return p()->talent.brutal_slash.ok() ? false : cat_attack_t::ready();
  }
...
// Thrash (Cat) =============================================================
struct thrash_cat_t : public druid_mixin_t<trigger_waning_twilight_t<cat_attack_t>>
{
...
struct ironfur_t : public rage_spender_t
{
  ...
  void execute() override
  {
    ...
    if ( p()->buff.guardian_of_elune->check() )
      dur += p()->buff.guardian_of_elune->data().effectN( 1 ).time_value();
...
struct raze_t : public druid_mixin_t<trigger_gore_t<rage_spender_t>>
{
  ...
  void execute() override
  {
    ...
    p()->buff.vicious_cycle_maul->expire();
    // PTR does not trigger vicious_cycle_mangle
    // p()->buff.vicious_cycle_mangle->tirgger();
  }
};

// Swipe (Bear) =============================================================
struct swipe_bear_t : public druid_mixin_t<trigger_gore_t<bear_attack_t>>
{
  swipe_bear_t( druid_t* p, std::string_view opt )
    : base_t( "swipe_bear", p, p->apply_override( maybe_ptr( p->dbc->ptr ) ? p->spec.swipe : p->talent.swipe, p->spec.bear_form_override ), opt )
  {
    aoe = -1;
    // target hit data stored in cat swipe
    reduced_aoe_targets = p->apply_override( maybe_ptr( p->dbc->ptr ) ? p->spec.swipe : p->talent.swipe, p->spec.cat_form_override )->effectN( 4 ).base_value();
...
void druid_action_t<Base>::impact( action_state_t* s )
{
  ...
  if ( p()->active.moonless_night && s->result_amount > 0 && can_proc_moonless_night() )
  {
    ...
    moonless->snapshot_and_execute( s, false, [ moonless, s ]( action_state_t* new_ ) {
...
struct astral_smolder_t
  : public residual_action::residual_periodic_action_t<druid_mixin_t<trigger_waning_twilight_t<druid_spell_t>>>
{
...
struct swipe_proxy_t : public druid_spell_t
{
  ...
  swipe_proxy_t( druid_t* p, std::string_view opt )
    : druid_spell_t( "swipe", p, maybe_ptr( p->dbc->ptr ) ? p->spec.swipe : p->talent.swipe, opt ),
...
// Stellar Flare ============================================================
struct stellar_flare_t : public druid_mixin_t<trigger_shooting_stars_t<trigger_waning_twilight_t<druid_spell_t>>>
{
...
{
  struct sunfire_damage_t : public druid_mixin_t<trigger_shooting_stars_t<trigger_waning_twilight_t<druid_spell_t>>>
  {
...
// Wrath ====================================================================
struct wrath_t : public druid_mixin_t<trigger_astral_smolder_t<consume_umbral_embrace_t<druid_spell_t>>>
{
  double gcd_mul;
  double sotf_mul;
  double smolder_mul;
...
void druid_t::init_base_stats()
{
  ...
  base.armor_multiplier *= 1.0 + query_aura_effect( talent.killer_instinct, A_MOD_BASE_RESISTANCE_PCT )->percent();
  ...
  if ( maybe_ptr( dbc->ptr ) )
    resources.base[ RESOURCE_ASTRAL_POWER ] += talent.astral_communion->effectN( 1 ).resource( RESOURCE_ASTRAL_POWER );
...
void druid_t::create_buffs()
{
  ...
  // Class
  buff.forestwalk = make_buff( this, "forestwalk", talent.forestwalk->effectN( 1 ).trigger() )
    ->set_duration_multiplier( 1.0 + talent.forestwalk->effectN( 3 ).percent() )
  ...
  buff.ironfur = make_buff( this, "ironfur", talent.ironfur )
    ...
    ->apply_affecting_aura( talent.reinforced_fur )
    ->apply_affecting_aura( maybe_ptr( dbc->ptr ) ? spec.ursine_adept : talent.ursine_adept )
    ->apply_affecting_aura( talent.ursocs_endurance )
...
void druid_t::init_special_effects()
{
  ...
  if ( talent.denizen_of_the_dream.ok() )
  {
    ...
    {
      denizen_of_the_dream_cb_t( druid_t* p, const special_effect_t& e ) : dbc_proc_callback_t( p, e ) {}
...
void druid_t::target_mitigation( school_e school, result_amount_type type, action_state_t* s )
{
  ...
  if ( ( maybe_ptr( dbc->ptr ) ? spec.ursine_adept->ok() : talent.ursine_adept.ok() ) && buff.bear_form->check() )
    s->result_amount *= 1.0 + ( maybe_ptr( dbc->ptr ) ? spec.ursine_adept : talent.ursine_adept )->effectN( 2 ).percent();
