  ...
  void execute() override
  {
    ...
    if ( p()->spec.starfall_2->ok() )
    {
      ...
      for ( size_t i = 0, actors = tl.size(); i < actors; i++ )
      {
        ...
        td( t )->dots.moonfire->extend_duration( timespan_t::from_seconds( ext ), timespan_t::min(), -1, false );
        td( t )->dots.sunfire->extend_duration( timespan_t::from_seconds( ext ), timespan_t::min(), -1, false );
      }
    }
