...
struct player_collected_data_t
{
  ...
  // For some actors (typically enemies), health and health percentage timelines might not match. Track health
  ...
  // Total number of iterations for the actor. Needed for single_actor_batch if target_error is used.
  ...
  struct action_sequence_data_t : noncopyable
  {
    ...
    struct record_t
    {
      ...
      record_t( T* o, int v, timespan_t tv ) : object( o ), value( v ), time_value( tv ) {}
    };
    ...
    action_sequence_data_t( const action_t* a, const player_t* t, timespan_t ts, const player_t* p )
      : action_sequence_data_t( a, t, ts, 0_ms, p )
    {}
  };
