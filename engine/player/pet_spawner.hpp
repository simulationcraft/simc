...
public:
  ...
  using expr_fn_t = std::function<std::unique_ptr<expr_t>(const util::span<const std::string>&)>;
...
public:
  ...
  std::unique_ptr<expr_t> create_expression( const util::span<const std::string>& expr ) override;
