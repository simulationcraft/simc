...
void item::readiness( special_effect_t& effect )
{
  ...
  double cdr = 1.0 / ( 1.0 + budget.p_epic.front() * cdr_spell -> effectN( 1 ).m_average() / 100.0 );
...
void item::amplification( special_effect_t& effect )
{
  ...
  const random_prop_data_t& budget = p -> dbc.random_property( effect.item -> item_level() );
  *amp_value = budget.p_epic.front() * amplify_spell -> effectN( 2 ).m_average() / 100.0;
  if ( p -> level() > 90 )
...
void item::cleave( special_effect_t& effect )
{
  ...
  effect.proc_flags2_ = PF2_ALL_HIT;
  effect.proc_chance_ = budget.p_epic.front() * cleave_driver_spell -> effectN( 1 ).m_average() / 10000.0;
