...
void items::storm_of_the_eternal_stats( special_effect_t& effect )
{
  ...
  stat_buff->add_stat( stat,
    effect.driver()->effectN( 1 ).average( effect.item ) * effect.player->sim->bfa_opts.storm_of_the_eternal_ratio );
...
void items::azsharas_font_of_power( special_effect_t& effect )
{
  ...
  if ( time > 0_ms )
  {
    ...
    auto total = effect.trigger()->duration() * ( channel.total_seconds() + 1 ); // total duration of the buff you got
    ...
    effect.player->sim->print_debug(
      "Azshara's Hack of Power started {}s before combat, channeled for {}s, giving {}s buff in combat", time, channel,
      actual );
