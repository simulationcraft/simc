  ...
  if ( !is_pet() && !is_enemy() && sim->pet_stat_delay )
  {
    sim->register_heartbeat_event_callback( [ this ]( sim_t* sim ) {
      for ( auto& pet : active_pets )
      {
        ...
        this_pet->current_pet_stats.composite_melee_speed = this_pet->owner->cache.attack_speed();
        this_pet->current_pet_stats.composite_spell_speed = this_pet->owner->cache.spell_speed();
      }
    } );
  }
}
