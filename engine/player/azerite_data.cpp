...
azerite_power_t::azerite_power_t() : m_player( nullptr ), m_spell( spell_data_t::not_found() ), m_data( nullptr )
{
}
...
double azerite_power_t::value( size_t index ) const
{
  ...
  if ( m_value.size() >= index && m_value[ index - 1 ] != std::numeric_limits<double>::lowest() )
  {
  ...
  for ( size_t budget_index = 0, end = budgets.size(); budget_index < end; ++budget_index )
  {
    ...
    if ( m_spell->scaling_class() == PLAYER_SPECIAL_SCALE7 || check_combat_rating_penalty( index ) )
    {
      value = item_database::apply_combat_rating_multiplier( m_player, CR_MULTIPLIER_ARMOR, actual_level, value ) + 0.5;
    }
...
bool azerite_power_t::check_combat_rating_penalty( size_t index ) const
{
  ...
  return m_spell->effectN( index ).subtype() == A_MOD_RATING && m_spell->effectN( index ).m_coefficient() != 0;
}
...
const spell_data_t* azerite_essence_t::spell( unsigned rank, essence_spell spell, essence_type type ) const
{
  ...
  // minor/major essence
  if ( ( type == essence_type::INVALID && m_type == essence_type::PASSIVE ) || type == essence_type::PASSIVE )
  {
...
azerite_power_t azerite_state_t::get_power( unsigned id )
{
  ...
  if ( override_it != m_overrides.end() )
  {
    ...
    if ( override_it->second[ 0 ] == 0 )
    {
      ...
      {
        m_player->sim->out_debug.printf( "%s disabling overridden azerite power %s", m_player->name(), power.name );
      }
    ...
    else
    {
      if ( m_player->sim->debug )
      {
        ...
        m_player->sim->out_debug.printf(
            "%s initializing overridden azerite power %s: "
            "ilevels=%s",
  ...
  else
  {
    ...
    if ( m_spell_items[ power.spell_id ].size() > 0 && m_player->sim->azerite_status == azerite_control::ENABLED )
    {
      if ( m_player->sim->debug )
      {
        std::stringstream s;
        ...
        {
          s << items[ i ]->full_name() << " (id=" << items[ i ]->parsed.data.id
        ...
        m_player->sim->out_debug.printf( "%s initializing azerite power %s: %s", m_player->name(), power.name,
                                         s.str().c_str() );
      }
...
bool azerite_state_t::parse_override( sim_t* sim, const std::string&, const std::string& value )
{
  ...
  for ( const auto& opt : override_split )
  {
    ...
    if ( opt_split.size() != 2 )
    {
      sim->errorf( "%s unknown azerite override string \"%s\"", sim->active_player->name(), opt.c_str() );
      return false;
    }
    ...
    {
      sim->errorf( "%s too high ilevel %u for azerite override of \"%s\"", sim->active_player->name(), ilevel,
                   power_str );
      return false;
    }
    ...
    if ( power->id == 0 )
    {
      sim->errorf( "%s unknown azerite power \"%s\" for override", sim->active_player->name(), power_str );
      return false;
    }
...
void azerite_essence_state_t::update_traversal_nodes()
{
  ...
  range::for_each( passives, [this]( unsigned id ) {
    auto it = range::find_if(
        m_state, [id]( const slot_state_t& slot ) { return id == slot.id() && slot.type() == essence_type::PASSIVE; } );
...
azerite_essence_t azerite_essence_state_t::get_essence( unsigned id ) const
{
  ...
  auto it = range::find_if( m_state, [&essence]( const slot_state_t& slot ) { return slot.id() == essence.id; } );
...
bool azerite_essence_state_t::parse_azerite_essence( sim_t* sim, const std::string& /* name */,
                                                     const std::string& value )
{
  ...
  for ( size_t i = 0u; i < splits.size(); ++i )
  {
    ...
    if ( token_split.size() > 1 )
    {
      ...
      {
        sim->error( "Invalid Azerite Essence rank, expected max of %u, got %s", MAX_AZERITE_ESSENCE_RANK,
                    token_split[ 1 ].c_str() );
        return false;
      }
    }
    ...
    if ( token_split.size() > 2 )
    {
      type = token_split[ 2 ].front() == '0'
                 ? essence_type::MINOR
                 : token_split[ 2 ].front() == '1' ? essence_type::MAJOR : essence_type::INVALID;

      if ( type == essence_type::INVALID )
      {
        sim->errorf( "Invalid Azerite Essence type, expected '0' or '1', got '%s'", token_split[ 2 ].c_str() );
        return false;
      }
    }
    ...
    switch ( token_split.size() )
    {
      ...
      case 1u:
      {
        if ( id == 0 )
        {
          sim->errorf( "Unable to parse passive Azerite Essence '%s' into a spell id", token_split[ 0 ].c_str() );
          return false;
        }
        ...
        if ( !spell->ok() )
        {
          sim->errorf( "Unable to find passive Azerite Essence '%s'", token_split[ 0 ].c_str() );
          return false;
        }
      ...
      case 2u:
      {
        ...
        m_state.emplace_back( major_parsed ? essence_type::MINOR : essence_type::MAJOR, id, rank );
...
void register_azerite_target_data_initializers( sim_t* sim )
{
  ...
  sim->register_target_data_initializer( []( actor_target_data_t* td ) {
    ...
    {
      td->debuff.battlefield_debuff = make_buff( *td, "battlefield_focus", td->source->find_spell( 280817 ) );
      td->debuff.battlefield_debuff->reset();
    }
    else if ( azerite->is_enabled( "Battlefield Precision" ) )
    {
      td->debuff.battlefield_debuff = make_buff( *td, "battlefield_precision", td->source->find_spell( 280855 ) );
      td->debuff.battlefield_debuff->reset();
    }
    else
    {
  ...
  sim->register_target_data_initializer( []( actor_target_data_t* td ) {
    ...
    if ( essence.enabled() )
    {
      td->debuff.blood_of_the_enemy =
          make_buff( *td, "blood_of_the_enemy", td->source->find_spell( 297108 ) )
  ...
  sim->register_target_data_initializer( []( actor_target_data_t* td ) {
    ...
    if ( essence.enabled() )
    {
      td->debuff.condensed_lifeforce =
          make_buff( *td, "condensed_lifeforce", td->source->find_spell( 295838 ) )
              ->set_default_value( td->source->find_spell( 295838 )->effectN( 1 ).percent() );
      td->debuff.condensed_lifeforce->reset();
    }
    else
    {
      td->debuff.condensed_lifeforce = make_buff( *td, "condensed_lifeforce" )->set_quiet( true );
    }
  } );
}
...
std::tuple<int, int, int> compute_value( const azerite_power_t& power, const spelleffect_data_t& effect )
{
  ...
  range::for_each( budgets, [&]( double budget ) {
    ...
    min_ += static_cast<int>( budget * effect.m_coefficient() * ( 1.0 - effect.m_delta() / 2 ) + 0.5 );
    max_ += static_cast<int>( budget * effect.m_coefficient() * ( 1.0 + effect.m_delta() / 2 ) + 0.5 );
  } );
  ...
  if ( data[ "azerite_details" ].HasMember( "selected_essences" ) )
  {
    for ( auto idx = 0u, end = data[ "azerite_details" ][ "selected_essences" ].Size(); idx < end; ++idx )
    {
      ...
      item.player->azerite_essence->add_essence(
          ...
          essence_data[ "essence" ][ "id" ].GetUint(), essence_data[ "rank" ].GetUint() );
    }
  }

  if ( data[ "azerite_details" ].HasMember( "level" ) && data[ "azerite_details" ][ "level" ].HasMember( "value" ) )
  {
...
void resounding_protection( special_effect_t& effect )
{
  ...
  public:
    rp_event_t( buff_t* b, const timespan_t& t ) : event_t( *b->source, t ), buff( b ), period( t )
    {
    }
  ...
  buff_t* buff =
      make_buff<absorb_buff_t>( effect.player, "resounding_protection", absorb )->set_default_value( amount );
...
void elemental_whirl( special_effect_t& effect )
{
  ...
  buff_t* crit =
      make_buff<stat_buff_t>( effect.player, "elemental_whirl_crit", crit_spell )->add_stat( STAT_CRIT_RATING, amount );
...
void unstable_flames( special_effect_t& effect )
{
  ...
  {
    effect.custom_buff =
        make_buff<stat_buff_t>( effect.player, "unstable_flames", spell )->add_stat( STAT_CRIT_RATING, amount );
  }
...
void thunderous_blast( special_effect_t& effect )
{
  struct thunderous_blast_t : public unique_gear::proc_spell_t
  {
      ...
      : proc_spell_t( "thunderous_blast", e.player, e.player->find_spell( 280384 ) )
    {
      ...
      {
        building_pressure = make_buff( e.player, "building_pressure", data().effectN( 3 ).trigger() )
      ...
      if ( !rolling_thunder )
      {
        rolling_thunder = make_buff( e.player, "rolling_thunder", e.player->find_spell( 280400 ) )
...
void blood_rite( special_effect_t& effect )
{
  ...
  {
    effect.custom_buff =
        make_buff<stat_buff_t>( effect.player, "blood_rite", spell )->add_stat( STAT_HASTE_RATING, power.value( 1 ) );
  }
...
struct reorigination_array_buff_t : public buff_t
{
  ...
  stat_e find_highest_stat() const
  {
    ...
    static const std::vector<rating_e> ratings{RATING_MELEE_CRIT, RATING_MELEE_HASTE, RATING_MASTERY,
    ...
    for ( auto r : ratings )
    {
      ...
      {
        composite -= sim->bfa_opts.reorigination_array_stacks * stat_value * source->composite_rating_multiplier( r );
      ...
      if ( sim->debug )
      {
        sim->out_debug.print( "{} reorigination_array check_stat current_stat={} stat={} value={}", source->name(),
                              util::stat_type_string( current_stat ), util::stat_type_string( rating_to_stat( r ) ),
                              composite );
      }
  ...
  void trigger_dynamic_stat_change()
  {
    ...
    if ( current_stat != STAT_NONE )
    {
      ...
      if ( sim->debug )
      {
        sim->out_debug.print( "{} reorigination_array stat change, current_stat={}, value={}, total={}", source->name(),
                              util::stat_type_string( current_stat ), -current_amount,
    ...
    if ( highest_stat != STAT_NONE )
    {
      ...
      if ( sim->debug )
      {
        sim->out_debug.print( "{} reorigination_array stat change, new_stat={}, value={}, total={}", source->name(),
                              util::stat_type_string( highest_stat ), new_amount,
  ...
  void execute( int stacks = 1, double value = DEFAULT_VALUE(), timespan_t duration = timespan_t::min() ) override
  {
    ...
    if ( current_stack == 0 )
    {
      ...
      make_repeating_event( *sim, timespan_t::from_seconds( 5.0 ), [this]() { trigger_dynamic_stat_change(); } );
    }
  }
...
void archive_of_the_titans( special_effect_t& effect )
{
  ...
  {
    reorg = unique_gear::create_buff<reorigination_array_buff_t>( effect.player, "reorigination_array", effect );
  }
  ...
  effect.player->register_combat_begin( [buff, driver, reorg]( player_t* ) {
    ...
    make_repeating_event( *buff->sim, driver->effectN( 1 ).period(), [buff]() { buff->trigger(); } );
...
void laser_matrix( special_effect_t& effect )
{
  ...
  if ( !reorg && effect.player->sim->bfa_opts.reorigination_array_stacks > 0 )
  {
    ...
    effect.player->register_combat_begin(
        [reorg]( player_t* ) { reorg->trigger( reorg->sim->bfa_opts.reorigination_array_stacks ); } );
  }
}
...
void azerite_globules( special_effect_t& effect )
{
  ...
  {
  public:
    azerite_globules_proc_cb_t( const special_effect_t& effect ) : dbc_proc_callback_t( effect.player, effect )
    {
    }
...
void earthlink( special_effect_t& effect )
{
  ...
  {
    earthlink_t( player_t* p ) : stat_buff_t( p, "earthlink", p->find_spell( 279928 ) )
    {
    }
...
void swirling_sands( special_effect_t& effect )
{
  struct swirling_sands_buff_t : public stat_buff_t
  {
    ...
    swirling_sands_buff_t( player_t* p, const azerite_power_t& power, dbc_proc_callback_t* e )
      : stat_buff_t( p, "swirling_sands", p->find_spell( 280433 ) ),
        extender( e ),
        max_extension( power.spell_ref().effectN( 3 ).time_value() )
    {
      add_stat( STAT_CRIT_RATING, power.value( 1 ) );
    }
  ...
  struct swirling_sands_extender_t : public dbc_proc_callback_t
  {
    ...
    swirling_sands_extender_t( const special_effect_t& effect, const azerite_power_t& power )
      : dbc_proc_callback_t( effect.player, effect ), extension( power.spell_ref().effectN( 2 ).time_value() )
    {
    }
...
void secrets_of_the_deep( special_effect_t& effect )
{
  ...
  auto normal_buff =
      unique_gear::create_buff<stat_buff_t>( effect.player, "secrets_of_the_deep", effect.player->find_spell( 273843 ) )
          ->add_stat( effect.player->convert_hybrid_stat( STAT_STR_AGI_INT ), power.value( 1 ) )
...
void combined_might( special_effect_t& effect )
{
  ...
  {
    buffs.push_back(
        unique_gear::create_buff<stat_buff_t>( effect.player, "might_of_the_orcs", effect.player->find_spell( 280841 ) )
            ->add_stat( STAT_ATTACK_POWER, power.value( 1 ) )
            ->add_stat( STAT_INTELLECT, power.value( 1 ) ) );
...
void relational_normalization_gizmo( special_effect_t& effect )
{
  ...
  for ( size_t i = 0, end = ilevels.size(); i < end; ++i )
  {
    ...
    haste_amount += floor(
        item_database::apply_combat_rating_multiplier( effect.player, CR_MULTIPLIER_ARMOR, ilevels[ i ], value ) );
...
void barrage_of_many_bombs( special_effect_t& effect )
{
  struct barrage_damage_t : public unique_gear::proc_spell_t
  {
    barrage_damage_t( const special_effect_t& effect, const azerite_power_t& power )
      : proc_spell_t( "barrage_of_many_bombs", effect.player, effect.player->find_spell( 280984 ), effect.item )
    {
      aoe         = -1;
  ...
  struct bomb_cb_t : public dbc_proc_callback_t
  {
    ...
    bomb_cb_t( const special_effect_t& effect, const azerite_power_t& power )
      : dbc_proc_callback_t( effect.player, effect ),
        damage( unique_gear::create_proc_action<barrage_damage_t>( "barrage_of_many_bombs", effect, power ) ),
...
void meticulous_scheming( special_effect_t& effect )
{
  ...
  struct seize_driver_t : public dbc_proc_callback_t
  {
    ...
    seize_driver_t( const special_effect_t& effect ) : dbc_proc_callback_t( effect.player, effect ), base( nullptr )
    {
    }
  ...
  secondary->type     = effect.type;
  secondary->source   = effect.source;
  secondary->proc_flags_ =
      PF_MELEE_ABILITY | PF_RANGED | PF_RANGED_ABILITY | PF_NONE_HEAL | PF_NONE_SPELL | PF_MAGIC_SPELL | PF_MAGIC_HEAL;
  ...
  auto haste_buff =
      unique_gear::create_buff<stat_buff_t>( effect.player, "seize_the_moment", effect.player->find_spell( 273714 ) )
          ->add_stat( STAT_HASTE_RATING, power.value( 4 ) );

  auto base_buff =
      unique_gear::create_buff<buff_t>( effect.player, "meticulous_scheming", effect.player->find_spell( 273685 ) )
  ...
  effect.proc_flags_ = PF_MELEE_ABILITY | PF_RANGED | PF_RANGED_ABILITY | PF_NONE_HEAL | PF_NONE_SPELL |
                       PF_MAGIC_SPELL | PF_MAGIC_HEAL | PF_PERIODIC;
...
void synaptic_spark_capacitor( special_effect_t& effect )
{
  struct spark_coil_t : public unique_gear::proc_spell_t
  {
    spark_coil_t( const special_effect_t& effect, const azerite_power_t& power )
      : proc_spell_t( "spark_coil", effect.player, effect.player->find_spell( 280847 ), effect.item )
    {
      aoe    = -1;
  ...
  struct spark_coil_driver_t : public unique_gear::proc_spell_t
  {
    ...
    spark_coil_driver_t( const special_effect_t& effect, const azerite_power_t& power )
      : proc_spell_t( "spark_coil_driver", effect.player, effect.player->find_spell( 280655 ), effect.item )
    {
      quiet          = true;
  ...
  effect.execute_action = unique_gear::create_proc_action<spark_coil_driver_t>( "spark_coil", effect, power );
...
void ricocheting_inflatable_pyrosaw( special_effect_t& effect )
{
  ...
  effect.execute_action = unique_gear::create_proc_action<rip_t>( "r.i.p.", effect, power );
...
void gutripper( special_effect_t& effect )
{
  ...
  struct gutripper_cb_t : public dbc_proc_callback_t
  {
    ...
    gutripper_cb_t( const special_effect_t& effect, const azerite_power_t& power )
      : dbc_proc_callback_t( effect.player, effect ), threshold( power.spell_ref().effectN( 2 ).base_value() )
    {
    }
...
void battlefield_focus_precision( special_effect_t& effect )
{
  ...
  {
    bf_damage_cb_t( const special_effect_t& effect ) : dbc_proc_callback_t( effect.player, effect )
    {
    }
  ...
  secondary->execute_action = unique_gear::create_proc_action<bf_damage_t>( effect.name(), effect, power );
...
void apothecarys_concoctions( special_effect_t& effect )
{
  ...
  effect.execute_action =
      unique_gear::create_proc_action<apothecarys_blight_t>( "apothecarys_concoctions", effect, power );
...
void seductive_power( special_effect_t& effect )
{
  ...
  {
    seductive_power_cb_t( special_effect_t& effect ) : dbc_proc_callback_t( effect.player, effect )
    {
    }
...
void bonded_souls( special_effect_t& effect )
{
  ...
  if ( !driver_buff )
  {
    ...
    driver_buff = make_buff( effect.player, "bonded_souls_driver", driver_buff_data )
                      ...
                      ->set_tick_zero( true )
                      ->set_tick_callback( [haste_buff]( buff_t*, int, const timespan_t& ) { haste_buff->trigger(); } );
  }
...
void fight_or_flight( special_effect_t& effect )
{
  ...
  if ( !fof_buff )
  {
    fof_buff = make_buff<stat_buff_t>( effect.player, "fight_or_flight", effect.player->find_spell( 287827 ) )
                   ->add_stat( effect.player->convert_hybrid_stat( STAT_STR_AGI_INT ), power.value( 1 ) )
                   // The buff's icd is handled by a debuff on the player ingame, we skip that and just use the debuff's
                   // duration as cooldown for the buff
...
void arcane_heart( special_effect_t& effect )
{
  ...
  effect.player->assessor_out_damage.add(
      assessor::TARGET_DAMAGE + 1, [buff, omni]( result_amount_type, action_state_t* state ) {
        ...
        if ( !buff->check() )  // special handling for damage from precombat actions
        {
          make_event( *buff->sim, [buff, amount] {
            ...
            buff->sim->print_debug( "arcane_heart_counter ability:precombat damage:{} counter now at:{}", amount,
                                    buff->current_value );
          } );
...
void clockwork_heart( special_effect_t& effect )
{
  ...
  if ( !ticker )
  {
    ticker = make_buff( effect.player, "clockwork_heart_ticker", effect.trigger() )
                 ...
                 ->set_tick_callback( [clockwork]( buff_t*, int, const timespan_t& ) { clockwork->trigger(); } );
  }

  effect.player->register_combat_begin( [ticker]( player_t* ) { ticker->trigger(); } );
}
...
void personcomputer_interface( special_effect_t& effect )
{
  ...
  switch ( stat )
  {
    case STAT_HASTE_RATING:
      ...
      break;
    case STAT_CRIT_RATING:
      effect.player->passive.crit_rating += power.value( 1 );
      break;
    case STAT_MASTERY_RATING:
      effect.player->passive.mastery_rating += power.value( 1 );
      break;
...
// Heart of Darkness
// TODO: Don't proc the buff if corruption is below the 25 threshold
...
void the_crucible_of_flame( special_effect_t& effect )
{
  ...
  struct ancient_flame_t : public unique_gear::proc_spell_t
  {
    ...
    {
      base_td = item_database::apply_combat_rating_multiplier(
          effect.player, combat_rating_multiplier_type::CR_MULTIPLIER_JEWLERY, essence.item()->item_level(),
          essence.spell_ref( 1u, essence_type::MINOR ).effectN( 3 ).average( essence.item() ) );

      base_td_multiplier *=
          1 + essence.spell_ref( 2u, essence_spell::UPGRADE, essence_type::MINOR ).effectN( 1 ).percent();
    }
  ...
  auto action = unique_gear::create_proc_action<ancient_flame_t>( "ancient_flame", effect, "ancient_flame", essence );
...
struct concentrated_flame_t : public azerite_essence_major_t
{
  struct missile_t : public unique_gear::proc_spell_t
  {
    ...
    missile_t( player_t* p, const azerite_essence_t& essence )
      : proc_spell_t( "concentrated_flame_missile", p, p->find_spell( 295374 ), essence.item() ),
        cast_number( 1u ),
        multiplier( essence.spell_ref( 1u ).effectN( 3 ).percent() )
    {
  ...
  concentrated_flame_t( player_t* p, const std::string& options_str )
    : azerite_essence_major_t( p, "concentrated_flame", p->find_spell( 295373 ) ), stack( 1u ), burn( nullptr )
  {
...
void blood_of_the_enemy( special_effect_t& effect )
{
  ...
  if ( essence.rank() >= 2 )
    counter->add_stat( STAT_CRIT_RATING,
                       essence.spell_ref( 1u, essence_type::MINOR ).effectN( 3 ).average( essence.item() ) );
  ...
  if ( !haste_buff )
  {
    ...
    haste_buff->add_stat( STAT_HASTE_RATING,
  ...
  if ( essence.rank() >= 3 )
  {
    ...
    // only lose 30 stacks
    decrement_stacks =
        as<int>( essence.spell_ref( 3u, essence_spell::UPGRADE, essence_type::MINOR ).effectN( 2 ).base_value() );
  }
...
void essence_of_the_focusing_iris( special_effect_t& effect )
{
  ...
  struct focused_energy_driver_t : dbc_proc_callback_t
  {
    ...
    focused_energy_driver_t( const special_effect_t& effect, int init_stacks )
      : dbc_proc_callback_t( effect.player, effect ), trigger_target(), init_stacks( init_stacks )
    {
    }
  ...
  effect.custom_buff =
      unique_gear::create_buff<stat_buff_t>( effect.player, "focused_energy", effect.player->find_spell( 295248 ) )
...
{
  focused_azerite_beam_tick_t( player_t* p, const std::string& n, double td ) : spell_t( n, p, p->find_spell( 295261 ) )
  {
    aoe               = -1;
    background        = true;
    may_crit          = true;
...
void condensed_life_force( special_effect_t& effect )
{
  ...
  auto action =
      unique_gear::create_proc_action<condensed_lifeforce_t>( "azerite_spike", effect, "azerite_spike", essence );
  ...
  struct guardian_of_azeroth_pet_t : public pet_t
  {
    ...
    struct azerite_volley_tick_t : public unique_gear::proc_spell_t
    {
        ...
        : proc_spell_t( "azerite_volley", p, p->find_spell( 303351 ), ess.item() )
      {
        aoe         = -1;
        base_dd_min = base_dd_max =
            ess.spell_ref( 2u, essence_spell::UPGRADE, essence_type::MAJOR ).effectN( 1 ).average( ess.item() );
      }
    };
...
void conflict_and_strife( special_effect_t& effect )
{
  ...
  {
    buff =
        make_buff<stat_buff_t>( effect.player, "strife", strife )
...
void purification_protocol( special_effect_t& effect )
{
  ...
  struct purification_protocol_t : public unique_gear::proc_spell_t
  {
    ...
    {
      base_dd_min = base_dd_max =
          essence.spell_ref( 1u, essence_type::MINOR ).effectN( 1 ).average( essence.item() ) *
    ...
    : azerite_essence_major_t( p, "ripple_in_space", p->find_spell( 302731 ) )
  {
    ...
    delay = essence.spell_ref( 1u, essence_type::MAJOR ).duration() +
            timespan_t::from_seconds(
                essence.spell_ref( 2u, essence_spell::UPGRADE, essence_type::MAJOR ).effectN( 1 ).base_value() / 1000 );
  }
...
void the_unbound_force( special_effect_t& effect )
{
  ...
  if ( essence.rank() >= 3 )
    crit_buff->buff_duration += timespan_t::from_millis(
        essence.spell_ref( 3u, essence_spell::UPGRADE, essence_type::MINOR ).effectN( 1 ).base_value() );

  if ( essence.rank() >= 2 )
    crit_buff->default_value +=
        essence.spell_ref( 2u, essence_spell::UPGRADE, essence_type::MINOR ).effectN( 1 ).percent();
...
struct the_unbound_force_t : public azerite_essence_major_t
{
  struct the_unbound_force_tick_t : public unique_gear::proc_spell_t
  {
    ...
    the_unbound_force_tick_t( player_t* p, const azerite_essence_t& essence, unsigned* max )
      : proc_spell_t( "the_unbound_force_tick", p, p->find_spell( 298453 ), essence.item() ),
        max_shard( max )  // missile spell
    {
...
void strive_for_perfection( special_effect_t& effect )
{
  ...
  effect.player->sim->print_debug( "{} reducing cooldown by {}%", effect.name(),
                                   vision_of_perfection_cdr( essence ) * -100 );
    ...
    if ( !effect.custom_buff )
    {
      effect.custom_buff =
          make_buff<stat_buff_t>( effect.player, "vision_of_perfection", effect.player->find_spell( 303344 ) )
              ->add_stat( STAT_HASTE_RATING, essence.spell_ref( 3u, essence_spell::UPGRADE, essence_type::MAJOR )
                                                 .effectN( 2 )
                                                 .average( essence.item() ) );
    }
  }
...
void worldvein_resonance( special_effect_t& effect )
{
  ...
  int period_min =
      as<int>( base_spell->effectN( 4 ).base_value() +
  ...
  struct lifeblood_event_t : public event_t
  {
    ...
    timespan_t next_event()
    {
      ...
      return std::max( 1.5_s,
                       timespan_t::from_seconds( buff->sim->rng().range( period_min, period_max + 1 ) ) - 0.5_s );
    }
...
struct worldvein_resonance_buff_t : public buff_t
{
  ...
  void adjust_stat( bool increase )
  {
    ...
    if ( lifeblood->check() )
    {
      ...
      sim->print_debug(
          "{} worldvein_resonance {}creases lifeblood stats by {}%,"
          " stacks={}, old={}, new={} ({}{})",
          player->name(), increase ? "in" : "de", data().effectN( 1 ).base_value(), lifeblood->current_stack,
          stat_entry.current_value, stat_entry.current_value + delta, increase ? "+" : "", delta );
      ...
      {
        player->stat_loss( stat_entry.stat, std::fabs( delta ), lifeblood->stat_gain, nullptr, true );
      }
...
void anima_of_life_and_death( special_effect_t& effect )
{
  ...
  effect.player->register_combat_begin( [buff, base_spell]( player_t* ) {
    make_repeating_event( *buff->sim, base_spell->effectN( 1 ).period(), [buff]() { buff->trigger(); } );
  } );
}
...
struct anima_of_death_t : public azerite_essence_major_t
{
    ...
    : azerite_essence_major_t( p, "anima_of_death", p->find_spell( 294926 ) )
  {
    ...
    snapshot_flags = update_flags =
        STATE_TGT_MUL_DA;  // The damage is based on the player's maximum health and doesn't seem to scale with anything
    if ( essence.rank() >= 2 )
    {
      cooldown->duration *=
          1.0 + essence.spell_ref( 2u, essence_spell::UPGRADE, essence_type::MAJOR ).effectN( 1 ).percent();
...
void nullification_dynamo( special_effect_t& effect )
{
  struct null_barrier_damage_t : public unique_gear::proc_spell_t
  {
    null_barrier_damage_t( const special_effect_t& effect, const azerite_essence_t& essence )
      : proc_spell_t( "null_barrier_damage", effect.player,
                      ...
                      effect.player->find_spell( 296061 ) == spell_data_t::not_found()
                          ? spell_data_t::nil()
  ...
  struct null_barrier_absorb_buff_t : absorb_buff_t
  {
    ...
    null_barrier_absorb_buff_t( player_t* p, const azerite_essence_t& essence, action_t* damage )
      : absorb_buff_t( p, "null_barrier", p->find_spell( 295842 ) ), null_barrier_damage( damage )
    {
  ...
  effect.player->register_combat_begin(
      [buff, interval]( player_t* ) { make_repeating_event( *buff->sim, interval, [buff]() { buff->trigger(); } ); } );
}
...
void aegis_of_the_deep( special_effect_t& effect )
{
  ...
  effect.player->register_combat_begin(
      [aegis_buff, effect]( player_t* ) { aegis_buff->trigger( effect.player->sim->desired_targets ); } );
}
...
void breath_of_the_dying( special_effect_t& effect )
{
  ...
  struct lethal_strikes_cb_t : public dbc_proc_callback_t
  {
    ...
    lethal_strikes_cb_t( const special_effect_t& e, const azerite_essence_t& ess ) : dbc_proc_callback_t( e.player, e )
    {
...
action_t* create_action( player_t* player, const std::string& name, const std::string& options )
{
  ...
  else if ( util::str_compare_ci( name, "heart_essence" ) )
  {
    ...
    else
    {
      ...
      {
        heart_essence_dummy_t( player_t* p ) : action_t( ACTION_OTHER, "heart_essence", p )
        {
