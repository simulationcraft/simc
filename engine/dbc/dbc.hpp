...
public:
  ...
  const spelleffect_data_t& effectN( unsigned idx ) const { assert( idx ); assert ( idx <= _effects -> size() ); if ( idx > _effects -> size() ) return *spelleffect_data_t::nil(); else return *_effects -> at( idx - 1 ); }
