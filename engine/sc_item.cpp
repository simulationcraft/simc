...
bool item_t::init()
{
  ...
  if ( ! option_id_str.empty() )
  {
    ...
    {
      util_t::fprintf( sim -> output_file, "\nsimucraft: Warning! Player %s at slot %s has inconsistency between name '%s' and '%s' for id '%s'\n",
...
bool item_t::decode_stats()
{
  ...
  for ( int i=0; i < num_tokens; i++ )
  {
    ...
    {
      util_t::fprintf( sim -> output_file, "simucraft: %s has unknown 'stats=' token '%s' at slot %s\n", player -> name(), t.full.c_str(), slot_name() );
      return false;
    }
  }
...
bool item_t::decode_gems()
{
  ...
  for ( int i=0; i < num_tokens; i++ )
  {
    ...
    {
      util_t::fprintf( sim -> output_file, "\nsimucraft: %s has unknown 'gems=' token '%s' at slot %s\n", player -> name(), t.full.c_str(), slot_name() );
      //return false;
    }
  }
...
bool item_t::decode_enchant()
{
  ...
  if( unique_gear_t::get_hidden_encoding( hidden_str, encoded_enchant_str ) )
  {
    ...
    for ( int i=0; i < num_tokens; i++ )
    {
      ...
      {
        util_t::fprintf( sim -> output_file, "simucraft: %s has unknown 'enchant=' token '%s' at slot %s\n", player -> name(), t.full.c_str(), slot_name() );
        return false;
      }
    }
    unique_enchant = true;
                             ...
                             const std::string& encoding )
{
  ...
  for ( int i=0; i < num_tokens; i++ )
  {
    ...
    {
      util_t::fprintf( sim -> output_file, "simucraft: %s has unknown 'use/equip=' token '%s' at slot %s\n", player -> name(), t.full.c_str(), slot_name() );
      return false;
    }
  }
...
bool item_t::decode_weapon()
{
  ...
  for ( int i=0; i < num_tokens; i++ )
  {
    ...
    {
      util_t::fprintf( sim -> output_file, "simucraft: %s has unknown 'weapon=' token '%s' at slot %s\n", player -> name(), t.full.c_str(), slot_name() );
      return false;
    }
  }
...
bool item_t::download_slot( item_t& item, const std::string& item_id, const std::string& enchant_id, const std::string gem_ids[ 3 ] )
{
  ...
  if ( ! success )
  {
    util_t::fprintf( item.sim -> output_file,
		     "\nsimucraft: Player %s unable to download slot '%s' info from wowhead.  Trying mmo-champion....\n", 
  ...
  {
    util_t::fprintf( item.sim -> output_file, 
		     "\nsimucraft: Player %s unable to download slot '%s' info from mmo-champion.  Trying wowarmory....\n", 
		     p -> name(), item.slot_name() );
...
bool item_t::download_item( item_t& item, const std::string& item_id )
{
  ...
  {
    util_t::fprintf( item.sim -> output_file,
		     "\nsimucraft: Player %s unable to download item '%s' info from mmo-champion.  Trying wowhead....\n", 
		     p -> name(), item.name() );
  ...
  {
    util_t::fprintf( item.sim -> output_file, 
		     "\nsimucraft: Player %s unable to download item '%s' info from mmo-champion.  Trying wowarmory....\n", 
		     p -> name(), item.name() );
...
bool item_t::download_glyph( sim_t* sim, std::string& glyph_name, const std::string& glyph_id )
{
  ...
  {
    util_t::fprintf( sim -> output_file,
		     "\nsimucraft: Unable to download glyph id '%s' info from wowhead.  Trying mmo-champion....\n", 
		     glyph_id.c_str() );
                       ...
                       const std::string& gem_id )
{
  ...
  util_t::fprintf( item.sim -> output_file,
		   "\nsimucraft: Unable to download gem id '%s' info from wowhead.  Trying mmo-champion....\n", 
		   gem_id.c_str() );
