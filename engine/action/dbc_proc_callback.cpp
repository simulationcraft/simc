...
struct proc_event_t : public event_t
{
      ...
      source_action( a )
  {
    ...
    if ( s )
    {
      if ( s->result_type == result_amount_type::ENERGIZE )
        source_state = s;
      else
  ...
  ~proc_event_t() override
  {
    ...
    if ( source_state && source_state->result_type != result_amount_type::ENERGIZE )
...
void dbc_proc_callback_t::trigger( action_t* a, action_state_t* state )
{
  ...
  if ( triggered )
  {
    assert( state && ( state->action || state->result_type == result_amount_type::ENERGIZE ) );
...
player_t* dbc_proc_callback_t::target( const action_state_t* state, action_t* proc ) const
{
  ...
  switch ( _action->type )
  {
    ...
    case ACTION_SPELL:
      ...
      if ( state->result_type == result_amount_type::ENERGIZE || state->action->player == listener )
