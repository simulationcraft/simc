                                  ...
                                  bool dynamic )
{
  ...
  else if ( name_str == "crit_pct" )
  {
    struct dot_crit_pct_expr_t : public dot_expr_t
    {
      ...
      virtual double evaluate() override { return dot() -> state ? dot() -> state -> crit_chance * 100.0 : 0; }
    };
...
void dot_t::start( timespan_t duration )
{
  ...
  if ( current_action && current_duration > time_to_tick && current_action -> need_to_trigger_costs_per_tick() )
  {
    current_action -> schedule_cost_tick_event( *this );
  }
