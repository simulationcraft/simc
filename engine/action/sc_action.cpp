...
struct action_execute_event_t : public player_event_t
{
  ...
  action_execute_event_t( action_t* a, timespan_t time_to_execute,
      ...
      player_event_t( *a->player, time_to_execute ), action( a ),
        execute_state( state )
  {
    sim().print_debug( "New Action Execute Event: {} {} {} (target={}, marker={})",
...
action_t::action_t( action_e       ty,
  ...
  ap_type( AP_DEFAULT ), // By default make actions use the new pure ap + mainhand dps value
...
double action_t::calculate_direct_amount( action_state_t* state ) const
{
  ...
  if ( sim -> debug )
  {
    sim -> out_debug.printf( "%s amount for %s: dd=%.0f i_dd=%.0f w_dd=%.0f b_dd=%.0f s_mod=%.2f s_power=%.0f a_mod=%.2f a_power=%.0f mult=%.2f w_mult=%.2f w_slot=%.1f bonus_da=%.0f",
                   ...
                   state -> composite_da_multiplier(), weapon_multiplier, weapon_slot_modifier, bonus_da( state ) );
  }
