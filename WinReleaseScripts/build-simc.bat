...
if not defined SIMCVERSION for /F "tokens=3 delims= " %%i in ('findstr /C:"#define SC_MAJOR_VERSION" %SIMCDIR%\engine\config.hpp') do set SC_MAJOR_VERSION=%%~i
if not defined SIMCVERSION for /F "tokens=3 delims= " %%i in ('findstr /C:"#define SC_MINOR_VERSION" %SIMCDIR%\engine\config.hpp') do set SC_MINOR_VERSION=%%~i
...
msbuild.exe "%SIMCDIR%\simulationcraft.sln" /maxcpucount /p:configuration=%1 /p:platform=%VSPLATFORM% /nr:true /m /p:QTDIR=%QTDIR% %TARGET%
if ERRORLEVEL 1 exit /b 1
...
robocopy %SIMCDIR%\qt\ %INSTALLDIR%\ Error.html Welcome.html Welcome.png jquery.js /NJH /NJS
robocopy %SIMCDIR%\locale\ %INSTALLDIR%\locale sc_de.qm sc_cn.qm sc_it.qm sc_ko.qm /NJH /NJS
...
:usage
echo Usage: build-simc.bat VS-Configuration [Target (Debug/Release)] [Architecture (x86/x64)]
call :help
goto :enderror
