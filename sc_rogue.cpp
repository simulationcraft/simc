...
struct apply_poison_t : public rogue_poison_t
{
      ...
      off_hand_poison( WEAPON_BUFF_NONE )
  {
    ...
    if ( main_hand_str.empty() &&
         off_hand_str.empty() )
    {
      util_t::sc_printf( "simcraft: At least one of 'main_hand/off_hand' must be specified in 'apply_poison'.  Accepted values are 'anesthetic/deadly/instant/wound'.\n" );
      exit( 0 );
    }
...
void rogue_t::combat_begin()
{
  ...
  if ( talents.honor_among_thieves )
  {
    ...
    {
      util_t::sc_printf( "simcraft: %s must have a party specification or 'honor_among_thieves_interval' must be set.\n", name() );
      exit( 0 );
    }
  }
}
...
bool rogue_t::save( FILE* file, int save_type )
{
  ...
  if ( save_type == SAVE_ALL || save_type == SAVE_ACTIONS )
  {
    if( honor_among_thieves_interval != 0 )
    {
      util_t::sc_fprintf( file, "# When using this profile with a real raid AND party setups, override this value to zero.\n" );
      util_t::sc_fprintf( file, "# This does not include HAT procs generated by the Rogue himself.\n" );
      util_t::sc_fprintf( file, "honor_among_thieves_interval=%.2f\n", honor_among_thieves_interval );
    }
  }
