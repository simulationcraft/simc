...
if [ -d $DEPLOY_DIR ]; then
  ...
  hdiutil create -srcfolder $DEPLOY_DIR/.Trash -srcfolder $DEPLOY_DIR/SimulationCraft.app -srcfolder profiles -srcfolder COPYING -srcfolder LICENSE.BSD -srcfolder LICENSE.MIT -srcfolder LICENSE.LGPL -srcfolder README.md  -srcfolder "simc" -nospotlight -format UDBZ -stretch 1g -volname "Simulationcraft v`defaults read $DEPLOY_DIR/SimulationCraft.app/Contents/Info CFBundleVersion`" -ov simc-$MAJOR_VERSION-$MINOR_VERSION-osx-x86.dmg
  rm -rf $DEPLOY_DIR
fi
