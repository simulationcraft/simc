...
DEPLOY_FILES="SimulationCraft.app profiles COPYING LICENSE.BOOST LICENSE.BSD LICENSE.MIT LICENSE.LGPL README.md simc"
...
if [ -d $DEPLOY_DIR ]; then
  ...
  hdiutil create -srcfolder $DEPLOY_DIR/ -format UDBZ -stretch 1g -volname "Simulationcraft v`defaults read $DEPLOY_DIR/SimulationCraft.app/Contents/Info CFBundleVersion`" -ov simc-$MAJOR_VERSION-$MINOR_VERSION-osx-x86.dmg
  rm -rf $DEPLOY_DIR
fi
