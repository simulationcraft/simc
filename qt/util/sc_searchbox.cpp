...
SC_SearchBox::SC_SearchBox( QWidget* parent, Qt::Corner corner, bool show_arrows, QBoxLayout::Direction direction )
  : QWidget( parent ),
    ...
    highlightTextOnShow( true )
{
  ...
  if ( enableReverseContextMenuItem || enableWrapAroundContextMenuItem || enableFindPrevNextContextMenuItem )
  {
    ...
    connect( searchBox, SIGNAL( customContextMenuRequested( const QPoint& ) ), this,
             SLOT( searchBoxContextMenuRequested( const QPoint& ) ) );
  }
  ...
  if ( enableHideArrowsContextMenuItem )
  {
    ...
    hideArrowsAction->setChecked(
        hide_arrows_setting );  // TODO connect to QSettings && sync all other SC_SearchBoxes on change
